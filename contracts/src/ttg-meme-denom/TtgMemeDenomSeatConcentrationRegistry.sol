...
/**
 * @title TtgMemeDenomSeatConcentrationRegistry
 * @notice DESIGN_ONLY V8 of TtgSeatConcentrationRegistry.
 * @dev Live GOV-03 cap is 400 bps. Fusion keeps 400 (permission KEEP). Absolute TTG
 *      amount is 400 bps × 25T. FR/ES 4.5% vs 400 is live Official TBD, not a V8 450 invention.
 */
contract TtgMemeDenomSeatConcentrationRegistry {
    ...
    function maxAggregateStakePerEntity() public pure returns (uint256) {
        return (TtgMemeDenomConstants.TTG_TOTAL_SUPPLY_UNITS
            * TtgMemeDenomConstants.MAX_AGGREGATE_SEAT_STAKE_PER_ENTITY_BPS) / 10_000;
    }
    ...
    function assertStakeAllowed(address staker, bytes2 jurisdiction, uint256 amount) external view {
        ...
        if (!entityHasActiveJurisdiction[entity][jurisdiction]) {
            if (activeSeatCountByEntity[entity]
                >= TtgMemeDenomConstants.MAX_ACTIVE_SEATS_PER_CONTROLLING_ENTITY) {
    ...
    function version() external pure returns (string memory) {
        return "ttg_primary_25t_seat_concentration_v8";
    }
}
