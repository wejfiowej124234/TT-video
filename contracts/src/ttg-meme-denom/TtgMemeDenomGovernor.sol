...
import "./TtgMemeDenomProposalThresholds.sol";

/**
 * @title TtgMemeDenomGovernor
 * @notice Fusion of live `TravelTrustGovernor` onto the 25T token. No O4 entity quorum split.
 */
interface ITtgMemeDenomVotes {
    function getPastVotes(address account, uint256 blockNumber) external view returns (uint256);

    function getPastTotalSupply(uint256 blockNumber) external view returns (uint256);
}

interface ITtgMemeDenomTimelock {
...
enum TtgMemeDenomProposalState {
    Pending,
    Active,
    Canceled,
    Defeated,
...
contract TtgMemeDenomGovernor {
    bytes32 internal constant REMOVE_COUNTRY_STEWARD = keccak256("REMOVE_COUNTRY_STEWARD");

    ITtgMemeDenomVotes public immutable token;
    ITtgMemeDenomTimelock public immutable timelock;
    ...
    constructor(
        ITtgMemeDenomVotes token_,
        ITtgMemeDenomTimelock timelock_,
        ...
        if (quorumNumeratorBps_ == 0) revert GovThreshold();
        token = token_;
        timelock = timelock_;
    ...
    function propose(
        ...
        return propose(targets, values, calldatas, description, TtgMemeDenomProposalThresholds.TIER_ORDINARY);
    }
    ...
    function propose(
        ...
        uint256 need = TtgMemeDenomProposalThresholds.requiredVotes(tier, supply);
    ...
    function proposeRemoveCountrySteward(
        ...
        proposalId = propose(targets, values, calldatas, description, TtgMemeDenomProposalThresholds.TIER_CORE);
        proposalTypeTags[proposalId] = REMOVE_COUNTRY_STEWARD;
    }

    function isRemoveCountryStewardProposal(uint256 proposalId) external view returns (bool) {
        return proposalTypeTags[proposalId] == REMOVE_COUNTRY_STEWARD;
    }
    ...
    function quorumReached(uint256 proposalId) public view returns (bool) {
        ...
        uint256 need = (supply * quorumNumeratorBps) / 10_000;
        return p.forVotes + p.abstainVotes >= need;
    }

    function state(uint256 proposalId) public view returns (TtgMemeDenomProposalState) {
        ProposalCore storage p = proposals[proposalId];
        if (p.proposer == address(0)) revert GovUnknownProposal();
        if (p.canceled) return TtgMemeDenomProposalState.Canceled;
        if (p.executed) return TtgMemeDenomProposalState.Executed;
        if (block.number < p.voteStart) return TtgMemeDenomProposalState.Pending;
        if (block.number <= p.voteEnd) return TtgMemeDenomProposalState.Active;
        if (p.forVotes <= p.againstVotes || !quorumReached(proposalId)) return TtgMemeDenomProposalState.Defeated;
        if (p.queuedOpId == bytes32(0)) return TtgMemeDenomProposalState.Succeeded;
        (, bool done,,,) = timelock.operations(p.queuedOpId);
        if (done) return TtgMemeDenomProposalState.Executed;
        return TtgMemeDenomProposalState.Queued;
    }

    function queue(uint256 proposalId) external returns (bytes32 opId) {
        if (state(proposalId) != TtgMemeDenomProposalState.Succeeded) revert GovBadState();
    ...
    function execute(uint256 proposalId) external {
        if (state(proposalId) != TtgMemeDenomProposalState.Queued) revert GovBadState();
        ProposalCore storage p = proposals[proposalId];
