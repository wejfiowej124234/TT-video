...
/**
 * @title MockV9Timelock
 * @notice Local Timelock stand-in for GovernorV9 + UUPS upgrade auth tests (English NatSpec only).
 */
contract MockV9Timelock {
    struct Operation {
        uint256 readyAt;
        bool done;
        address target;
    ...
    /// @dev Sepolia/local deploy bootstrap: admin may call as Timelock until `endBootstrap`.
    ...
    /// @notice One-shot setup: execute as this Timelock (bindMarket / seedBatches). Then `endBootstrap`.
    function bootstrapCall(address target, bytes calldata data) external returns (bytes memory ret) {
        if (msg.sender != admin) revert OnlyAdmin();
        if (!bootstrapMode) revert BootstrapClosed();
