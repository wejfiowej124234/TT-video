...
| **GovernanceTimelock** | **B-089** | **`schedule`****（****admin****）** + **`scheduleByGovernor`****（****Governor****）** + **`setGovernor`** + **`execute`**；测试 **`GovernanceTimelock.t.sol`**、**`TravelTrustGovernor.t.sol`**；ABI **`GovernanceTimelock.json`**（含 **`governor`****/****`scheduleByGovernor`**） |
...
| **GovernanceVotesToken** | **Completion（B-089）** | **`GovernanceVotesToken.sol`**：**`getPastVotes`****/****`getPastTotalSupply`**（checkpoint）；供 Governor **快照** |
