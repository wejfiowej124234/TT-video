...
- **D-4555-B Country Pool net profit（G23-04 冻结 · Gate-2.4 输入）**：`CountryPoolNetProfitLedger.json` · `StewardPathVault.json` · `UnallocatedStewardPathVault.json` · **`manifests/country-pool-net-profit-v1.json`** · **`CountryPoolNetProfitGovernancePayload.json`**（library selector 表 · 非 forge ABI）· **`registry/event-decoders/country-pool-net-profit-v1.yaml`**。验证：`bash scripts/dev/check-country-pool-net-profit-abi-freeze.sh`（`check-55-s13` 已串联）。**禁止** 在 Gate-2.4 前 rename P0 事件（`StewardPathDeposit` / `UnallocatedStewardDeposit` 等）。
