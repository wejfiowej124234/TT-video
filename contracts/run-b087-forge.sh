#!/usr/bin/env bash
# B-087：本地跑 InvestorDistributionClaim.t.sol 中 test_B087_*（须 forge 在 PATH）
set -euo pipefail
...
forge test --root "$CONTRACTS_DIR" --match-path test/InvestorDistributionClaim.t.sol --match-test "B087" -vv
