#!/usr/bin/env bash
# B-090：GovernanceTreasury.t.sol 中 test*_b090_* / test*_B090_*（须 forge 在 PATH）
set -euo pipefail
...
forge test --root "$CONTRACTS_DIR" --match-path test/GovernanceTreasury.t.sol --match-test "[Bb]090" -vv
