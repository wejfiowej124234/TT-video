#!/usr/bin/env bash
# 公开只读 API 冒烟（缺口总表 P1-E：可执行联调/回归留痕用，非 E2E 三项）。
# 用法：API 已监听后执行
#   ./scripts/smoke-api-public-routes.sh
#   API_BASE_URL=http://127.0.0.1:8080 ./scripts/smoke-api-public-routes.sh
# 若 .env 中 PORT 非 8080（如 3012），可：PORT=3012 ./scripts/smoke-api-public-routes.sh
#   或显式 API_BASE_URL=http://127.0.0.1:3012 ./scripts/smoke-api-public-routes.sh
set -euo pipefail
PORT="${PORT:-8080}"
BASE="${API_BASE_URL:-http://127.0.0.1:${PORT}}"
BASE="${BASE%/}"

fail() { echo "smoke-api-public-routes: FAIL $1" >&2; exit 1; }

code() {
  curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 "$1" || echo "000"
}

h="$(code "$BASE/health")"
[[ "$h" == "200" ]] || fail "/health expected 200 got $h ($BASE)"

m="$(code "$BASE/meta")"
[[ "$m" == "200" ]] || fail "/meta expected 200 got $m ($BASE)"
mbuild="$(code "$BASE/meta/build")"
[[ "$mbuild" == "200" ]] || fail "/meta/build expected 200 got $mbuild ($BASE)"
mt="$(code "$BASE/metrics")"
[[ "$mt" == "200" ]] || fail "/metrics expected 200 got $mt ($BASE)"
if command -v jq >/dev/null 2>&1; then
  mb=$(curl -sS --connect-timeout 5 "$BASE/meta" || true)
  svc=$(echo "$mb" | jq -r '.service // empty')
  [[ "$svc" == "traveltrust-api" ]] || fail "/meta JSON .service expected traveltrust-api, got \"$svc\""
  st=$(echo "$mb" | jq -r '.strict_mode | type')
  [[ "$st" == "object" ]] || fail "/meta JSON .strict_mode must be object"
  dwt=$(echo "$mb" | jq -r '.dual_write | type')
  [[ "$dwt" == "object" ]] || fail "/meta JSON .dual_write must be object"
  ...
  echo "$mb" | jq -e '.product_roles.me_public_role_mapping == {"tourist":"traveler"}' >/dev/null \
    || fail "/meta JSON .product_roles.me_public_role_mapping tourist→traveler (690)"
  echo "$mb" | jq -e '.product_roles.protocol_roles_target_87 == ["traveler","guide","provider","region_steward"]' >/dev/null \
    || fail "/meta JSON .product_roles.protocol_roles_target_87 (690)"
  pif=$(echo "$mb" | jq -r '.product_roles.provider_in_users_role | tostring')
  [[ "$pif" == "true" ]] || fail "/meta JSON .product_roles.provider_in_users_role expected true (692), got $pif"
  rsf=$(echo "$mb" | jq -r '.product_roles.region_steward_in_users_role | tostring')
  [[ "$rsf" == "true" ]] || fail "/meta JSON .product_roles.region_steward_in_users_role expected true (692), got $rsf"
  prul=$(echo "$mb" | jq -r '.product_roles.rule // empty')
  [[ -n "$prul" ]] || fail "/meta JSON .product_roles.rule must be non-empty (692)"
  regtype=$(echo "$mb" | jq -r '.auth.registration | type')
  [[ "$regtype" == "object" ]] || fail "/meta JSON .auth.registration must be object, got $regtype (694)"
  ...
  defr=$(echo "$mb" | jq -r '.auth.registration.default_role // empty')
  [[ "$defr" == "tourist" ]] || fail "/meta JSON .auth.registration.default_role expected tourist, got \"$defr\" (694)"
  ierr=$(echo "$mb" | jq -r '.auth.registration.invalid_role_error_key // empty')
  [[ "$ierr" == "invalid_registration_role" ]] || fail "/meta JSON .auth.registration.invalid_role_error_key (694)"
  gvf=$(echo "$mb" | jq -r '.auth.registration.guide_via_separate_flow_only | tostring')
  [[ "$gvf" == "true" ]] || fail "/meta JSON .auth.registration.guide_via_separate_flow_only expected true (694)"
  r694=$(echo "$mb" | jq -r '.auth.registration.rule // empty')
  [[ -n "$r694" ]] || fail "/meta JSON .auth.registration.rule must be non-empty (694)"
  ...
  bb=$(curl -sS --connect-timeout 5 "$BASE/meta/build" || true)
  exp=$(echo "$mb" | jq -c '.build')
  got=$(echo "$bb" | jq -c '.')
  [[ "$exp" == "$got" ]] || fail "/meta/build JSON must equal /meta .build (688)"
fi

d="$(code "$BASE/api/v1/discover/orders")"
[[ "$d" == "200" ]] || fail "/api/v1/discover/orders expected 200 got $d"

dp="$(code "$BASE/api/v1/discover/orders?limit=1")"
[[ "$dp" == "200" ]] || fail "/api/v1/discover/orders?limit=1 expected 200 got $dp"
if command -v jq >/dev/null 2>&1; then
  db=$(curl -sS --connect-timeout 5 "$BASE/api/v1/discover/orders?limit=1" || true)
  st=$(echo "$db" | jq -r '.status // empty')
  plim=$(echo "$db" | jq -r '.page.limit | tostring // empty')
  [[ "$st" == "ok" ]] || fail "discover?limit=1 JSON .status expected ok, got \"$st\""
  [[ "$plim" == "1" ]] || fail "discover?limit=1 JSON .page.limit expected 1, got \"$plim\""
  itype=$(echo "$db" | jq -r '.items | type')
  [[ "$itype" == "array" ]] || fail "discover?limit=1 JSON .items must be array, got $itype"
  hm=$(echo "$db" | jq -r '.page.has_more | tostring // empty')
  [[ "$hm" == "true" || "$hm" == "false" ]] || fail "discover?limit=1 JSON .page.has_more must be boolean"
fi

r="$(code "$BASE/api/v1/did-rank/itineraries")"
[[ "$r" == "200" ]] || fail "/api/v1/did-rank/itineraries expected 200 got $r"
rg="$(code "$BASE/api/v1/did-rank/guides")"
...
if command -v jq >/dev/null 2>&1; then
  ...
  [[ "$irb" =~ ^(order_completed_at|itinerary_created_at_fallback|itinerary_created_at_proxy)$ ]] || fail "did-rank itineraries JSON .rank_basis unexpected: \"$irb\""
  itype=$(echo "$ib" | jq -r '.itineraries | type')
  [[ "$itype" == "array" ]] || fail "did-rank itineraries JSON .itineraries must be array, got $itype"
  gb=$(curl -sS --connect-timeout 5 "$BASE/api/v1/did-rank/guides?period=week" || true)
  grb=$(echo "$gb" | jq -r '.rank_basis // empty')
  ...
  gb_rev=$(curl -sS --connect-timeout 5 "$BASE/api/v1/did-rank/guides?period=week&sort=reviews" || true)
  grb_rev=$(echo "$gb_rev" | jq -r '.rank_basis // empty')
  ...
  gb_w=$(curl -sS --connect-timeout 5 "$BASE/api/v1/did-rank/guides?period=week&sort=weighted" || true)
  grb_w=$(echo "$gb_w" | jq -r '.rank_basis // empty')
