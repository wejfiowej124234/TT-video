# 公开只读 API 冒烟（PowerShell）：与 smoke-api-public-routes.sh 等价；须各端点 200（discover 非 503）。
# 用法：API 已监听后，仓库根执行 .\scripts\smoke-api-public-routes.ps1
# 环境变量：$env:API_BASE_URL 优先；否则 $env:PORT（默认 8080）→ http://127.0.0.1:$PORT

$ErrorActionPreference = "Stop"
$port = if ($env:PORT) { $env:PORT } else { "8080" }
$Base = if ($env:API_BASE_URL) { $env:API_BASE_URL.TrimEnd("/") } else { "http://127.0.0.1:$port" }

function fail { param($msg) Write-Error "smoke-api-public-routes: FAIL $msg"; exit 1 }
function ok { param($msg) Write-Host "smoke-api-public-routes OK: $msg" }
...
$hc = Get-StatusCode "$Base/health"
if ($hc -ne 200) { fail "/health expected 200 got $hc ($Base)" }
ok "/health 200"

try {
    $rm = Invoke-WebRequest -Uri "$Base/meta" -UseBasicParsing -TimeoutSec 5
} catch { $rm = $null }
if (-not $rm -or $rm.StatusCode -ne 200) { fail "/meta expected 200 got $(if ($rm) { $rm.StatusCode } else { 'no response' }) ($Base)" }
ok "/meta 200"
try {
    $jm = $rm.Content | ConvertFrom-Json
} catch { fail "/meta JSON parse failed: $_" }
if ($jm.service -ne "traveltrust-api") { fail "/meta JSON .service expected traveltrust-api, got $($jm.service)" }
if ($null -eq $jm.strict_mode) { fail "/meta JSON .strict_mode missing or null (expected object)" }
if ($null -eq $jm.dual_write) { fail "/meta JSON .dual_write missing or null (expected object)" }
...
if ($icp.source -notin @("runtime", "startup_snapshot")) {
    fail "/meta JSON .indexer.checkpoint.source expected runtime|startup_snapshot, got $($icp.source)"
}
...
if (-not (Test-MetaJsonNumber $icp.block_number)) {
    fail "/meta JSON .indexer.checkpoint.block_number must be numeric"
}
if (-not (Test-MetaJsonNumber $icp.log_index)) {
    fail "/meta JSON .indexer.checkpoint.log_index must be numeric"
}
...
for ($i = 0; $i -lt 4; $i++) {
    if ([string]$arr[$i] -ne $exp[$i]) { fail "/meta JSON .auth.registration.self_serve_roles_allowed[$i] expected $($exp[$i]), got $($arr[$i]) (697)" }
}
if ([string]$reg.default_role -ne "tourist") { fail "/meta JSON .auth.registration.default_role expected tourist (694)" }
if ([string]$reg.invalid_role_error_key -ne "invalid_registration_role") { fail "/meta JSON .auth.registration.invalid_role_error_key (694)" }
if ($reg.guide_via_separate_flow_only -isnot [bool] -or $reg.guide_via_separate_flow_only -ne $true) {
    fail "/meta JSON .auth.registration.guide_via_separate_flow_only expected true (694)"
}
if ([string]::IsNullOrWhiteSpace([string]$reg.rule)) { fail "/meta JSON .auth.registration.rule must be non-empty (694)" }
...
try {
    $rmet = Invoke-WebRequest -Uri "$Base/metrics" -UseBasicParsing -TimeoutSec 5
} catch { $rmet = $null }
if (-not $rmet -or $rmet.StatusCode -ne 200) { fail "/metrics expected 200 got $(if ($rmet) { $rmet.StatusCode } else { 'no response' })" }
ok "/metrics 200"

$d = Get-StatusCode "$Base/api/v1/discover/orders"
if ($d -ne 200) { fail "/api/v1/discover/orders expected 200 got $d" }
ok "/api/v1/discover/orders 200"

try {
    $r2p = Invoke-WebRequest -Uri "$Base/api/v1/discover/orders?limit=1" -UseBasicParsing -TimeoutSec 5
} catch { $r2p = $null }
...
ok "discover?limit=1 JSON shape"

$dr = Get-StatusCode "$Base/api/v1/did-rank/itineraries"
if ($dr -ne 200) { fail "/api/v1/did-rank/itineraries expected 200 got $dr" }
ok "/api/v1/did-rank/itineraries 200"
try {
    $riw = Invoke-WebRequest -Uri "$Base/api/v1/did-rank/itineraries?period=week" -UseBasicParsing -TimeoutSec 5
} catch { $riw = $null }
if (-not $riw -or $riw.StatusCode -ne 200) {
    fail "did-rank itineraries ?period=week expected 200, got $(if ($riw) { $riw.StatusCode } else { 'no response' })"
}
try {
    $jiw = $riw.Content | ConvertFrom-Json
} catch { fail "did-rank itineraries week JSON parse failed: $_" }
$allowedIt = @("order_completed_at", "itinerary_created_at_fallback", "itinerary_created_at_proxy")
if ($allowedIt -notcontains $jiw.rank_basis) {
    fail "did-rank itineraries JSON .rank_basis unexpected: $($jiw.rank_basis) (expected order_completed_at|itinerary_created_at_fallback|itinerary_created_at_proxy)"
}
if ($null -eq $jiw.itineraries) { fail "did-rank itineraries JSON .itineraries missing" }
if ($jiw.itineraries -isnot [System.Array]) { fail "did-rank itineraries JSON .itineraries must be array" }
ok "did-rank/itineraries JSON .rank_basis + itineraries[] (period=week)"

$drg = Get-StatusCode "$Base/api/v1/did-rank/guides"
if ($drg -ne 200) { fail "/api/v1/did-rank/guides expected 200 got $drg" }
ok "/api/v1/did-rank/guides 200"
try {
    $rgw = Invoke-WebRequest -Uri "$Base/api/v1/did-rank/guides?period=week" -UseBasicParsing -TimeoutSec 5
} catch { $rgw = $null }
if (-not $rgw -or $rgw.StatusCode -ne 200) {
    fail "did-rank guides ?period=week expected 200, got $(if ($rgw) { $rgw.StatusCode } else { 'no response' })"
}
try {
    $jgw = $rgw.Content | ConvertFrom-Json
} catch { fail "did-rank guides week JSON parse failed: $_" }
if ($jgw.rank_basis -ne "guide_reception_gross_total_then_completed_count") {
    fail "did-rank guides JSON .rank_basis expected guide_reception_gross_total_then_completed_count, got $($jgw.rank_basis)"
}
ok "did-rank/guides JSON .rank_basis (period=week)"
try {
    $rgwRev = Invoke-WebRequest -Uri "$Base/api/v1/did-rank/guides?period=week&sort=reviews" -UseBasicParsing -TimeoutSec 5
} catch { $rgwRev = $null }
if (-not $rgwRev -or $rgwRev.StatusCode -ne 200) {
    fail "did-rank guides ?period=week&sort=reviews expected 200, got $(if ($rgwRev) { $rgwRev.StatusCode } else { 'no response' })"
}
try {
    $jgwRev = $rgwRev.Content | ConvertFrom-Json
} catch { fail "did-rank guides sort=reviews week JSON parse failed: $_" }
$expGRev = "guide_avg_received_review_then_reception_gross_then_completed_count_min_completed_ge_3"
if ($jgwRev.rank_basis -ne $expGRev) {
    fail "did-rank guides sort=reviews .rank_basis expected $expGRev, got $($jgwRev.rank_basis)"
}
ok "did-rank/guides JSON .rank_basis (period=week, sort=reviews)"
try {
    $rgwW = Invoke-WebRequest -Uri "$Base/api/v1/did-rank/guides?period=week&sort=weighted" -UseBasicParsing -TimeoutSec 5
} catch { $rgwW = $null }
if (-not $rgwW -or $rgwW.StatusCode -ne 200) {
    fail "did-rank guides ?period=week&sort=weighted expected 200, got $(if ($rgwW) { $rgwW.StatusCode } else { 'no response' })"
}
try {
    $jgwW = $rgwW.Content | ConvertFrom-Json
} catch { fail "did-rank guides sort=weighted week JSON parse failed: $_" }
$expGW = "guide_weighted_volume_norm_w60_review_avg_norm_w40_then_reception_gross_then_completed_count_min_completed_ge_3"
if ($jgwW.rank_basis -ne $expGW) {
    fail "did-rank guides sort=weighted .rank_basis expected $expGW, got $($jgwW.rank_basis)"
}
ok "did-rank/guides JSON .rank_basis (period=week, sort=weighted)"

$drt = Get-StatusCode "$Base/api/v1/did-rank/travelers"
if ($drt -ne 200) { fail "/api/v1/did-rank/travelers expected 200 got $drt" }
ok "/api/v1/did-rank/travelers 200"
try {
    $rtw = Invoke-WebRequest -Uri "$Base/api/v1/did-rank/travelers?period=week" -UseBasicParsing -TimeoutSec 5
} catch { $rtw = $null }
if (-not $rtw -or $rtw.StatusCode -ne 200) {
    fail "did-rank travelers ?period=week expected 200, got $(if ($rtw) { $rtw.StatusCode } else { 'no response' })"
}
try {
    $jtw = $rtw.Content | ConvertFrom-Json
} catch { fail "did-rank travelers week JSON parse failed: $_" }
if ($jtw.rank_basis -ne "tourist_completed_orders_in_window") {
...
} catch { fail "community stats JSON parse failed: $_" }
...
ok "community/stats/posts-by-tag JSON shape"

Write-Host ""
Write-Host "smoke-api-public-routes: OK ($Base — health + meta + metrics + meta.indexer.checkpoint + discover + discover?limit=1 + did-rank/itineraries + did-rank/guides + did-rank/travelers + community/stats/posts-by-tag; /meta order_chain_sync_status 722/723/724/725 + finality_discipline 726 + indexer 727 + indexer.memory 757 + indexer.checkpoint 758 top_keys + database 760 + rate_limits.guide_upload 761 + chain.rule 762/763/765/766/767/768/769/770/771/772/773/774/775/776/777/778/779/780/781/782/783/784/785/786/787/788/789/790/791/792/793/794/795/796/797/798/799/800/801/802/803/804/805/806 + build 730 + dual_write 732 + strict_mode 731 + ssot 733 + admin_exports 734 + chargeback_policy 735 + authority 736 + pause 737 + evidence 738 + order_messages 739 + reviews 740 + dispute_open 741 + dispute_resolve 742 + itineraries 743 + orders 744 + discover 745 + product_countries 746 + did_rank 747 + product_roles 748 + auth.registration 749 + auth 750 + chain 729 + chain.contracts 759 top_keys when object + root 728 lock when jq)"
