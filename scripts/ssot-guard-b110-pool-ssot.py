...
Static checks:
  • Root **country_pool*** / **treasury_pool*** / **treasury_erc20_pool*** `m.insert("…".to_string(), …)`
    only in `crates/api/src/routes/governance.rs` (GET …/governance/pool merge helpers + tests).
...
GOV = API_SRC / "routes" / "governance.rs"
...
def slice_fn_rust(text: str, sig: str) -> str:
    """Slice `fn name(...) { ... }` or `async fn ...` by brace depth from first `{` after sig."""
    start = text.find(sig)
    if start < 0:
        fail(f"missing {sig!r} in routes/governance.rs")
...
def main() -> None:
        ...
        for m in INSERT_KEY_RE.finditer(txt):
            if path.resolve() != allowed:
                ...
                fail(
                    ...
                    f"routes/governance.rs (seen in {rel})"
                )

    g = GOV.read_text(encoding="utf-8")

    cty = slice_fn_rust(g, "fn merge_country_pool_chain_ssot_fields")
    tr = slice_fn_rust(g, "fn merge_treasury_pool_chain_ssot_fields")
    er = slice_fn_rust(g, "fn merge_treasury_erc20_pool_chain_ssot_fields")
    ...
    pos = g.find('"pool_balance": hex')
    if pos < 0:
        fail('governance.rs must contain "pool_balance": hex (chain SSOT branch)')
    chunk = g[max(0, pos - 120) : pos + 420]
    ...
    agg = slice_fn_rust(g, "fn build_fee_pool_aggregate_body")
