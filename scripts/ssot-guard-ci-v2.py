...
human hints (see scripts/templates/SSOT_GUARD_FAILURE_REPORT.md).

Run from repo root: python3 scripts/ssot-guard-ci-v2.py
"""
...
REMEDIATION = [
    "Review scripts/templates/SSOT_GUARD_FAILURE_REPORT.md for report field meanings.",
...
def main() -> int:
    ...
    for script_id, relpath in (
        ("static_b097_escrow", "scripts/ssot-guard-escrow-orders-detail.py"),
        ("static_b110_pool", "scripts/ssot-guard-b110-pool-ssot.py"),
        ("response_contract_snapshots", "scripts/ssot-guard-response-contract.py"),
    ):
