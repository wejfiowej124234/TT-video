#!/usr/bin/env bash
# B-424 · **`GET /meta`** **↔** **`GET …/admin/observability/overview`**：**Leg 1** **`build`** **深相等** **（** **`/meta.build`** **vs** **`…/overview.meta.build`** **）** **；** **Leg 2** **`indexer`** **子树** **深相等** **（** **`/meta.indexer`** **vs** **`…/overview.indexer`** **）** **。**
#
# 环境：**`ADMIN_BEARER_TOKEN`** **、** **`jq`** **、** **`curl`** **；** **`API_BASE_URL`** **默认** **`http://127.0.0.1:8080`** **。**
#
# 用法（仓库根）：**`bash scripts/check-meta-overview-deepeq-gate.sh`** **[** **`--json`** **]**
set -euo pipefail
...
JSON=false
for a in "$@"; do
  if [[ "$a" == "--json" ]]; then JSON=true; else echo "check-meta-overview-deepeq-gate: unknown option: $a" >&2; exit 1; fi
done
