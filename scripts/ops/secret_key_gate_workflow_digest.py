#!/usr/bin/env python3
# B-355: CI leg — machine digest of `.github/workflows/secret-key-gate.yml` (230 / admin secret fail-closed gate).
# Pre-commit / third-party secret scanners are not in-repo yet; this script does not claim to inventory them.
...
ANCHOR = "CI-SECRET-KEY-GATE-WORKFLOW-DIGEST-V1"
IMPLEMENTATION_TT = "TT-B355-SECRETS-SCAN-PRECOMMIT-SPEC-001"
MOTHER_TABLE = "B-355"
...
def _artifact_paths(yml: str) -> list[str]:
    """Lines under upload-artifact `path: |` for secret-key-evidence."""
    marker = "name: secret-key-evidence"
    idx = yml.find(marker)
    if idx < 0:
        return []
    sub = yml[idx:]
...
def build_digest(yml_text: str) -> dict[str, Any]:
    ...
    return {
        ...
        "workflow_file": ".github/workflows/secret-key-gate.yml",
        "workflow_name": "Secret Key Gate",
        "job_id": "secret-key",
...
def verify(repo: Path) -> tuple[bool, str]:
    wf = repo / ".github" / "workflows" / "secret-key-gate.yml"
    if not wf.is_file():
        return False, f"missing {wf}"
    text = wf.read_text(encoding="utf-8")
    if "Secret Key Gate" not in text:
        return False, "unexpected workflow header"
    if not _RE_EVIDENCE_DIR.search(text):
        return False, "missing EVIDENCE_DIR echo"
    d = build_digest(text)
    if d["route_checks_total"] != 3:
        return False, f"route_checks_total expected 3 got {d['route_checks_total']}"
    if d["policy_checks_total"] != 2:
        return False, f"policy_checks_total expected 2 got {d['policy_checks_total']}"
    if d["if_check_anchor_count"] != 5:
        return (
            False,
            f"if check_anchor count expected 5 got {d['if_check_anchor_count']}",
        )
    paths = d["upload_artifact"]["paths_under_go_wave"]
    expected = [
        "secret_key_routes.log",
        "secret_key_gate_summary.json",
        "secret_key_gate_metrics.json",
        "secret_rotation_sampling_report.json",
        "secret_key_evidence_field_report.json",
    ]
    if paths != expected:
        return False, f"artifact paths drift: {paths!r}"
    return True, f"OK ({ANCHOR}; routes=3 policies=2 anchors=5; {IMPLEMENTATION_TT})"
...
def cmd_verify(_: argparse.Namespace) -> int:
    ...
    if not ok:
        print(f"secret_key_gate_workflow_digest: FAIL: {msg}", file=sys.stderr)
        return 1
    print(f"secret_key_gate_workflow_digest: {msg}", file=sys.stderr)
    return 0


def cmd_self_test(_: argparse.Namespace) -> int:
    assert verify(repo_root_from_here())[0]
    print("secret_key_gate_workflow_digest self-test OK", file=sys.stderr)
    return 0


def main() -> int:
    ap = argparse.ArgumentParser(
        description="B-355 CI leg: digest secret-key-gate workflow (read-only).",
    )
