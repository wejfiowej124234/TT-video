...
# 依赖：curl、jq
#
# **stdout** 为 **`traveltrust.ops_artifact.v1`**：**`artifact_schema_id`/`artifact_schema_version`/`artifact_version`/`artifact_type`:`snapshot_public`/`captured_at`/`epic_task_id`/`provenance`/`api_context`/`payload`**；**`payload`** 内合并 **`/health`**、**`/meta`** 及可选 admin/internal 段，且含 **`snapshot_provenance`**：**`script`**（固定 **`indexer-public-snapshot.sh`**）、**`script_semver`**（本常量）、**`host_git_commit`**、**`host_git_branch`**、**`host_repo_dirty`**（语义同前）。**形状或语义变时须 bump `script_semver`** 并与 **04 §3.4**/**110 §3.1.2**/**`indexer-reconcile-gate`** 同批。
...
# 1.4.0 — stdout 外层 **`traveltrust.ops_artifact.v1`**（**`artifact_type`:`snapshot_public`**）；**`payload`** 内为原合并体（含 **`snapshot_provenance`**）。
INDEXER_PUBLIC_SNAPSHOT_SCRIPT_SEMVER="1.4.0"
...
internal_invoked_json='false'
if [[ -n "${INTERNAL_API_SECRET:-}" ]]; then
  internal_invoked_json='true'
fi
...
jq -n \
  ...
  --arg host_branch "$HOST_GIT_BRANCH" \
  --argjson host_repo_dirty "$HOST_REPO_DIRTY_JSON" \
  --argjson internal_invoked "$internal_invoked_json" \
  '{
    artifact_schema_id: "traveltrust.ops_artifact.v1",
    artifact_schema_version: $ver,
    artifact_version: $av,
    artifact_type: "snapshot_public",
    captured_at: (now | todate),
    epic_task_id: "Epic-D-D02",
    provenance: {
      script: "indexer-public-snapshot.sh",
      script_semver: $snap_semver,
      host_git_commit: (if ($host_commit | length) > 0 then $host_commit else null end),
    ...
    api_context: {
      ...
      chain_id: null,
      internal_invoked: $internal_invoked
    },
    payload: $payload
  }'
