#!/usr/bin/env node
/**
 * Los Angeles Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-los-angeles-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LOS_ANGELES-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LOS_ANGELES-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/los-angeles');

const LOS_ANGELES = getCityPilot('洛杉矶');
const POI_COUNT = LOS_ANGELES.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LOS_ANGELES-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of LOS_ANGELES.matrix_ids) {
    const poi = LOS_ANGELES.pois[LOS_ANGELES.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '洛杉矶');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent('洛杉矶')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '洛杉矶', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(LOS_ANGELES);
    ...
    const shotPath = path.join(SHOT_DIR, `los-angeles-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < LOS_ANGELES.matrix_ids.length; i++) {
      const matrixId = LOS_ANGELES.matrix_ids[i];
      const poi = LOS_ANGELES.pois[i];
...
function checkNoBacklogUnlock() {
  const open = LOS_ANGELES.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = LOS_ANGELES.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Los Angeles Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_LOS_ANGELES_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '洛杉矶', city_en: 'Los Angeles', country_iso: 'US', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_LOS_ANGELES_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_LOS_ANGELES_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_LOS_ANGELES_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_LOS_ANGELES_CONTENT_QA_EXIT}`);
