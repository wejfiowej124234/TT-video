#!/usr/bin/env node
/**
 * POI City Closure Evidence · TH · 曼谷 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-bangkok-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('曼谷');
...
async function main() {
  ...
  const evidenceRows = BANGKOK.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent(BANGKOK.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of BANGKOK.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || BANGKOK.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, BANGKOK.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === BANGKOK.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= BANGKOK.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${BANGKOK.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, BANGKOK.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: BANGKOK.country_iso,
      country_zh: BANGKOK.country_zh,
      city_zh: BANGKOK.city_zh,
      city_en: BANGKOK.city_en,
    },
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === BANGKOK.country_iso && c.city_zh === BANGKOK.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: BANGKOK.matrix_ids.length,
        closure_evidence: BANGKOK.closure_latest,
        [BANGKOK.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.bangkok_city_closure = { status: 'CLOSED', evidence: BANGKOK.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_TH_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'BANGKOK_CLOSED';
    kickoff[BANGKOK.closure_key] = 'CLOSED';
