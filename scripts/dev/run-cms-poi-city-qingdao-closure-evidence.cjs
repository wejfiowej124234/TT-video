#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 青岛 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-qingdao-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('青岛');
...
async function main() {
  ...
  const evidenceRows = QINGDAO.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(QINGDAO.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of QINGDAO.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || QINGDAO.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, QINGDAO.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === QINGDAO.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= QINGDAO.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${QINGDAO.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, QINGDAO.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: QINGDAO.country_iso,
      country_zh: QINGDAO.country_zh,
      city_zh: QINGDAO.city_zh,
      city_en: QINGDAO.city_en,
    },
    poi_count: QINGDAO.matrix_ids.length,
    matrix_ids: QINGDAO.matrix_ids,
    pois: QINGDAO.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === QINGDAO.country_iso && c.city_zh === QINGDAO.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: QINGDAO.matrix_ids.length,
        closure_evidence: QINGDAO.closure_latest,
        [QINGDAO.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: QINGDAO.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'QINGDAO_CLOSED';
    kickoff[QINGDAO.closure_key] = 'CLOSED';
