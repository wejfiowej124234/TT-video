#!/usr/bin/env node
/**
 * Australia L5 Content QA · ② staging · JP + KR + TH + SG + FR 五模板 · 不调整标准
 *
 *   node scripts/dev/run-cms-australia-content-qa.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AUSTRALIA-CONTENT-QA-LATEST.json');
...
function main() {
  ...
  console.log(`TT_CMS_AU_CONTENT_QA: ${report.TT_CMS_AU_CONTENT_QA}`);
  console.log(`TT_CMS_AU_COUNTRY: ${report.TT_CMS_AU_COUNTRY}`);
