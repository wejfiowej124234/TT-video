#!/usr/bin/env node
/**
 * POI City Closure Evidence · JP · 京都 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-kyoto-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('京都');
...
async function main() {
  if (!NEXT_CITY) throw new Error('no next city after Kyoto');
  ...
  const evidenceRows = KYOTO.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent(KYOTO.city_zh)}&limit=50`;
  ...
  const loopPath = path.join(ROOT, KYOTO.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === KYOTO.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= KYOTO.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${KYOTO.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, KYOTO.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: KYOTO.country_iso,
      country_zh: '日本',
      city_zh: KYOTO.city_zh,
      city_en: KYOTO.city_en,
    },
    poi_count: KYOTO.matrix_ids.length,
    matrix_ids: KYOTO.matrix_ids,
    pois: KYOTO.pois,
    checks,
    ...
    catalog_probe: {
      ...
      legacy_values: KYOTO.pois.map((p) => ({ legacy_value: p, ok: Boolean(catalogByLegacy[p]?.image_url) })),
    },
    closed_loop_evidence: KYOTO.closed_loop_latest,
    next_city: {
  ...
  fs.writeFileSync(path.join(stampedDir, 'CMS-POI-CITY-KYOTO-CLOSURE.json'), JSON.stringify(closure, null, 2) + '\n');

  const prev = readRegistry() || {};
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === KYOTO.country_iso && c.city_zh === KYOTO.city_zh),
  );
  const registry = {
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: KYOTO.matrix_ids.length,
        closure_evidence: KYOTO.closure_latest,
        [KYOTO.closure_key]: 'CLOSED',
      },
    ],
    active_city: {
  ...
  if (kickoff) {
    ...
    kickoff.kyoto_city_closure = { status: 'CLOSED', evidence: KYOTO.closure_latest };
    ...
    kickoff[KYOTO.closure_key] = 'CLOSED';
  ...
  if (pilotWave) {
    ...
    pilotWave[KYOTO.closure_key] = 'CLOSED';
