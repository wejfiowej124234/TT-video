#!/usr/bin/env bash
# ① Guide Workbench L5 全页证据：vitest 绿集 + Playwright（收件箱双角色 + 全页探针）
#
# 用法（仓库根）：
...
EVID="$ROOT/frontend/evidence/GO_local_guide_workbench_l5"
mkdir -p "$EVID"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_LOG="$EVID/GUIDE-WORKBENCH-L5-${STAMP}.log"
...
start_api_bg() {
  echo "record-guide-workbench-l5: starting API (P3_CHAIN_OFF=${P3_CHAIN_OFF})…"
  (
    cd "$ROOT"
  ...
  ) >"$EVID/.api-bg-gwb-full-${STAMP}.log" 2>&1 &
  echo $! >"$EVID/.api-bg-gwb-full-${STAMP}.pid"
}

cleanup_api_bg() {
  local pidfile="$EVID/.api-bg-gwb-full-${STAMP}.pid"
  [[ -f "$pidfile" ]] || return 0
  local pid
  pid="$(cat "$pidfile")"
...
if api_up && [[ "${TRAVELTRUST_EVIDENCE_REUSE_API:-0}" == "1" ]]; then
  echo "record-guide-workbench-l5: reusing API on :${API_PORT} (TRAVELTRUST_EVIDENCE_REUSE_API=1)"
elif api_up; then
  echo "record-guide-workbench-l5: restarting API on :${API_PORT} (API_RATE_LIMIT_PER_MINUTE=0)…"
  kill_api_on_port
fi
if ! api_up; then
  start_api_bg
...
{
  echo "TT_GUIDE_WORKBENCH_L5_EVIDENCE: START ${STAMP}"
  echo "phase: ① local only"
  echo "api: ${API_BASE}"
  echo "chain: tourist+guide seed (guide@test.com)"

  echo ""
  echo "== Step A: vitest Guide Workbench L5 contracts =="
  cd "$ROOT/frontend"
  npx vitest run \
  ...
  echo ""
  echo "== Step B: Playwright dual-role inbox corridor =="
  ...
  node ./scripts/run-e2e-default.mjs --project=chromium e2e/guide-workbench-inbox-l5.spec.ts

  echo ""
  echo "== Step C: Playwright full-page L5 probes =="
  node ./scripts/run-e2e-default.mjs --project=chromium e2e/guide-workbench-full-l5.spec.ts

  echo ""
  echo "TT_GUIDE_WORKBENCH_L5_EVIDENCE: OK ${STAMP}"
  echo "TT_GUIDE_WORKBENCH_L5_SUMMARY: exit=0 phase=① vitest+playwright inbox_dual_role+full_page_probes"
} 2>&1 | tee "$RUN_LOG"

grep -q "TT_GUIDE_WORKBENCH_L5_EVIDENCE: OK" "$RUN_LOG" || {
