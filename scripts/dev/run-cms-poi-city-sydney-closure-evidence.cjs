#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 悉尼 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-sydney-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('悉尼');
...
async function main() {
  ...
  const evidenceRows = SYDNEY.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent(SYDNEY.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SYDNEY.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SYDNEY.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SYDNEY.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SYDNEY.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SYDNEY.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SYDNEY.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SYDNEY.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SYDNEY.country_iso,
      country_zh: SYDNEY.country_zh,
      city_zh: SYDNEY.city_zh,
      city_en: SYDNEY.city_en,
    },
    poi_count: SYDNEY.matrix_ids.length,
    matrix_ids: SYDNEY.matrix_ids,
    pois: SYDNEY.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SYDNEY.country_iso && c.city_zh === SYDNEY.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SYDNEY.matrix_ids.length,
        closure_evidence: SYDNEY.closure_latest,
        [SYDNEY.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.sydney_city_closure = { status: 'CLOSED', evidence: SYDNEY.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_AU_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SYDNEY_CLOSED';
    kickoff[SYDNEY.closure_key] = 'CLOSED';
