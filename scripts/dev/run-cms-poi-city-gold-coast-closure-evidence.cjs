#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 黄金海岸 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-gold-coast-closure-evidence.cjs
 */
const fs = require('fs');
...
const US_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-POI-WAVE-KICKOFF-LATEST.json');
const US_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-POI-PILOT-WAVE-LATEST.json');
...
const NEXT_CITY = nextCityPilot('黄金海岸');
...
async function main() {
  ...
  const evidenceRows = GOLD_COAST.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent(GOLD_COAST.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of GOLD_COAST.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || GOLD_COAST.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, GOLD_COAST.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === GOLD_COAST.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= GOLD_COAST.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${GOLD_COAST.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, GOLD_COAST.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: GOLD_COAST.country_iso,
      country_zh: GOLD_COAST.country_zh,
      city_zh: GOLD_COAST.city_zh,
      city_en: GOLD_COAST.city_en,
    },
    poi_count: GOLD_COAST.matrix_ids.length,
    matrix_ids: GOLD_COAST.matrix_ids,
    pois: GOLD_COAST.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === GOLD_COAST.country_iso && c.city_zh === GOLD_COAST.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: GOLD_COAST.matrix_ids.length,
        closure_evidence: GOLD_COAST.closure_latest,
        [GOLD_COAST.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.gold_coast_city_closure = { status: 'CLOSED', evidence: GOLD_COAST.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_AU_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'GOLD_COAST_CLOSED';
    kickoff[GOLD_COAST.closure_key] = 'CLOSED';
