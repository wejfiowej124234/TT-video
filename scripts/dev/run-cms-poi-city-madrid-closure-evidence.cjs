#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 马德里 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-madrid-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('马德里');
...
async function main() {
  ...
  for (const matrixId of MADRID.matrix_ids) {
  ...
  const evidenceRows = MADRID.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent(MADRID.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of MADRID.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || MADRID.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, MADRID.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === MADRID.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= MADRID.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${MADRID.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, MADRID.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: MADRID.country_iso,
      country_zh: MADRID.country_zh,
      city_zh: MADRID.city_zh,
      city_en: MADRID.city_en,
    },
    poi_count: MADRID.matrix_ids.length,
    matrix_ids: MADRID.matrix_ids,
    pois: MADRID.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === MADRID.country_iso && c.city_zh === MADRID.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: MADRID.matrix_ids.length,
        closure_evidence: MADRID.closure_latest,
        [MADRID.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.madrid_city_closure = { status: 'CLOSED', evidence: MADRID.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_ES_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'MADRID_CLOSED';
    kickoff[MADRID.closure_key] = 'CLOSED';
