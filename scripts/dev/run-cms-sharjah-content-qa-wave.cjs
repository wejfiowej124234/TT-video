#!/usr/bin/env node
/**
 * Sharjah · Execution + Content QA Wave · JP + KR 双模板同源
 *
 *   TRAVELTRUST_STAGING_RC_BASELINE_OVERRIDE=1 \
 *   API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-sharjah-content-qa-wave.cjs
 *
...
assertStagingBaselineMutationAuthorized('cms_sharjah_content_qa_wave');

const ROOT = path.join(__dirname, '../..');
const API = (process.env.API || 'https://tt-api-staging.fly.dev').replace(/\/$/, '');
const SHARJAH = getCityPilot('沙迦');

const SHARJAH_HEROES = SHARJAH.hero_files;

async function syncAllHeroes() {
  const unique = [...new Set(Object.values(SHARJAH_HEROES))];
  console.log(`\n===== Pre-sync staging heroes (${unique.length} unique / ${SHARJAH.matrix_ids.length} POI) =====`);
...
async function runExitCheckWithRetry(maxAttempts = 12) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      runNode('scripts/dev/run-cms-poi-city-sharjah-content-qa-exit-check.cjs');
      return;
    } catch {
      if (attempt === maxAttempts) throw new Error('Sharjah exit check failed after retries');
...
async function main() {
  console.log('TT_CMS_SHARJAH_CONTENT_QA_WAVE: START');

  console.log('\n===== Catalog Build · 沙迦 =====');
  runNode('scripts/dev/run-cms-phase1-poi-pilot-catalog-build.cjs', '--city-zh 沙迦');
  ...
  for (const matrixId of SHARJAH.matrix_ids) {
    ...
    runNode(
      ...
      `--city-zh 沙迦 --matrix-id ${matrixId} --poi "${poi}"`,
    );
  }

  console.log('\n===== Execution closed loop · 沙迦 8/8 =====');
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs', '--city-zh 沙迦 --skip-refresh');

  console.log('\n===== Execution CLOSED · 沙迦 =====');
  runNode('scripts/dev/run-cms-poi-city-sharjah-closure-evidence.cjs');

  runNode('scripts/dev/run-cms-poi-city-sharjah-content-qa-closure-evidence.cjs');
  await runExitCheckWithRetry();

  console.log('\nTT_CMS_SHARJAH_CONTENT_QA_WAVE: DONE');
}
