#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 普吉（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '普吉');
  if (!cityQa) throw new Error('普吉 city QA missing');

  const checks = {
    assets_locked_9_9: lock.locked === PHUKET.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_PHUKET_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'TH', city_zh: '普吉', city_en: 'Phuket', poi_count: PHUKET.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CLOSURE-LATEST.json',
    ...
    TT_CMS_POI_CITY_PHUKET_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_PHUKET_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${PHUKET.matrix_ids.length}`);
