#!/usr/bin/env node
/**
 * POI City Closure Evidence · KR · 首尔 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-seoul-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('首尔');
...
async function main() {
  ...
  const evidenceRows = SEOUL.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent(SEOUL.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SEOUL.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SEOUL.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SEOUL.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SEOUL.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SEOUL.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SEOUL.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SEOUL.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SEOUL.country_iso,
      country_zh: SEOUL.country_zh,
      city_zh: SEOUL.city_zh,
      city_en: SEOUL.city_en,
    },
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SEOUL.country_iso && c.city_zh === SEOUL.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SEOUL.matrix_ids.length,
        closure_evidence: SEOUL.closure_latest,
        [SEOUL.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.seoul_city_closure = { status: 'CLOSED', evidence: SEOUL.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: 'POI-KR-CITY-BUSAN',
      ...
      : null;
    kickoff.TT_CMS_KR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SEOUL_CLOSED';
    kickoff[SEOUL.closure_key] = 'CLOSED';
