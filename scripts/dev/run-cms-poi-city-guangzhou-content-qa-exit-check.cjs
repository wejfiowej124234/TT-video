#!/usr/bin/env node
/**
 * Guangzhou Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-guangzhou-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/guangzhou');

const GUANGZHOU = getCityPilot('广州');
const POI_COUNT = GUANGZHOU.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  const issues = [];
  const rows = [];
  for (const matrixId of GUANGZHOU.matrix_ids) {
    const poi = GUANGZHOU.pois[GUANGZHOU.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '广州');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent('广州')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '中国' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '广州', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(GUANGZHOU);
    ...
    const shotPath = path.join(SHOT_DIR, `guangzhou-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < GUANGZHOU.matrix_ids.length; i++) {
      const matrixId = GUANGZHOU.matrix_ids[i];
      const poi = GUANGZHOU.pois[i];
...
function checkNoBacklogUnlock() {
  const open = GUANGZHOU.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = GUANGZHOU.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Guangzhou Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '广州', city_en: 'Guangzhou', country_iso: 'CN', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_CN_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA_EXIT}`);
