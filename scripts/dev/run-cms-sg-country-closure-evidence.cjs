#!/usr/bin/env node
/**
 * Singapore Country CLOSED · national_runtime + 新加坡 Content QA 汇总
 *
 * 前置：run-cms-sg-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-sg-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildSingaporeContentQa, SG_CITY_DISPLAY_ORDER } = require('./lib/cms-singapore-content-qa.cjs');
const { EVIDENCE, readJson, SG_CITIES, TH_CITIES, KR_CITIES, JP_CITIES } = require('./lib/cms-sg-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return SG_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_SG_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_SG_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_SG_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_SG_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }
  ...
  if (report.summary.cities_execution_pass !== 1) issues.push(`execution ${report.summary.cities_execution_pass}/1`);
  if (report.summary.cities_content_qa_closed !== 1) issues.push(`content_qa_closed ${report.summary.cities_content_qa_closed}/1`);
  ...
  if (report.country?.TT_CMS_SG_COUNTRY !== 'CLOSED') {
  ...
  const closure = {
    schema: 'traveltrust.cms_sg_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    singapore_content_qa_ssot: 'evidence/GO_cms_operation/CMS-SINGAPORE-CONTENT-QA-LATEST.json',
    ...
    checks: {
      singapore_city_execution_closed: true,
      singapore_city_content_qa_closed: true,
      singapore_city_exit_checks: true,
  ...
  const refreshed = buildSingaporeContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_SG_COUNTRY: CLOSED');
  console.log('TT_CMS_SG_CONTENT_QA: CLOSED');
