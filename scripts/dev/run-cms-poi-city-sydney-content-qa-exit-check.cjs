#!/usr/bin/env node
/**
 * Sydney Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-sydney-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SYDNEY-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SYDNEY-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/sydney');

const SYDNEY = getCityPilot('悉尼');
const POI_COUNT = SYDNEY.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SYDNEY-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '悉尼');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent('悉尼')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '悉尼', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(SYDNEY);
    ...
    const shotPath = path.join(SHOT_DIR, `sydney-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < SYDNEY.matrix_ids.length; i++) {
      const matrixId = SYDNEY.matrix_ids[i];
      const poi = SYDNEY.pois[i];
...
function checkNoBacklogUnlock() {
  const open = SYDNEY.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = SYDNEY.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Sydney Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_SYDNEY_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '悉尼', city_en: 'Sydney', country_iso: 'AU', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_SYDNEY_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_SYDNEY_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_SYDNEY_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_SYDNEY_CONTENT_QA_EXIT}`);
