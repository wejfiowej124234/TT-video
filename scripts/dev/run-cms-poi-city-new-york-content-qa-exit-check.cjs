#!/usr/bin/env node
/**
 * New York Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-new-york-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NEW_YORK-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NEW_YORK-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/new-york');

const NEW_YORK = getCityPilot('纽约');
const POI_COUNT = NEW_YORK.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NEW_YORK-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of NEW_YORK.matrix_ids) {
    const poi = NEW_YORK.pois[NEW_YORK.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '纽约');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent('纽约')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '纽约', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(NEW_YORK);
    ...
    const shotPath = path.join(SHOT_DIR, `new-york-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < NEW_YORK.matrix_ids.length; i++) {
      const matrixId = NEW_YORK.matrix_ids[i];
      const poi = NEW_YORK.pois[i];
...
function checkNoBacklogUnlock() {
  const open = NEW_YORK.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = NEW_YORK.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# New York Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_NEW_YORK_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '纽约', city_en: 'New York', country_iso: 'US', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_NEW_YORK_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_NEW_YORK_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_NEW_YORK_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_NEW_YORK_CONTENT_QA_EXIT}`);
