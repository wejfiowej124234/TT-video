#!/usr/bin/env node
/**
 * France Country CLOSED · national_runtime + 三城 Content QA 汇总
 *
 * 前置：run-cms-fr-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH/SG LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-fr-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildFranceContentQa, FR_CITY_DISPLAY_ORDER } = require('./lib/cms-france-content-qa.cjs');
const {
  EVIDENCE,
  readJson,
  FR_CITIES,
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return FR_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_FR_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_FR_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_FR_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_FR_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }
  ...
  if (report.country?.TT_CMS_FR_COUNTRY !== 'CLOSED') {
  ...
  const closure = {
    schema: 'traveltrust.cms_fr_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    france_content_qa_ssot: 'evidence/GO_cms_operation/CMS-FRANCE-CONTENT-QA-LATEST.json',
    ...
    checks: {
      ...
      fr_country_runtime_pass: true,
      content_accuracy_100: true,
      cross_region_0: report.country.cross_region_total === 0,
    },
    TT_CMS_FR_COUNTRY: 'CLOSED',
    TT_CMS_FR_CONTENT_QA: 'CLOSED',
    verdict: 'FR_COUNTRY_CLOSED',
  };
  ...
  const refreshed = buildFranceContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_FR_COUNTRY: CLOSED');
  console.log('TT_CMS_FR_CONTENT_QA: CLOSED');
