#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 大理 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-dali-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('大理');
...
async function main() {
  ...
  const evidenceRows = DALI.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(DALI.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of DALI.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || DALI.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, DALI.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === DALI.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= DALI.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${DALI.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, DALI.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: DALI.country_iso,
      country_zh: DALI.country_zh,
      city_zh: DALI.city_zh,
      city_en: DALI.city_en,
    },
    poi_count: DALI.matrix_ids.length,
    matrix_ids: DALI.matrix_ids,
    pois: DALI.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === DALI.country_iso && c.city_zh === DALI.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: DALI.matrix_ids.length,
        closure_evidence: DALI.closure_latest,
        [DALI.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: DALI.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'DALI_CLOSED';
    kickoff[DALI.closure_key] = 'CLOSED';
