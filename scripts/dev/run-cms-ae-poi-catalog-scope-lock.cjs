#!/usr/bin/env node
/**
 * AE POI Catalog Scope Lock · Country → City → POI 分母 · ② staging
 *
 * 前置: TT_CMS_JP/KR/TH/SG/FR/US_COUNTRY: CLOSED
 * 不得修改 JP 41 · KR 31 · TH 28 · SG 10 · FR 24 · US 33 · AU 24 · ES 24 LOCK · Content QA 标准
 */
const fs = require('fs');
const path = require('path');
const { buildPoiCatalogScope } = require('./lib/cms-poi-catalog-scope.cjs');
const { buildAePilotWaves } = require('./lib/cms-ae-poi-pilot-waves.cjs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-POI-CATALOG-SCOPE-LOCK-LATEST.json');
const PILOT_WAVE_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-POI-PILOT-WAVE-LATEST.json');
const REGISTRY_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CLOSURE-REGISTRY-LATEST.json');
const POI_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-POI-WAVE-KICKOFF-LATEST.json');
const LOCK_REG = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-ASSET-LOCK-REGISTRY-LATEST.json');

async function probeAbuDhabiCatalog(api) {
  const r = await request(`${api}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent('阿布扎比')}&limit=50`);
  const items = r.json?.items || [];
  return { http: r.status, count: items.length, catalog_empty: items.length === 0 };
}

function assertCountryClosed(p, key) {
  if (!fs.existsSync(p)) throw new Error(`${key} closure missing — abort AE kickoff`);
  const doc = JSON.parse(fs.readFileSync(p, 'utf8'));
  if (doc[key] !== 'CLOSED') throw new Error(`${key}=${doc[key] || 'MISSING'} — abort AE kickoff`);
}
    ...
    JSON.stringify(
      {
        ...
        active_city: { country_iso: 'AE', city_zh: '阿布扎比', city_en: 'Abu Dhabi', status: 'ACTIVE' },
        active_wave: pilot.active_wave,
        template_countries: ['JP', 'KR', 'TH', 'SG', 'FR', 'US', 'AU', 'ES'],
        ae_scope_lock: 'evidence/GO_cms_operation/CMS-AE-POI-CATALOG-SCOPE-LOCK-LATEST.json',
...
async function main() {
  ...
  const aeRows = scope.rows.filter((r) => r.country_iso === 'AE');
  const aeByCity = {};
  for (const row of aeRows) {
    aeByCity[row.city_zh] = aeByCity[row.city_zh] || { attraction: 0, food: 0, total: 0, matrix_ids: [] };
    aeByCity[row.city_zh][row.poi_type === 'food' ? 'food' : 'attraction'] += 1;
    aeByCity[row.city_zh].total += 1;
    aeByCity[row.city_zh].matrix_ids.push(row.matrix_id);
  }

  const pilot = buildAePilotWaves(scope);
  const active = pilot.active_catalog_build;
  const auh = getCityPilot('阿布扎比');
  const catalog = await probeAbuDhabiCatalog(API);

  const report = {
    schema: 'traveltrust.cms_ae_poi_catalog_scope_lock.v1',
    ...
    country: { country_iso: 'AE', country_zh: '阿联酋' },
    ae_denominator: {
      cities: Object.keys(aeByCity).sort(),
      city_breakdown: aeByCity,
      poi_total: aeRows.length,
      attraction: aeRows.filter((r) => r.poi_type === 'attraction').length,
      food: aeRows.filter((r) => r.poi_type === 'food').length,
    },
    pilot_wave: pilot,
    ...
    abu_dhabi_pilot: {
      ...
      acceptance: '阿布扎比 CLOSED',
    },
    catalog_probe: catalog,
    workflow: ['AE Scope Lock', '阿布扎比 → 迪拜 → 沙迦', 'AE Country Runtime', 'TT_CMS_AE_COUNTRY: CLOSED'],
    ...
    TT_CMS_AE_POI_CATALOG_SCOPE_LOCK: 'FROZEN',
    TT_CMS_AE_POI_DENOMINATOR_TOTAL: aeRows.length,
    TT_CMS_AE_POI_PILOT_ACCEPTANCE: '阿布扎比 CLOSED',
    TT_CMS_AE_POI_NEXT_STAGE: 'CATALOG_BUILD',
    uae_status: 'ACTIVE',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(report, null, 2) + '\n');
  fs.writeFileSync(PILOT_WAVE_LATEST, JSON.stringify({ schema: 'traveltrust.cms_ae_poi_pilot_wave.v1', stamp_utc: stamp, ...pilot }, null, 2) + '\n');
  writeRegistry(stamp, pilot);
  fs.writeFileSync(
    POI_KICKOFF,
    JSON.stringify(
      {
        schema: 'traveltrust.cms_ae_poi_wave_kickoff.v1',
        stamp_utc: stamp,
        country_iso: 'AE',
        status: 'SCOPE_LOCKED',
        active_city: '阿布扎比',
        template: 'JP + KR + TH + SG + FR + US + AU + ES Golden Template',
        TT_CMS_AE_POI_WAVE: 'SCOPE_LOCKED_CATALOG_BUILD_NEXT',
      },
      null,
      2,
    ) + '\n',
  ...
  const hierarchy = buildCmsOpsHierarchy({ scope: { ...scope, pilot_waves: pilot, ae_active: true } });
  writeHierarchyLatest(hierarchy, stamp.replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z'));

  const { buildUAEContentQa } = require('./lib/cms-uae-content-qa.cjs');
  fs.writeFileSync(
    path.join(ROOT, 'evidence/GO_cms_operation/CMS-UAE-CONTENT-QA-LATEST.json'),
    JSON.stringify(buildUAEContentQa({ stamp_utc: stamp, skip_template_guard: true }), null, 2) + '\n',
  );

  console.log(`TT_CMS_AE_POI_CATALOG_SCOPE_LOCK: ${report.TT_CMS_AE_POI_CATALOG_SCOPE_LOCK}`);
  console.log(`TT_CMS_AE_POI_DENOMINATOR_TOTAL: ${report.TT_CMS_AE_POI_DENOMINATOR_TOTAL} (3 cities · pilot 阿布扎比 ${active.poi_count})`);
  console.log(`JP 41 · KR 31 · TH 28 · SG 10 · FR 24 · US 33 · AU 24 · ES 24 LOCK unchanged`);
  console.log(`Evidence: ${OUT_LATEST}`);
  console.log('\nNext: node scripts/dev/run-cms-abu-dhabi-content-qa-wave.cjs');
}
