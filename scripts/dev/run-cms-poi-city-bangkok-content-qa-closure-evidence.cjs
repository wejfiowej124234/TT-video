#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 曼谷（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '曼谷');
  if (!cityQa) throw new Error('曼谷 city QA missing');

  const checks = {
    assets_locked_10_10: lock.locked === BANGKOK.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_BANGKOK_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'TH', city_zh: '曼谷', city_en: 'Bangkok', poi_count: BANGKOK.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CLOSURE-LATEST.json',
  ...
  console.log('TT_CMS_POI_CITY_BANGKOK_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${BANGKOK.matrix_ids.length}`);
