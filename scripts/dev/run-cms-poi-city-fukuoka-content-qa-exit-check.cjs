#!/usr/bin/env node
/**
 * Fukuoka Content QA Exit Check · Osaka Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-fukuoka-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-FUKUOKA-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-FUKUOKA-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/fukuoka');

const FUKUOKA = getCityPilot('福冈');
const POI_COUNT = FUKUOKA.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-FUKUOKA-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '福冈');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent('福冈')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '福冈', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = FUKUOKA.pois.slice(0, 4);
    const foodPois = FUKUOKA.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `fukuoka-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < FUKUOKA.matrix_ids.length; i++) {
      const matrixId = FUKUOKA.matrix_ids[i];
      const poi = FUKUOKA.pois[i];
      const expectedFn = catalogByMatrix[matrixId] || getAsset(matrixId).hero_file;
      // Duplicate legacy names (屋台 ×2): match by hero filename, not POI label.
      let match = domImages.find((img) => !usedSrc.has(img.src) && filename(img.src) === expectedFn);
      if (!match) {
...
function checkNoBacklogUnlock() {
  const open = FUKUOKA.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = FUKUOKA.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Fukuoka Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_FUKUOKA_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '福冈', city_en: 'Fukuoka', country_iso: 'JP', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_FUKUOKA_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_FUKUOKA_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_FUKUOKA_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_FUKUOKA_CONTENT_QA_EXIT}`);
