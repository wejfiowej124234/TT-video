#!/usr/bin/env node
/**
 * POI City Closure Evidence · FR · 巴黎 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-paris-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('巴黎');
...
async function main() {
  ...
  const evidenceRows = PARIS.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent(PARIS.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of PARIS.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || PARIS.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, PARIS.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === PARIS.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= PARIS.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${PARIS.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, PARIS.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: PARIS.country_iso,
      country_zh: PARIS.country_zh,
      city_zh: PARIS.city_zh,
      city_en: PARIS.city_en,
    },
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === PARIS.country_iso && c.city_zh === PARIS.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: PARIS.matrix_ids.length,
        closure_evidence: PARIS.closure_latest,
        [PARIS.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.paris_city_closure = { status: 'CLOSED', evidence: PARIS.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_FR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'PARIS_CLOSED';
    kickoff[PARIS.closure_key] = 'CLOSED';
