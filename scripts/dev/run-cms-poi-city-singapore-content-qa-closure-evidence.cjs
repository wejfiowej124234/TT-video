#!/usr/bin/env node
/**
 * POI City Content QA Closure · SG · 新加坡（JP + KR + TH Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '新加坡');
  if (!cityQa) throw new Error('新加坡 city QA missing');

  const checks = {
    assets_locked_10_10: lock.locked === SINGAPORE.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'SG', city_zh: '新加坡', city_en: 'Singapore', poi_count: SINGAPORE.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CLOSURE-LATEST.json',
    ...
    cross_region_remediated: 'all_10_execution_plus_geo',
    checks,
  ...
  console.log('TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${SINGAPORE.matrix_ids.length}`);
