#!/usr/bin/env node
/**
 * USA Country Runtime Audit · national_runtime · ② staging
 *
 * 验证美国相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · 四城 LOCK 不变 · JP/KR/TH/SG LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-us-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  filename,
  fetchJson,
} = require('./lib/cms-us-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-US-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/us-country');

const CONSUMER_SURFACES = [
  { id: 'home_usa', path: '/', setup: 'home_usa' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_us', path: '/market', setup: 'market_fr_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'us_poi_runtime_exit_aggregate',
    label: '四城 Market POI Runtime · Exit Check Gate 4 聚合',
...
function isUsaScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /usa|us\b|san francisco|las vegas|los angeles|new york|美国|旧金山|拉斯维加斯|洛杉矶|纽约|金门|好莱坞|自由女神|时代广场|百乐宫|天使湾/.test(
    blob,
  );
}

async function scanPageImages(page, scope, { usaOnly = false } = {}) {
  ...
  for (const img of images) {
    if (usaOnly && !isUsaScopedImage(img)) continue;
...
async function catalogUsaAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=US`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const usBtn = page.locator('#landing-hero-form').getByRole('button', { name: '美国', exact: true });
    const usBtnVisible = await usBtn.isVisible().catch(() => false);
    if (usBtnVisible) {
      ...
      surfaceRows.push({ surface: 'home_usa_ambient', pass: ambientOk, runtime_src: ambientSrc });
      if (!ambientOk) issues.push(`home 美国 ambient: ${ambientSrc || 'MISSING'}`);
    } else {
      const ambientCatalog = await catalogUsaAmbientUrl();
      const ambientOk =
      ...
      surfaceRows.push({
        surface: 'home_usa_ambient_catalog_fallback',
        pass: ambientOk,
        runtime_src: ambientCatalog.url,
        note: 'landing 美国 pill not visible · catalog media fallback',
      });
      if (!ambientOk) issues.push(`home 美国 ambient catalog fallback: ${ambientCatalog.url || 'MISSING'}`);
    }
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `us-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { usaOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页美国 Ambient + Market 壳 + 美国相关内容无 fallback',
...
function formatMd(report) {
  const lines = [
    ...
    `**TT_CMS_US_COUNTRY_RUNTIME:** \`${report.TT_CMS_US_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
  ...
  lines.push('', `**US POI Runtime:** ${report.poi_runtime_pass}/${report.poi_runtime_required} PASS`);
  lines.push('', '**四城汇总**', '');
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateUsCityExitChecks(),
    validateUsaAmbient(),
    await validateCatalogApiAllUsPois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = US_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = US_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'four_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows =
      staticChecks.find((c) => c.id === 'catalog_api_us_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_us_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'US', country_zh: '美国', cities: US_CITIES },
    ...
    verdict: allPass ? 'US_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_US_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_US_COUNTRY_RUNTIME: ${report.TT_CMS_US_COUNTRY_RUNTIME}`);
  console.log(`US POI Runtime: ${poiRuntimePass}/${poiRequired} PASS`);
