#!/usr/bin/env node
/**
 * Dubai · Execution + Content QA Wave · JP + KR 双模板同源
 *
 *   TRAVELTRUST_STAGING_RC_BASELINE_OVERRIDE=1 \
 *   API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-dubai-content-qa-wave.cjs
 *
...
assertStagingBaselineMutationAuthorized('cms_dubai_content_qa_wave');

const ROOT = path.join(__dirname, '../..');
const API = (process.env.API || 'https://tt-api-staging.fly.dev').replace(/\/$/, '');
const DUBAI = getCityPilot('迪拜');

const DUBAI_HEROES = DUBAI.hero_files;

async function syncAllHeroes() {
  const unique = [...new Set(Object.values(DUBAI_HEROES))];
  console.log(`\n===== Pre-sync staging heroes (${unique.length} unique / ${DUBAI.matrix_ids.length} POI) =====`);
...
async function runExitCheckWithRetry(maxAttempts = 12) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      runNode('scripts/dev/run-cms-poi-city-dubai-content-qa-exit-check.cjs');
      return;
    } catch {
      if (attempt === maxAttempts) throw new Error('Dubai exit check failed after retries');
...
async function main() {
  console.log('TT_CMS_DUBAI_CONTENT_QA_WAVE: START');

  console.log('\n===== Catalog Build · 迪拜 =====');
  runNode('scripts/dev/run-cms-phase1-poi-pilot-catalog-build.cjs', '--city-zh 迪拜');
  ...
  for (const matrixId of DUBAI.matrix_ids) {
    ...
    runNode(
      ...
      `--city-zh 迪拜 --matrix-id ${matrixId} --poi "${poi}"`,
    );
  }

  console.log('\n===== Execution closed loop · 迪拜 8/8 =====');
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs', '--city-zh 迪拜 --skip-refresh');

  console.log('\n===== Execution CLOSED · 迪拜 =====');
  runNode('scripts/dev/run-cms-poi-city-dubai-closure-evidence.cjs');

  runNode('scripts/dev/run-cms-poi-city-dubai-content-qa-closure-evidence.cjs');
  await runExitCheckWithRetry();

  console.log('\nTT_CMS_DUBAI_CONTENT_QA_WAVE: DONE');
}
