#!/usr/bin/env node
/**
 * Sharjah Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-sharjah-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SHARJAH-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SHARJAH-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/sharjah');

const SHARJAH = getCityPilot('沙迦');
const POI_COUNT = SHARJAH.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SHARJAH-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '沙迦');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent('沙迦')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '沙迦', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(SHARJAH);
    ...
    const shotPath = path.join(SHOT_DIR, `sharjah-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < SHARJAH.matrix_ids.length; i++) {
      const matrixId = SHARJAH.matrix_ids[i];
      const poi = SHARJAH.pois[i];
...
function checkNoBacklogUnlock() {
  const open = SHARJAH.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = SHARJAH.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Sharjah Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_SHARJAH_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '沙迦', city_en: 'Sharjah', country_iso: 'AE', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_SHARJAH_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_SHARJAH_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_SHARJAH_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_SHARJAH_CONTENT_QA_EXIT}`);
