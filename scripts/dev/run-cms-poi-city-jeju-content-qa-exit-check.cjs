#!/usr/bin/env node
/**
 * Jeju Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-jeju-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-JEJU-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-JEJU-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/jeju');

const JEJU = getCityPilot('济州');
const POI_COUNT = JEJU.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-JEJU-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '济州');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent('济州')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '济州', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = JEJU.pois.slice(0, 4);
    const foodPois = JEJU.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `jeju-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < JEJU.matrix_ids.length; i++) {
      const matrixId = JEJU.matrix_ids[i];
      const poi = JEJU.pois[i];
...
function checkNoBacklogUnlock() {
  const open = JEJU.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = JEJU.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Jeju Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_JEJU_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '济州', city_en: 'Jeju', country_iso: 'KR', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_JEJU_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_JEJU_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_JEJU_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_JEJU_CONTENT_QA_EXIT}`);
