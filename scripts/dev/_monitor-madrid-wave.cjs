#!/usr/bin/env node
/**
 * Monitor Madrid wave · 5min stall → kill + resume · 8/8 → closure + exit
 * 不得改动 JP/KR/TH/SG/FR/US/AU LOCK · 巴塞罗那 LOCK · 不启动塞维利亚 / ES Country Runtime
 */
const fs = require('fs');
...
const TERM_LOG = process.env.TERM_LOG || path.join(ROOT, 'evidence/GO_cms_operation/_madrid-wave-monitor.log');
...
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MADRID-CLOSURE-LATEST.json');
const QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MADRID-CONTENT-QA-CLOSURE-LATEST.json');
const EXIT_CHECK = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MADRID-CONTENT-QA-EXIT-CHECK-LATEST.json');
const WAVE_SCRIPT = 'scripts/dev/run-cms-madrid-content-qa-wave.cjs';
...
function madridLockedCount() {
  return MADRID.matrix_ids.filter((id) => getAsset(id).state === 'LOCKED').length;
}
...
function verifyPriorLocks() {
  ...
  const counts = { JP: 0, KR: 0, TH: 0, SG: 0, FR: 0, US: 0, AU: 0, ES: 0 };
  ...
  return { pass: issues.length === 0, issues, counts, bcnLocked };
}
...
function wavePid() {
  ...
  try {
    ...
    for (const line of out.split('\n')) {
      if (line.includes('run-cms-madrid-content-qa-wave.cjs') && !line.includes('_monitor-madrid-wave')) {
...
function runClosurePipeline() {
  ...
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs --city-zh 马德里 --skip-refresh');
  runNode('scripts/dev/run-cms-poi-city-madrid-closure-evidence.cjs');
  runNode('scripts/dev/run-cms-poi-city-madrid-content-qa-closure-evidence.cjs');
  for (let i = 1; i <= 12; i++) {
    try {
      runNode('scripts/dev/run-cms-poi-city-madrid-content-qa-exit-check.cjs');
      break;
    } catch {
...
function verifyDone() {
  ...
  return {
    exec: execDoc?.TT_CMS_POI_CITY_MADRID === 'CLOSED',
    qa: qaDoc?.TT_CMS_POI_CITY_MADRID_CONTENT_QA === 'CLOSED',
    exit: exitDoc?.TT_CMS_POI_CITY_MADRID_CONTENT_QA_EXIT === 'PASS' && exitDoc?.all_pass === true,
    locked: madridLockedCount(),
  };
}
...
async function main() {
  ...
  log(`MONITOR START stall=${STALL_MS}ms · prior locks OK (Barcelona ${guard0.bcnLocked}/8)`);

  let lastLockCount = madridLockedCount();
  ...
  while (true) {
    const locked = madridLockedCount();
  ...
  log('MADRID TRIPLE PASS — next: Sevilla');
}
