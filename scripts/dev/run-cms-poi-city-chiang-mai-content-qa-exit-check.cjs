#!/usr/bin/env node
/**
 * Chiang Mai Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-chiang-mai-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/chiang-mai');

const CHIANG_MAI = getCityPilot('清迈');
const POI_COUNT = CHIANG_MAI.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '清迈');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent('清迈')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '清迈', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = CHIANG_MAI.pois.slice(0, 5);
    const foodPois = CHIANG_MAI.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `chiang-mai-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < CHIANG_MAI.matrix_ids.length; i++) {
      const matrixId = CHIANG_MAI.matrix_ids[i];
      const poi = CHIANG_MAI.pois[i];
...
function checkNoBacklogUnlock() {
  const open = CHIANG_MAI.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = CHIANG_MAI.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Chiang Mai Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '清迈', city_en: 'Chiang Mai', country_iso: 'TH', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA_EXIT}`);
