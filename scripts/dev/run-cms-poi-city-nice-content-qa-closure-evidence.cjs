#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 尼斯（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '尼斯');
  if (!cityQa) throw new Error('尼斯 city QA missing');

  const checks = {
    assets_locked_8_8: lock.locked === NICE.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_NICE_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'FR', city_zh: '尼斯', city_en: 'Chiang Mai', poi_count: NICE.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CLOSURE-LATEST.json',
    ...
    cross_region_remediated: 'all_9_execution_plus_geo',
    checks,
    ...
    TT_CMS_POI_CITY_NICE_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_NICE_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${NICE.matrix_ids.length}`);
