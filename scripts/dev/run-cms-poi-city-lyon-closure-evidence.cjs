#!/usr/bin/env node
/**
 * POI City Closure Evidence · FR · 里昂 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-lyon-closure-evidence.cjs
 */
const fs = require('fs');
...
const LYON = getCityPilot('里昂');
const NEXT_CITY = nextCityPilot('里昂');
...
async function main() {
  ...
  for (const matrixId of LYON.matrix_ids) {
  ...
  const evidenceRows = LYON.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent(LYON.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of LYON.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || LYON.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, LYON.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === LYON.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= LYON.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${LYON.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, LYON.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: LYON.country_iso,
      country_zh: LYON.country_zh,
      city_zh: LYON.city_zh,
      city_en: LYON.city_en,
    },
    poi_count: LYON.matrix_ids.length,
    matrix_ids: LYON.matrix_ids,
    pois: LYON.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === LYON.country_iso && c.city_zh === LYON.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: LYON.matrix_ids.length,
        closure_evidence: LYON.closure_latest,
        [LYON.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.lyon_city_closure = { status: 'CLOSED', evidence: LYON.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_FR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'LYON_CLOSED';
    kickoff[LYON.closure_key] = 'CLOSED';
