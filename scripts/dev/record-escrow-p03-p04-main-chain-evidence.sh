#!/usr/bin/env bash
# ① P03/P04 主链接入验收证据：vitest 绿集 + Playwright itinerary-first P03/P04
#
# 用法（仓库根）：
#   bash scripts/dev/record-escrow-p03-p04-main-chain-evidence.sh
#
...
RUN_LOG="$EVID/ESCROW-P03-P04-MAIN-CHAIN-${STAMP}.log"
...
start_api_bg() {
  echo "record-escrow-p03-p04: starting API (P3_CHAIN_OFF=${P3_CHAIN_OFF})…"
  (
    cd "$ROOT"
  ...
  ) >"$EVID/.api-bg-p03p04-${STAMP}.log" 2>&1 &
  echo $! >"$EVID/.api-bg-p03p04-${STAMP}.pid"
}

cleanup_api_bg() {
  local pidfile="$EVID/.api-bg-p03p04-${STAMP}.pid"
  [[ -f "$pidfile" ]] || return 0
  local pid
  pid="$(cat "$pidfile")"
...
if api_up && [[ "${TRAVELTRUST_EVIDENCE_REUSE_API:-0}" != "1" ]]; then
  echo "record-escrow-p03-p04: restarting API on :${API_PORT} (API_RATE_LIMIT_PER_MINUTE=0)…"
  kill_api_on_port
fi
if ! api_up; then
  start_api_bg
...
{
  echo "TT_ESCROW_P03_P04_MAIN_CHAIN_EVIDENCE: START ${STAMP}"
  ...
  echo ""
  echo "== Step B: vitest P03/P04 unit =="
  cd "$ROOT/frontend"
  npx vitest run lib/escrowExperienceP03P04.test.ts lib/escrowDraftFlow.test.ts

  echo ""
  echo "== Step C: Playwright P03/P04 itinerary-first escrow =="
  ...
  node ./scripts/run-e2e-default.mjs --project=chromium e2e/escrow-p03-p04-itinerary-first.spec.ts

  echo ""
  echo "TT_ESCROW_P03_P04_MAIN_CHAIN_EVIDENCE: OK ${STAMP}"
  echo "TT_ESCROW_P03_P04_MAIN_CHAIN_SUMMARY: exit=0 phase=① green_set+vitest+playwright"
} 2>&1 | tee "$RUN_LOG"

grep -q "TT_ESCROW_P03_P04_MAIN_CHAIN_EVIDENCE: OK" "$RUN_LOG" || {
