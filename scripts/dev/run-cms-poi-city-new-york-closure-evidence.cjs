#!/usr/bin/env node
/**
 * POI City Closure Evidence · US · 纽约 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-new-york-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEW_YORK = getCityPilot('纽约');
const NEXT_CITY = nextCityPilot('纽约');
...
async function main() {
  ...
  for (const matrixId of NEW_YORK.matrix_ids) {
  ...
  const evidenceRows = NEW_YORK.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent(NEW_YORK.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of NEW_YORK.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || NEW_YORK.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, NEW_YORK.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === NEW_YORK.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= NEW_YORK.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${NEW_YORK.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, NEW_YORK.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: NEW_YORK.country_iso,
      country_zh: NEW_YORK.country_zh,
      city_zh: NEW_YORK.city_zh,
      city_en: NEW_YORK.city_en,
    },
    poi_count: NEW_YORK.matrix_ids.length,
    matrix_ids: NEW_YORK.matrix_ids,
    pois: NEW_YORK.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === NEW_YORK.country_iso && c.city_zh === NEW_YORK.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: NEW_YORK.matrix_ids.length,
        closure_evidence: NEW_YORK.closure_latest,
        [NEW_YORK.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'US_CITY_PILOT_COMPLETE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : null;
    kickoff.new_york_city_closure = { status: 'CLOSED', evidence: NEW_YORK.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_US_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'NEW_YORK_CLOSED';
    kickoff[NEW_YORK.closure_key] = 'CLOSED';
