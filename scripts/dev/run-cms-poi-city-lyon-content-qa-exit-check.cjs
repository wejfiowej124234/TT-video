#!/usr/bin/env node
/**
 * Lyon Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-lyon-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LYON-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LYON-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/lyon');

const LYON = getCityPilot('里昂');
const POI_COUNT = LYON.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LYON-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of LYON.matrix_ids) {
    const poi = LYON.pois[LYON.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '里昂');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent('里昂')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '里昂', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = LYON.pois.slice(0, 4);
    const foodPois = LYON.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `lyon-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < LYON.matrix_ids.length; i++) {
      const matrixId = LYON.matrix_ids[i];
      const poi = LYON.pois[i];
...
function checkNoBacklogUnlock() {
  const open = LYON.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = LYON.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Lyon Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_LYON_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '里昂', city_en: 'Lyon', country_iso: 'FR', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_LYON_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_LYON_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_LYON_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_LYON_CONTENT_QA_EXIT}`);
