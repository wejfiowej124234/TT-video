#!/usr/bin/env bash
# O-S4 · Cold Start Campaigns smoke（① 本地 · 须 API+PG 已起）
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
API="${TRAVELTRUST_API_BASE:-${API_BASE:-http://127.0.0.1:8080}}"
FE="${TRAVELTRUST_FE_BASE:-http://127.0.0.1:3012}"

echo "== smoke-admin-official-cold-start-p0-local (O-S4) =="
echo "API=$API FE=$FE"

for path in \
  "/api/v1/admin/official/cold-start/campaigns"; do
...
echo "smoke-admin-official-cold-start-p0-local: exit 0"
