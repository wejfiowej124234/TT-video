#!/usr/bin/env bash
# ① GD/P06 + P03–P06 公众 catalog 向导主链验收证据（Playwright 日历红/白 + 绿集回归）
#
# 用法（仓库根）：
...
RUN_LOG="$EVID/ESCROW-GD-P06-PUBLIC-CATALOG-${STAMP}.log"
...
start_api_bg() {
  echo "record-escrow-gd-p06: starting API (P3_CHAIN_OFF=${P3_CHAIN_OFF})…"
  (
    cd "$ROOT"
  ...
  ) >"$EVID/.api-bg-gd-p06-${STAMP}.log" 2>&1 &
  echo $! >"$EVID/.api-bg-gd-p06-${STAMP}.pid"
}

cleanup_api_bg() {
  local pidfile="$EVID/.api-bg-gd-p06-${STAMP}.pid"
  [[ -f "$pidfile" ]] || return 0
  local pid
  pid="$(cat "$pidfile")"
...
if api_up && [[ "${TRAVELTRUST_EVIDENCE_REUSE_API:-0}" != "1" ]]; then
  echo "record-escrow-gd-p06: restarting API on :${API_PORT} (API_RATE_LIMIT_PER_MINUTE=0)…"
  kill_api_on_port
fi
if ! api_up; then
  start_api_bg
...
{
  echo "TT_ESCROW_GD_P06_PUBLIC_CATALOG_EVIDENCE: START ${STAMP}"
  echo "phase: ① local only"
  echo "api: ${API_BASE}"
  echo "guide: f0e0b101-0001-4001-8001-000000000001 (tg_guide_main@trustgate-e2e.local)"
  ...
  echo ""
  echo "== Step B: Playwright GD/P06 public catalog P03–P06 =="
  ...
  echo ""
  echo "TT_ESCROW_GD_P06_PUBLIC_CATALOG_EVIDENCE: OK ${STAMP}"
  echo "TT_ESCROW_GD_P06_PUBLIC_CATALOG_SUMMARY: exit=0 phase=① playwright+green_set"
} 2>&1 | tee "$RUN_LOG"

grep -q "TT_ESCROW_GD_P06_PUBLIC_CATALOG_EVIDENCE: OK" "$RUN_LOG" || {
