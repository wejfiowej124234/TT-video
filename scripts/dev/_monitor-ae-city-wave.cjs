#!/usr/bin/env node
/**
 * Monitor AE city wave · CITY_ZH required · dynamic POI count · 5min stall → kill + resume
 * Prior guard: JP/KR/TH/SG/FR/US/AU/ES LOCK unchanged + prior AE cities LOCK
 */
const fs = require('fs');
...
function wavePid() {
  try {
    ...
    for (const line of out.split('\n')) {
      if (line.includes(WAVE_SCRIPT) && !line.includes('_monitor-ae-city-wave')) {
