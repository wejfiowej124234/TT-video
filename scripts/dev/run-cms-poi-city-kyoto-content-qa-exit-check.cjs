#!/usr/bin/env node
/**
 * Kyoto Content QA Exit Check · Osaka Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-kyoto-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-KYOTO-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-KYOTO-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/kyoto');

const KYOTO = getCityPilot('京都');
const POI_COUNT = KYOTO.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-KYOTO-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '京都');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const expected = {};
  for (const matrixId of KYOTO.matrix_ids) {
    expected[KYOTO.pois[KYOTO.matrix_ids.indexOf(matrixId)]] = getAsset(matrixId).hero_file;
  }
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent('京都')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '京都', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = KYOTO.pois.slice(0, 4);
    const foodPois = KYOTO.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `kyoto-poi-runtime-${stamp}.png`);
    ...
    for (const poi of KYOTO.pois) {
      const expectedFn = catalogByPoi[poi] || getAsset(KYOTO.matrix_ids[KYOTO.pois.indexOf(poi)]).hero_file;
...
function checkNoBacklogUnlock() {
  const open = KYOTO.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = KYOTO.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Kyoto Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_KYOTO_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '京都', city_en: 'Kyoto', country_iso: 'JP', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_KYOTO_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_KYOTO_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_KYOTO_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_KYOTO_CONTENT_QA_EXIT}`);
