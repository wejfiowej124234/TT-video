#!/usr/bin/env node
/**
 * POI City Closure Evidence · KR · 济州 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-jeju-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('济州');
...
async function main() {
  ...
  const evidenceRows = JEJU.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent(JEJU.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of JEJU.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || JEJU.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, JEJU.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === JEJU.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= JEJU.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${JEJU.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, JEJU.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: JEJU.country_iso,
      country_zh: JEJU.country_zh,
      city_zh: JEJU.city_zh,
      city_en: JEJU.city_en,
    },
    poi_count: JEJU.matrix_ids.length,
    matrix_ids: JEJU.matrix_ids,
    pois: JEJU.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === JEJU.country_iso && c.city_zh === JEJU.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: JEJU.matrix_ids.length,
        closure_evidence: JEJU.closure_latest,
        [JEJU.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.jeju_city_closure = { status: 'CLOSED', evidence: JEJU.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_KR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'JEJU_CLOSED';
    kickoff[JEJU.closure_key] = 'CLOSED';
