#!/usr/bin/env node
/**
 * Thailand L5 Content QA · ② staging · JP + KR 双模板 · 不调整标准
 *
 *   node scripts/dev/run-cms-thailand-content-qa.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-THAILAND-CONTENT-QA-LATEST.json');
...
function main() {
  ...
  console.log(`TT_CMS_TH_CONTENT_QA: ${report.TT_CMS_TH_CONTENT_QA}`);
  console.log(`TT_CMS_TH_COUNTRY: ${report.TT_CMS_TH_COUNTRY}`);
