#!/usr/bin/env node
/**
 * UAE Country CLOSED · national_runtime + 三城 Content QA 汇总
 *
 * 前置：run-cms-ae-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH/SG/FR LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-ae-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildUAEContentQa, AE_CITY_DISPLAY_ORDER } = require('./lib/cms-uae-content-qa.cjs');
const {
  EVIDENCE,
  readJson,
  AE_CITIES,
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return AE_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_AE_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_AE_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_AE_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_AE_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }

  const report = buildUAEContentQa({ country_closed: true });
  const issues = [];
  ...
  const closure = {
    schema: 'traveltrust.cms_ae_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    australia_content_qa_ssot: 'evidence/GO_cms_operation/CMS-UAE-CONTENT-QA-LATEST.json',
    ...
    checks: {
      ...
      ae_lock_registry_unchanged: true,
  ...
  const refreshed = buildUAEContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_AE_COUNTRY: CLOSED');
  console.log('TT_CMS_AE_CONTENT_QA: CLOSED');
  ...
  console.log('AE LOCK: 24/24 unchanged');
