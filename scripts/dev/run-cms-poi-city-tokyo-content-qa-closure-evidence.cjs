#!/usr/bin/env node
/**
 * POI City Content QA Closure · JP · 东京（Osaka Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '东京');
  if (!cityQa) throw new Error('东京 city QA missing');

  const checks = {
    assets_locked_9_9: lock.locked === TOKYO.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_TOKYO_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'JP', city_zh: '东京', city_en: 'Tokyo', poi_count: TOKYO.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CLOSURE-LATEST.json',
    ...
    cross_region_remediated: ['寿司', '拉面', '天妇罗'],
    checks,
    ...
    TT_CMS_POI_CITY_TOKYO_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_TOKYO_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${TOKYO.matrix_ids.length}`);
