#!/usr/bin/env node
/**
 * Korea Country Runtime Audit · national_runtime · ② staging
 *
 * 验证韩国相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · 四城 LOCK 不变 · 日本 41 LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-kr-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  filename,
  fetchJson,
} = require('./lib/cms-kr-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/kr-country');

const CONSUMER_SURFACES = [
  { id: 'home_korea', path: '/', setup: 'home_korea' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_kr', path: '/market', setup: 'market_kr_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'kr_poi_runtime_exit_aggregate',
...
function isKoreaScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /korea|kr|seoul|busan|jeju|incheon|韩国|首尔|釜山|济州|仁川|景福宫|韩屋|明洞|汉江|海云台|牛岛|月尾岛/.test(blob);
}

async function scanPageImages(page, scope, { koreaOnly = false } = {}) {
  ...
  for (const img of images) {
    if (koreaOnly && !isKoreaScopedImage(img)) continue;
...
async function catalogKoreaAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=KR`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const krBtn = page.locator('#landing-hero-form').getByRole('button', { name: '韩国', exact: true });
    await krBtn.scrollIntoViewIfNeeded();
    await krBtn.click();
    ...
    surfaceRows.push({ surface: 'home_korea_ambient', pass: ambientOk, runtime_src: ambientSrc });
    if (!ambientOk) issues.push(`home 韩国 ambient: ${ambientSrc || 'MISSING'}`);
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `kr-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { koreaOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页韩国 Ambient + Market 壳 + 韩国相关内容无 fallback',
...
async function liveConsumerProbeWithRetry(poiAggregate, attempts = 5) {
  let last = null;
...
function formatMd(report) {
  const lines = [
    ...
    `**TT_CMS_KR_COUNTRY_RUNTIME:** \`${report.TT_CMS_KR_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
  ...
  lines.push('', `**KR POI Runtime:** ${report.poi_runtime_pass}/${report.poi_runtime_required} PASS`);
  lines.push('', '**四城汇总**', '');
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateFourCityExitChecks(),
    validateKoreaAmbient(),
    await validateCatalogApiAllKrPois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = KR_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = KR_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'four_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows = staticChecks.find((c) => c.id === 'catalog_api_kr_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_kr_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'KR', country_zh: '韩国', cities: KR_CITIES },
    ...
    verdict: allPass ? 'KR_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_KR_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_KR_COUNTRY_RUNTIME: ${report.TT_CMS_KR_COUNTRY_RUNTIME}`);
  console.log(`KR POI Runtime: ${poiRuntimePass}/${poiRequired} PASS`);
