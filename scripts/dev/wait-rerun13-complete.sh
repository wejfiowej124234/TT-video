#!/usr/bin/env bash
# Wait for Site10 rerun13 matrix lock release, then check-gates + parse-fails.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
EVID="$ROOT/frontend/evidence/GO_local_phase1"
LOCK="$EVID/site10-matrix-run.lock"
OUT="$EVID/site10-rerun13-complete.status.txt"
WAIT_LOG="$EVID/site10-rerun13-wait.log"

log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] $*" | tee -a "$OUT"; }

: >"$OUT"
log "wait-rerun13: polling lock $LOCK"
while [[ -f "$LOCK" ]]; do
  prog="$(grep -oE '\[[0-9]+/844\]' "$WAIT_LOG" 2>/dev/null | tail -1 || echo "pre-matrix")"
  log "progress=$prog"
  sleep 120
done
