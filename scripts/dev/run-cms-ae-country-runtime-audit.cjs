#!/usr/bin/env node
/**
 * UAE Country Runtime Audit · national_runtime · ② staging
 *
 * 验证阿联酋相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · 三城 LOCK 不变 · JP/KR/TH/SG LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-ae-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  cityExitPath,
  isForbiddenFallback,
  validateAeCityExitChecks,
  validateAeCityContentQaClosures,
  validateAeCityExecutionClosures,
  validateCatalogApiAllAePois,
  validateUAEAmbient,
  ...
  filename,
  fetchJson,
} = require('./lib/cms-ae-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/ae-country');

const CONSUMER_SURFACES = [
  { id: 'home_ae', path: '/', setup: 'home_ae' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_au', path: '/market', setup: 'market_ae_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'ae_poi_runtime_exit_aggregate',
...
function isUAEScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /australia|au\b|abu-dhabi|dubai|sharjah|阿联酋|阿布扎比|迪拜|沙迦|歌剧院|邦迪|冲浪者/.test(
    blob,
  );
}

async function scanPageImages(page, scope, { aeOnly = false } = {}) {
  ...
  for (const img of images) {
    if (aeOnly && !isUAEScopedImage(img)) continue;
...
async function catalogUAEAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=AE`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const auBtn = page.locator('#landing-hero-form').getByRole('button', { name: '阿联酋', exact: true });
    ...
    if (auBtnVisible) {
      ...
      surfaceRows.push({ surface: 'home_ae_ambient', pass: ambientOk, runtime_src: ambientSrc });
      if (!ambientOk) issues.push(`home 阿联酋 ambient: ${ambientSrc || 'MISSING'}`);
    } else {
      const ambientCatalog = await catalogUAEAmbientUrl();
      const ambientOk =
      ...
      surfaceRows.push({
        surface: 'home_ae_ambient_catalog_fallback',
        pass: ambientOk,
        runtime_src: ambientCatalog.url,
        note: 'landing 阿联酋 pill not visible · catalog media fallback',
      });
      if (!ambientOk) issues.push(`home 阿联酋 ambient catalog fallback: ${ambientCatalog.url || 'MISSING'}`);
    }
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `ae-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { aeOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页阿联酋 Ambient + Market 壳 + 阿联酋相关内容无 fallback',
...
function formatMd(report) {
  const lines = [
    ...
    `**TT_CMS_AE_COUNTRY_RUNTIME:** \`${report.TT_CMS_AE_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateAeCityExitChecks(),
    validateUAEAmbient(),
    await validateCatalogApiAllAePois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = AE_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = AE_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'four_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows =
      staticChecks.find((c) => c.id === 'catalog_api_ae_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_ae_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'AE', country_zh: '阿联酋', cities: AE_CITIES },
    ...
    ae_lock_guard: { required: 24, unchanged: true },
    ...
    verdict: allPass ? 'AE_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_AE_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_AE_COUNTRY_RUNTIME: ${report.TT_CMS_AE_COUNTRY_RUNTIME}`);
