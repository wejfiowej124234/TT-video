#!/usr/bin/env node
/**
 * POI City Closure Evidence · FR · 尼斯 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-nice-closure-evidence.cjs
 */
const fs = require('fs');
...
const FR_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-POI-WAVE-KICKOFF-LATEST.json');
const FR_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-POI-PILOT-WAVE-LATEST.json');
...
const NICE = getCityPilot('尼斯');
const NEXT_CITY = nextCityPilot('尼斯');
...
async function main() {
  ...
  for (const matrixId of NICE.matrix_ids) {
  ...
  const evidenceRows = NICE.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent(NICE.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of NICE.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || NICE.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, NICE.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === NICE.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= NICE.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${NICE.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, NICE.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: NICE.country_iso,
      country_zh: NICE.country_zh,
      city_zh: NICE.city_zh,
      city_en: NICE.city_en,
    },
    poi_count: NICE.matrix_ids.length,
    matrix_ids: NICE.matrix_ids,
    pois: NICE.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === NICE.country_iso && c.city_zh === NICE.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: NICE.matrix_ids.length,
        closure_evidence: NICE.closure_latest,
        [NICE.closure_key]: 'CLOSED',
      },
    ],
  ...
  const kickoff = readJsonIfExists(FR_KICKOFF);
  if (kickoff) {
    kickoff.stamp_utc = stamp;
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'FR_CITY_PILOT_COMPLETE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : null;
    kickoff.nice_city_closure = { status: 'CLOSED', evidence: NICE.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-FR-CITY-${NEXT_CITY.slug.toUpperCase()}`,
          label: `FR · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_FR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'NICE_CLOSED';
    kickoff[NICE.closure_key] = 'CLOSED';
    kickoff.TT_CMS_POI_ACTIVE_CITY = NEXT_CITY ? `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}` : null;
    fs.writeFileSync(FR_KICKOFF, JSON.stringify(kickoff, null, 2) + '\n');
  }

  const pilotWave = readJsonIfExists(FR_PILOT_WAVE);
  if (pilotWave) {
    pilotWave.stamp_utc = stamp;
    pilotWave.nice_closed = true;
    pilotWave.active_catalog_build = kickoff?.active_catalog_build || registry.active_city;
    pilotWave[NICE.closure_key] = 'CLOSED';
    fs.writeFileSync(FR_PILOT_WAVE, JSON.stringify(pilotWave, null, 2) + '\n');
  }
