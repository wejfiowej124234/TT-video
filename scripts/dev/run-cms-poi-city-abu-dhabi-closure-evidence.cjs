#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 阿布扎比 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-abu-dhabi-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('阿布扎比');
...
async function main() {
  ...
  const evidenceRows = ABU_DHABI.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent(ABU_DHABI.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of ABU_DHABI.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || ABU_DHABI.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, ABU_DHABI.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === ABU_DHABI.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= ABU_DHABI.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${ABU_DHABI.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, ABU_DHABI.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: ABU_DHABI.country_iso,
      country_zh: ABU_DHABI.country_zh,
      city_zh: ABU_DHABI.city_zh,
      city_en: ABU_DHABI.city_en,
    },
    poi_count: ABU_DHABI.matrix_ids.length,
    matrix_ids: ABU_DHABI.matrix_ids,
    pois: ABU_DHABI.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === ABU_DHABI.country_iso && c.city_zh === ABU_DHABI.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: ABU_DHABI.matrix_ids.length,
        closure_evidence: ABU_DHABI.closure_latest,
        [ABU_DHABI.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: ABU_DHABI.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_AE_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'ABU_DHABI_CLOSED';
    kickoff[ABU_DHABI.closure_key] = 'CLOSED';
