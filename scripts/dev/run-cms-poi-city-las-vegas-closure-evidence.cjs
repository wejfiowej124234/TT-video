#!/usr/bin/env node
/**
 * POI City Closure Evidence · US · 拉斯维加斯 · ② staging
 *
...
const NEXT_CITY = nextCityPilot('拉斯维加斯');
...
async function main() {
  ...
  const evidenceRows = LAS_VEGAS.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent(LAS_VEGAS.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of LAS_VEGAS.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || LAS_VEGAS.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, LAS_VEGAS.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === LAS_VEGAS.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= LAS_VEGAS.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${LAS_VEGAS.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, LAS_VEGAS.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: LAS_VEGAS.country_iso,
      country_zh: LAS_VEGAS.country_zh,
      city_zh: LAS_VEGAS.city_zh,
      city_en: LAS_VEGAS.city_en,
    },
    poi_count: LAS_VEGAS.matrix_ids.length,
    matrix_ids: LAS_VEGAS.matrix_ids,
    pois: LAS_VEGAS.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === LAS_VEGAS.country_iso && c.city_zh === LAS_VEGAS.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: LAS_VEGAS.matrix_ids.length,
        closure_evidence: LAS_VEGAS.closure_latest,
        [LAS_VEGAS.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.las_vegas_city_closure = { status: 'CLOSED', evidence: LAS_VEGAS.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_US_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'LAS_VEGAS_CLOSED';
    kickoff[LAS_VEGAS.closure_key] = 'CLOSED';
