#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 广州 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-guangzhou-closure-evidence.cjs
 */
const fs = require('fs');
...
const AE_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-POI-WAVE-KICKOFF-LATEST.json');
const AE_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-POI-PILOT-WAVE-LATEST.json');
...
const NEXT_CITY = nextCityPilot('广州');
...
async function main() {
  ...
  const evidenceRows = GUANGZHOU.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(GUANGZHOU.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of GUANGZHOU.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || GUANGZHOU.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, GUANGZHOU.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === GUANGZHOU.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= GUANGZHOU.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${GUANGZHOU.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, GUANGZHOU.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: GUANGZHOU.country_iso,
      country_zh: GUANGZHOU.country_zh,
      city_zh: GUANGZHOU.city_zh,
      city_en: GUANGZHOU.city_en,
    },
    poi_count: GUANGZHOU.matrix_ids.length,
    matrix_ids: GUANGZHOU.matrix_ids,
    pois: GUANGZHOU.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === GUANGZHOU.country_iso && c.city_zh === GUANGZHOU.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: GUANGZHOU.matrix_ids.length,
        closure_evidence: GUANGZHOU.closure_latest,
        [GUANGZHOU.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: GUANGZHOU.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-AE-CITY-${NEXT_CITY.slug.replace(/-/g, '_').toUpperCase()}`,
          label: `CN · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'GUANGZHOU_CLOSED';
    kickoff[GUANGZHOU.closure_key] = 'CLOSED';
