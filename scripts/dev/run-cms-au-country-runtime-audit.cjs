#!/usr/bin/env node
/**
 * Australia Country Runtime Audit · national_runtime · ② staging
 *
 * 验证澳大利亚相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · 三城 LOCK 不变 · JP/KR/TH/SG LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-au-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  cityExitPath,
  isForbiddenFallback,
  validateAuCityExitChecks,
  validateAuCityContentQaClosures,
  validateAuCityExecutionClosures,
  validateCatalogApiAllAuPois,
  validateAustraliaAmbient,
  ...
  filename,
  fetchJson,
} = require('./lib/cms-au-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/au-country');

const CONSUMER_SURFACES = [
  { id: 'home_au', path: '/', setup: 'home_au' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_au', path: '/market', setup: 'market_au_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'au_poi_runtime_exit_aggregate',
...
function isAustraliaScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /australia|au\b|sydney|melbourne|gold coast|澳大利亚|悉尼|墨尔本|黄金海岸|歌剧院|邦迪|冲浪者/.test(
    blob,
  );
}

async function scanPageImages(page, scope, { auOnly = false } = {}) {
  ...
  for (const img of images) {
    if (auOnly && !isAustraliaScopedImage(img)) continue;
...
async function catalogAustraliaAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=AU`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const auBtn = page.locator('#landing-hero-form').getByRole('button', { name: '澳大利亚', exact: true });
    ...
    if (auBtnVisible) {
      ...
      surfaceRows.push({ surface: 'home_au_ambient', pass: ambientOk, runtime_src: ambientSrc });
      if (!ambientOk) issues.push(`home 澳大利亚 ambient: ${ambientSrc || 'MISSING'}`);
    } else {
      const ambientCatalog = await catalogAustraliaAmbientUrl();
      const ambientOk =
      ...
      surfaceRows.push({
        surface: 'home_au_ambient_catalog_fallback',
        pass: ambientOk,
        runtime_src: ambientCatalog.url,
        note: 'landing 澳大利亚 pill not visible · catalog media fallback',
      });
      if (!ambientOk) issues.push(`home 澳大利亚 ambient catalog fallback: ${ambientCatalog.url || 'MISSING'}`);
    }
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `au-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { auOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页澳大利亚 Ambient + Market 壳 + 澳大利亚相关内容无 fallback',
...
function formatMd(report) {
  const lines = [
    '# Australia Country Runtime Audit',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**TT_CMS_AU_COUNTRY_RUNTIME:** \`${report.TT_CMS_AU_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateAuCityExitChecks(),
    validateAustraliaAmbient(),
    await validateCatalogApiAllAuPois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = AU_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = AU_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'four_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows =
      staticChecks.find((c) => c.id === 'catalog_api_au_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_au_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'AU', country_zh: '澳大利亚', cities: AU_CITIES },
    ...
    au_lock_guard: { required: 24, unchanged: true },
    ...
    verdict: allPass ? 'AU_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_AU_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_AU_COUNTRY_RUNTIME: ${report.TT_CMS_AU_COUNTRY_RUNTIME}`);
