#!/usr/bin/env node
/**
 * San Francisco · Execution + Content QA Wave · JP + KR 双模板同源
 *
 *   TRAVELTRUST_STAGING_RC_BASELINE_OVERRIDE=1 \
 *   API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-san-francisco-content-qa-wave.cjs
 *
...
assertStagingBaselineMutationAuthorized('cms_san-francisco_content_qa_wave');

const ROOT = path.join(__dirname, '../..');
const API = (process.env.API || 'https://tt-api-staging.fly.dev').replace(/\/$/, '');
const SAN_FRANCISCO = getCityPilot('旧金山');

const SAN_FRANCISCO_HEROES = SAN_FRANCISCO.hero_files;

async function syncAllHeroes() {
  const unique = [...new Set(Object.values(SAN_FRANCISCO_HEROES))];
  console.log(`\n===== Pre-sync staging heroes (${unique.length} unique / ${SAN_FRANCISCO.matrix_ids.length} POI) =====`);
...
async function runExitCheckWithRetry(maxAttempts = 12) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      runNode('scripts/dev/run-cms-poi-city-san-francisco-content-qa-exit-check.cjs');
      return;
    } catch {
      if (attempt === maxAttempts) throw new Error('San Francisco exit check failed after retries');
...
async function main() {
  console.log('TT_CMS_SAN_FRANCISCO_CONTENT_QA_WAVE: START');

  console.log('\n===== Catalog Build · 旧金山 =====');
  runNode('scripts/dev/run-cms-phase1-poi-pilot-catalog-build.cjs', '--city-zh 旧金山');
  ...
  for (const matrixId of SAN_FRANCISCO.matrix_ids) {
    ...
    runNode(
      ...
      `--city-zh 旧金山 --matrix-id ${matrixId} --poi "${poi}"`,
    );
  }

  console.log('\n===== Execution closed loop · 旧金山 8/8 =====');
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs', '--city-zh 旧金山 --skip-refresh');

  console.log('\n===== Execution CLOSED · 旧金山 =====');
  runNode('scripts/dev/run-cms-poi-city-san-francisco-closure-evidence.cjs');

  runNode('scripts/dev/run-cms-poi-city-san-francisco-content-qa-closure-evidence.cjs');
  await runExitCheckWithRetry();

  console.log('\nTT_CMS_SAN_FRANCISCO_CONTENT_QA_WAVE: DONE');
}
