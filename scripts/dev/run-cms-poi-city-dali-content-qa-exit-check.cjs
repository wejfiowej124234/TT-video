#!/usr/bin/env node
/**
 * Dali Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-dali-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-DALI-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-DALI-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/dali');

const DALI = getCityPilot('大理');
const POI_COUNT = DALI.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-DALI-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  const issues = [];
  const rows = [];
  for (const matrixId of DALI.matrix_ids) {
    const poi = DALI.pois[DALI.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '大理');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent('大理')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '大理', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(DALI);
    ...
    const shotPath = path.join(SHOT_DIR, `dali-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < DALI.matrix_ids.length; i++) {
      const matrixId = DALI.matrix_ids[i];
      const poi = DALI.pois[i];
...
function checkNoBacklogUnlock() {
  const open = DALI.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = DALI.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Dali Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_DALI_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '大理', city_en: 'Dali', country_iso: 'CN', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_DALI_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_DALI_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_DALI_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_DALI_CONTENT_QA_EXIT}`);
