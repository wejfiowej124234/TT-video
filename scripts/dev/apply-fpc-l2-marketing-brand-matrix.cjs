#!/usr/bin/env node
/**
 * Apply L2 marketing_brand cluster certification → page matrix (B25-C1).
 *
 *   node scripts/dev/apply-fpc-l2-marketing-brand-matrix.cjs
 */
'use strict';
...
const EVID_DIR = path.join(
  ROOT,
  'docs/spec/governance-token/evidence/phase3-production-entry-baseline/FPC-100/B25-C1-marketing-brand'
);

const CLUSTER = 'marketing_brand';
const UX_PASS = 'PASS';
const UX_WARN = 'WARN';
...
const PAGE_SCORES = {
  '/': {
    ...
    user_goal: 'Discover destination · create itinerary draft',
    primary_cta: 'Generate itinerary',
    notes: 'LANDING-MARKET SSOT · localStorage session · ② cross-device bookmark deferred',
  },
  '/did-rank': {
    ui: 9,
    ux: 9,
    ...
    function_flow: 9,
    production_ready: 'YES',
    user_goal: 'View trust rankings',
    primary_cta: 'Explore rank board',
    notes: 'SSR is_me · DID-RANK-PHASE1-FREEZE',
  },
  '/traveltrust': {
    ui: 9,
    ux: 9,
    ...
    function_flow: 8,
    production_ready: 'YES',
    user_goal: 'Understand TravelTrust Network brand',
    primary_cta: 'Scroll cinematic story',
    notes: 'PH-1 cinematic freeze · optional e2e at ①',
  },
  '/traveltrust/announcements': {
    ...
    function_flow: 8,
    production_ready: 'YES',
    user_goal: 'Read protocol announcements',
    primary_cta: 'View announcement lane',
    notes: 'Governance lane SSOT · TTG public rounds registry',
  },
};
...
function applyUxStates(page, stamp) {
  ...
  page.layer4_enterprise = {
    ...
    mobile_375: UX_PASS,
    i18n: UX_PASS,
    api_data_chain: page.route === '/' || page.route === '/did-rank' ? UX_PASS : UX_WARN,
  };
  ...
  page.fpc_batch = 'B25-C1';
  page.verified_at_utc = stamp;
  page.evidence_path = path
    .relative(ROOT, path.join(EVID_DIR, 'FPC-100-MARKETING-BRAND-CHECKLIST-BASELINE.v1.json'))
    .replace(/\\/g, '/');
}
...
matrix.five_layers = {
  ...
  L2_per_page_l5_ux_ui: clusterCertified
    ? `marketing_brand ${clusterPages.length}/${clusterPages.length} L2 PASS — B25-C1 at ①`
...
console.log(
  `TT_FPC_L2_MARKETING_BRAND_APPLY: updated=${updated} cluster_certified=${clusterCertified} ui_scored=${uiDone}`
);
