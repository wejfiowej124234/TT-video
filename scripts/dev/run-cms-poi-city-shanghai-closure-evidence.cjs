#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 上海 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-shanghai-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('上海');
...
async function main() {
  ...
  const evidenceRows = SHANGHAI.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(SHANGHAI.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SHANGHAI.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SHANGHAI.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SHANGHAI.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SHANGHAI.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SHANGHAI.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SHANGHAI.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SHANGHAI.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SHANGHAI.country_iso,
      country_zh: SHANGHAI.country_zh,
      city_zh: SHANGHAI.city_zh,
      city_en: SHANGHAI.city_en,
    },
    poi_count: SHANGHAI.matrix_ids.length,
    matrix_ids: SHANGHAI.matrix_ids,
    pois: SHANGHAI.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SHANGHAI.country_iso && c.city_zh === SHANGHAI.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SHANGHAI.matrix_ids.length,
        closure_evidence: SHANGHAI.closure_latest,
        [SHANGHAI.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: SHANGHAI.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SHANGHAI_CLOSED';
    kickoff[SHANGHAI.closure_key] = 'CLOSED';
