#!/usr/bin/env node
/**
 * Monitor Melbourne wave · 5min stall → kill + resume · 8/8 → closure + exit
 * 不得改动 JP/KR/TH/SG/FR/US · 悉尼 LOCK · 不启动黄金海岸 / AU Country Runtime
 */
const fs = require('fs');
...
const TERM_LOG = process.env.TERM_LOG || path.join(ROOT, 'evidence/GO_cms_operation/_melbourne-wave-monitor.log');
...
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CLOSURE-LATEST.json');
const QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CONTENT-QA-CLOSURE-LATEST.json');
const EXIT_CHECK = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CONTENT-QA-EXIT-CHECK-LATEST.json');
const WAVE_SCRIPT = 'scripts/dev/run-cms-melbourne-content-qa-wave.cjs';

const PRIOR_LOCK_REQUIRED = { JP: 41, KR: 31, TH: 28, SG: 10, FR: 24, US: 33, SYDNEY: 8 };
...
function melbourneLockedCount() {
  return MELBOURNE.matrix_ids.filter((id) => getAsset(id).state === 'LOCKED').length;
}
...
function verifyPriorLocks() {
  ...
  return { pass: issues.length === 0, issues, counts, sydneyLocked };
}
...
function wavePid() {
  ...
  try {
    ...
    for (const line of out.split('\n')) {
      if (line.includes('run-cms-melbourne-content-qa-wave.cjs') && !line.includes('_monitor-melbourne-wave')) {
...
function runClosurePipeline() {
  ...
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs --city-zh 墨尔本 --skip-refresh');
  runNode('scripts/dev/run-cms-poi-city-melbourne-closure-evidence.cjs');
  runNode('scripts/dev/run-cms-poi-city-melbourne-content-qa-closure-evidence.cjs');
  for (let i = 1; i <= 12; i++) {
    try {
      runNode('scripts/dev/run-cms-poi-city-melbourne-content-qa-exit-check.cjs');
      break;
    } catch {
...
function verifyDone() {
  ...
  return {
    exec: execDoc?.TT_CMS_POI_CITY_MELBOURNE === 'CLOSED',
    qa: qaDoc?.TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA === 'CLOSED',
    exit: exitDoc?.TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA_EXIT === 'PASS' && exitDoc?.all_pass === true,
    locked: melbourneLockedCount(),
  };
}
...
async function main() {
  ...
  log(`MONITOR START stall=${STALL_MS}ms · prior locks OK`);

  let lastLockCount = melbourneLockedCount();
  ...
  while (true) {
    const locked = melbourneLockedCount();
  ...
  log('MELBOURNE TRIPLE PASS — paused · awaiting Gold Coast confirmation (not auto-starting)');
}
