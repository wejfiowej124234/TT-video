#!/usr/bin/env node
/**
 * POI City Closure Evidence · JP · 札幌 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-sapporo-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('札幌');
...
async function main() {
  if (!NEXT_CITY) throw new Error('no next city after Sapporo');
  ...
  const evidenceRows = SAPPORO.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent(SAPPORO.city_zh)}&limit=50`;
  ...
  const loopPath = path.join(ROOT, SAPPORO.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SAPPORO.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SAPPORO.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SAPPORO.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SAPPORO.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SAPPORO.country_iso,
      country_zh: '日本',
      city_zh: SAPPORO.city_zh,
      city_en: SAPPORO.city_en,
    },
    poi_count: SAPPORO.matrix_ids.length,
    matrix_ids: SAPPORO.matrix_ids,
    pois: SAPPORO.pois,
    checks,
    ...
    catalog_probe: {
      ...
      legacy_values: SAPPORO.pois.map((p) => ({ legacy_value: p, ok: Boolean(catalogByLegacy[p]?.image_url) })),
    },
    closed_loop_evidence: SAPPORO.closed_loop_latest,
    next_city: {
  ...
  fs.writeFileSync(path.join(stampedDir, 'CMS-POI-CITY-SAPPORO-CLOSURE.json'), JSON.stringify(closure, null, 2) + '\n');

  const prev = readRegistry() || {};
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SAPPORO.country_iso && c.city_zh === SAPPORO.city_zh),
  );
  const registry = {
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SAPPORO.matrix_ids.length,
        closure_evidence: SAPPORO.closure_latest,
        [SAPPORO.closure_key]: 'CLOSED',
      },
    ],
    active_city: {
  ...
  if (kickoff) {
    ...
    kickoff.sapporo_city_closure = { status: 'CLOSED', evidence: SAPPORO.closure_latest };
    ...
    kickoff[SAPPORO.closure_key] = 'CLOSED';
  ...
  if (pilotWave) {
    ...
    pilotWave[SAPPORO.closure_key] = 'CLOSED';
