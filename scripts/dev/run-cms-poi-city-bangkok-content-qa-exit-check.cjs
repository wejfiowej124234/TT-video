#!/usr/bin/env node
/**
 * Bangkok Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-bangkok-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/bangkok');

const BANGKOK = getCityPilot('曼谷');
const POI_COUNT = BANGKOK.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BANGKOK-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '曼谷');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent('曼谷')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '曼谷', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = BANGKOK.pois.slice(0, 5);
    const foodPois = BANGKOK.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `bangkok-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < BANGKOK.matrix_ids.length; i++) {
      const matrixId = BANGKOK.matrix_ids[i];
      const poi = BANGKOK.pois[i];
...
function checkNoBacklogUnlock() {
  const open = BANGKOK.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = BANGKOK.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Bangkok Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_BANGKOK_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '曼谷', city_en: 'Bangkok', country_iso: 'TH', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_BANGKOK_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_BANGKOK_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_BANGKOK_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_BANGKOK_CONTENT_QA_EXIT}`);
