#!/usr/bin/env node
/**
 * Sapporo Content QA Exit Check · Osaka Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-sapporo-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/sapporo');

const SAPPORO = getCityPilot('札幌');
const POI_COUNT = SAPPORO.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of SAPPORO.matrix_ids) {
    const poi = SAPPORO.pois[SAPPORO.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '札幌');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const expected = {};
  for (const matrixId of SAPPORO.matrix_ids) {
    expected[SAPPORO.pois[SAPPORO.matrix_ids.indexOf(matrixId)]] = getAsset(matrixId).hero_file;
  }
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent('札幌')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await page.getByRole('button', { name: /Custom itinerary|自定义行程/i }).first().click({ force: true, timeout: 60000 });
    ...
    await day1.getByRole('button', { name: '札幌', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = SAPPORO.pois.slice(0, 5);
    const foodPois = SAPPORO.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `sapporo-poi-runtime-${stamp}.png`);
    ...
    for (const poi of SAPPORO.pois) {
      const expectedFn = catalogByPoi[poi] || getAsset(SAPPORO.matrix_ids[SAPPORO.pois.indexOf(poi)]).hero_file;
...
function checkNoBacklogUnlock() {
  const open = SAPPORO.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = SAPPORO.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Sapporo Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_SAPPORO_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '札幌', city_en: 'Sapporo', country_iso: 'JP', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_SAPPORO_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_SAPPORO_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_JP_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_SAPPORO_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_SAPPORO_CONTENT_QA_EXIT}`);
