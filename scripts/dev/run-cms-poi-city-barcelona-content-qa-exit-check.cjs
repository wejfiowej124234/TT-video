#!/usr/bin/env node
/**
 * Barcelona Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-barcelona-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/barcelona');

const BARCELONA = getCityPilot('巴塞罗那');
const POI_COUNT = BARCELONA.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of BARCELONA.matrix_ids) {
    const poi = BARCELONA.pois[BARCELONA.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  const report = buildSpainContentQa();
  const city = report.cities.find((c) => c.city_zh === '巴塞罗那');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent('巴塞罗那')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '西班牙' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '巴塞罗那', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(BARCELONA);
    ...
    const shotPath = path.join(SHOT_DIR, `barcelona-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < BARCELONA.matrix_ids.length; i++) {
      const matrixId = BARCELONA.matrix_ids[i];
      const poi = BARCELONA.pois[i];
...
function checkNoBacklogUnlock() {
  const open = BARCELONA.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = BARCELONA.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Barcelona Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_BARCELONA_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '巴塞罗那', city_en: 'Barcelona', country_iso: 'ES', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_BARCELONA_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_BARCELONA_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_ES_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_BARCELONA_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_BARCELONA_CONTENT_QA_EXIT}`);
