#!/usr/bin/env node
/**
 * FPC-100 Batch B25-C3 · L2 transaction & escrow cluster (① local)
 *
 *   node scripts/dev/run-fpc-batch-b25-c3-transaction-escrow.cjs
 */
'use strict';
...
const OUT = path.join(EVID, 'FPC-100-BATCH-B25-C3-LATEST.json');
const EVID_DIR = path.join(EVID, 'B25-C3-transaction-escrow');
const CHECKLIST_PATH = path.join(EVID_DIR, 'FPC-100-TRANSACTION-ESCROW-CHECKLIST-BASELINE.v1.json');
...
const GATE_ENV = checklist.business_gate_env || {};
...
const QUALITY_CHECKS = [
  {
    ...
    must_contain: ['id: B25-C3', 'transaction_escrow'],
  },
  {
    id: 'Q4_escrow_closure',
    domain: 'Q4',
    path: 'frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md',
    must_contain: ['/escrow/[id]', 'Phase ①'],
  },
  {
    id: 'Q5_orders_corridor',
    domain: 'Q5',
    path: 'frontend/evidence/GO_local_orders_l5/README.md',
    must_contain: ['TT_ORDERS_CORRIDOR_LOCAL', '/pay'],
  },
  {
    id: 'Q5_escrow_checklist',
    domain: 'Q5',
    path: 'docs/spec/governance-token/evidence/phase3-production-entry-baseline/FPC-100/B25-C3-transaction-escrow/FPC-100-TRANSACTION-ESCROW-CHECKLIST-BASELINE.v1.json',
    must_contain: ['fpc_100_transaction_escrow_checklist', 'state_machine_ssot'],
  },
];
...
const {
  ...
  runDisplayChainSsot,
  runStaticSsotChecks,
} = require('./lib/fpc-transaction-escrow-probes.cjs');
...
function runGate(g, findings, gateResults, env = {}) {
  ...
  try {
    ...
    stdout = sh(cmd, ROOT, {
      FPC_GATE_TIMEOUT_MS: g.includes('orders-corridor') ? '600000' : '600000',
      ...gateEnvFor(g),
      ...env,
    });
  } catch (e) {
...
function buildFourQuestions(businessVerdict, qualityVerdict, findings, recertPass) {
  ...
  return {
    business_correct: {
      ...
      notes: 'transaction_escrow 8 routes · web3 green + full-chain smoke + orders corridor',
    },
    quality_meets_standard: {
      answer: qualityVerdict === 'PASS' || qualityVerdict === 'PASS_WITH_WARN',
      verdict: qualityVerdict,
      notes: 'State machine SSOT · confirm-completion→Completed · bilateral service separate · display chain',
    },
    findings_identified: {
    ...
    recertification_passed: {
      answer: recertPass,
      notes: recertPass
        ? '8/8 transaction_escrow L2 PASS · dashboard next=B25-C4 · B21-B25-C2 frozen'
...
(async () => {
  ...
  const gate = assertCanRun('B25-C3');
  if (!gate.ok) {
    findings.push({
      id: 'no_batch_skip',
      severity: 'P0',
      detail: `Cannot run B25-C3 before ${gate.missing_prerequisites?.join(', ')}`,
    });
  }

  for (const g of REGISTRY_GATES) {
    if (!preflight.pass && g.includes('smoke-web3')) {
      gateResults.push({ gate: g, exit_code: 1, pass: false, summary_line: 'skipped: preflight failed' });
      findings.push({ id: 'gate_skip:smoke-web3', severity: 'P0', detail: 'API required' });
      continue;
    }
  ...
  for (const g of BUSINESS_GATES) {
    if (g.includes('orders-corridor') && !preflight.pass) {
      gateResults.push({ gate: g, exit_code: 1, pass: false, summary_line: 'skipped: preflight failed' });
      findings.push({ id: 'gate_skip:orders-corridor', severity: 'P0', detail: 'API required' });
      continue;
    }
    runGate(g, findings, gateResults, {
      API_BASE: preflight.api_base || 'http://127.0.0.1:8080',
  ...
  const stateMachine = runStateMachineSsot(checklist, findings);
  ...
  const report = {
    ...
    batch_id: 'B25-C3',
    title: 'L2 · Page certification — transaction & escrow cluster',
    layer: 'L2',
    phase: '① local',
    timestamp_utc: stamp,
    verifier: 'run-fpc-batch-b25-c3-transaction-escrow.cjs',
    product_version: 'v1.0',
    code_anchor_commit: codeAnchor,
    git: { head, branch: sh('git branch --show-current').trim() },
    depends_on: ['B05', 'B10', 'B20', 'B23', 'B25-C2'],
    route_clusters: ['transaction_escrow'],
    cluster_routes: checklist.cluster_routes,
    gates: [...REGISTRY_GATES, ...BUSINESS_GATES],
    gate_results: gateResults,
    gate_pass: businessGatePass,
    runtime_preflight: preflight,
    transaction_escrow_live: {
      cluster_checks: clusterChecks,
      dashboard_parity: dashboardParity,
      display_parity: displayParity,
      state_machine: stateMachine,
    ...
    transaction_static_ssot: staticSsot,
    ...
    next_batch: pass ? 'B25-C4' : 'B25-C3-remediation',
    ai_review: {
    ...
    human_verifier: null,
    human_note:
      'transaction_escrow 8 routes ① — order/escrow/settlement state machine + display parity; defer-commit',
    traceability: {
      requirements: [
        '8/8 transaction_escrow L2 certified (2 CONDITIONAL: pay · chain sync)',
        'confirm-completion → Completed (no service-completion delegation)',
        'confirm-service-completion bilateral Escrowed-only path separate',
        'web3 green + full-chain smoke + orders corridor SKIP_E2E PASS',
        'page matrix b25_c3_apply cluster_certified',
      ],
      spec_refs: [
        ...
        'frontend/evidence/GO_local_orders_l5/README.md',
        'crates/api/src/chain_off/orders_flow/accept_cancel_pay_complete.rs',
        'FPC-100/B25-C3-transaction-escrow/FPC-100-TRANSACTION-ESCROW-CHECKLIST-BASELINE.v1.json',
      ],
      evidence_path: 'FPC-100/FPC-100-BATCH-B25-C3-LATEST.json',
      certification_batch: 'B25-C3',
      product_version: 'v1.0',
    },
  };
  ...
  console.log(`TT_FPC_100_BATCH_B25-C3: ${report.verdict}`);
