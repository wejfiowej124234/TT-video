#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 成都 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-chengdu-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('成都');
...
async function main() {
  ...
  const evidenceRows = CHENGDU.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(CHENGDU.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of CHENGDU.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || CHENGDU.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, CHENGDU.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === CHENGDU.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= CHENGDU.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${CHENGDU.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, CHENGDU.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: CHENGDU.country_iso,
      country_zh: CHENGDU.country_zh,
      city_zh: CHENGDU.city_zh,
      city_en: CHENGDU.city_en,
    },
    poi_count: CHENGDU.matrix_ids.length,
    matrix_ids: CHENGDU.matrix_ids,
    pois: CHENGDU.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === CHENGDU.country_iso && c.city_zh === CHENGDU.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: CHENGDU.matrix_ids.length,
        closure_evidence: CHENGDU.closure_latest,
        [CHENGDU.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: CHENGDU.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'CHENGDU_CLOSED';
    kickoff[CHENGDU.closure_key] = 'CLOSED';
