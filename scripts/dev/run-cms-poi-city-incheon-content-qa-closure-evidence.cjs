#!/usr/bin/env node
/**
 * POI City Content QA Closure · KR · 仁川（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '仁川');
  if (!cityQa) throw new Error('仁川 city QA missing');

  const checks = {
    assets_locked_6_6: lock.locked === INCHEON.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_INCHEON_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'KR', city_zh: '仁川', city_en: 'Incheon', poi_count: INCHEON.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CLOSURE-LATEST.json',
    ...
    cross_region_remediated: 'all_6_execution_plus_geo',
    checks,
    ...
    TT_CMS_POI_CITY_INCHEON_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_INCHEON_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${INCHEON.matrix_ids.length}`);
