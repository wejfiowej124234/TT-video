#!/usr/bin/env node
/**
 * POI City Closure Evidence · JP · 大阪 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-osaka-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('大阪');
...
async function main() {
  if (!NEXT_CITY) throw new Error('no next city after Osaka');
  ...
  const evidenceRows = OSAKA.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent(OSAKA.city_zh)}&limit=50`;
  ...
  const loopPath = path.join(ROOT, OSAKA.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === OSAKA.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= OSAKA.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${OSAKA.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, OSAKA.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: OSAKA.country_iso,
      country_zh: '日本',
      city_zh: OSAKA.city_zh,
      city_en: OSAKA.city_en,
    },
    poi_count: OSAKA.matrix_ids.length,
    matrix_ids: OSAKA.matrix_ids,
    pois: OSAKA.pois,
    checks,
    ...
    catalog_probe: {
      ...
      legacy_values: OSAKA.pois.map((p) => ({ legacy_value: p, ok: Boolean(catalogByLegacy[p]?.image_url) })),
    },
    closed_loop_evidence: OSAKA.closed_loop_latest,
    next_city: {
    ...
    closure_keys: {
      [OSAKA.closure_key]: 'CLOSED',
      TT_CMS_POI_ACTIVE_CITY: `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}`,
      [`TT_CMS_POI_CITY_${NEXT_CITY.city_en.toUpperCase()}`]: 'ACTIVE',
    },
    frozen: true,
    [OSAKA.closure_key]: 'CLOSED',
  };
  ...
  fs.writeFileSync(path.join(stampedDir, 'CMS-POI-CITY-OSAKA-CLOSURE.json'), JSON.stringify(closure, null, 2) + '\n');

  const prev = readRegistry() || {};
  const closedBefore = (prev.closed_cities || []).filter((c) => !(c.country_iso === OSAKA.country_iso && c.city_zh === OSAKA.city_zh));
  const registry = {
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: OSAKA.matrix_ids.length,
        closure_evidence: OSAKA.closure_latest,
        [OSAKA.closure_key]: 'CLOSED',
      },
    ],
    active_city: {
    ...
    TT_CMS_POI_ACTIVE_CITY: `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}`,
    TT_CMS_POI_CITY_TOKYO: prev.closed_cities?.find((c) => c.city_zh === '东京')?.TT_CMS_POI_CITY_TOKYO || 'CLOSED',
    [OSAKA.closure_key]: 'CLOSED',
  };
  ...
  if (kickoff) {
    ...
    kickoff.osaka_city_closure = { status: 'CLOSED', evidence: OSAKA.closure_latest };
    kickoff.active_catalog_build = {
      wave_id: `POI-JP-CITY-${NEXT_CITY.city_en.toUpperCase()}`,
    ...
    kickoff[OSAKA.closure_key] = 'CLOSED';
  ...
  if (pilotWave) {
    ...
    pilotWave[OSAKA.closure_key] = 'CLOSED';
