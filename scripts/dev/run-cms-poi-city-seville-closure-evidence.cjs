#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 塞维利亚 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-seville-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('塞维利亚');
...
async function main() {
  ...
  for (const matrixId of SEVILLE.matrix_ids) {
  ...
  const evidenceRows = SEVILLE.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent(SEVILLE.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SEVILLE.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SEVILLE.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SEVILLE.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SEVILLE.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SEVILLE.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SEVILLE.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SEVILLE.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SEVILLE.country_iso,
      country_zh: SEVILLE.country_zh,
      city_zh: SEVILLE.city_zh,
      city_en: SEVILLE.city_en,
    },
    poi_count: SEVILLE.matrix_ids.length,
    matrix_ids: SEVILLE.matrix_ids,
    pois: SEVILLE.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SEVILLE.country_iso && c.city_zh === SEVILLE.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SEVILLE.matrix_ids.length,
        closure_evidence: SEVILLE.closure_latest,
        [SEVILLE.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.seville_city_closure = { status: 'CLOSED', evidence: SEVILLE.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_ES_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SEVILLE_CLOSED';
    kickoff[SEVILLE.closure_key] = 'CLOSED';
