#!/usr/bin/env node
/**
 * POI City Closure Evidence · TH · 普吉 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-phuket-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('普吉');
...
async function main() {
  ...
  const evidenceRows = PHUKET.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent(PHUKET.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of PHUKET.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || PHUKET.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, PHUKET.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === PHUKET.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= PHUKET.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${PHUKET.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, PHUKET.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: PHUKET.country_iso,
      country_zh: PHUKET.country_zh,
      city_zh: PHUKET.city_zh,
      city_en: PHUKET.city_en,
    },
    poi_count: PHUKET.matrix_ids.length,
    matrix_ids: PHUKET.matrix_ids,
    pois: PHUKET.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === PHUKET.country_iso && c.city_zh === PHUKET.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: PHUKET.matrix_ids.length,
        closure_evidence: PHUKET.closure_latest,
        [PHUKET.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.phuket_city_closure = { status: 'CLOSED', evidence: PHUKET.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_TH_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'PHUKET_CLOSED';
    kickoff[PHUKET.closure_key] = 'CLOSED';
