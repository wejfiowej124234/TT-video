...
[[ -n "$STAMP" ]] || { echo "record-cert4: --stamp required" >&2; exit 2; }
[[ -n "$SIGNER" ]] || { echo "record-cert4: --signer required" >&2; exit 2; }
ARGS=(python "$ROOT/scripts/dev/record-cert4-safe-walkthrough-signoff.py" --stamp "$STAMP" --signer "$SIGNER")
