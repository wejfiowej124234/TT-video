#!/usr/bin/env node
/**
 * Singapore Content QA Exit Check · JP + KR + TH Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-singapore-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/singapore');

const SINGAPORE = getCityPilot('新加坡');
const POI_COUNT = SINGAPORE.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SINGAPORE-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of SINGAPORE.matrix_ids) {
    const poi = SINGAPORE.pois[SINGAPORE.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '新加坡');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=SG&city=${encodeURIComponent('新加坡')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '新加坡' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '新加坡', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = SINGAPORE.pois.slice(0, 5);
    const foodPois = SINGAPORE.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `singapore-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < SINGAPORE.matrix_ids.length; i++) {
      const matrixId = SINGAPORE.matrix_ids[i];
      const poi = SINGAPORE.pois[i];
...
function checkNoBacklogUnlock() {
  const open = SINGAPORE.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = SINGAPORE.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Singapore Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '新加坡', city_en: 'Singapore', country_iso: 'SG', poi_count: POI_COUNT },
    replicated_from: 'JP + KR + TH Golden Template (TT_CMS_JP_COUNTRY: CLOSED)',
    web_base: WEB,
    api_base: API,
    checks,
    all_pass: allPass,
    ...
    TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_SG_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_SINGAPORE_CONTENT_QA_EXIT}`);
