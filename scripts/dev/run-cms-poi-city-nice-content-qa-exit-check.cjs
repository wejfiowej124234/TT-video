#!/usr/bin/env node
/**
 * Nice Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-nice-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/nice');

const NICE = getCityPilot('尼斯');
const POI_COUNT = NICE.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-NICE-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of NICE.matrix_ids) {
    const poi = NICE.pois[NICE.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  const report = buildFranceContentQa();
  const city = report.cities.find((c) => c.city_zh === '尼斯');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent('尼斯')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '法国' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '尼斯', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = NICE.pois.slice(0, 4);
    const foodPois = NICE.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `nice-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < NICE.matrix_ids.length; i++) {
      const matrixId = NICE.matrix_ids[i];
      const poi = NICE.pois[i];
...
function checkNoBacklogUnlock() {
  const open = NICE.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = NICE.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Nice Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_NICE_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '尼斯', city_en: 'Nice', country_iso: 'FR', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_NICE_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_NICE_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_NICE_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_NICE_CONTENT_QA_EXIT}`);
