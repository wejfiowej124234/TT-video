/**
 * Singapore L5 Content QA · Phase② · JP + KR + TH Country CLOSED 三模板 · 不调整标准
 */
const fs = require('fs');
...
const SG_CITY_DISPLAY_ORDER = ['新加坡'];

const SG_COUNTRY_CRITERIA = [
...
function assertTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort SG kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort SG kickoff`);
  }
}

function assessSgCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_SG_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'Singapore Country Runtime audit PASS · 新加坡 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_SG_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'Singapore Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 SG Country Runtime audit · 先跑 run-cms-sg-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessSgCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessSgCountryRuntimeConsumer();
  const sgRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-SG-'),
  ).length;
  const sgRuntimePass = sgRuntimeAudit?.TT_CMS_SG_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
  criteria.unsplash_0 = sgRuntimePass || poiUnsplashRuntime === 0;
  criteria.ocs_runtime_0 = sgRuntimePass;
...
function buildSingaporeBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildSingaporeContentQa(options = {}) {
  ...
  const cities = SG_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'SG').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildSingaporeBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessSgCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_singapore_content_qa.v1',
    ...
    summary: {
      ...
      active_city: registry?.active_city || { country_iso: 'SG', city_zh: '新加坡' },
    },
    TT_CMS_SG_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_SG_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_SG_COUNTRY: country.TT_CMS_SG_COUNTRY,
  };
}

module.exports = {
  SG_CITY_DISPLAY_ORDER,
  buildSingaporeContentQa,
