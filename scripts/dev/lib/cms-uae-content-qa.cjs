/**
 * UAE L5 Content QA · Phase② · JP/KR/TH/SG/FR/US Country CLOSED 六模板 · 不调整标准
 */
const fs = require('fs');
...
const AE_CITY_DISPLAY_ORDER = ['阿布扎比', '迪拜', '沙迦'];

const AE_COUNTRY_CRITERIA = [
...
function assertAeTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort AE kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort AE kickoff`);
  }
}

function assessUAECountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_AE_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'UAE Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_AE_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'UAE Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 AE Country Runtime audit · 先跑 run-cms-ae-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessUAECountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessUAECountryRuntimeConsumer();
  const usRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-AE-'),
  ).length;
  const usRuntimePass = usRuntimeAudit?.TT_CMS_AE_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
...
function buildUAEBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildUAEContentQa(options = {}) {
  if (!options.skip_template_guard) assertAeTemplateCountriesClosed();
  ...
  const cities = AE_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'AE').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildUAEBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessUAECountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_uae_content_qa.v1',
    ...
    TT_CMS_AE_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_AE_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_AE_COUNTRY: country.TT_CMS_AE_COUNTRY,
  };
}

module.exports = {
  AE_CITY_DISPLAY_ORDER,
  buildUAEContentQa,
  assertAeTemplateCountriesClosed: assertAeTemplateCountriesClosed,
  assessUAECountry,
};
