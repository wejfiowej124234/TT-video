/**
 * Spain L5 Content QA · Phase② · JP/KR/TH/SG/FR/US Country CLOSED 六模板 · 不调整标准
 */
const fs = require('fs');
...
const ES_CITY_DISPLAY_ORDER = ['巴塞罗那', '马德里', '塞维利亚'];

const ES_COUNTRY_CRITERIA = [
...
function assertEsTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort ES kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort ES kickoff`);
  }
}

function assessSpainCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_ES_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'Spain Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_ES_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'Spain Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 ES Country Runtime audit · 先跑 run-cms-es-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessSpainCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessSpainCountryRuntimeConsumer();
  const usRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-ES-'),
  ).length;
  const usRuntimePass = usRuntimeAudit?.TT_CMS_ES_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
...
function buildSpainBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildSpainContentQa(options = {}) {
  if (!options.skip_template_guard) assertEsTemplateCountriesClosed();
  ...
  const cities = ES_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'ES').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildSpainBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessSpainCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_spain_content_qa.v1',
    ...
    TT_CMS_ES_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_ES_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_ES_COUNTRY: country.TT_CMS_ES_COUNTRY,
  };
}

module.exports = {
  ES_CITY_DISPLAY_ORDER,
  buildSpainContentQa,
  assertEsTemplateCountriesClosed: assertEsTemplateCountriesClosed,
  assessSpainCountry,
};
