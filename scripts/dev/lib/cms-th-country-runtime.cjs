/**
 * Thailand Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-TH-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-TH-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-THAILAND-CONTENT-QA-LATEST.json'),
  frozen_standard: path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-STANDARD-FROZEN.v1.json'),
  th_scope_lock: path.join(ROOT, 'evidence/GO_cms_operation/CMS-TH-POI-CATALOG-SCOPE-LOCK-LATEST.json'),
};
...
/** 清迈 evidence 文件名用 CHIANG_MAI / CHIANG-MAI（非 CHIANG-MAI token） */
function cityEvidenceToken(cityZh, kind) {
  if (cityZh === '清迈') {
    return kind === 'exec' ? 'CHIANG-MAI' : 'CHIANG_MAI';
  }
...
function cityExitPath(cityZh) {
  return path.join(
    ...
    `CMS-POI-CITY-${cityEvidenceToken(cityZh, 'exit')}-CONTENT-QA-EXIT-CHECK-LATEST.json`,
  );
}
...
function cityContentQaClosurePath(cityZh) {
  return path.join(
    ROOT,
    'evidence/GO_cms_operation',
    `CMS-POI-CITY-${cityEvidenceToken(cityZh, 'content_qa')}-CONTENT-QA-CLOSURE-LATEST.json`,
  );
}
...
function cityExecClosurePath(cityZh) {
  return path.join(
    ROOT,
    'evidence/GO_cms_operation',
    `CMS-POI-CITY-${cityEvidenceToken(cityZh, 'exec')}-CLOSURE-LATEST.json`,
  );
}
...
function validateFourCityExitChecks() {
  ...
  return {
    id: 'three_city_exit_checks',
    label: '三城 Golden Template Exit Check · city_consumer_runtime',
...
function validateFourCityContentQaClosures() {
  ...
  return {
    id: 'three_city_content_qa_closed',
    label: '三城 Content QA CLOSED · LOCK 不变',
    pass: issues.length === 0,
    rows,
    issues,
  };
...
function validateFourCityExecutionClosures() {
  ...
  return {
    id: 'three_city_execution_closed',
    label: '三城 Execution CLOSED',
    pass: issues.length === 0,
    rows,
    issues,
  };
...
async function validateCatalogApiAllThPois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of TH_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
  ...
  return {
    id: 'catalog_api_th_poi',
    label: `Catalog API TH POI ${passCount}/${total} 与 LOCK 一致`,
    pass: issues.length === 0,
...
function validateThailandAmbient() {
  const ambient = loadAmbientRuntimeWiringSsot();
  const th = ambient.by_iso?.TH;
  const issues = [];
  if (!ambient.is_closed) issues.push('Ambient wiring SSOT not CLOSED (10/10)');
  if (!th?.runtime_reads_cms_catalog) issues.push('泰国 Ambient Runtime 未读 CMS catalog');
  if (th?.still_unsplash) issues.push('泰国 Ambient 仍 Unsplash');
  if (th?.still_ts_fallback) issues.push('泰国 Ambient 仍 TS fallback');
  return {
    id: 'thailand_ambient_runtime',
    label: '首页 Destination Ambient · 泰国读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: th || null,
...
function validateThScopeLock() {
  const doc = readJson(EVIDENCE.th_scope_lock);
  const issues = [];
  const poiTotal = doc?.th_denominator?.poi_total;
  const cityCount = doc?.th_denominator?.cities?.length;
  if (!doc) issues.push('CMS-TH-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 28) issues.push(`TH scope poi_total=${poiTotal || 'MISSING'} (expected 28)`);
  if (cityCount !== 3) issues.push(`TH scope city_count=${cityCount || 'MISSING'} (expected 3)`);
  if (doc?.TT_CMS_TH_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_TH_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_TH_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'th_scope_lock',
    label: 'TH POI Catalog Scope Lock · 28 POI / 3 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_TH_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
