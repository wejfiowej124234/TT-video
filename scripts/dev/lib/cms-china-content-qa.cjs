/**
 * China L5 Content QA · Phase② · JP/KR/TH/SG/FR/US Country CLOSED 六模板 · 不调整标准
 */
const fs = require('fs');
...
const CN_CITY_DISPLAY_ORDER = ['北京', '上海', '广州', '成都', '杭州', '西安', '厦门', '青岛', '大理'];

const CN_COUNTRY_CRITERIA = [
...
function assertCnTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort CN kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort CN kickoff`);
  }
}

function assessChinaCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_CN_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'China Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-CN-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_CN_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'China Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-CN-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 CN Country Runtime audit · 先跑 run-cms-cn-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessChinaCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessChinaCountryRuntimeConsumer();
  const usRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-CN-'),
  ).length;
  const usRuntimePass = usRuntimeAudit?.TT_CMS_CN_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
...
function buildChinaBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildChinaContentQa(options = {}) {
  if (!options.skip_template_guard) assertCnTemplateCountriesClosed();
  ...
  const cities = CN_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'CN').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildChinaBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessChinaCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_china_content_qa.v1',
    ...
    TT_CMS_CN_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_CN_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_CN_COUNTRY: country.TT_CMS_CN_COUNTRY,
  };
}

module.exports = {
  CN_CITY_DISPLAY_ORDER,
  buildChinaContentQa,
  assertCnTemplateCountriesClosed: assertCnTemplateCountriesClosed,
  assessChinaCountry,
};
