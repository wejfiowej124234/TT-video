/**
 * Australia Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AUSTRALIA-CONTENT-QA-LATEST.json'),
  frozen_standard: path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-STANDARD-FROZEN.v1.json'),
  au_scope_lock: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-POI-CATALOG-SCOPE-LOCK-LATEST.json'),
};
...
function validateAuCityExitChecks() {
  const rows = [];
  const issues = [];
...
function validateAuCityContentQaClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of AU_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
...
function validateAuCityExecutionClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of AU_CITIES) {
...
async function validateCatalogApiAllAuPois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of AU_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
  ...
  return {
    id: 'catalog_api_au_poi',
    label: `Catalog API AU POI ${passCount}/${total} 与 LOCK 一致`,
    pass: issues.length === 0,
...
function validateAustraliaAmbient() {
  const ambient = loadAmbientRuntimeWiringSsot();
  const us = ambient.by_iso?.AU;
  const issues = [];
  if (!ambient.is_closed) issues.push('Ambient wiring SSOT not CLOSED (10/10)');
  if (!us?.runtime_reads_cms_catalog) issues.push('澳大利亚 Ambient Runtime 未读 CMS catalog');
  if (us?.still_unsplash) issues.push('澳大利亚 Ambient 仍 Unsplash');
  if (us?.still_ts_fallback) issues.push('澳大利亚 Ambient 仍 TS fallback');
  return {
    id: 'australia_ambient_runtime',
    label: '首页 Destination Ambient · 澳大利亚读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: us || null,
...
function validateAuLockRegistryUnchanged() {
  ...
  return {
    id: 'au_lock_registry_unchanged',
    label: '三城 AU LOCK 资产 (24/24)',
    pass: issues.length === 0,
...
function validateAuScopeLock() {
  const doc = readJson(EVIDENCE.au_scope_lock);
  const issues = [];
  const poiTotal = doc?.au_denominator?.poi_total;
  const cityCount = doc?.au_denominator?.cities?.length;
  if (!doc) issues.push('CMS-AU-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 24) issues.push(`AU scope poi_total=${poiTotal || 'MISSING'} (expected 24)`);
  if (cityCount !== 3) issues.push(`AU scope city_count=${cityCount || 'MISSING'} (expected 3)`);
  if (doc?.TT_CMS_AU_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_AU_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_AU_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'au_scope_lock',
    label: 'AU POI Catalog Scope Lock · 24 POI / 3 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_AU_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
