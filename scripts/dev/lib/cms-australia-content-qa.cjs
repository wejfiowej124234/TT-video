/**
 * Australia L5 Content QA · Phase② · JP/KR/TH/SG/FR/US Country CLOSED 六模板 · 不调整标准
 */
const fs = require('fs');
...
const AU_CITY_DISPLAY_ORDER = ['悉尼', '墨尔本', '黄金海岸'];

const AU_COUNTRY_CRITERIA = [
...
function assertAuTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort AU kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort AU kickoff`);
  }
}

function assessAuCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_AU_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'Australia Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_AU_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'Australia Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 AU Country Runtime audit · 先跑 run-cms-au-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessAuCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessAuCountryRuntimeConsumer();
  const usRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-AU-'),
  ).length;
  const usRuntimePass = usRuntimeAudit?.TT_CMS_AU_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
...
function buildAustraliaBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildAustraliaContentQa(options = {}) {
  if (!options.skip_template_guard) assertAuTemplateCountriesClosed();
  ...
  const cities = AU_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'AU').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildAustraliaBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessAuCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_australia_content_qa.v1',
    ...
    TT_CMS_AU_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_AU_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_AU_COUNTRY: country.TT_CMS_AU_COUNTRY,
  };
}

module.exports = {
  AU_CITY_DISPLAY_ORDER,
  buildAustraliaContentQa,
  assertAuTemplateCountriesClosed: assertAuTemplateCountriesClosed,
  assessAuCountry,
};
