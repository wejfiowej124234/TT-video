/**
 * Thailand L5 Content QA · Phase③ · JP + KR Country CLOSED 双模板 · 不调整标准
 */
const fs = require('fs');
...
const TH_CITY_DISPLAY_ORDER = ['曼谷', '普吉', '清迈'];

const TH_COUNTRY_CRITERIA = [
...
function assertTemplateCountriesClosed() {
  for (const [label, p] of [
    ['JP', JP_COUNTRY_CLOSURE],
    ['KR', KR_COUNTRY_CLOSURE],
  ]) {
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort TH kickoff`);
    const doc = readJson(p);
    const key = label === 'JP' ? 'TT_CMS_JP_COUNTRY' : 'TT_CMS_KR_COUNTRY';
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort TH kickoff`);
  }
}

function assessThCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-TH-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_TH_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'Thailand Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-TH-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_TH_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'Thailand Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-TH-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 TH Country Runtime audit · 先跑 run-cms-th-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessThCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessThCountryRuntimeConsumer();
  const thRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-TH-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-TH-'),
  ).length;
  const thRuntimePass = thRuntimeAudit?.TT_CMS_TH_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
  criteria.unsplash_0 = thRuntimePass || poiUnsplashRuntime === 0;
  criteria.ocs_runtime_0 = thRuntimePass;
...
function buildThailandBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildThailandContentQa(options = {}) {
  if (!options.skip_template_guard) assertTemplateCountriesClosed();
  ...
  const cities = TH_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'TH').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildThailandBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessThCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_thailand_content_qa.v1',
    ...
    summary: {
      ...
      active_city: registry?.active_city || { country_iso: 'TH', city_zh: '曼谷' },
    },
    TT_CMS_TH_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_TH_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_TH_COUNTRY: country.TT_CMS_TH_COUNTRY,
  };
}

module.exports = {
  TH_CITY_DISPLAY_ORDER,
  buildThailandContentQa,
  assertTemplateCountriesClosed,
  assessThCountry,
};
