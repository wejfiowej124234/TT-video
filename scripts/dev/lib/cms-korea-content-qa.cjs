/**
 * Korea L5 Content QA · Phase③ · 日本 Golden Template 同源 · 不调整标准
 */
const fs = require('fs');
const path = require('path');
const { CITY_PILOTS, readRegistry } = require('./cms-poi-city-pilot.cjs');
const { getAsset, loadStandard } = require('./cms-content-qa-asset-lock.cjs');
const {
  buildCityQa,
...
function assessKrCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_KR_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'Korea Country Runtime audit PASS · 四城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_KR_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'Korea Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 KR Country Runtime audit · 先跑 run-cms-kr-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessKrCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessKrCountryRuntimeConsumer();
  const krRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-KR-'),
  ).length;
  const krRuntimePass = krRuntimeAudit?.TT_CMS_KR_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
  criteria.unsplash_0 = krRuntimePass || poiUnsplashRuntime === 0;
  criteria.ocs_runtime_0 = krRuntimePass;
...
function buildKoreaBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildKoreaContentQa(options = {}) {
  if (!fs.existsSync(JP_COUNTRY_CLOSURE)) {
    throw new Error('JP country not CLOSED — run CMS-JP-COUNTRY-CLOSURE first');
  }
  ...
  const cities = KR_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'KR').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildKoreaBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessKrCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_korea_content_qa.v1',
    ...
    summary: {
      ...
      active_city: registry?.active_city || { country_iso: 'KR', city_zh: '首尔' },
    },
    TT_CMS_KR_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) ? 'CLOSED' : 'OPEN',
    TT_CMS_KR_COUNTRY: country.TT_CMS_KR_COUNTRY,
  };
}

module.exports = {
  KR_CITY_DISPLAY_ORDER,
  KR_COUNTRY_CRITERIA,
  buildKoreaContentQa,
  assessKrCountry,
};
