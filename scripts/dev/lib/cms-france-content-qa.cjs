/**
 * France L5 Content QA · Phase② · JP + KR + TH + SG Country CLOSED 四模板 · 不调整标准
 */
const fs = require('fs');
...
const FR_CITY_DISPLAY_ORDER = ['巴黎', '里昂', '尼斯'];

const FR_COUNTRY_CRITERIA = [
...
function assertTemplateCountriesClosed() {
  for (const [label, p, key] of [
    ...
    if (!fs.existsSync(p)) throw new Error(`${label} country not CLOSED — abort FR kickoff`);
    const doc = readJson(p);
    if (doc?.[key] !== 'CLOSED') throw new Error(`${key}=${doc?.[key] || 'MISSING'} — abort FR kickoff`);
  }
}

function assessFrCountryRuntimeConsumer() {
  const auditPath = path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json');
  const audit = readJson(auditPath);
  if (audit?.TT_CMS_FR_COUNTRY_RUNTIME === 'PASS') {
    return {
      verdict: 'PASS',
      reason: 'France Country Runtime audit PASS · 三城 Consumer + Catalog + Ambient',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
      poi_runtime: `${audit.poi_runtime_pass}/${audit.poi_runtime_required}`,
    };
  }
  if (audit?.TT_CMS_FR_COUNTRY_RUNTIME === 'FAIL') {
    return {
      verdict: 'FAIL',
      reason: 'France Country Runtime audit FAIL',
      tier: 'national_runtime',
      ssot: 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
  ...
  return {
    verdict: 'UNKNOWN',
    reason: '缺少 FR Country Runtime audit · 先跑 run-cms-fr-country-runtime-audit.cjs',
    tier: 'national_runtime',
  };
}

function assessFrCountry(cities, visualGap, ambientWiring, options = {}) {
  ...
  const countryRuntime = assessFrCountryRuntimeConsumer();
  const frRuntimeAudit = readJson(path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json'));
  ...
  const poiUnsplashRuntime = (visualGap?.assets || []).filter(
    (a) =>
      ...
      String(a.matrix_id || '').startsWith('PH-FR-'),
  ).length;
  const frRuntimePass = frRuntimeAudit?.TT_CMS_FR_COUNTRY_RUNTIME === 'PASS' || options.country_closed === true;
  criteria.unsplash_0 = frRuntimePass || poiUnsplashRuntime === 0;
  criteria.ocs_runtime_0 = frRuntimePass;
...
function buildFranceBacklog(cities) {
  const tree = {};
  let totalIssues = 0;
...
function buildFranceContentQa(options = {}) {
  ...
  const cities = FR_CITY_DISPLAY_ORDER.filter((zh) => CITY_PILOTS[zh]?.country_iso === 'FR').map((cityZh) =>
    buildCityQa(cityZh, matrixText, registry, visualGap),
  );

  const backlog = buildFranceBacklog(cities);
  const activeCities = cities.filter((c) => c.execution.verdict !== 'WAITING');
  const country = assessFrCountry(cities, visualGap, ambientWiring, options);

  return {
    schema: 'traveltrust.cms_france_content_qa.v1',
    ...
    summary: {
      ...
      active_city: registry?.active_city || { country_iso: 'FR', city_zh: '巴黎' },
    },
    TT_CMS_FR_CONTENT_QA: activeCities.every((c) => c.content_qa_closed) && country.TT_CMS_FR_COUNTRY === 'CLOSED' ? 'CLOSED' : 'OPEN',
    TT_CMS_FR_COUNTRY: country.TT_CMS_FR_COUNTRY,
  };
}

module.exports = {
  FR_CITY_DISPLAY_ORDER,
  buildFranceContentQa,
