/**
 * China Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const CN_CITIES = ["北京","上海","广州","成都","杭州","西安","厦门","青岛","大理"];
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-CN-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-China-CONTENT-QA-LATEST.json'),
...
function cityQaEvidenceToken(cityZh) {
  return CN_CITY_QA_EVIDENCE_TOKEN[cityZh] || cityEvidenceToken(cityZh);
}
...
function cityExecClosurePath(cityZh) {
  return path.join(
    ROOT,
    'evidence/GO_cms_operation',
    `CMS-POI-CITY-${cityQaEvidenceToken(cityZh)}-CLOSURE-LATEST.json`,
  );
}
...
function validateCnCityExitChecks() {
  const rows = [];
  const issues = [];
...
function validateCnCityContentQaClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of CN_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
...
function validateCnCityExecutionClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of CN_CITIES) {
...
async function validateCatalogApiAllCnPois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of CN_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
  ...
  return {
    id: 'catalog_api_cn_poi',
    label: `Catalog API CN POI ${passCount}/${total} 与 LOCK 一致`,
    pass: issues.length === 0,
...
function validateChinaAmbient() {
  ...
  if (!us?.runtime_reads_cms_catalog) issues.push('中国 Ambient Runtime 未读 CMS catalog');
  if (us?.still_unsplash) issues.push('中国 Ambient 仍 Unsplash');
  if (us?.still_ts_fallback) issues.push('中国 Ambient 仍 TS fallback');
  return {
    id: 'china_ambient_runtime',
    label: '首页 Destination Ambient · 中国读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: us || null,
...
function validateCnLockRegistryUnchanged() {
  const issues = [];
  const rows = [];
  for (const cityZh of CN_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
  ...
  return {
    id: 'cn_lock_registry_unchanged',
    label: '九城 CN LOCK 资产 (24/24)',
    pass: issues.length === 0,
...
function validateCnScopeLock() {
  const doc = readJson(EVIDENCE.cn_scope_lock);
  const issues = [];
  const poiTotal = doc?.cn_denominator?.poi_total;
  const cityCount = doc?.cn_denominator?.cities?.length;
  if (!doc) issues.push('CMS-CN-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 91) issues.push(`CN scope poi_total=${poiTotal || 'MISSING'} (expected 91)`);
  if (cityCount !== 9) issues.push(`CN scope city_count=${cityCount || 'MISSING'} (expected 9)`);
  if (doc?.TT_CMS_CN_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_CN_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_CN_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'cn_scope_lock',
    label: 'CN POI Catalog Scope Lock · 91 POI / 9 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_CN_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
