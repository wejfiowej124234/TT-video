/**
 * Spain Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-SPAIN-CONTENT-QA-LATEST.json'),
...
function cityQaEvidenceToken(cityZh) {
  return ES_CITY_QA_EVIDENCE_TOKEN[cityZh] || cityEvidenceToken(cityZh);
}
...
function validateEsCityExitChecks() {
  const rows = [];
  const issues = [];
...
function validateEsCityContentQaClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of ES_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
...
function validateEsCityExecutionClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of ES_CITIES) {
...
async function validateCatalogApiAllEsPois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of ES_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
...
function validateSpainAmbient() {
  ...
  if (!us?.runtime_reads_cms_catalog) issues.push('西班牙 Ambient Runtime 未读 CMS catalog');
  if (us?.still_unsplash) issues.push('西班牙 Ambient 仍 Unsplash');
  if (us?.still_ts_fallback) issues.push('西班牙 Ambient 仍 TS fallback');
  return {
    id: 'spain_ambient_runtime',
    label: '首页 Destination Ambient · 西班牙读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: us || null,
...
function validateEsLockRegistryUnchanged() {
  ...
  return {
    id: 'es_lock_registry_unchanged',
    label: '三城 ES LOCK 资产 (24/24)',
    pass: issues.length === 0,
...
function validateEsScopeLock() {
  const doc = readJson(EVIDENCE.es_scope_lock);
  const issues = [];
  const poiTotal = doc?.es_denominator?.poi_total;
  const cityCount = doc?.es_denominator?.cities?.length;
  if (!doc) issues.push('CMS-ES-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 24) issues.push(`ES scope poi_total=${poiTotal || 'MISSING'} (expected 24)`);
  if (cityCount !== 3) issues.push(`ES scope city_count=${cityCount || 'MISSING'} (expected 3)`);
  if (doc?.TT_CMS_ES_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_ES_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_ES_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'es_scope_lock',
    label: 'ES POI Catalog Scope Lock · 24 POI / 3 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_ES_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
