/**
 * UAE Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-AE-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-UAE-CONTENT-QA-LATEST.json'),
...
function cityQaEvidenceToken(cityZh) {
  return AE_CITY_QA_EVIDENCE_TOKEN[cityZh] || cityEvidenceToken(cityZh);
}
...
function validateAeCityExitChecks() {
  const rows = [];
  const issues = [];
...
function validateAeCityContentQaClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of AE_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
...
function validateAeCityExecutionClosures() {
  const rows = [];
  const issues = [];
  for (const cityZh of AE_CITIES) {
...
async function validateCatalogApiAllAePois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of AE_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
...
function validateUAEAmbient() {
  ...
  if (!us?.runtime_reads_cms_catalog) issues.push('阿联酋 Ambient Runtime 未读 CMS catalog');
  if (us?.still_unsplash) issues.push('阿联酋 Ambient 仍 Unsplash');
  if (us?.still_ts_fallback) issues.push('阿联酋 Ambient 仍 TS fallback');
  return {
    id: 'uae_ambient_runtime',
    label: '首页 Destination Ambient · 阿联酋读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: us || null,
...
function validateAeLockRegistryUnchanged() {
  ...
  return {
    id: 'ae_lock_registry_unchanged',
    label: '三城 AE LOCK 资产 (24/24)',
    pass: issues.length === 0,
...
function validateAeScopeLock() {
  const doc = readJson(EVIDENCE.ae_scope_lock);
  const issues = [];
  const poiTotal = doc?.ae_denominator?.poi_total;
  const cityCount = doc?.ae_denominator?.cities?.length;
  if (!doc) issues.push('CMS-AE-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 24) issues.push(`AE scope poi_total=${poiTotal || 'MISSING'} (expected 24)`);
  if (cityCount !== 3) issues.push(`AE scope city_count=${cityCount || 'MISSING'} (expected 3)`);
  if (doc?.TT_CMS_AE_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_AE_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_AE_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'ae_scope_lock',
    label: 'AE POI Catalog Scope Lock · 24 POI / 3 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_AE_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
