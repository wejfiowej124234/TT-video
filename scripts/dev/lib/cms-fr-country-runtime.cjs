/**
 * France Country Runtime · national_runtime tier · Content QA frozen standard 同源
...
const EVIDENCE = {
  runtime_audit: path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-COUNTRY-RUNTIME-AUDIT-LATEST.json'),
  country_closure: path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-COUNTRY-CLOSURE-LATEST.json'),
  content_qa: path.join(ROOT, 'evidence/GO_cms_operation/CMS-FRANCE-CONTENT-QA-LATEST.json'),
  frozen_standard: path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-STANDARD-FROZEN.v1.json'),
  fr_scope_lock: path.join(ROOT, 'evidence/GO_cms_operation/CMS-FR-POI-CATALOG-SCOPE-LOCK-LATEST.json'),
};
...
function cityExitPath(cityZh) {
  return path.join(
    ...
    `CMS-POI-CITY-${cityEvidenceToken(cityZh)}-CONTENT-QA-EXIT-CHECK-LATEST.json`,
  );
}
...
function cityContentQaClosurePath(cityZh) {
  return path.join(
    ROOT,
    'evidence/GO_cms_operation',
    `CMS-POI-CITY-${cityEvidenceToken(cityZh)}-CONTENT-QA-CLOSURE-LATEST.json`,
  );
}
...
async function validateCatalogApiAllFrPois(apiBase) {
  const issues = [];
  const rows = [];
  for (const cityZh of FR_CITIES) {
    const pilot = CITY_PILOTS[cityZh];
    const r = await fetchJson(
      `${apiBase}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent(cityZh)}&limit=50`,
    );
    if (!r.ok) {
  ...
  return {
    id: 'catalog_api_fr_poi',
    label: `Catalog API FR POI ${passCount}/${total} 与 LOCK 一致`,
    pass: issues.length === 0,
...
function validateFranceAmbient() {
  const ambient = loadAmbientRuntimeWiringSsot();
  const fr = ambient.by_iso?.FR;
  const issues = [];
  if (!ambient.is_closed) issues.push('Ambient wiring SSOT not CLOSED (10/10)');
  if (!fr?.runtime_reads_cms_catalog) issues.push('法国 Ambient Runtime 未读 CMS catalog');
  if (fr?.still_unsplash) issues.push('法国 Ambient 仍 Unsplash');
  if (fr?.still_ts_fallback) issues.push('法国 Ambient 仍 TS fallback');
  return {
    id: 'france_ambient_runtime',
    label: '首页 Destination Ambient · 法国读 CMS · 无 Unsplash',
    pass: issues.length === 0,
    row: fr || null,
...
function validateFrScopeLock() {
  const doc = readJson(EVIDENCE.fr_scope_lock);
  const issues = [];
  const poiTotal = doc?.fr_denominator?.poi_total;
  const cityCount = doc?.fr_denominator?.cities?.length;
  if (!doc) issues.push('CMS-FR-POI-CATALOG-SCOPE-LOCK-LATEST.json missing');
  if (poiTotal !== 24) issues.push(`FR scope poi_total=${poiTotal || 'MISSING'} (expected 24)`);
  if (cityCount !== 3) issues.push(`FR scope city_count=${cityCount || 'MISSING'} (expected 3)`);
  if (doc?.TT_CMS_FR_POI_CATALOG_SCOPE_LOCK !== 'FROZEN') {
    issues.push(`TT_CMS_FR_POI_CATALOG_SCOPE_LOCK=${doc?.TT_CMS_FR_POI_CATALOG_SCOPE_LOCK || 'MISSING'}`);
  }
  return {
    id: 'fr_scope_lock',
    label: 'FR POI Catalog Scope Lock · 24 POI / 3 cities 不变',
    pass: issues.length === 0,
    doc: doc
      ? { poi_total: poiTotal, city_count: cityCount, TT: doc.TT_CMS_FR_POI_CATALOG_SCOPE_LOCK }
      : null,
    issues,
  };
}
