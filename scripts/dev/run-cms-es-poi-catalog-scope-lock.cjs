#!/usr/bin/env node
/**
 * ES POI Catalog Scope Lock · Country → City → POI 分母 · ② staging
 *
 * 前置: TT_CMS_JP/KR/TH/SG/FR/US_COUNTRY: CLOSED
 * 不得修改 JP 41 · KR 31 · TH 28 · SG 10 · FR 24 · US 33 · AU 24 LOCK · Content QA 标准
 */
const fs = require('fs');
const path = require('path');
const { buildPoiCatalogScope } = require('./lib/cms-poi-catalog-scope.cjs');
const { buildEsPilotWaves } = require('./lib/cms-es-poi-pilot-waves.cjs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-POI-CATALOG-SCOPE-LOCK-LATEST.json');
const PILOT_WAVE_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-POI-PILOT-WAVE-LATEST.json');
const REGISTRY_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CLOSURE-REGISTRY-LATEST.json');
const POI_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-POI-WAVE-KICKOFF-LATEST.json');
const LOCK_REG = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-ASSET-LOCK-REGISTRY-LATEST.json');

async function probeBarcelonaCatalog(api) {
  const r = await request(`${api}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent('巴塞罗那')}&limit=50`);
  const items = r.json?.items || [];
  return { http: r.status, count: items.length, catalog_empty: items.length === 0 };
}

function assertCountryClosed(p, key) {
  if (!fs.existsSync(p)) throw new Error(`${key} closure missing — abort ES kickoff`);
  const doc = JSON.parse(fs.readFileSync(p, 'utf8'));
  if (doc[key] !== 'CLOSED') throw new Error(`${key}=${doc[key] || 'MISSING'} — abort ES kickoff`);
}
    ...
    JSON.stringify(
      {
        ...
        active_country: { country_iso: 'ES', country_zh: '西班牙', status: 'ACTIVE' },
        active_city: { country_iso: 'ES', city_zh: '巴塞罗那', city_en: 'Barcelona', status: 'ACTIVE' },
        active_wave: pilot.active_wave,
        template_countries: ['JP', 'KR', 'TH', 'SG', 'FR', 'US', 'AU'],
        es_scope_lock: 'evidence/GO_cms_operation/CMS-ES-POI-CATALOG-SCOPE-LOCK-LATEST.json',
...
async function main() {
  ...
  const esRows = scope.rows.filter((r) => r.country_iso === 'ES');
  const esByCity = {};
  for (const row of esRows) {
    esByCity[row.city_zh] = esByCity[row.city_zh] || { attraction: 0, food: 0, total: 0, matrix_ids: [] };
    esByCity[row.city_zh][row.poi_type === 'food' ? 'food' : 'attraction'] += 1;
    esByCity[row.city_zh].total += 1;
    esByCity[row.city_zh].matrix_ids.push(row.matrix_id);
  }

  const pilot = buildEsPilotWaves(scope);
  const active = pilot.active_catalog_build;
  const bcn = getCityPilot('巴塞罗那');
  const catalog = await probeBarcelonaCatalog(API);

  const report = {
    schema: 'traveltrust.cms_es_poi_catalog_scope_lock.v1',
    ...
    country: { country_iso: 'ES', country_zh: '西班牙' },
    es_denominator: {
      cities: Object.keys(esByCity).sort(),
      city_breakdown: esByCity,
      poi_total: esRows.length,
      attraction: esRows.filter((r) => r.poi_type === 'attraction').length,
      food: esRows.filter((r) => r.poi_type === 'food').length,
    },
    pilot_wave: pilot,
    active_catalog_build: active,
    barcelona_pilot: {
      city_zh: '巴塞罗那',
      poi_count: bcn.matrix_ids.length,
      matrix_ids: bcn.matrix_ids,
      acceptance: '巴塞罗那 CLOSED',
    },
    catalog_probe: catalog,
    workflow: ['ES Scope Lock', '巴塞罗那 → 马德里 → 塞维利亚', 'ES Country Runtime', 'TT_CMS_ES_COUNTRY: CLOSED'],
    ...
    TT_CMS_ES_POI_CATALOG_SCOPE_LOCK: 'FROZEN',
    TT_CMS_ES_POI_DENOMINATOR_TOTAL: esRows.length,
    TT_CMS_ES_POI_PILOT_ACCEPTANCE: '巴塞罗那 CLOSED',
    TT_CMS_ES_POI_NEXT_STAGE: 'CATALOG_BUILD',
    spain_status: 'ACTIVE',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(report, null, 2) + '\n');
  fs.writeFileSync(PILOT_WAVE_LATEST, JSON.stringify({ schema: 'traveltrust.cms_es_poi_pilot_wave.v1', stamp_utc: stamp, ...pilot }, null, 2) + '\n');
  writeRegistry(stamp, pilot);
  fs.writeFileSync(
    POI_KICKOFF,
    JSON.stringify(
      {
        schema: 'traveltrust.cms_es_poi_wave_kickoff.v1',
        stamp_utc: stamp,
        country_iso: 'ES',
        status: 'SCOPE_LOCKED',
        active_city: '巴塞罗那',
        template: 'JP + KR + TH + SG + FR + US + AU Golden Template',
        TT_CMS_ES_POI_WAVE: 'SCOPE_LOCKED_CATALOG_BUILD_NEXT',
      },
      null,
      2,
    ) + '\n',
  ...
  const hierarchy = buildCmsOpsHierarchy({ scope: { ...scope, pilot_waves: pilot, es_active: true } });
  writeHierarchyLatest(hierarchy, stamp.replace(/[-:]/g, '').replace(/\.\d{3}Z$/, 'Z'));

  const { buildSpainContentQa } = require('./lib/cms-spain-content-qa.cjs');
  fs.writeFileSync(
    path.join(ROOT, 'evidence/GO_cms_operation/CMS-SPAIN-CONTENT-QA-LATEST.json'),
    JSON.stringify(buildSpainContentQa({ stamp_utc: stamp, skip_template_guard: true }), null, 2) + '\n',
  );

  console.log(`TT_CMS_ES_POI_CATALOG_SCOPE_LOCK: ${report.TT_CMS_ES_POI_CATALOG_SCOPE_LOCK}`);
  console.log(`TT_CMS_ES_POI_DENOMINATOR_TOTAL: ${report.TT_CMS_ES_POI_DENOMINATOR_TOTAL} (3 cities · pilot 巴塞罗那 ${active.poi_count})`);
  console.log(`JP 41 · KR 31 · TH 28 · SG 10 · FR 24 · US 33 · AU 24 LOCK unchanged`);
  console.log(`Evidence: ${OUT_LATEST}`);
  console.log('\nNext: node scripts/dev/run-cms-barcelona-content-qa-wave.cjs');
}
