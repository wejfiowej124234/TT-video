#!/usr/bin/env node
/**
 * Australia Country CLOSED · national_runtime + 三城 Content QA 汇总
 *
 * 前置：run-cms-au-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH/SG/FR LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-au-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildAustraliaContentQa, AU_CITY_DISPLAY_ORDER } = require('./lib/cms-australia-content-qa.cjs');
const {
  EVIDENCE,
  readJson,
  AU_CITIES,
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-AU-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return AU_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_AU_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_AU_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_AU_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_AU_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }

  const report = buildAustraliaContentQa({ country_closed: true });
  const issues = [];
  ...
  if (report.country?.TT_CMS_AU_COUNTRY !== 'CLOSED') {
  ...
  const closure = {
    schema: 'traveltrust.cms_au_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-AU-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    australia_content_qa_ssot: 'evidence/GO_cms_operation/CMS-AUSTRALIA-CONTENT-QA-LATEST.json',
    ...
    checks: {
      ...
      au_lock_registry_unchanged: true,
  ...
  const refreshed = buildAustraliaContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_AU_COUNTRY: CLOSED');
  console.log('TT_CMS_AU_CONTENT_QA: CLOSED');
  ...
  console.log('AU LOCK: 24/24 unchanged');
