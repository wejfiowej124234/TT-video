...
[[ -n "$STAMP" ]] || { echo "record-cert6: --stamp required" >&2; exit 2; }
[[ -n "$SIGNER" ]] || { echo "record-cert6: --signer required" >&2; exit 2; }
ARGS=(python "$ROOT/scripts/dev/record-cert6-phase-b-unpause-signoff.py" --stamp "$STAMP" --signer "$SIGNER")
