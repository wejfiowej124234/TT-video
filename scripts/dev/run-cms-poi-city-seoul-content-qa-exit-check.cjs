#!/usr/bin/env node
/**
 * Seoul Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-seoul-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/seoul');

const SEOUL = getCityPilot('首尔');
const POI_COUNT = SEOUL.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '首尔');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent('首尔')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '首尔', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = SEOUL.pois.slice(0, 5);
    const foodPois = SEOUL.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `seoul-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < SEOUL.matrix_ids.length; i++) {
      const matrixId = SEOUL.matrix_ids[i];
      const poi = SEOUL.pois[i];
...
function checkNoBacklogUnlock() {
  const open = SEOUL.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = SEOUL.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Seoul Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_SEOUL_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '首尔', city_en: 'Seoul', country_iso: 'KR', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_SEOUL_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_SEOUL_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_SEOUL_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_SEOUL_CONTENT_QA_EXIT}`);
