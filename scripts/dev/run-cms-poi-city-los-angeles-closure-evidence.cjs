#!/usr/bin/env node
/**
 * POI City Closure Evidence · US · 洛杉矶 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-san-francisco-closure-evidence.cjs
 */
const fs = require('fs');
...
const US_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-US-POI-WAVE-KICKOFF-LATEST.json');
const US_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-US-POI-PILOT-WAVE-LATEST.json');
...
const LOS_ANGELES = getCityPilot('洛杉矶');
const NEXT_CITY = nextCityPilot('洛杉矶');
...
async function main() {
  ...
  for (const matrixId of LOS_ANGELES.matrix_ids) {
  ...
  const evidenceRows = LOS_ANGELES.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent(LOS_ANGELES.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of LOS_ANGELES.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || LOS_ANGELES.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, LOS_ANGELES.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === LOS_ANGELES.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= LOS_ANGELES.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${LOS_ANGELES.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, LOS_ANGELES.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: LOS_ANGELES.country_iso,
      country_zh: LOS_ANGELES.country_zh,
      city_zh: LOS_ANGELES.city_zh,
      city_en: LOS_ANGELES.city_en,
    },
    poi_count: LOS_ANGELES.matrix_ids.length,
    matrix_ids: LOS_ANGELES.matrix_ids,
    pois: LOS_ANGELES.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === LOS_ANGELES.country_iso && c.city_zh === LOS_ANGELES.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: LOS_ANGELES.matrix_ids.length,
        closure_evidence: LOS_ANGELES.closure_latest,
        [LOS_ANGELES.closure_key]: 'CLOSED',
      },
    ],
  ...
  const kickoff = readJsonIfExists(US_KICKOFF);
  if (kickoff) {
    kickoff.stamp_utc = stamp;
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'US_CITY_PILOT_ACTIVE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : null;
    kickoff.los_angeles_city_closure = { status: 'CLOSED', evidence: LOS_ANGELES.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-US-CITY-${NEXT_CITY.slug.toUpperCase()}`,
          label: `US · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_US_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'LOS_ANGELES_CLOSED';
    kickoff[LOS_ANGELES.closure_key] = 'CLOSED';
    kickoff.TT_CMS_POI_ACTIVE_CITY = NEXT_CITY ? `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}` : null;
    fs.writeFileSync(US_KICKOFF, JSON.stringify(kickoff, null, 2) + '\n');
  }

  const pilotWave = readJsonIfExists(US_PILOT_WAVE);
  if (pilotWave) {
    pilotWave.stamp_utc = stamp;
    pilotWave.los_angeles_closed = true;
    pilotWave.active_catalog_build = kickoff?.active_catalog_build || registry.active_city;
    pilotWave[LOS_ANGELES.closure_key] = 'CLOSED';
    fs.writeFileSync(US_PILOT_WAVE, JSON.stringify(pilotWave, null, 2) + '\n');
  }
