#!/usr/bin/env node
/**
 * Phuket Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-phuket-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/phuket');

const PHUKET = getCityPilot('普吉');
const POI_COUNT = PHUKET.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PHUKET-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  const issues = [];
  const rows = [];
  for (const matrixId of PHUKET.matrix_ids) {
    const poi = PHUKET.pois[PHUKET.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '普吉');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=TH&city=${encodeURIComponent('普吉')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '泰国' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '普吉', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = PHUKET.pois.slice(0, 5);
    const foodPois = PHUKET.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `phuket-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < PHUKET.matrix_ids.length; i++) {
      const matrixId = PHUKET.matrix_ids[i];
      const poi = PHUKET.pois[i];
...
function checkNoBacklogUnlock() {
  const open = PHUKET.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = PHUKET.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Phuket Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_PHUKET_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '普吉', city_en: 'Phuket', country_iso: 'TH', poi_count: POI_COUNT },
    replicated_from: 'JP Golden Template (TT_CMS_JP_COUNTRY: CLOSED)',
    web_base: WEB,
    api_base: API,
    checks,
    all_pass: allPass,
    ...
    TT_CMS_POI_CITY_PHUKET_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_PHUKET_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_TH_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_PHUKET_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_PHUKET_CONTENT_QA_EXIT}`);
