#!/usr/bin/env node
/**
 * Seville · Execution + Content QA Wave · JP + KR 双模板同源
 *
 *   TRAVELTRUST_STAGING_RC_BASELINE_OVERRIDE=1 \
 *   API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-seville-content-qa-wave.cjs
 *
...
assertStagingBaselineMutationAuthorized('cms_seville_content_qa_wave');

const ROOT = path.join(__dirname, '../..');
const API = (process.env.API || 'https://tt-api-staging.fly.dev').replace(/\/$/, '');
const SEVILLE = getCityPilot('塞维利亚');

const SEVILLE_HEROES = SEVILLE.hero_files;

async function syncAllHeroes() {
  const unique = [...new Set(Object.values(SEVILLE_HEROES))];
  console.log(`\n===== Pre-sync staging heroes (${unique.length} unique / ${SEVILLE.matrix_ids.length} POI) =====`);
...
async function runExitCheckWithRetry(maxAttempts = 12) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (attempt === maxAttempts) throw new Error('Seville exit check failed after retries');
...
async function main() {
  console.log('TT_CMS_SEVILLE_CONTENT_QA_WAVE: START');

  console.log('\n===== Catalog Build · 塞维利亚 =====');
  runNode('scripts/dev/run-cms-phase1-poi-pilot-catalog-build.cjs', '--city-zh 塞维利亚');
  ...
  for (const matrixId of SEVILLE.matrix_ids) {
    ...
    runNode(
      ...
      `--city-zh 塞维利亚 --matrix-id ${matrixId} --poi "${poi}"`,
    );
  }

  console.log('\n===== Execution closed loop · 塞维利亚 8/8 =====');
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs', '--city-zh 塞维利亚 --skip-refresh');

  console.log('\n===== Execution CLOSED · 塞维利亚 =====');
  ...
  console.log('\nTT_CMS_SEVILLE_CONTENT_QA_WAVE: DONE');
}
