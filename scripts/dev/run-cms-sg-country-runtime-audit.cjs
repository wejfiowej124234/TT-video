#!/usr/bin/env node
/**
 * Singapore Country Runtime Audit · national_runtime · ② staging
 *
 * 验证新加坡相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · SG LOCK · JP/KR/TH LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-sg-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  filename,
  fetchJson,
} = require('./lib/cms-sg-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/sg-country');

const CONSUMER_SURFACES = [
  { id: 'home_singapore', path: '/', setup: 'home_singapore' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_sg', path: '/market', setup: 'market_sg_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'sg_poi_runtime_exit_aggregate',
    label: '新加坡 Market POI Runtime · Exit Check Gate 4 聚合',
...
function isSingaporeScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /singapore|sg\b|marina|sentosa|chinatown|little india|新加坡|滨海湾|圣淘沙|牛车水|小印度|叻沙|肉骨茶|辣椒螃蟹|椰浆饭|海南鸡饭/.test(
    blob,
  );
}

async function scanPageImages(page, scope, { singaporeOnly = false } = {}) {
  ...
  for (const img of images) {
    if (singaporeOnly && !isSingaporeScopedImage(img)) continue;
...
async function catalogSingaporeAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=SG`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const sgBtn = page.locator('#landing-hero-form').getByRole('button', { name: '新加坡', exact: true });
    const sgBtnVisible = await sgBtn.isVisible().catch(() => false);
    if (sgBtnVisible) {
      await sgBtn.scrollIntoViewIfNeeded();
      await sgBtn.click();
      ...
      surfaceRows.push({ surface: 'home_singapore_ambient', pass: ambientOk, runtime_src: ambientSrc });
      if (!ambientOk) issues.push(`home 新加坡 ambient: ${ambientSrc || 'MISSING'}`);
    } else {
      const ambientCatalog = await catalogSingaporeAmbientUrl();
      const ambientOk =
      ...
      surfaceRows.push({
        surface: 'home_singapore_ambient_catalog_fallback',
        pass: ambientOk,
        runtime_src: ambientCatalog.url,
        note: 'landing 新加坡 pill not visible · catalog media fallback',
      });
      if (!ambientOk) issues.push(`home 新加坡 ambient catalog fallback: ${ambientCatalog.url || 'MISSING'}`);
    }
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `sg-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { singaporeOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页新加坡 Ambient + Market 壳 + 新加坡相关内容无 fallback',
...
function formatMd(report) {
  const lines = [
    '# Singapore Country Runtime Audit',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**TT_CMS_SG_COUNTRY_RUNTIME:** \`${report.TT_CMS_SG_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
  ...
  lines.push('', `**SG POI Runtime:** ${report.poi_runtime_pass}/${report.poi_runtime_required} PASS`);
  lines.push('', '**新加坡汇总**', '');
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateCityExitChecks(),
    validateSingaporeAmbient(),
    await validateCatalogApiAllSgPois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = SG_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = SG_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'singapore_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows = staticChecks.find((c) => c.id === 'catalog_api_sg_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_sg_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'SG', country_zh: '新加坡', cities: SG_CITIES },
    ...
    verdict: allPass ? 'SG_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_SG_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_SG_COUNTRY_RUNTIME: ${report.TT_CMS_SG_COUNTRY_RUNTIME}`);
  console.log(`SG POI Runtime: ${poiRuntimePass}/${poiRequired} PASS`);
