#!/usr/bin/env node
/**
 * Monitor Barcelona wave · 5min stall → kill + resume · 8/8 → closure + exit
 * 不得改动 JP/KR/TH/SG/FR/US/AU LOCK · 不启动马德里 / ES Country Runtime
 */
const fs = require('fs');
...
const TERM_LOG = process.env.TERM_LOG || path.join(ROOT, 'evidence/GO_cms_operation/_barcelona-wave-monitor.log');
...
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CLOSURE-LATEST.json');
const QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CONTENT-QA-CLOSURE-LATEST.json');
const EXIT_CHECK = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-BARCELONA-CONTENT-QA-EXIT-CHECK-LATEST.json');
const WAVE_SCRIPT = 'scripts/dev/run-cms-barcelona-content-qa-wave.cjs';
...
function barcelonaLockedCount() {
  return BARCELONA.matrix_ids.filter((id) => getAsset(id).state === 'LOCKED').length;
}
...
function wavePid() {
  ...
  try {
    ...
    for (const line of out.split('\n')) {
      if (line.includes('run-cms-barcelona-content-qa-wave.cjs') && !line.includes('_monitor-barcelona-wave')) {
...
function runClosurePipeline() {
  ...
  runNode('scripts/dev/run-cms-phase1-poi-wave1-closed-loop.cjs --city-zh 巴塞罗那 --skip-refresh');
  runNode('scripts/dev/run-cms-poi-city-barcelona-closure-evidence.cjs');
  runNode('scripts/dev/run-cms-poi-city-barcelona-content-qa-closure-evidence.cjs');
  for (let i = 1; i <= 12; i++) {
    try {
      runNode('scripts/dev/run-cms-poi-city-barcelona-content-qa-exit-check.cjs');
      break;
    } catch {
...
function verifyDone() {
  ...
  return {
    exec: execDoc?.TT_CMS_POI_CITY_BARCELONA === 'CLOSED',
    qa: qaDoc?.TT_CMS_POI_CITY_BARCELONA_CONTENT_QA === 'CLOSED',
    exit: exitDoc?.TT_CMS_POI_CITY_BARCELONA_CONTENT_QA_EXIT === 'PASS' && exitDoc?.all_pass === true,
    locked: barcelonaLockedCount(),
  };
}
...
async function main() {
  ...
  let lastLockCount = barcelonaLockedCount();
  ...
  while (true) {
    const locked = barcelonaLockedCount();
  ...
  log('BARCELONA TRIPLE PASS — paused · awaiting Madrid confirmation (not auto-starting)');
}
