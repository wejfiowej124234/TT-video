#!/usr/bin/env node
/**
 * Incheon Content QA Exit Check · JP Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-incheon-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/incheon');

const INCHEON = getCityPilot('仁川');
const POI_COUNT = INCHEON.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-INCHEON-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '仁川');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent('仁川')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '韩国' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '仁川', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = INCHEON.pois.slice(0, 3);
    const foodPois = INCHEON.pois.slice(3);
    ...
    const shotPath = path.join(SHOT_DIR, `incheon-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < INCHEON.matrix_ids.length; i++) {
      const matrixId = INCHEON.matrix_ids[i];
      const poi = INCHEON.pois[i];
...
function checkNoBacklogUnlock() {
  const open = INCHEON.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = INCHEON.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Incheon Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_INCHEON_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '仁川', city_en: 'Incheon', country_iso: 'KR', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_INCHEON_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_INCHEON_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_KR_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_INCHEON_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_INCHEON_CONTENT_QA_EXIT}`);
