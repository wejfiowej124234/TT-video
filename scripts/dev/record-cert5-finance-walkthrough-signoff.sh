...
[[ -n "$STAMP" ]] || { echo "record-cert5: --stamp required" >&2; exit 2; }
[[ -n "$SIGNER" ]] || { echo "record-cert5: --signer required" >&2; exit 2; }
ARGS=(python "$ROOT/scripts/dev/record-cert5-finance-walkthrough-signoff.py" --stamp "$STAMP" --signer "$SIGNER")
