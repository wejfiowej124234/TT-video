#!/usr/bin/env node
/**
 * POI City Closure Evidence · KR · 仁川 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-incheon-closure-evidence.cjs
 */
const fs = require('fs');
...
const KR_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-POI-WAVE-KICKOFF-LATEST.json');
const KR_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-POI-PILOT-WAVE-LATEST.json');
...
const NEXT_CITY = nextCityPilot('仁川');
...
async function main() {
  ...
  const evidenceRows = INCHEON.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent(INCHEON.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of INCHEON.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || INCHEON.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, INCHEON.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === INCHEON.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= INCHEON.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${INCHEON.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, INCHEON.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: INCHEON.country_iso,
      country_zh: INCHEON.country_zh,
      city_zh: INCHEON.city_zh,
      city_en: INCHEON.city_en,
    },
    poi_count: INCHEON.matrix_ids.length,
    matrix_ids: INCHEON.matrix_ids,
    pois: INCHEON.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === INCHEON.country_iso && c.city_zh === INCHEON.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: INCHEON.matrix_ids.length,
        closure_evidence: INCHEON.closure_latest,
        [INCHEON.closure_key]: 'CLOSED',
      },
    ],
  ...
  const kickoff = readJsonIfExists(KR_KICKOFF);
  if (kickoff) {
    kickoff.stamp_utc = stamp;
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'KR_CITY_PILOT_COMPLETE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : null;
    kickoff.incheon_city_closure = { status: 'CLOSED', evidence: INCHEON.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-KR-CITY-${NEXT_CITY.city_en.toUpperCase()}`,
          label: `KR · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_KR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'INCHEON_CLOSED';
    kickoff[INCHEON.closure_key] = 'CLOSED';
    kickoff.TT_CMS_POI_ACTIVE_CITY = NEXT_CITY ? `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}` : null;
    fs.writeFileSync(KR_KICKOFF, JSON.stringify(kickoff, null, 2) + '\n');
  }

  const pilotWave = readJsonIfExists(KR_PILOT_WAVE);
  if (pilotWave) {
    pilotWave.stamp_utc = stamp;
    pilotWave.incheon_closed = true;
    pilotWave.active_catalog_build = kickoff?.active_catalog_build || registry.active_city;
    pilotWave[INCHEON.closure_key] = 'CLOSED';
    fs.writeFileSync(KR_PILOT_WAVE, JSON.stringify(pilotWave, null, 2) + '\n');
  }
