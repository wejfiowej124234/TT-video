#!/usr/bin/env node
/**
 * France L5 Content QA · ② staging · JP + KR + TH 三模板 · 不调整标准
 *
 *   node scripts/dev/run-cms-france-content-qa.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-FRANCE-CONTENT-QA-LATEST.json');
...
function main() {
  ...
  console.log(`TT_CMS_FR_CONTENT_QA: ${report.TT_CMS_FR_CONTENT_QA}`);
  console.log(`TT_CMS_FR_COUNTRY: ${report.TT_CMS_FR_COUNTRY}`);
