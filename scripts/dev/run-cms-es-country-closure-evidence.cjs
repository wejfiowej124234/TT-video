#!/usr/bin/env node
/**
 * Spain Country CLOSED · national_runtime + 三城 Content QA 汇总
 *
 * 前置：run-cms-es-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH/SG/FR LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-es-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildSpainContentQa, ES_CITY_DISPLAY_ORDER } = require('./lib/cms-spain-content-qa.cjs');
const {
  EVIDENCE,
  readJson,
  ES_CITIES,
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return ES_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_ES_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_ES_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_ES_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_ES_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }

  const report = buildSpainContentQa({ country_closed: true });
  const issues = [];
  ...
  const closure = {
    schema: 'traveltrust.cms_es_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    australia_content_qa_ssot: 'evidence/GO_cms_operation/CMS-SPAIN-CONTENT-QA-LATEST.json',
    ...
    checks: {
      ...
      es_lock_registry_unchanged: true,
      ...
      es_country_runtime_pass: true,
      content_accuracy_100: true,
      cross_region_0: report.country.cross_region_total === 0,
    },
    TT_CMS_ES_COUNTRY: 'CLOSED',
    TT_CMS_ES_CONTENT_QA: 'CLOSED',
    verdict: 'ES_COUNTRY_CLOSED',
  };
  ...
  const refreshed = buildSpainContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_ES_COUNTRY: CLOSED');
  console.log('TT_CMS_ES_CONTENT_QA: CLOSED');
  ...
  console.log('ES LOCK: 24/24 unchanged');
