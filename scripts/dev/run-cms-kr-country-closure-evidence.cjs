#!/usr/bin/env node
/**
 * Korea Country CLOSED · national_runtime + 四城 Content QA 汇总
 *
 * 前置：run-cms-kr-country-runtime-audit.cjs PASS
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-kr-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildKoreaContentQa, KR_CITY_DISPLAY_ORDER } = require('./lib/cms-korea-content-qa.cjs');
const { EVIDENCE, readJson, KR_CITIES } = require('./lib/cms-kr-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-KR-COUNTRY-CLOSURE-LATEST.md');
...
function buildCitySummary(report, runtimeAudit) {
  return KR_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_KR_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_KR_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_KR_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_KR_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }
  ...
  if (report.summary.cities_execution_pass !== 4) issues.push(`execution ${report.summary.cities_execution_pass}/4`);
  if (report.summary.cities_content_qa_closed !== 4) issues.push(`content_qa_closed ${report.summary.cities_content_qa_closed}/4`);
  ...
  if (report.country?.TT_CMS_KR_COUNTRY !== 'CLOSED') {
  ...
  const closure = {
    schema: 'traveltrust.cms_kr_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-KR-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    korea_content_qa_ssot: 'evidence/GO_cms_operation/CMS-KOREA-CONTENT-QA-LATEST.json',
    ...
    checks: {
      four_city_execution_closed: true,
      four_city_content_qa_closed: true,
      four_city_exit_checks: true,
      kr_lock_registry_unchanged: true,
      jp_lock_registry_unchanged: true,
      kr_country_runtime_pass: true,
      content_accuracy_100: true,
      cross_region_0: report.country.cross_region_total === 0,
    },
    TT_CMS_KR_COUNTRY: 'CLOSED',
    TT_CMS_KR_CONTENT_QA: 'CLOSED',
    verdict: 'KR_COUNTRY_CLOSED',
  };
  ...
  // Refresh Korea Content QA board with CLOSED country
  const refreshed = buildKoreaContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_KR_COUNTRY: CLOSED');
  console.log('TT_CMS_KR_CONTENT_QA: CLOSED');
