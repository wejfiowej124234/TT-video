#!/usr/bin/env node
/**
 * POI City Closure Evidence · US · 旧金山 · ② staging
 *
...
const NEXT_CITY = nextCityPilot('旧金山');
...
async function main() {
  ...
  const evidenceRows = SAN_FRANCISCO.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=US&city=${encodeURIComponent(SAN_FRANCISCO.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SAN_FRANCISCO.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SAN_FRANCISCO.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SAN_FRANCISCO.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SAN_FRANCISCO.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SAN_FRANCISCO.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SAN_FRANCISCO.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SAN_FRANCISCO.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SAN_FRANCISCO.country_iso,
      country_zh: SAN_FRANCISCO.country_zh,
      city_zh: SAN_FRANCISCO.city_zh,
      city_en: SAN_FRANCISCO.city_en,
    },
    poi_count: SAN_FRANCISCO.matrix_ids.length,
    matrix_ids: SAN_FRANCISCO.matrix_ids,
    pois: SAN_FRANCISCO.pois,
    checks,
    ...
    closed_loop_evidence: SAN_FRANCISCO.closed_loop_latest,
    next_city: NEXT_CITY
      ? { country_iso: NEXT_CITY.country_iso, city_zh: NEXT_CITY.city_zh, city_en: NEXT_CITY.city_en }
      : null,
    closure_keys: {
      [SAN_FRANCISCO.closure_key]: 'CLOSED',
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SAN_FRANCISCO.country_iso && c.city_zh === SAN_FRANCISCO.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SAN_FRANCISCO.matrix_ids.length,
        closure_evidence: SAN_FRANCISCO.closure_latest,
        [SAN_FRANCISCO.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.san_francisco_city_closure = { status: 'CLOSED', evidence: SAN_FRANCISCO.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_US_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SAN_FRANCISCO_CLOSED';
    kickoff[SAN_FRANCISCO.closure_key] = 'CLOSED';
