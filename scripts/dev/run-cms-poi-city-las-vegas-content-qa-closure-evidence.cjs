#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 拉斯维加斯（JP + KR + TH + US Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LAS_VEGAS-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-LAS_VEGAS-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '拉斯维加斯');
  if (!cityQa) throw new Error('拉斯维加斯 city QA missing');

  const checks = {
    assets_locked_7_7: lock.locked === LAS_VEGAS.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_LAS_VEGAS_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'US', city_zh: '拉斯维加斯', city_en: 'Las Vegas', poi_count: LAS_VEGAS.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-LAS_VEGAS-CLOSURE-LATEST.json',
    ...
    cross_region_remediated: 'all_7_execution_plus_geo',
    checks,
    ...
    TT_CMS_POI_CITY_LAS_VEGAS_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_LAS_VEGAS_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${LAS_VEGAS.matrix_ids.length}`);
