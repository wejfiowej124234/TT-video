#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 清迈（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '清迈');
  if (!cityQa) throw new Error('清迈 city QA missing');

  const checks = {
    assets_locked_9_9: lock.locked === CHIANG_MAI.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'TH', city_zh: '清迈', city_en: 'Chiang Mai', poi_count: CHIANG_MAI.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-CHIANG_MAI-CLOSURE-LATEST.json',
    ...
    TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_CHIANG_MAI_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${CHIANG_MAI.matrix_ids.length}`);
