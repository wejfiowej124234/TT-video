#!/usr/bin/env node
/**
 * Melbourne Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-melbourne-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/melbourne');

const MELBOURNE = getCityPilot('墨尔本');
const POI_COUNT = MELBOURNE.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-MELBOURNE-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of MELBOURNE.matrix_ids) {
    const poi = MELBOURNE.pois[MELBOURNE.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  const report = buildAustraliaContentQa();
  const city = report.cities.find((c) => c.city_zh === '墨尔本');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent('墨尔本')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '澳大利亚' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '墨尔本', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(MELBOURNE);
    ...
    const shotPath = path.join(SHOT_DIR, `melbourne-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < MELBOURNE.matrix_ids.length; i++) {
      const matrixId = MELBOURNE.matrix_ids[i];
      const poi = MELBOURNE.pois[i];
...
function checkNoBacklogUnlock() {
  const open = MELBOURNE.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = MELBOURNE.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Melbourne Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '墨尔本', city_en: 'Melbourne', country_iso: 'AU', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_AU_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_MELBOURNE_CONTENT_QA_EXIT}`);
