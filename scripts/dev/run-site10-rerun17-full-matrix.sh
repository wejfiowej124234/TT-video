#!/usr/bin/env bash
# ① Site10 · rerun17 全矩阵（844 · guest auth-gate helper 窄切片绿后 · 单实例锁）
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
EVID="$ROOT/frontend/evidence/GO_local_phase1"
ORCH="$EVID/site10-rerun17-orchestrator.log"
RERUN_LOG="$EVID/site10-full-rerun6.log"
GATES="$EVID/site10-rerun17-gates.txt"
PARSE="$EVID/site10-rerun17-parse.txt"
WAIT="$EVID/site10-rerun17-wait.log"

log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] $*" | tee -a "$ORCH"; }

log "rerun17 start · expectCommunityMeGuestInvalidAuthGate (reports Suspense + authPending)"
echo "=== rerun17 $(date -u +%Y-%m-%dT%H:%M:%SZ) · post guest auth-gate fix (target REAL FAIL << 49) ===" | tee -a "$RERUN_LOG"
...
log "rerun17 orchestrator complete · compare REAL FAIL vs rerun16=49"
