#!/usr/bin/env node
/**
 * Paris Content QA Exit Check · JP + KR + TH + SG Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-paris-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PARIS-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PARIS-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/paris');

const PARIS = getCityPilot('巴黎');
const POI_COUNT = PARIS.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-PARIS-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '巴黎');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=FR&city=${encodeURIComponent('巴黎')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '巴黎', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = PARIS.pois.slice(0, 4);
    const foodPois = PARIS.pois.slice(4);
    ...
    const shotPath = path.join(SHOT_DIR, `paris-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < PARIS.matrix_ids.length; i++) {
      const matrixId = PARIS.matrix_ids[i];
      const poi = PARIS.pois[i];
...
function checkNoBacklogUnlock() {
  const open = PARIS.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = PARIS.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Paris Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_PARIS_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '巴黎', city_en: 'Paris', country_iso: 'FR', poi_count: POI_COUNT },
    replicated_from: 'JP + KR + TH + SG Golden Template (TT_CMS_JP_COUNTRY: CLOSED)',
    web_base: WEB,
    api_base: API,
    checks,
    all_pass: allPass,
    ...
    TT_CMS_POI_CITY_PARIS_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_PARIS_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_FR_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_PARIS_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_PARIS_CONTENT_QA_EXIT}`);
