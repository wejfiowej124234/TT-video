...
TMP="$(mktemp)"
{
  ...
  # PER: optional UI / feature flags — root .env NEXT_PUBLIC_* or synced keys (same name)
  for k in \
    NEXT_PUBLIC_ENABLE_LEGACY_BEARER_STORAGE \
    NEXT_PUBLIC_SKIP_ME_FETCH \
    NEXT_PUBLIC_COMMUNITY_ME_AVATAR_UPLOAD \
    NEXT_PUBLIC_TRAVELTRUST_THEATER_MEDIA_MODE \
    NEXT_PUBLIC_TRAVELTRUST_HERO_LOOP \
    NEXT_PUBLIC_TRAVELTRUST_HERO_LOOP_POSTER \
    ...
    NEXT_PUBLIC_TRAVELTRUST_COMMUNITY_SHOWCASE \
    NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID; do
    v="$(get_val "$k")" && [[ -n "$v" ]] && echo "${k}=${v}"
  done
  sync_sha="$(git -C "$ROOT" rev-parse --short HEAD 2>/dev/null || echo unknown)"
  sync_utc="$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u)"
  echo "# SYNCED_FROM_ROOT_ENV_SHA=${sync_sha}@${sync_utc}"
  echo "$MARK_END"
} > "$TMP"

MANAGED='^(NEXT_PUBLIC_API_BASE_URL|NEXT_PUBLIC_CHAIN_ID|NEXT_PUBLIC_RPC_URL|NEXT_PUBLIC_FEE_ROUTER_ADDRESS|NEXT_PUBLIC_ESCROW_FACTORY_ADDRESS|NEXT_PUBLIC_REGISTRY_ADDRESS|NEXT_PUBLIC_GUIDE_STAKING_ADDRESS|NEXT_PUBLIC_STAKING_PROVIDER_ADDRESS|NEXT_PUBLIC_GOVERNANCE_TOKEN_ADDRESS|NEXT_PUBLIC_SETTLEMENT_TOKEN_ADDRESS|NEXT_PUBLIC_INVESTOR_DISTRIBUTION_CLAIM_ADDRESS|NEXT_PUBLIC_REGION_STEWARD_STAKE_POOL_ADDRESS|NEXT_PUBLIC_COUNTRY_POOL_REDEMPTION_EPOCH_CN_ADDRESS|NEXT_PUBLIC_ONBOARDING_LOCAL_DEV_TOOLS|NEXT_PUBLIC_COMMUNITY_MEDIA_S3_PUBLIC_BASE_URL|NEXT_PUBLIC_TRAVELTRUST_COMMUNITY_MEDIA_ASSET_MAX_BYTES|NEXT_PUBLIC_TRAVELTRUST_COMMUNITY_POST_MEDIA_URL_PREFIXES|NEXT_PUBLIC_ENABLE_LEGACY_BEARER_STORAGE|NEXT_PUBLIC_SKIP_ME_FETCH|NEXT_PUBLIC_COMMUNITY_ME_AVATAR_UPLOAD|NEXT_PUBLIC_TRAVELTRUST_THEATER_MEDIA_MODE|NEXT_PUBLIC_TRAVELTRUST_HERO_LOOP|NEXT_PUBLIC_TRAVELTRUST_HERO_LOOP_POSTER|NEXT_PUBLIC_TRAVELTRUST_ROLE_VIDEO_TRAVELER|NEXT_PUBLIC_TRAVELTRUST_ROLE_VIDEO_GUIDE|NEXT_PUBLIC_TRAVELTRUST_ROLE_VIDEO_MERCHANT|NEXT_PUBLIC_TRAVELTRUST_ROLE_VIDEO_ACQUISITION|NEXT_PUBLIC_TRAVELTRUST_ROLE_VIDEO_REGION_STEWARD|NEXT_PUBLIC_TRAVELTRUST_COMMUNITY_SHOWCASE|NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID|SYNCED_FROM_ROOT_ENV_SHA)='
KEPT="$(mktemp)"
if [[ -f "$OUT" ]]; then
  inside=0
  while IFS= read -r line || [[ -n "$line" ]]; do
