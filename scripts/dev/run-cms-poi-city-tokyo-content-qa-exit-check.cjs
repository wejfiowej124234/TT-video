#!/usr/bin/env node
/**
 * Tokyo Content QA Exit Check · Osaka Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-tokyo-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/tokyo');

const TOKYO = getCityPilot('东京');
const POI_COUNT = TOKYO.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-TOKYO-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkLockRegistry() {
  ...
  for (const matrixId of TOKYO.matrix_ids) {
    const poi = TOKYO.pois[TOKYO.matrix_ids.indexOf(matrixId)];
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '东京');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const expected = {};
  for (const matrixId of TOKYO.matrix_ids) {
    expected[TOKYO.pois[TOKYO.matrix_ids.indexOf(matrixId)]] = getAsset(matrixId).hero_file;
  }
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=JP&city=${encodeURIComponent('东京')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntime(catalogRows) {
  ...
  try {
    ...
    await day1.getByRole('button', { name: '东京', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const attrPois = TOKYO.pois.slice(0, 5);
    const foodPois = TOKYO.pois.slice(5);
    ...
    const shotPath = path.join(SHOT_DIR, `tokyo-poi-runtime-${stamp}.png`);
    ...
    for (const poi of TOKYO.pois) {
      const expectedFn = catalogByPoi[poi] || getAsset(TOKYO.matrix_ids[TOKYO.pois.indexOf(poi)]).hero_file;
...
function checkNoBacklogUnlock() {
  const open = TOKYO.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = TOKYO.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Tokyo Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_TOKYO_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '东京', city_en: 'Tokyo', country_iso: 'JP', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_TOKYO_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_TOKYO_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
  ...
  console.log(`TT_CMS_POI_CITY_TOKYO_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_TOKYO_CONTENT_QA_EXIT}`);
