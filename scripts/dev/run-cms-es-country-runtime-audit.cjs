#!/usr/bin/env node
/**
 * Spain Country Runtime Audit · national_runtime · ② staging
 *
 * 验证西班牙相关内容 Consumer 读 CMS · 无 Unsplash/OCS/fallback · 三城 LOCK 不变 · JP/KR/TH/SG LOCK 不变
 * 不修改 LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-es-country-runtime-audit.cjs
 */
const fs = require('fs');
...
const {
  ...
  cityExitPath,
  isForbiddenFallback,
  validateEsCityExitChecks,
  validateEsCityContentQaClosures,
  validateEsCityExecutionClosures,
  validateCatalogApiAllEsPois,
  validateSpainAmbient,
  ...
  filename,
  fetchJson,
} = require('./lib/cms-es-country-runtime.cjs');
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-COUNTRY-RUNTIME-AUDIT-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/es-country');

const CONSUMER_SURFACES = [
  { id: 'home_es', path: '/', setup: 'home_es' },
  { id: 'market', path: '/market', setup: 'market_base' },
  { id: 'market_itinerary_au', path: '/market', setup: 'market_es_poi' },
...
function aggregateExitCheckPoiRuntime() {
  ...
  return {
    id: 'es_poi_runtime_exit_aggregate',
...
function isSpainScopedImage(img) {
  const blob = `${img.src || ''} ${img.alt || ''} ${img.context || ''}`.toLowerCase();
  return /australia|au\b|barcelona|madrid|seville|西班牙|巴塞罗那|马德里|塞维利亚|歌剧院|邦迪|冲浪者/.test(
    blob,
  );
}

async function scanPageImages(page, scope, { esOnly = false } = {}) {
  ...
  for (const img of images) {
    if (esOnly && !isSpainScopedImage(img)) continue;
...
async function catalogSpainAmbientUrl() {
  const r = await fetchJson(`${API}/api/v1/catalog/media?asset_kind=landing_ambient&country_iso=ES`);
...
async function liveConsumerProbe(poiAggregate) {
  ...
  try {
    ...
    const auBtn = page.locator('#landing-hero-form').getByRole('button', { name: '西班牙', exact: true });
    ...
    if (auBtnVisible) {
      ...
      surfaceRows.push({ surface: 'home_es_ambient', pass: ambientOk, runtime_src: ambientSrc });
      if (!ambientOk) issues.push(`home 西班牙 ambient: ${ambientSrc || 'MISSING'}`);
    } else {
      const ambientCatalog = await catalogSpainAmbientUrl();
      const ambientOk =
      ...
      surfaceRows.push({
        surface: 'home_es_ambient_catalog_fallback',
        pass: ambientOk,
        runtime_src: ambientCatalog.url,
        note: 'landing 西班牙 pill not visible · catalog media fallback',
      });
      if (!ambientOk) issues.push(`home 西班牙 ambient catalog fallback: ${ambientCatalog.url || 'MISSING'}`);
    }
    ...
    await page.screenshot({ path: path.join(SHOT_DIR, `es-country-runtime-${stamp}.png`), fullPage: false });
    ...
    for (const spec of CONSUMER_SURFACES.filter((s) => s.setup === 'scroll')) {
      ...
      const scan = await scanPageImages(page, spec.id, { esOnly: true });
      surfaceRows.push({
        surface: spec.id,
    ...
    return {
      ...
      label: 'Live Consumer · 首页西班牙 Ambient + Market 壳 + 西班牙相关内容无 fallback',
...
function formatMd(report) {
  const lines = [
    '# Spain Country Runtime Audit',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**TT_CMS_ES_COUNTRY_RUNTIME:** \`${report.TT_CMS_ES_COUNTRY_RUNTIME}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const staticChecks = [
    ...
    validateEsCityExitChecks(),
    validateSpainAmbient(),
    await validateCatalogApiAllEsPois(API),
    poiAggregate,
  ];
  ...
  const poiRequired = ES_CITIES.reduce((n, zh) => n + CITY_PILOTS[zh].matrix_ids.length, 0);
  const poiRuntimePass = (poiAggregate.poi_rows || []).filter((r) => r.cms_runtime_ok).length;

  const citySummary = ES_CITIES.map((cityZh) => {
    const pilot = CITY_PILOTS[cityZh];
    const exit = staticChecks.find((c) => c.id === 'four_city_exit_checks')?.rows?.find((r) => r.city_zh === cityZh);
    const catalogRows =
      staticChecks.find((c) => c.id === 'catalog_api_es_poi')?.rows?.filter((r) => r.city_zh === cityZh) || [];
  ...
  const report = {
    schema: 'traveltrust.cms_es_country_runtime_audit.v1',
    recorded_at_utc: new Date().toISOString(),
    phase: '② staging',
    country: { country_iso: 'ES', country_zh: '西班牙', cities: ES_CITIES },
    ...
    es_lock_guard: { required: 24, unchanged: true },
    ...
    verdict: allPass ? 'ES_COUNTRY_RUNTIME_PASS' : 'NOT_READY',
    TT_CMS_ES_COUNTRY_RUNTIME: allPass ? 'PASS' : 'FAIL',
  ...
  console.log(`TT_CMS_ES_COUNTRY_RUNTIME: ${report.TT_CMS_ES_COUNTRY_RUNTIME}`);
