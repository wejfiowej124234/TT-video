#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 厦门 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-xiamen-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('厦门');
...
async function main() {
  ...
  const evidenceRows = XIAMEN.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=CN&city=${encodeURIComponent(XIAMEN.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of XIAMEN.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || XIAMEN.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, XIAMEN.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === XIAMEN.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= XIAMEN.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${XIAMEN.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, XIAMEN.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: XIAMEN.country_iso,
      country_zh: XIAMEN.country_zh,
      city_zh: XIAMEN.city_zh,
      city_en: XIAMEN.city_en,
    },
    poi_count: XIAMEN.matrix_ids.length,
    matrix_ids: XIAMEN.matrix_ids,
    pois: XIAMEN.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === XIAMEN.country_iso && c.city_zh === XIAMEN.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: XIAMEN.matrix_ids.length,
        closure_evidence: XIAMEN.closure_latest,
        [XIAMEN.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.abu_dhabi_city_closure = { status: 'CLOSED', evidence: XIAMEN.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_CN_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'XIAMEN_CLOSED';
    kickoff[XIAMEN.closure_key] = 'CLOSED';
