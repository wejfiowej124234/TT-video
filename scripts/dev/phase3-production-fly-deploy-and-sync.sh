...
DATABASE_URL_ON_FLY=0
if fly secrets list -a "$APP" 2>/dev/null | grep -q '^DATABASE_URL[[:space:]]'; then
  DATABASE_URL_ON_FLY=1
fi
if [[ -z "${DATABASE_URL:-}" && "$DATABASE_URL_ON_FLY" != "1" ]]; then
  fail "DATABASE_URL required for prod (local env or fly mpg attach)"
fi
[[ "$DATABASE_URL_ON_FLY" == "1" && -z "${DATABASE_URL:-}" ]] && \
  ok "DATABASE_URL on Fly only — deploy continues without local copy"
...
add_secret TRAVELTRUST_DEPLOYMENT_PROFILE "production"
add_secret TRAVELTRUST_COMMUNITY_PUBLIC_SHOWCASE "0"
add_secret TRAVELTRUST_MARKET_PUBLIC_SHOWCASE "0"
add_secret PUBLIC_API_BASE_URL "${PUBLIC_API_BASE_URL:-${PROD_API_BASE:-}}"

for k in CHAIN_RPC_URL CHAIN_RPC_URL_BACKUP CHAIN_ID SETTLEMENT_TOKEN \
  ESCROW_FACTORY_ADDRESS FEE_ROUTER_ADDRESS REGION_VAULT_ADDRESS \
  REGISTRY_ADDRESS GOVERNOR_ADDRESS \
  TIMELOCK_ADDRESS GOVERNANCE_TIMELOCK_ADDRESS GOV_FREEZE_V1_TIMELOCK_ADDRESS \
  GOVERNANCE_TOKEN_ADDRESS GOVERNANCE_VOTES_TOKEN_ADDRESS \
  TREASURY_ADDRESS \
  REGION_STEWARD_STAKE_POOL_ADDRESS REGION_STEWARD_STAKE_POOL_PROXY_ADDRESS \
  GUIDE_STAKING_ADDRESS STAKING_ADDRESS GUIDE_STAKING_POOL_ADDRESS \
  STAKING_PROVIDER_ADDRESS PROVIDER_STAKING_POOL_ADDRESS \
  STRICT_SSOT SSOT_VERSION SSOT_SHA256 CHARGEBACK_POLICY \
...
# Alias fallbacks for /meta ChainConfig (no business logic — deploy config only)
...
if [[ -z "${TIMELOCK_ADDRESS:-}" && -n "${GOVERNANCE_TIMELOCK_ADDRESS:-}" ]]; then
  add_secret TIMELOCK_ADDRESS "${GOVERNANCE_TIMELOCK_ADDRESS}"
fi
