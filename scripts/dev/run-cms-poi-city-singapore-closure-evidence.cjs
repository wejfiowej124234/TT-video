#!/usr/bin/env node
/**
 * POI City Closure Evidence · SG · 新加坡 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-singapore-closure-evidence.cjs
 */
const fs = require('fs');
...
const SG_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-POI-WAVE-KICKOFF-LATEST.json');
const SG_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-SG-POI-PILOT-WAVE-LATEST.json');
...
const NEXT_CITY = nextCityPilot('新加坡');
...
async function main() {
  ...
  for (const matrixId of SINGAPORE.matrix_ids) {
  ...
  const evidenceRows = SINGAPORE.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=SG&city=${encodeURIComponent(SINGAPORE.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SINGAPORE.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SINGAPORE.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SINGAPORE.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SINGAPORE.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SINGAPORE.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SINGAPORE.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SINGAPORE.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SINGAPORE.country_iso,
      country_zh: SINGAPORE.country_zh,
      city_zh: SINGAPORE.city_zh,
      city_en: SINGAPORE.city_en,
    },
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SINGAPORE.country_iso && c.city_zh === SINGAPORE.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SINGAPORE.matrix_ids.length,
        closure_evidence: SINGAPORE.closure_latest,
        [SINGAPORE.closure_key]: 'CLOSED',
      },
    ],
  ...
  const kickoff = readJsonIfExists(SG_KICKOFF);
  if (kickoff) {
    kickoff.stamp_utc = stamp;
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SG_CITY_PILOT_COMPLETE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : 'SG_COUNTRY_RUNTIME';
    kickoff.singapore_city_closure = { status: 'CLOSED', evidence: SINGAPORE.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-SG-CITY-${NEXT_CITY.slug.toUpperCase()}`,
          label: `SG · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_SG_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SINGAPORE_CLOSED';
    kickoff[SINGAPORE.closure_key] = 'CLOSED';
    kickoff.TT_CMS_POI_ACTIVE_CITY = NEXT_CITY ? `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}` : null;
    fs.writeFileSync(SG_KICKOFF, JSON.stringify(kickoff, null, 2) + '\n');
  }

  const pilotWave = readJsonIfExists(SG_PILOT_WAVE);
  if (pilotWave) {
    pilotWave.stamp_utc = stamp;
    pilotWave.singapore_closed = true;
    pilotWave.active_catalog_build = kickoff?.active_catalog_build || registry.active_city;
    pilotWave[SINGAPORE.closure_key] = 'CLOSED';
    fs.writeFileSync(SG_PILOT_WAVE, JSON.stringify(pilotWave, null, 2) + '\n');
  }
