#!/usr/bin/env bash
# ① Site10 · rerun15 全矩阵（844 · 窄切片绿后 · 单实例锁）
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"
EVID="$ROOT/frontend/evidence/GO_local_phase1"
ORCH="$EVID/site10-rerun15-orchestrator.log"
RERUN_LOG="$EVID/site10-full-rerun6.log"
GATES="$EVID/site10-rerun15-gates.txt"
PARSE="$EVID/site10-rerun15-parse.txt"
WAIT="$EVID/site10-rerun15-wait.log"

log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] $*" | tee -a "$ORCH"; }

log "rerun15 start · carousel/image-counter + auth reload /me + gotoSmoke import"
echo "=== rerun15 $(date -u +%Y-%m-%dT%H:%M:%SZ) · post narrow-slice fixes (target REAL FAIL << 56) ===" | tee -a "$RERUN_LOG"
...
log "rerun15 orchestrator complete · compare REAL FAIL vs rerun14=56"
