#!/usr/bin/env node
/**
 * POI City Closure Evidence · KR · 釜山 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-busan-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('釜山');
...
async function main() {
  ...
  const evidenceRows = BUSAN.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=KR&city=${encodeURIComponent(BUSAN.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of BUSAN.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || BUSAN.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, BUSAN.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === BUSAN.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= BUSAN.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${BUSAN.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, BUSAN.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: BUSAN.country_iso,
      country_zh: BUSAN.country_zh,
      city_zh: BUSAN.city_zh,
      city_en: BUSAN.city_en,
    },
    poi_count: BUSAN.matrix_ids.length,
    matrix_ids: BUSAN.matrix_ids,
    pois: BUSAN.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === BUSAN.country_iso && c.city_zh === BUSAN.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: BUSAN.matrix_ids.length,
        closure_evidence: BUSAN.closure_latest,
        [BUSAN.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.busan_city_closure = { status: 'CLOSED', evidence: BUSAN.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_KR_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'BUSAN_CLOSED';
    kickoff[BUSAN.closure_key] = 'CLOSED';
