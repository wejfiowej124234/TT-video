#!/usr/bin/env bash
# C-S2 · Admin POI image batch review smoke（① 本地 · 须 API+PG 已起）
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
API="${TRAVELTRUST_API_BASE:-${API_BASE:-http://127.0.0.1:8080}}"
FE="${TRAVELTRUST_FE_BASE:-http://127.0.0.1:3012}"

echo "== smoke-admin-content-poi-media-p0-local (C-S2) =="
echo "API=$API FE=$FE"

for path in \
  "/api/v1/admin/content/poi-image-batches" \
  "/api/v1/admin/content/poi-image-batches/00000000-0000-0000-0000-000000000001" \
  "/api/v1/admin/content/poi-image-batches/00000000-0000-0000-0000-000000000001/candidates"; do
...
for fe_path in \
  ...
  "/admin/content/poi-images/batches/00000000-0000-0000-0000-000000000001"; do
...
echo "smoke-admin-content-poi-media-p0-local: exit 0"
