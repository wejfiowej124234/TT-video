#!/usr/bin/env node
/**
 * POI City Content QA Closure · JP · 札幌（Osaka Golden Template 同源）
 *
 *   node scripts/dev/run-cms-poi-city-sapporo-content-qa-closure-evidence.cjs
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '札幌');
  if (!cityQa) throw new Error('札幌 city QA missing');

  const checks = {
    assets_locked_9_9: lock.locked === SAPPORO.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_SAPPORO_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'JP', city_zh: '札幌', city_en: 'Sapporo', poi_count: SAPPORO.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-SAPPORO-CLOSURE-LATEST.json',
    ...
    TT_CMS_POI_CITY_SAPPORO_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_SAPPORO_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${SAPPORO.matrix_ids.length}`);
