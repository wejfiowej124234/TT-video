#!/usr/bin/env node
/**
 * POI City Content QA Closure · TH · 广州（JP + KR + TH + US Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '广州');
  if (!cityQa) throw new Error('广州 city QA missing');

  const checks = {
    assets_locked_8_8: lock.locked === GUANGZHOU.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'CN', city_zh: '广州', city_en: 'Guangzhou', poi_count: GUANGZHOU.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-GUANGZHOU-CLOSURE-LATEST.json',
    ...
    TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA: 'CLOSED',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_GUANGZHOU_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${GUANGZHOU.matrix_ids.length}`);
