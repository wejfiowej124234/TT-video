#!/usr/bin/env node
/**
 * POI City Content QA Closure · JP · 大阪
 *
 *   node scripts/dev/run-cms-poi-city-osaka-content-qa-closure-evidence.cjs
 *
 * TT_CMS_POI_CITY_OSAKA_CONTENT_QA: CLOSED ≠ TT_CMS_POI_CITY_OSAKA: CLOSED (Execution)
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-OSAKA-CONTENT-QA-CLOSURE-LATEST.json');
const LOCK_REG = path.join(ROOT, 'evidence/GO_cms_operation/CMS-CONTENT-QA-ASSET-LOCK-REGISTRY-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-OSAKA-CLOSURE-LATEST.json');
...
function assessLockedAssets() {
  ...
  for (const matrixId of OSAKA.matrix_ids) {
    ...
    if (asset.replace_count !== 1) issues.push(`${matrixId}: replace_count=${asset.replace_count} (expected 1)`);
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '大阪');
  if (!cityQa) throw new Error('大阪 city QA missing');

  const checks = {
    assets_locked_8_8: lock.locked === OSAKA.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_OSAKA_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'JP', city_zh: '大阪', city_en: 'Osaka', poi_count: OSAKA.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-OSAKA-CLOSURE-LATEST.json',
    ...
    TT_CMS_POI_CITY_OSAKA_CONTENT_QA: 'CLOSED',
    note: 'Content QA CLOSED · 不等于 Execution CLOSED 重复劳动',
  };

  fs.mkdirSync(path.dirname(OUT_LATEST), { recursive: true });
  fs.writeFileSync(OUT_LATEST, JSON.stringify(closure, null, 2) + '\n');
  console.log('TT_CMS_POI_CITY_OSAKA_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${OSAKA.matrix_ids.length}`);
