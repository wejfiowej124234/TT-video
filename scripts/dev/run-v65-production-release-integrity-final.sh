#!/usr/bin/env bash
# V65 Production Release Integrity Final Audit
# Unlock: TRAVELTRUST_V65_PRODUCTION_RELEASE_INTEGRITY_OK=1
# Forbidden: Web3 mainnet · Admin IA redesign · TT_PRODUCTION_GO flip · Human UAT substitute
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

if [[ "${TRAVELTRUST_V65_PRODUCTION_RELEASE_INTEGRITY_OK:-}" != "1" ]]; then
  echo "FAIL: set TRAVELTRUST_V65_PRODUCTION_RELEASE_INTEGRITY_OK=1" >&2
  exit 2
fi
...
# shellcheck disable=SC1091
[[ -f .env.prod-owner-business-probe.local ]] && set -a && . ./.env.prod-owner-business-probe.local && set +a
...
exec python scripts/dev/run-v65-production-release-integrity-final.py "$@"
