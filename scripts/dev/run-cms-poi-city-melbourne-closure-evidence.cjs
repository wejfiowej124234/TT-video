#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 墨尔本 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-melbourne-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('墨尔本');
...
async function main() {
  ...
  for (const matrixId of MELBOURNE.matrix_ids) {
  ...
  const evidenceRows = MELBOURNE.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=AU&city=${encodeURIComponent(MELBOURNE.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of MELBOURNE.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || MELBOURNE.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, MELBOURNE.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === MELBOURNE.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= MELBOURNE.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${MELBOURNE.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, MELBOURNE.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: MELBOURNE.country_iso,
      country_zh: MELBOURNE.country_zh,
      city_zh: MELBOURNE.city_zh,
      city_en: MELBOURNE.city_en,
    },
    poi_count: MELBOURNE.matrix_ids.length,
    matrix_ids: MELBOURNE.matrix_ids,
    pois: MELBOURNE.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === MELBOURNE.country_iso && c.city_zh === MELBOURNE.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: MELBOURNE.matrix_ids.length,
        closure_evidence: MELBOURNE.closure_latest,
        [MELBOURNE.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.melbourne_city_closure = { status: 'CLOSED', evidence: MELBOURNE.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_AU_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'MELBOURNE_CLOSED';
    kickoff[MELBOURNE.closure_key] = 'CLOSED';
