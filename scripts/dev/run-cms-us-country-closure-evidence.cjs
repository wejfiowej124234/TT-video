#!/usr/bin/env node
/**
 * USA Country CLOSED · national_runtime + 四城 Content QA 汇总
 *
 * 前置：run-cms-us-country-runtime-audit.cjs PASS
 * 不修改 JP/KR/TH/SG/FR LOCK · 不调整 Content QA 标准 · 不得进入下一国家或 ③ Production GO
 *
 *   node scripts/dev/run-cms-us-country-closure-evidence.cjs
 */
const fs = require('fs');
const path = require('path');
const { buildUsaContentQa, US_CITY_DISPLAY_ORDER } = require('./lib/cms-usa-content-qa.cjs');
const {
  EVIDENCE,
  readJson,
  US_CITIES,
...
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-US-COUNTRY-CLOSURE-LATEST.md');
const RUNTIME_AUDIT = EVIDENCE.runtime_audit;
const NOW = new Date().toISOString();

function buildCitySummary(report, runtimeAudit) {
  return US_CITY_DISPLAY_ORDER.map((cityZh) => {
...
function main() {
  ...
  if (!runtimeAudit || runtimeAudit.TT_CMS_US_COUNTRY_RUNTIME !== 'PASS') {
    console.error('TT_CMS_US_COUNTRY: NOT_CLOSED');
    console.error('  prerequisite: CMS-US-COUNTRY-RUNTIME-AUDIT-LATEST.json TT_CMS_US_COUNTRY_RUNTIME=PASS');
    console.error(`  current: ${runtimeAudit?.TT_CMS_US_COUNTRY_RUNTIME || 'MISSING'}`);
    process.exit(1);
  }

  const report = buildUsaContentQa({ country_closed: true });
  const issues = [];

  if (report.summary.cities_execution_pass !== 4) issues.push(`execution ${report.summary.cities_execution_pass}/4`);
  if (report.summary.cities_content_qa_closed !== 4) {
    issues.push(`content_qa_closed ${report.summary.cities_content_qa_closed}/4`);
  }
  ...
  if (report.country?.TT_CMS_US_COUNTRY !== 'CLOSED') {
  ...
  const closure = {
    schema: 'traveltrust.cms_us_country_closure.v1',
    recorded_at_utc: NOW,
    phase: '② staging',
    layer: 'CONTENT_QA',
    country: {
    ...
    runtime_audit_ssot: 'evidence/GO_cms_operation/CMS-US-COUNTRY-RUNTIME-AUDIT-LATEST.json',
    usa_content_qa_ssot: 'evidence/GO_cms_operation/CMS-USA-CONTENT-QA-LATEST.json',
  ...
  const refreshed = buildUsaContentQa({ stamp_utc: NOW, country_closed: true });
  fs.writeFileSync(EVIDENCE.content_qa, JSON.stringify(refreshed, null, 2) + '\n');

  console.log('TT_CMS_US_COUNTRY: CLOSED');
  console.log('TT_CMS_US_CONTENT_QA: CLOSED');
