...
foreach ($uk in $uiKeys) {
    $uv = Get-Var $uk
    if (-not [string]::IsNullOrWhiteSpace($uv)) { $syncLines.Add("$uk=$uv") }
}

$gitDir = Join-Path $Root ".git"
$syncSha = "unknown"
if (Test-Path $gitDir) {
    try { $syncSha = (git -C $Root rev-parse --short HEAD 2>$null).Trim() } catch {}
}
$syncUtc = (Get-Date).ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
$syncLines.Add("# SYNCED_FROM_ROOT_ENV_SHA=${syncSha}@${syncUtc}")
...
$prefixesManaged = @(
    ...
    "NEXT_PUBLIC_COUNTRY_POOL_REDEMPTION_EPOCH_CN_ADDRESS=",
