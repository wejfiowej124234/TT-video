#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 巴塞罗那 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-barcelona-closure-evidence.cjs
 */
const fs = require('fs');
...
const ES_KICKOFF = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-POI-WAVE-KICKOFF-LATEST.json');
const ES_PILOT_WAVE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-ES-POI-PILOT-WAVE-LATEST.json');
...
const NEXT_CITY = nextCityPilot('巴塞罗那');
...
async function main() {
  ...
  for (const matrixId of BARCELONA.matrix_ids) {
  ...
  const evidenceRows = BARCELONA.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=ES&city=${encodeURIComponent(BARCELONA.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of BARCELONA.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || BARCELONA.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, BARCELONA.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === BARCELONA.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= BARCELONA.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${BARCELONA.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, BARCELONA.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: BARCELONA.country_iso,
      country_zh: BARCELONA.country_zh,
      city_zh: BARCELONA.city_zh,
      city_en: BARCELONA.city_en,
    },
    poi_count: BARCELONA.matrix_ids.length,
    matrix_ids: BARCELONA.matrix_ids,
    pois: BARCELONA.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === BARCELONA.country_iso && c.city_zh === BARCELONA.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: BARCELONA.matrix_ids.length,
        closure_evidence: BARCELONA.closure_latest,
        [BARCELONA.closure_key]: 'CLOSED',
      },
    ],
  ...
  const kickoff = readJsonIfExists(ES_KICKOFF);
  if (kickoff) {
    kickoff.stamp_utc = stamp;
    kickoff.status = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'AU_CITY_PILOT_ACTIVE';
    kickoff.next_stage = NEXT_CITY ? 'CATALOG_BUILD' : null;
    kickoff.barcelona_city_closure = { status: 'CLOSED', evidence: BARCELONA.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ? {
          wave_id: `POI-AU-CITY-${NEXT_CITY.slug.toUpperCase()}`,
          label: `AU · ${NEXT_CITY.city_zh}`,
          country_iso: NEXT_CITY.country_iso,
      ...
      : null;
    kickoff.TT_CMS_ES_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'BARCELONA_CLOSED';
    kickoff[BARCELONA.closure_key] = 'CLOSED';
    kickoff.TT_CMS_POI_ACTIVE_CITY = NEXT_CITY ? `${NEXT_CITY.country_iso} · ${NEXT_CITY.city_zh}` : null;
    fs.writeFileSync(ES_KICKOFF, JSON.stringify(kickoff, null, 2) + '\n');
  }

  const pilotWave = readJsonIfExists(ES_PILOT_WAVE);
  if (pilotWave) {
    pilotWave.stamp_utc = stamp;
    pilotWave.barcelona_closed = true;
    pilotWave.active_catalog_build = kickoff?.active_catalog_build || registry.active_city;
    pilotWave[BARCELONA.closure_key] = 'CLOSED';
    fs.writeFileSync(ES_PILOT_WAVE, JSON.stringify(pilotWave, null, 2) + '\n');
  }
