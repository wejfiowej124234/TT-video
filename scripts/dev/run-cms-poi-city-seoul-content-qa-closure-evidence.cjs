#!/usr/bin/env node
/**
 * POI City Content QA Closure · KR · 首尔（JP Golden Template 同源）
 */
const fs = require('fs');
...
const OUT_LATEST = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CONTENT-QA-CLOSURE-LATEST.json');
const EXEC_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CLOSURE-LATEST.json');
...
function main() {
  ...
  const cityQa = report.cities.find((c) => c.city_zh === '首尔');
  if (!cityQa) throw new Error('首尔 city QA missing');

  const checks = {
    assets_locked_9_9: lock.locked === SEOUL.matrix_ids.length,
  ...
  if (issues.length) {
    console.error('TT_CMS_POI_CITY_SEOUL_CONTENT_QA: NOT_CLOSED');
  ...
  const closure = {
    ...
    layer: 'CONTENT_QA',
    not_execution: true,
    city: { country_iso: 'KR', city_zh: '首尔', city_en: 'Seoul', poi_count: SEOUL.matrix_ids.length },
    execution_closure_ssot: 'evidence/GO_cms_operation/CMS-POI-CITY-SEOUL-CLOSURE-LATEST.json',
  ...
  console.log('TT_CMS_POI_CITY_SEOUL_CONTENT_QA: CLOSED');
  console.log(`Assets LOCKED: ${lock.locked}/${SEOUL.matrix_ids.length}`);
