#!/usr/bin/env node
/**
 * POI City Closure Evidence · AU · 沙迦 · ② staging
 *
 *   API=https://tt-api-staging.fly.dev node scripts/dev/run-cms-poi-city-sharjah-closure-evidence.cjs
 */
const fs = require('fs');
...
const NEXT_CITY = nextCityPilot('沙迦');
...
async function main() {
  ...
  const evidenceRows = SHARJAH.matrix_ids.map((id) => ({ matrix_id: id, ...readEvidence(id) }));
  const evidenceMissing = evidenceRows.filter((r) => !r.ok);

  const catalogUrl = `${API}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent(SHARJAH.city_zh)}&limit=50`;
  const catalog = await fetchJson(catalogUrl);
  const catalogItems = catalog.json?.items || [];
  const catalogIssues = [];
  for (const matrixId of SHARJAH.matrix_ids) {
    const heroFile = getAsset(matrixId).hero_file || SHARJAH.hero_files[matrixId];
  ...
  const loopPath = path.join(ROOT, SHARJAH.closed_loop_latest);
  const closedLoop = readJsonIfExists(loopPath);
  const loopOk =
    closedLoop?.TT_CMS_POI_CITY_CLOSED_LOOP === 'PASS' &&
    (closedLoop?.results || []).length === SHARJAH.matrix_ids.length;

  const checks = {
    matrix_pass_live: matrixIssues.length === 0,
    evidence_complete: evidenceMissing.length === 0,
    catalog_readable: catalog.ok && catalogItems.length >= SHARJAH.matrix_ids.length && catalogIssues.length === 0,
    city_closed_loop: loopOk,
  };
  const allOk = Object.values(checks).every(Boolean);

  if (!allOk) {
    console.error(`${SHARJAH.closure_key}: NOT_CLOSED`);
  ...
  const OUT_LATEST = path.join(ROOT, SHARJAH.closure_latest);
  const closure = {
    ...
    city: {
      country_iso: SHARJAH.country_iso,
      country_zh: SHARJAH.country_zh,
      city_zh: SHARJAH.city_zh,
      city_en: SHARJAH.city_en,
    },
    poi_count: SHARJAH.matrix_ids.length,
    matrix_ids: SHARJAH.matrix_ids,
    pois: SHARJAH.pois,
    checks,
  ...
  const closedBefore = (prev.closed_cities || []).filter(
    (c) => !(c.country_iso === SHARJAH.country_iso && c.city_zh === SHARJAH.city_zh),
  );
  const registry = {
    ...prev,
    ...
    closed_cities: [
      ...closedBefore,
      {
        ...
        closed_at_utc: NOW,
        poi_count: SHARJAH.matrix_ids.length,
        closure_evidence: SHARJAH.closure_latest,
        [SHARJAH.closure_key]: 'CLOSED',
      },
    ],
  ...
  if (kickoff) {
    ...
    kickoff.sharjah_city_closure = { status: 'CLOSED', evidence: SHARJAH.closure_latest };
    kickoff.active_catalog_build = NEXT_CITY
      ...
      : null;
    kickoff.TT_CMS_AE_POI_WAVE = NEXT_CITY ? 'CATALOG_BUILD_ACTIVE' : 'SHARJAH_CLOSED';
    kickoff[SHARJAH.closure_key] = 'CLOSED';
