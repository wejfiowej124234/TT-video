#!/usr/bin/env node
/**
 * Abu Dhabi Content QA Exit Check · JP + KR + TH + SG + FR Golden Template 同源
 *
 *   WEB=https://tt-web-staging.fly.dev API=https://tt-api-staging.fly.dev \
 *     node scripts/dev/run-cms-poi-city-abu-dhabi-content-qa-exit-check.cjs
 */
const fs = require('fs');
...
const OUT_JSON = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-ABU_DHABI-CONTENT-QA-EXIT-CHECK-LATEST.json');
const OUT_MD = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-ABU_DHABI-CONTENT-QA-EXIT-CHECK-LATEST.md');
const SHOT_DIR = path.join(ROOT, 'evidence/GO_cms_operation/content-qa-exit-check/abu-dhabi');

const ABU_DHABI = getCityPilot('阿布扎比');
const POI_COUNT = ABU_DHABI.matrix_ids.length;
const CONTENT_QA_CLOSURE = path.join(ROOT, 'evidence/GO_cms_operation/CMS-POI-CITY-ABU_DHABI-CONTENT-QA-CLOSURE-LATEST.json');
...
function checkSixDimensions(runtimeCheck) {
  ...
  const city = report.cities.find((c) => c.city_zh === '阿布扎比');
  const issues = [];
...
async function checkCatalogApi() {
  ...
  const r = await fetchJson(
    `${API}/api/v1/catalog/poi-images?country_iso=AE&city=${encodeURIComponent('阿布扎比')}&limit=50`,
  );
  if (!r.ok) {
...
async function checkConsumerRuntimeOnce(catalogRows) {
  ...
  try {
    ...
    await dlg.getByRole('option', { name: '阿联酋' }).click({ timeout: 30000 });
    ...
    await day1.getByRole('button', { name: '阿布扎比', exact: true }).click({ timeout: 30000 });

    await page.waitForTimeout(1500);

    const { attrPois, foodPois } = poiAttrFoodGroups(ABU_DHABI);
    ...
    const shotPath = path.join(SHOT_DIR, `abu-dhabi-poi-runtime-${stamp}.png`);
    ...
    for (let i = 0; i < ABU_DHABI.matrix_ids.length; i++) {
      const matrixId = ABU_DHABI.matrix_ids[i];
      const poi = ABU_DHABI.pois[i];
...
function checkNoBacklogUnlock() {
  const open = ABU_DHABI.matrix_ids.filter((id) => getAsset(id).state !== 'LOCKED');
  const unlocks = ABU_DHABI.matrix_ids.filter((id) => getAsset(id).unlock_reason);
  const issues = [];
...
function formatMd(report) {
  const lines = [
    '# Abu Dhabi Content QA Exit Check',
    '',
    `**Verdict:** \`${report.verdict}\``,
    `**Content QA:** \`${report.TT_CMS_POI_CITY_ABU_DHABI_CONTENT_QA}\``,
    '',
    '| # | 检查项 | 结果 |',
    '|---|--------|------|',
  ];
...
async function main() {
  ...
  const report = {
    ...
    city: { city_zh: '阿布扎比', city_en: 'Abu Dhabi', country_iso: 'AE', poi_count: POI_COUNT },
    ...
    TT_CMS_POI_CITY_ABU_DHABI_CONTENT_QA: fs.existsSync(CONTENT_QA_CLOSURE) ? 'CLOSED' : 'OPEN',
    TT_CMS_POI_CITY_ABU_DHABI_CONTENT_QA_EXIT: allPass ? 'PASS' : 'FAIL',
    runtime_tiers: {
      city: { tier: 'city_consumer_runtime', gate_2_equals_gate_4: true },
      country: { tier: 'national_runtime', when: 'TT_CMS_AE_COUNTRY: CLOSED', not_in_scope: true },
    },
  ...
  console.log(`TT_CMS_POI_CITY_ABU_DHABI_CONTENT_QA_EXIT: ${report.TT_CMS_POI_CITY_ABU_DHABI_CONTENT_QA_EXIT}`);
