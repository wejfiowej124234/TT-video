...
**变更纪律（与 `registry/README.md`、CONTRIBUTING、AGENTS、`.cursor/rules` 同源）**：凡是**新增、删除或改动** **`docs/spec` 路径依赖**（含：`docs/spec/` 下被 **`scripts/`**、**`docs/`**（spec 外）、**`.github/workflows/`** 等硬编码消费的路径；上述消费方对 **`docs/spec/…`** 的锚点/门禁目标变更；**`registry/spec-path-dependencies*.yaml`** / 本文所记条目的增删改），**须同步**更新 **本文** 与 **[`registry/spec-path-dependencies.v1.yaml`](../registry/spec-path-dependencies.v1.yaml)**，并运行 **`python registry/validate-spec-path-dependencies-registry.py`**（**exit 0**）。**不**借此迁移或删除 **`docs/spec`**，**除非单独立项**；**不**改 **`build.yml`** 主链默认必过组合。低频 CI：**[`.github/workflows/registry-spec-path-dependencies-validate.yml`](../.github/workflows/registry-spec-path-dependencies-validate.yml)**。
