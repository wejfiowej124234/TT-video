...
7. **动商家入驻链（`app/provider/register*`、`provider-applications`、市场 listing 三门闸等 · ①）**：`bash scripts/dev/smoke-provider-onboarding-local.sh`（**[provider/register README](../frontend/app/provider/register/README.md)** · **[dev-local-smoke-baseline §8](dev-local-smoke-baseline.md)**；须 **`DATABASE_URL`** · **`INTERNAL_API_SECRET`** · **`SEED_TEST_ACCOUNTS=1`**；**非 ②③ GO**）
8. **动旅行收购 PD-009 链（`app/market/acquisition*`、收购 `acquisition_publish_gate` / `GET /me.trust` 收购字段等 · ①）**：`bash scripts/dev/smoke-acquisition-pd009-local.sh`（**[market/acquisition README](../frontend/app/market/acquisition/README.md)** · **[dev-local-smoke-baseline §9](dev-local-smoke-baseline.md)** · **[acquisition-publish-trust-rules §8.1](spec/artifacts/acquisition-publish-trust-rules.v1.md#81-第一阶段--本地--closed2026-05-27)**；须 **`DATABASE_URL`** · **`SEED_TEST_ACCOUNTS=1`**；**非 ②③ GO**）
9. **宣称 ① 全站企业 10 分（L5 全链路全矩阵）**：`bash scripts/dev/run-enterprise-site-10-local.sh`（**[ENTERPRISE-SITE-10-L5-MATRIX](runbook/ENTERPRISE-SITE-10-L5-MATRIX.md)**；末行 **`TT_ENTERPRISE_SITE_10_LOCAL: OK`**；**≠** ②③ · **≠** 仅走廊 `TT_ENTERPRISE_LOCAL_10`）
10. **动 Web3 旅行首页 `/` 或自由市场主入口 `/market` 数据链（`landingItinerarySession` · `useMarketPage` · 收藏 **`localStorage`** 等 · ①）**：`bash scripts/dev/run-web3-itinerary-l5-green.sh` + **`bash scripts/dev/smoke-web3-itinerary-full-chain-local.sh`**（**[LANDING-MARKET-PAGES-CODE-SSOT](../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** · **[dev-local-smoke-baseline §10](dev-local-smoke-baseline.md)** · **[GO_local_web3_itinerary_l5](../frontend/evidence/GO_local_web3_itinerary_l5/README.md)**；**非 ②③ GO**）
11. **动治理域 / TTG Cert 脚本 / `docs/spec/governance-token` / HAT-R1 / Four-Ledger / Admin RBAC 对齐证据（② · Sepolia 验收轨）**：`bash scripts/dev/run-ttg-governance-cert-post-change-gate.sh`（**已完成 Cert #1～N 机读 smoke 回归**；**新 Cert** 须同步 **`registry/ttg-governance-cert-gates.v1.yaml`** + **`python scripts/dev/validate-ttg-governance-cert-gates-registry.py`**；**active Cert finalize** 仍单独 `--finalize` — **[TTG-CERT-EXECUTION-SESSION-RUNBOOK](runbook/TTG-CERT-EXECUTION-SESSION-RUNBOOK.md)**；**≠ ① Phase① FULL MASTER** · **≠ ③ GO**）
