...
|----------------------------|---------------------------|---------------------------------------------------------|--------------------------------------------|------------------------------------------------------------------------------------------------------------------|---------------------|-----------------------------------|------------------------------------------------------------------|
| **`refunded`** | `test_B094_executeResolution_refunded_full_traveler` · `orderId = keccak256("b094-refund")` | **0** · **1_000_000_000** · **0** | **无**（仅 **forge** 闭环；补钉真 tx **单点** 更新 **§3.1** 或 evidence bundle） | 旅行者 **+1000e6**；向导/平台 **不变** | **`ResolutionExecuted`** | 有则填执行块；否则 **NULL**（以迁移/handler 为准） | 与 **`orders.id`** 按 indexer 规则映射之 **32-byte** 投影键（**§3.1**） |
| **`partially_refunded`** | `test_B094_executeResolution_partially_refunded_split` · `orderId = keccak256("b094-partial")` | **300_000_000** · **650_000_000** · **50_000_000** | **`0x11841400150ad0a9668a50652dca2d91c953ee50e230a601a6c1de777b2cb75c`**（[`rpc-tt9-eth-getTransactionByHash-resolution-input.json`](rpc-tt9-eth-getTransactionByHash-resolution-input.json) **`result.hash`**） | 向导 **+300e6**、旅行者 **+650e6**、平台 **+50e6** | **`ResolutionExecuted`** | **`6`**（[`sql-tt9-orders-projection-b094.txt`](sql-tt9-orders-projection-b094.txt)） | **`0000000000000000000000000000000033333333333343338333333333333301`**（TT-9 seed / SQL 导出一致） |
| **`slashed`** | `test_B094_executeResolution_slashed_guide_zero_platform_fee` · `orderId = keccak256("b094-slash")` | **0** · **800_000_000** · **200_000_000** | **无**（策略同 **refunded**；补钉 **§3.3**） | 旅行者 **+800e6**、平台 **+200e6**、向导 **不变** | **`ResolutionExecuted`** | 同 **refunded** 列规则 | 同 **refunded** 列规则 |
...
|------|---------------|------------------|---------------|---------------------|----------------------------------|
| 全额退旅行者 | 0 | 1_000_000_000 | 0 | `Refunded` | `Resolved` |
...
| **余额摘要（`executeResolution(0, TOTAL, 0)` 前后）** | 旅行者 **`+TOTAL`**；向导、平台 **不变**；**`token.balanceOf(escrowAddr)`**：`TOTAL` → **`0`**。 |
...
| **余额摘要（与 calldata 三腿一致）** | 向导 **`+300e6`**、旅行者 **`+650e6`**、平台 **`+50e6`**；合约余额 **`→ 0`**（Foundry 断言同逻辑）。 |
...
| **余额摘要（`executeResolution(0, 800e6, 200e6)` 前后）** | 旅行者 **`+800e6`**；平台 **`+200e6`**；向导 **不变**；**`token.balanceOf(escrowAddr)`**：`TOTAL` → **`0`**。 |
