# DApp 架构总览（Next.js 前端 · DApp 模式）

**旅游项目 DApp**：Next.js / React / TS（同一前端，Web + DApp 两模式）。技术栈与落点以 [09-技术架构总览 §2.7 DApp+3D 定稿方案](09-技术架构总览-v1.0.md) 为准（wagmi + viem + WalletConnect v2、EIP-712、自研 tx、viem watchContractEvent、12 块 finality）。

本文是 TravelTrust **DApp 侧**的架构总览：DApp 在**同一 Next.js 前端**（frontend/）中的定位、与 Web 的架构关系、数据流与实现要点。与 [01-总库总览](01-总库总览.md)、[02-架构设计](02-架构设计.md)、[05-前端总览](05-前端总览.md)、[09-技术架构总览](09-技术架构总览-v1.0.md) 配合阅读；DApp 为 05 中 **dapp/** 模块的架构展开，链与 DB 边界、用户签 vs 执行器代发以 01/02 为准。**DApp 技术栈（已锁定）**：**wagmi + viem + WalletConnect v2**；EIP-712 TypedData 签名；自研 **txMachine** 管理交易状态；**viem watchContractEvent** 监听链上事件；**Finality 12 blocks** 确认后再视为终态。  
**全系统大图中的 DApp 位置**（链上·用户签、与 Escrow/订单主链关系）：[18-TravelTrust-全系统架构图](18-TravelTrust-全系统架构图.md) §二/§七、[18-补充-TravelTrust-全系统架构层级图-最终版](18-补充-TravelTrust-全系统架构层级图-最终版.md)；可执行步骤与协议区见 [53-阶段开发技术文档](53-阶段开发技术文档.md)、[14-合约-API-ABI-前后端对齐](14-合约-API-ABI-前后端对齐.md)。

**与 01/02/05 对应**：01 §7 钱包/代付/EIP-712、§5 链与代币 → 本文签名与合约交互；02 表现层与「链上·用户签」边界 → 本文 DApp 职责；裁决上链由执行器代发（01 执行器 P0）。05 为唯一前端总览，本文为其 **DApp 模式**专篇；业务数据以 [04-后端与API](04-后端与API.md) 为唯一来源；超时/争议见 01、[03-业务流程与风控](03-业务流程与风控.md)；阶段见 [07-开发流程与顺序](07-开发流程与顺序.md) **§二 Phase 3/4**。**DApp 位置（已锁定）**：与普通 Web 共用同一 Next.js 前端，通过「是否连接钱包」区分；一前端、Web+DApp 两模式、业务数据同源（05 §三）。

**受众与用法**：做**钱包/签名/支付/质押/争议**时 05 为主、06 为辅；仅做普通 Web 读 05+04 即可。受众：DApp 与钱包/链实现、前端与 05 对接人；建议先读 01、02、05；API 与规则以 04、01、03 为准。**与 08 门禁对应**：DApp/前端可替代性与可验证发布（08 Gate-5、W-Q6-FE/W-BOMB-10）见 [08-4 第 7 章](08-4-对外口径包.md)、[08-1 附录 Gate-5](08-1-战略与合规风险检查清单.md#附录上线-go-no-go-gate发布级门禁)；05 已含与 08 的落点说明。**文档维护责任**：前端或技术负责人；DApp 变更须与 01 §7/§10、02 §十三 13.1、05 一致并同步 04；§6.3 开工前检查清单变更时建议同步 [07-开发流程与顺序](07-开发流程与顺序.md) §四 4.3。文档版本与最后更新见 [00-文档索引](00-文档索引.md)。

### 读前摘要

| 你要找什么 | 单源 |
|------------|------|
| **wagmi / viem / txMachine / finality** | **§三～§五**；技术定稿 **[09 §2.7](09-技术架构总览-v1.0.md)** |
| **数据流（deposit、release、争议）** | **§四** |
| **业务 API** | **[04 §3.4](04-后端与API.md)** — 本文**不**列全量路由 |
| **合约方法与 ABI** | **[14](14-合约-API-ABI-前后端对齐.md)**、`contracts/abi`、`frontend/dapp/abis` |
| **链上部署顺序（钱包所连网络与合约地址）** | **[14 §6](14-合约-API-ABI-前后端对齐.md)**、**[Runbook §2.56](../../ops/RUNBOOK.md)**、[governance-token/02 §1.3](governance-token/02-对内技术规格-草案.md)、**[07 §二 Phase 3](07-开发流程与顺序.md)** |
| **前端总览与目录** | **[05](05-前端总览.md)**（**06** 仅展开 **dapp/** 模式） |
| **Escrow 参与方签名语义（Traveler / Guide vs `provider`）** | **[87](87-TravelTrust-角色体系技术文档-融合架构版.md)** §8、§10；ABI 真值 **[14](14-合约-API-ABI-前后端对齐.md)** |
...
| deposit/**release**/openDispute 须钱包签；链下确认行程完成见 04 §3.4 | 01 §7 | §一、§三 签名与交易、§四 1～4 | 一致 |
...
| **07 §二 Phase 3/4** 链与 DApp、05+06 配合 | 07 §二 Phase 3/4 | 开篇与 01/02 对应、受众与用法、6.3 检查清单 | 一致 |
...
|------------------|-------------|--------|
| DApp/钱包/EIP-712/代付 P0、UI 事实 | 钱包连接与签名、deposit/**release**/openDispute 须钱包签；链下 `confirm-completion` 与 **release** 解耦（04 §3.4）；EIP-712 Domain Separator 写死；不托管、不代签；已支付仅链上 Paid/对账 | §一、§三、§四、§五 |
| Web 与 DApp 业务数据同源 | 业务数据唯一来自 04 API；DApp 与 Web 共用同一 API、同一 Next.js 应用 | §二 |
...
| **DApp** | 前端连接钱包 → 用户对「支付 / 质押 / **release** / 发起争议」等操作**签名** → 交易发到链上，合约执行；**确认行程完成（链下）**走 API，与 **release** 分列（04 §3.4）；前端或后端再读链状态与 API 一起展示结果。 |

**DApp 与 Web 业务数据同步（P0）**（与 05 §二 一致）：业务数据**唯一数据源 = 04 API**（01 §9）；DApp 与 Web **共用同一 API**，不因钱包分源。链上仅负责支付/质押/**release**/openDispute 等钱包签；链下确认行程完成见 API；业务展示以 API 为准。同一订单在 Web 与 DApp 页行程/接待人/状态须一致；订单详情、向导、行程**一律从 API 获取**。超时/争议窗口以 01、[03-业务流程与风控](03-业务流程与风控.md) 定稿为准（争议窗口 disputeDeadline≥autoCompleteAt，01 §10 12 缝 #3），前端从 API 获取并展示。
...
- **钱包切链/换号**：必须把 `chainChanged/accountsChanged` 视为“身份失效事件”，触发状态清理并强制刷新（最小实现见 05 §七点十）。
- **Gas 预估与上限**：必须按 EIP-1559 做预估 + 缓冲，并写死 `maxFeePerGas` 上限策略；超过上限时提示用户稍后再试，不得无限重试（见 05 §七点十一）。
- **ERC20 非标准行为（必须写死支持范围）**：默认仅支持“标准 ERC20”（例如 USDC/DAI），并在合约/后端层显式拒绝：
    1) **fee-on-transfer**（转账扣税导致到账<amount）
    2) **non-standard return bool**（不返回 bool 或返回非标准值）
    3) **ERC777 hooks/reentrancy 风险**（若允许任意 token 则必须以 SafeERC20 + reentrancy guard 设计封死）
    当前阶段的最小落点是：token allowlist 只允许经过审计的标准 token；任何“非标准 token 兼容”都必须进入 08-3/08-4 的明示范围，禁止默许。
- **小数精度与 rounding**：合约计算与签名参数必须使用最小单位整数；前端仅格式化展示，不得自行四舍五入影响结算参数（见 05 §七点十二）。
...
| [02-架构设计](02-架构设计.md) | 表现层 = 前端（Next.js）；分层与「链上·用户签 vs 执行器代发」、§十三 13.1 前端/代付与资金边界。 |
| [03-业务流程与风控](03-业务流程与风控.md) | 超时/争议窗口与前端展示一致；业务规则定稿见 01/03。 |
| [04-后端与API](04-后端与API.md) | 业务数据以 04 API 为**唯一来源**；订单/向导/争议/档期、Escrow 地址与参数从 API 获取。 |
| [05-前端总览](05-前端总览.md) | 本文为 05 中 **dapp/** 模块的架构总览；页面、路由、lib/ 与 05 共用；做 DApp 功能时 05 为主、06 为辅。 |
| [07-开发流程与顺序](07-开发流程与顺序.md) | **§二 Phase 3**（链对接）、**§二 Phase 4**（前端与 DApp）须同时参 06；业务数据同源在 **07 §二 Phase 4** 须满足。 |
...
| 维度 | 说明 | 06 落点 |
|------|------|---------|
| 与 01/02/03/04/05/07 一致性 | 01 §7/§9/§10（12 缝 #3/#11、17 条 #2/#14）、02 §十三 13.1、03 超时/争议窗口、04 唯一来源、05+06 配合及 §九 敏感操作、07 §二 Phase 3/4 | 开篇两表、§一～§五 |
...
| 检查项 | 要求 |
|--------|------|
| 01 §7 | 钱包/SIWE、EIP-712 写死、UI 事实、**代付 P0（不托管、代付法务门禁见 01）**、deposit/**release**/openDispute 须钱包签已载明；链下确认行程完成见 04 §3.4（§一、§三、§四、§五） |
...
| 04/05/07 | 业务数据从 API 获取；05 为主 06 为辅、05 §九 9.0.5 敏感操作见 §五 安全；**07 §二 Phase 3/4** 参 06、**07 §二 Phase 4** 业务数据同源 |

**附录〇 收口（2026-04-07）**：本 **§6.3** 与 [15 附录〇 第 13 项](15-多维度文档与技术检查报告.md#发版前勾选总表) 联合勾选；上表与 01/02/03/04/05/07 引用已交叉核对，发版前由责任人做最后一轮回归确认。

---

以上即 TravelTrust **DApp 架构设计总览**，便于与 01/02/03/04/05/07 及后端、合约对齐实现。

*本文与 [01-总库总览](01-总库总览.md)、[02-架构设计](02-架构设计.md)、[03-业务流程与风控](03-业务流程与风控.md)、[04-后端与API](04-后端与API.md)、[05-前端总览](05-前端总览.md)、[07-开发流程与顺序](07-开发流程与顺序.md) 配套；**治理扩展（Target）** 与 [13-1](13-1-UI产品级SSOT与页面规范.md) **表 2-续**、[83](83-区域治理与收益分配-协议白皮书.md) **附录 I.0**、[82](82-治理币-文档总览.md)、[08-4](08-4-对外口径包.md) 交叉见 §三末段。文档版本与最后更新见 [00-文档索引](00-文档索引.md)。*
