...
| **`/` + 三页市场 · ① 代码/UI 真源** | **[LANDING-MARKET-PAGES-CODE-SSOT](../../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** · **[`(home)/README`](../../frontend/app/(home)/README.md)** · **[`market/README`](../../frontend/app/market/README.md)** |
...
**当前落点（简要）**：  
- **首页**（**`app/(home)/page.tsx`**，路由 **`/`**）：`LandingHeroForm` + `useLandingPage`，提交后 **单次** `postItineraryCreate` 创建 **1 份** Draft 订单；**`ITINERARY_CARD_COUNT=1`**（`components/landing/constants.ts`）— **1 张预览卡**绑定该 `order_id`（**非** 多次创单；历史 5 卡同单展示已收敛为 1，见 **53**「自由市场重复行程」）；**`localStorage`**（`landingItinerarySession.ts` · 跨 tab · 旧 session 一次迁移）恢复预览/解锁 id；`ItineraryResultsSection` 预览，**预览解锁** 调用 `getOrder(orderId)`（**① 无真实 USDC 扣款**）。  
...
- **自由市场**（`app/market` + **`/market/provider|acquisition`** 子站）：主入口 **`useMarketPage`** — URL 筛选 · **`getDiscoverOrders`/`getGuides`** · **300ms debounce**（`MARKET_LIST_REFETCH_DEBOUNCE_MS`）；收藏 **① FE `localStorage`**（**`marketFavoritesStorage.ts`** · **F-020 best-effort 已接线（①）· ② SLA**）。`CustomItineraryModal`（42）旅行者/向导手填行程 + 平台定价（lib/countries，44）。**❶ 自定义行程走后端（已闭合，见 §0.3）**：提交须 **POST /api/v1/itineraries/custom** → 后端创建 Draft order + 写 itineraries → 返回 `order_id` → 市场页重新拉取 `getDiscoverOrders`；若代码回退为「仅前端列表」，即回到协议级缺陷，须按 §0.3 门禁修复。持久化订单来源应为：首页行程生成、**自定义行程接口**、或 `/orders/new` 的 `postOrder`。子站列表 **PG catalog** 优先（**94**）；demo 仅 API 失败降级 — **[LANDING-MARKET-PAGES-CODE-SSOT](../../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** §3～§5。  
- **订单详情/托管**（`app/escrow/[id]`）：`EscrowDetail` 用 `getOrder(id)` 拉取 order + itinerary（daily_itinerary、amount_breakdown、snapshot_hash）；**预链上草稿**（`isPreEscrowProtocol && !hasEscrow`）为 **Experience 暖色壳**（**UI 已冻结 2026-05-28** → [`ESCROW-DRAFT-EXPERIENCE-FREEZE.md`](../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-DRAFT-EXPERIENCE-FREEZE.md)）；3 步紧凑步骤条、发布横幅、预览缩略图、页脚「更多」取消；**高级协议区默认隐藏**（仅 `NEXT_PUBLIC_ESCROW_DEV_TOOLS=1`）。Draft 且无 snapshot 时 `ConfirmFinalPlanBlock` → `POST /api/v1/orders/:id/confirm-final-plan`；**已上链**仍为协议 DID 壳（存款/争议，**未** UI 冻结）。
...
|------|-----------|--------------------|-----------------------------|
| **/** | **`app/(home)/page.tsx`** | `LandingHeroForm`（国家/城市/日期/天数/景区/餐饮酒店/预算/`party_size`/`num_rooms`）、`ItineraryResultsSection`（**1** 预览位 · 同单；解锁后 daily_itinerary + USDT/USDC）、`UnlockModal`（① `getOrder` 预览加载）；**`landingItinerarySession`**（**`localStorage`** · 跨 tab · **`marketFavoritesStorage.ts`**） | `postItineraryCreate`（**1 次**）、`getOrder`（预览解锁） |
...
| **/market** | `app/market/page.tsx` | **`useMarketPage`**（URL 筛选 · **300ms debounce** · 收藏 **`localStorage`**）；`MarketPageHero`（「自定义行程」打开 `CustomItineraryModal`）、`MarketContent`（订单/向导卡片）、`OrderDetailDrawer`、`GuideDetailDrawer`、`BookGuideModal`；`CustomItineraryModal` 手填行程 + lib/countries 定价（44） | `getDiscoverOrders`、`getGuides`。自定义行程：**POST /api/v1/itineraries/custom** → Draft + `order_id` → 刷新列表（与 §0.3 ❶ 已闭合一致；回归须门禁） |
| **/market/provider** · **/market/acquisition** | `app/market/provider/*` · `app/market/acquisition/*` | **`MarketStandaloneBusinessPage`** · **`marketSubsiteFilters.ts`** · PG catalog 列表/详情 | **`GET …/market/provider|acquisition/listings`**（**04 §3.4** · **94**）；见 **[LANDING-MARKET-PAGES-CODE-SSOT](../../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** §4～§5 |
| **/discover** | `app/discover/page.tsx` | 重定向到 `/market`（29 约定） | — |
| **/traveltrust** | `app/traveltrust/page.tsx` | **融资向网络落地页**（[85](85-TravelTrust网络落地页-融资级设计与开发规格.md)）：**`layout.tsx` 深色壳**；**L1** **`TravelTrustHomeLandingNavSlot`** portal + **CSS** 公告跑马灯；**layout lock** `hero→roles→…`（**无** `#overview` — **[FIVE-MAIN-ROUTES](../../frontend/evidence/GO_local_marketing_front_closure/FIVE-MAIN-ROUTES-PHASE1-FREEZE.md)**）；**非** ICO | **`modules/traveltrust-home`**；**04 §3.4**、**13-1** |
...
| **/escrow/[id]** | `app/escrow/[id]/page.tsx` · [`app/escrow/[id]/README.md`](../frontend/app/escrow/[id]/README.md) | **双壳**：① **Experience**（`experienceDraft`）— [`EscrowDraft*`](../frontend/components/escrow/EscrowDetail/README.md) 编排、Tab 行程编辑、发布→`/market?view=split&bindGuideToOrder=`、预览 `richCollapsedPreview`；**UI 冻结** [`ESCROW-DRAFT-EXPERIENCE-FREEZE`](../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-DRAFT-EXPERIENCE-FREEZE.md)。② **协议 DID**（已上链）— 8 步 cyan、链上动作、风险栈。共用：`ConfirmFinalPlanBlock`、`QuoteSummaryCard`、`getOrder` | `getOrder` · `PATCH …/itinerary` · `PATCH …/guide` · `POST …/confirm-final-plan` · `POST …/cancel`（见 **04 §3.4**） |
...
1. **首页生成单份草稿**：用户在 `/` 填写偏好（`LandingHeroForm`）→ 提交后 **单次** `postItineraryCreate` 创建 **1** 份 Draft 订单 → **`ITINERARY_CARD_COUNT=1`** 预览卡展示 → 解锁时 `getOrder(orderId)` 拉取 daily_itinerary、amount_breakdown → 进入 `/escrow/[id]` 继续草稿发布与绑向导（见 [`app/(home)/README.md`](../../frontend/app/(home)/README.md) · **[LANDING-MARKET-PAGES-CODE-SSOT](../../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** §2）。
...
3. **市场与新建订单**：`/market` 展示发现订单（**`useMarketPage`** · **`getDiscoverOrders`** · **300ms debounce** · 收藏 **`localStorage`**）与向导（**`getGuides`**）。子站 **`/market/provider|acquisition`** 见 **§0.1** 与 **CODE SSOT** §4～§5。「自定义行程」打开 CustomItineraryModal，手填行程 + lib/countries 定价。**目标流程（§0.3 ❶ 已闭合）**：提交 → POST `/api/v1/itineraries/custom` → 后端创建 Draft order + 写 itineraries → 返回 order_id → 前端刷新列表或跳转 `/escrow/[id]`。持久化订单来源：首页行程生成、**自定义行程接口**、或 `/orders/new` 的 postOrder。
4. **订单详情与确认最终版本**：进入 `/escrow/[id]`，`EscrowDetail` 经 `getOrder(id)` 拉取 order + itinerary。**预链上草稿**走 **Experience 壳**（保存行程 → 发布自由市场 → 双栏市场选向导 → 确认终版 → ① mock-pay）；**无** 旅行者默认 FeeRouter/接单/聊天块（开发者仅 `NEXT_PUBLIC_ESCROW_DEV_TOOLS=1`）。`ConfirmFinalPlanBlock` → `POST /api/v1/orders/:id/confirm-final-plan` → `snapshot_hash` 写库后进入付款/链上阶段（28、13-1）。**已上链**订单仍为协议 DID 壳。

### 0.2.2 `/escrow/[id]` Experience 草稿 · 代码与文档 SSOT（2026-05-28）

| 维度 | 真源 |
|------|------|
| **Phase ① 收口** | [`ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md`](../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md) |
| **UI 冻结** | [`ESCROW-DRAFT-EXPERIENCE-FREEZE.md`](../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-DRAFT-EXPERIENCE-FREEZE.md) |
| **组件编排** | [`components/escrow/EscrowDetail/README.md`](../frontend/components/escrow/EscrowDetail/README.md) · `index.tsx` |
| **走廊证据** | [`GO_local_web3_itinerary_l5`](../frontend/evidence/GO_local_web3_itinerary_l5/README.md) |
| **机读绿集** | `bash scripts/dev/run-web3-itinerary-l5-green.sh`（含 `escrowDraftExperienceUiFreeze`） |
| **API 烟测** | `bash scripts/dev/smoke-web3-itinerary-full-chain-local.sh`（**①**；非 ②③ GO） |
| **逐文件映射** | [`62-补充-03-Escrow路由逐文件代码映射`](code-maps/62-补充-03-Escrow路由逐文件代码映射-20260306.md) §3.1 |
| **未冻结** | `hasEscrow` 后协议壳、`/escrow/[id]/rate` 评分页 |
...
| **Itinerary Preview（杂志式）** | 展示日程 + 预算拆分 + 高亮亮点。 | **已实现**：`/` 使用 `ItineraryResultsSection`（**`ITINERARY_CARD_COUNT=1`** 预览卡 + 解锁后 daily_itinerary、amount_breakdown）；`/itinerary/new` 结果区 Day N 分段、费用明细、`AgreementSummaryAccordion`（25/27-P22）。 |
