...
- **自由市场**（`app/market`）：订单列表来自 `getDiscoverOrders`（GET `/api/v1/discover/orders`，仅 Draft），向导列表来自 `getGuides`。`CustomItineraryModal`（42）游客/向导手填行程 + 平台定价（lib/countries，44）。**❶ 自定义行程走后端（已闭合，见 §0.3）**：提交须 **POST /api/v1/itineraries/custom** → 后端创建 Draft order + 写 itineraries → 返回 `order_id` → 市场页重新拉取 `getDiscoverOrders`；若代码回退为「仅前端列表」，即回到协议级缺陷，须按 §0.3 门禁修复。持久化订单来源应为：首页行程生成、**自定义行程接口**、或 `/orders/new` 的 `postOrder`。  
...
**正确顺序**：① 游客发布行程（Draft）→ ② 向导接单（Accepted）→ ③ 双方**聊天确认行程**（可修改、可多轮）→ ④ **双方均确认无异议后**，才执行「确认最终行程」并**生成智能合约绑定**（confirm-final-plan → snapshotHash → 创建/绑定 Escrow）。**不得**在未完成聊天确认前就生成合约。

**互斥约束（向导档期）**：向导在**档期被锁定的时间段内**不得再接其它人行程；**游客**在「进行中订单」限制内不得再发布新行程（或按产品规则限制数量）。**锁定生效点、锁定范围、档期精度、取消恢复**等**必须写死**，见 **§4.15 档期与锁定规则**；概要如下：

- **锁定生效点**：**不是** confirm-final-plan，而是 **deposit 成功并达链上 finality**。confirm 后、未 deposit 前向导**不被锁定**，可接其他订单；否则向导在「游客 2 小时才支付」等场景下被长时间白锁（见 §4.15）。
...
- **payment_window**：confirm 后**限定时间**（如 30 分钟）内必须 deposit；**超时自动取消 + 解除向导锁定**，避免游客不支付、向导被锁、订单未开始的运营死局（§4.9、§4.15）。
...
| **4️⃣** | **confirm-final-plan 无乐观锁** | 向导改 v3、游客在旧页确认 v2 → 错误 snapshot。 | **P0 Gate**：confirm 请求**必须带 expected_version**，后端 **CAS 校验**（当前 version === expected_version 才写入 snapshot_hash），否则返回 409 并提示刷新；见 §4.5。 |
...
| # | 缺口 | 闭环要求 | 落点 |
|---|------|----------|------|
| **1️⃣3️⃣** | **订单撤回与信誉惩罚** | 游客频繁生成→撤回→不支付，向导时间被浪费。 | **必须**：**限制撤回次数**和/或**扣游客信誉**；成文；见 03、§4.6。 |
...
| **4️⃣** | **资金卡死场景** | 向导/游客失联、双方都不操作，资金永远锁仓。 | **必须**：Escrow 有**强制 timeout**、**强制 dispute 入口**、**强制清算机制**（如超时自动可争议、仲裁后执行）；见 01 订单状态机、03。 |
...
|--------|------------------------------|------|
| A. confirm-final-plan 成功 | **否** | 若以此为准，游客 2 小时后才支付则向导被白锁 2 小时；**禁止**。 |
...
见 **§4.9**：confirm 后 **payment_window**（建议 30 分钟）内必须 deposit；**超时自动取消 + 解除向导锁定**。否则游客不支付、向导被锁、订单未开始 = 严重运营问题。

#### 4.15.3 防「锁单攻击」（confirm 滥用）

游客恶意：反复 confirm 不支付，向导被反复锁定。**必须成文**：

| 措施 | 要求 |
|------|------|
| **confirm 次数/频率限制** | 每订单 confirm 仅一次（已满足）；同一用户**短时内对多订单 confirm 不支付**可设上限（如 24h 内 N 单未支付则限制再 confirm）。 |
| **用户信誉惩罚** | confirm 后超时未支付：扣**游客信誉**或记入「未支付次数」；与 03、81 信誉规则一致。 |
| **冷却时间** | 同一游客对同一向导、或全局「confirm 未支付」后，**冷却期内**限制再次 confirm 新单（可选）。 |
...
| 场景 | 要求 |
|------|------|
| **游客取消 / 合约终止** | 向导档期**是否立即解锁**须成文。建议：**立即解锁**（取消/终止即释放档期）。 |
...
| **订单转让** | 是否允许**订单转让**（换向导、换游客）；与 01/03 状态机、合约一致。 |
...
**订单撤回与信誉（0.5 1️⃣3️⃣）**：游客频繁生成→撤回→不支付会浪费向导档期；须成文**限制撤回次数**和/或**扣游客信誉**，与 03 一致。**市场操纵（0.5 1️⃣4️⃣）**：多账号接单、抬高排序须防范；须成文 **anti-sybil**、**stake-weight 排序**、**信誉加权**等，与 29 一致。
...
| 17 | **核心流程与互斥约定**（§0.2.1）：游客发布→向导接单→聊天确认行程→**双方确认后**才生成智能合约；确认后至行程结束前**游客不得再发布、向导不得再接单**，本单结束后才恢复 | **待补**：01/03 订单状态与业务规则成文；后端接单/发布时校验进行中订单与档期。 |
