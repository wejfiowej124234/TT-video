...
| **五数字锁定（FeeRouter 经济链）** | **45% / 55%**（第一层，相对 **可分配平台费用 100%**，定义 **[84 §1.1.1](84-第一阶段10国Country-Pool发行参数总表.md)**）+ **65% / 20% / 15%**（**Global 内部**，相对 Global 自身 100%，**[84 §1.2](84-第一阶段10国Country-Pool发行参数总表.md)**）；正文与表见 **[83 §3](83-区域治理与收益分配-协议白皮书.md)、[83 附录 B.2～B.3](83-区域治理与收益分配-协议白皮书.md#附录-b--收益资金流模型)** |
...
- 本图描述的是 **订单完成、平台收取的可分配手续费**（进入 **FeeRouter** 的基数 = **100%**），**不是** Escrow 内旅行者/向导订单本金；**本金**仍按 Escrow 合约释放，与本图并行独立。**100% 的闭合定义**（含 **不扣减** 用户自付 **L1/L2 gas**）见 **[84 §1.1.1](84-第一阶段10国Country-Pool发行参数总表.md)**。
- **仲裁费、向导侧 `IdentityStakingPool` 系 `slash` 罚没**（**81/14**；历史文案 **`Staking.slash`** 已弃用）与上图 **45/55** **正交**；归宿与科目见 **[Runbook](../../ops/RUNBOOK.md) §7.1** 专表、[08-4](08-4-对外口径包.md) 第 2 章；**不得**与本图混为一条线而不加脚注。
...
**锁定（对读）**：上列五处百分数与 **[84 §1.1 表、§1.2 表](84-第一阶段10国Country-Pool发行参数总表.md)**、**[83 §3 流程与比例表](83-区域治理与收益分配-协议白皮书.md)**、**[83 附录 B](83-区域治理与收益分配-协议白皮书.md#附录-b--收益资金流模型)** 及 **`governance_doc_reference::protocol_reference_json` → `fee_router`** 同批；**不得**只改图示或只改镜像。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.0.6 | 2026-04-19 | **§1 状态行**：**55-S13** 可执行路径写 **`bash scripts/check-55-s13.sh`**（与 **83/84/14** 同口径）。 |
| 1.0.5 | 2026-04-19 | **§1**：**FeeRouter** 仓库态 **Partial/MVP** 与 **02/14/B-116** 对齐；**slash** 用语与 **`IdentityStakingPool` 系** 同源。 |
| 1.0.4 | 2026-04-08 | **B-116-4-2**：读前摘要增 **五数字锁定** 行；§2 Mermaid 后增 **锁定（对读）** 段，与 **83/84/API `fee_router`** 闭合。 |
