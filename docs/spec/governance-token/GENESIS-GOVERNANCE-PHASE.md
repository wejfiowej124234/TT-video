...
| 层 | SSOT |
|----|------|
| 供应四块 · 10M（Genesis V2） | [TTG-TOKENOMICS-GENESIS-V2](TTG-TOKENOMICS-GENESIS-V2.md) · [protocol-ssot.v1 §1](protocol-ssot.v1.md) |
| GOV-01～04 | [TTG-TOKENOMICS-FREEZE-V1](TTG-TOKENOMICS-FREEZE-V1.md) |
| 分配 · 权限 · 流程图 | [ttg-allocation-permissions-flows-ssot-v1](ttg-allocation-permissions-flows-ssot-v1.md)（分配表 → Genesis V2） |
| 公众三轮 · P4 · Seat 退出 | [ttg-primary-market-and-exit-policy-v1-draft](ttg-primary-market-and-exit-policy-v1-draft.md) · Registry 轮次初值 |
...
```text
Tokenomics（FROZEN · Genesis V2 四块 + GOV-01～04）
        ↓
Allocation / 四轨 / 两轨收益（FROZEN · Genesis V2 + protocol-ssot）
        ↓
...
2. **稀释是设计内建的：** 随 **公众三轮**（Registry 初值 800K + 1.2M + 3M = 5M）、**Community Incentive Program**、**DAO Treasury** 按 SSOT 释放/拨付，**team 占 Active Voting Supply 比例自然下降**，长期 **team 桶仍 ≈ 总供应 15%**（非 100%）。**无 Country Shelf 创世桶** — Steward 以 **自持 TTG** 质押达门槛。  
...
5. **One TTG, Same Protocol Rights** — 协议不记来源；DAO Treasury **≠** 投票权来源；**禁止** Mint 补仓扩张供应叙事。
...
Treasury Safe / Timelock 治理边界
        ↓
Allocation Pool / Vesting（team · Community Incentive Program · public rounds · …）
```
...
- **公众三轮尚未全部开启**，且 **team 仍处 cliff/vest 早期**  
- **DAO Treasury / Community Incentive 多数尚未进入 Active Voting Supply**
...
下列为 **披露用简化模型**（假设：team 桶 **1.5M** 在 Genesis 窗口 **已全部可投票**；公募按三轮 **全部售完** 且 **购买者均参与投票**；**忽略** vesting 锁、委托率、CIP/DAO 桶 — **实际 snapshot 以链上为准**）。**Round 3 后 Active ≈6.5M 示例** 与 Registry 默认 `active_voting_supply_min_bps: 3500`（≥3.5M）**兼容** — **非 Genesis Exit 硬编码**（Exit 见 §7.1 AND + Registry）。

| 阶段 | Team 可投票 | Public 可投票 | Active Voting Supply（示例） | Team 占 Active % |
|------|-------------|---------------|------------------------------|------------------|
| **Genesis** | 1.5M | 0 | 1.5M | **100%** |
| **Round 1 后** | 1.5M | 0.8M | 2.3M | **≈65%** |
| **Round 2 后** | 1.5M | 2.0M | 3.5M | **≈43%** |
| **Round 3 后** | 1.5M | 5.0M | 6.5M | **≈23%** |

**公众三轮（Registry 初值 · 合计 5M）：** R1 **800K** · R2 **1.2M** · R3 **3M**（见 [ttg-vesting-registry](../../../registry/ttg-vesting-registry.v1.yaml) · [TTG-TOKENOMICS-GENESIS-V2](TTG-TOKENOMICS-GENESIS-V2.md)）。

**再往后（叙事 · 非精确演算）：** Community Incentive Program、DAO Treasury 按 SSOT 拨付 → **team 占全供应长期 ≈15%**，**占 Active Voting Supply 持续下降** → **Community Governance**。Steward 以 **自持 TTG** 质押（**无 Country Shelf**）。
...
**冲突披露（GOV-02 · FROZEN）：** team / 初创 founding multisig **须** 披露利益 · **禁止** 未披露投票（Genesis V2 **无**独立 advisors 创世桶）。
...
| 用户叙事 | SSOT 桶 / 模块 |
|----------|----------------|
| Region Steward Seat | **自持 TTG** 质押达 Governance Parameter 门槛（**无** `country_pool_shelf` 创世桶） |
| Community Incentive | `community_incentive` 5% · Program（非 vesting） |
| DAO TTG 桶 | `treasury_dao` 30% · **3M TTG**（战略托管 · **≠** 投票权来源 · 禁 Mint）· **≠** USDC Global Treasury |
...
| Public Sale | `public_sale` 50% · **5M** · 三轮（Registry 初值 800K/1.2M/3M） |
...
| `public_governance_threshold.public_bucket_votable_min_bps` | Public Sale 桶可投票 TTG ≥ bps × **`public_sale` 创世 Allocation（5M）** |
...
| **G-VOTE-02** | **Community Incentive Program** | `community_incentive` 5% | 按 Program / Policy 进入流通后可 Vote | 可 `delegate` | Program custody | 将 Program 误读为 advisors vesting |
| **G-VOTE-03** | **DAO Treasury（TTG 桶 30%）** | `treasury_dao` | **默认不 Vote** | **不对外委托** | **Timelock / Safe** | **不能自己投自己** — 动用须 **Proposal → Vote → Execute** · **≠** 投票权来源 |
...
| **G-VOTE-06** | **Seat 质押（自持 TTG）** | 锁定于 StakePool | **Stake 期间** 通常 **不可转移** · Vote 权随 token custody（② 以合约为准） | 视 custody | StakePool | 一国多 Seat（GOV-03）· **无 Country Shelf** |

**已取消（Genesis V2）：** 独立 **Advisors Vesting** 创世轨（原 G-VOTE-02 advisors）— 披露对象改为 team / founding multisig。

**Team Vesting 商业参数（③ 前须 Owner 填）：** [registry/ttg-vesting-registry.v1.yaml](../../../registry/ttg-vesting-registry.v1.yaml) — **`team` 1,500,000 TTG（FROZEN）** · commercial **OWNER_INPUT** · **`community_incentive` 500,000 Allocation（Program）** · **`public_sale` 5M = Primary Market R1/R2/R3（800K+1.2M+3M Registry 初值）** · **`treasury_dao` 3M** · 无独立 investor / advisors / country_pool_shelf 创世池。
...
Active Voting Supply @ snapshot =
  ...
  （含 team 已解锁 + public 已购 + CIP 等已进入流通并委托的 TTG）
