...
- `contracts/src/` 已实现 `Escrow.sol`、`EscrowFactory.sol`、**`GuideIdentityStakingPool.sol`** / **`ProviderIdentityStakingPool.sol`**（继承 **`IdentityStakingPool`**）、**`Registry.sol`**、**`FeeRouter.sol`**（经济路由 **Partial**）、**`RegionVault.sol`**（国家桶池 **Partial · MVP**）、**`InvestorDistributionClaim.sol`**（份额分红领取 **Partial · B-087**）、**`GovernanceTimelock.sol`**（**B-089**）、**`TravelTrustGovernor.sol`**、**`GovernanceVotesToken.sol`**、**`GovernanceTreasury.sol`**（治理金库支出 **Partial · B-090**）等。
- `contracts/abi/` 已入仓：`Escrow.json`、`EscrowFactory.json`、**`GuideIdentityStakingPool.json`**、**`ProviderIdentityStakingPool.json`**、**`Registry.json`**、**`FeeRouter.json`**、**`RegionVault.json`**、**`InvestorDistributionClaim.json`**、**`GovernanceTimelock.json`**、**`GovernanceTreasury.json`**（**Governor/VotesToken** 以 **`forge inspect`** 补 ABI 入仓可与 **14** 对齐；完整清单可由 `forge inspect <Contract> abi` 覆盖更新）。
...
| **智能合约实现** | **已实现** | `contracts/src/` 已有 `Escrow.sol`、`EscrowFactory.sol`、**Guide/Provider 身份质押池**（**`IdentityStakingPool`** 系）、`Registry.sol`、`FeeRouter.sol`、`RegionVault.sol`、`InvestorDistributionClaim.sol`、`GovernanceTimelock.sol`、`GovernanceTreasury.sol`、`MockERC20.sol` 等（**旧 `Staking.sol` 已移除**，见 **contracts/README**）。 |
| **ABI 入仓状态** | **已入仓（55-S13 字节对齐子集）** | `contracts/abi/` 与 `frontend/dapp/abis/` 含 **Escrow / EscrowFactory / GuideIdentityStakingPool / ProviderIdentityStakingPool / Registry / FeeRouter / RegionVault** 等；`forge build` + **`scripts/sync-abi-from-forge`** 维护。 |
