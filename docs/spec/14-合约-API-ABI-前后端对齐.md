...
|------|------|-----------------------------|----------------|
| **Escrow** | 订单托管 | **EscrowFactory**：**`createEscrow`**（**B-091**：**`factoryPaused`** 时 **`FactoryPaused`**，仅阻断**新**实例；**已部署** Escrow **不**受影响）；实例：**deposit** / **release**（**B-093 / 01 §10**：**Completed** 收平台费；**`guideAmount = totalAmount * (10000 - platformFeeBps) / 10000`**（向下取整），**`platformFeeAmount = totalAmount - guideAmount`**（BPS 余数归平台费腿）；**`init`** 拒 **`platformFeeBps > 10000`**；**非** `POST .../confirm-completion`，后者仅链下行程完成，见 04 §3.4）/ refund / openDispute / **executeResolution**（执行器代发裁决；**B-094 Partial**：三腿 **`guideAmount + travelerRefund + platformFee == totalAmount`**，Foundry **`Escrow.t.sol`** **`test_B094_executeResolution_*`**；**`ResolutionExecuted`** **无**三腿金额，链下 **`orders_projection`** 映 **Refunded / PartiallyRefunded / Slashed** 须 **calldata** 或 outbox 侧车 + **`traveltrust_core::terminal_order_state_from_resolution_amounts`**）；事件 EscrowCreated、Deposited、Released、Refunded、DisputeOpened、ResolutionExecuted（01 §5；**Paid** 为产品口径，链上对应 **Deposited**；**Released** 的 **`guideAmount`/`platformFeeAmount`** 与上式可复算） | 前端：用户签 deposit/**release**/refund/openDispute；**createEscrow** 见 **EscrowFactory.json**（**55-S13**）；后端：Indexer 消费事件、Executor 调 executeResolution |
| **Staking** | 向导/仲裁员质押 | **stake** / **withdraw** / **slash**（仅 **`slasher`**，构造注入 immutable）；`MIN_STAKE`；**无**按单 bps/Cap（81 §4.2 路线图）；档位与接单上限 SSOT **08-3** | 前端：`/staking` 页 ERC-20 approve + stake；**slash** 由执行器/治理多签，非用户页；后端：读链、对账；ABI 见 `Staking.json` |
...
| **GovernanceTimelock（Partial · B-089）** | 治理参数 **`execute`** **延迟壳** | **`schedule(target,value,data,salt)`**（`onlyAdmin`）写入 **`operations[id]`**；**`delay`** 后 **`execute(id)`** 对 **`target`** **`call{value}(data)`**；**`hashOperation`** 纯函数 | ABI：`contracts/abi/GovernanceTimelock.json`；测试 **`GovernanceTimelock.t.sol`**（**`FeeRouter.transferOwnership`**、**`FeeRouter.setRoutingConfig`** 经 **`execute`**，**TT-COMP-B089**）；**`Deploy.s.sol`** + **`GOVERNANCE_TIMELOCK_DELAY_SECONDS`**；**不含**链上投票/Snapshot |
...
| 模块（工作名） | 状态（摘要） | 与现行 Escrow/Staking 关系 |
|----------------|--------------|------------------------------|
| **RegionVault** | **Partial（MVP）**：`forward` + **`RegionVaultForwarded`**；`Deploy.s.sol` 将 **FeeRouter `countryBucket`** 设为 Vault 地址 | 资金池与 Escrow 隔离；承接 **FeeRouter** 国家桶入账；**逐国链上再分 / Snapshot** 仍 Target |
...
| **GovernanceTimelock** | **Partial（B-089）**：`schedule` / `execute` + **`FeeRouter.transferOwnership`** / **`FeeRouter.setRoutingConfig`**（**TT-COMP-B089**） | **非** Governor 全栈 |
...
| **Governance token / TTG 激励（若启用）** | **Target** | 与 83 Global slice 等叙事对齐；**独立**于向导质押 `Staking.sol`（[81](81-经济模型-向导质押与订单押金.md)） |

**落地 checklist（FeeRouter / RegionVault / 110 · 2026-03）**：① ~~Solidity FeeRouter（MVP）~~ ② ~~Foundry `FeeRouter.t.sol`~~ ③ **Partial**：indexer-tick 合并 **`PlatformFeeRouted`** + DB **`fee_router_routed_events`** + **`GET /meta` 暴露** + governance/admin 只读 API（**[110 §3.1.1](110-阶段开发链上索引器与事件同步器.md)**）④ **Partial**：**`RegionVault.sol`** + **`RegionVault.t.sol`**（含 FeeRouter→Vault 串联）+ **`contracts/abi/RegionVault.json`** ↔ **`frontend/dapp/abis`** + **`verify-abi-forge.py`** / **55-S13** 纳入；**~~Deploy：`countryBucket` → Vault~~**（`Deploy.s.sol`）⑤ **Partial**：**`RegionVaultForwarded`** — **`REGION_VAULT_ADDRESS`**、**`region_vault_forwarded_events`**、indexer-tick、reorg 链域回滚含该表、governance/admin 只读、前端 **`/governance/vault-forwards`**；**110 仍 Target**：对账导出等深化 ⑥ [02-架构设计](02-架构设计.md) **§十**（若需图示）⑦ CI：`contract-abi-gate`、`check-governance-doc-linkage.sh` ⑧ [07 §二 2.4](07-开发流程与顺序.md) 若动经济百分数。
...
| **frontend** | 前端需用 ABI 调用 viem readContract/writeContract/signTypedData | **单源**：`contracts/abi/`；部署前同步至 **frontend/dapp/abis/**。**Staking / Registry** 两目录须**字节一致**（`scripts/check-55-s13`）。**Escrow**：canonical 全量在 `contracts/abi/Escrow.json`；`frontend/dapp/abis/Escrow.json` 可为精简 ABI，须与调用一致且含 **openDispute** 等（门禁同脚本）。`frontend/lib/stakingAbi.ts`、`registryAbi.ts` 为 viem `as const` 镜像，改接口须同步 JSON 与 TS |
| **后端（crates/api 或链客户端）** | 链下读链、对账、执行器调合约 | 若用 Rust 与链交互（alloy 等），ABI 或等效接口定义须与 contracts/abi 一致（可脚本从 JSON 生成 Rust 绑定） |

**当前仓库实况（2026-03）**：

- `contracts/src/` 已实现 `Escrow.sol`、`EscrowFactory.sol`、`Staking.sol`、`Registry.sol`、**`FeeRouter.sol`**（经济路由 **Partial**）、**`RegionVault.sol`**（国家桶池 **Partial · MVP**）、**`InvestorDistributionClaim.sol`**（份额分红领取 **Partial · B-087**）、**`GovernanceTimelock.sol`**（治理延迟执行 **Partial · B-089**）、**`GovernanceTreasury.sol`**（治理金库支出 **Partial · B-090**）等。
- `contracts/abi/` 已入仓：`Escrow.json`、`EscrowFactory.json`、**`Staking.json`**、**`Registry.json`**、**`FeeRouter.json`**、**`RegionVault.json`**、**`InvestorDistributionClaim.json`**、**`GovernanceTimelock.json`**、**`GovernanceTreasury.json`**（与 `contracts/src` 对齐；完整清单可由 `forge inspect <Contract> abi` 覆盖更新）。
- `frontend/dapp/abis/`：**Staking.json、Registry.json、EscrowFactory.json、FeeRouter.json、RegionVault.json** 与 `contracts/abi/` **字节一致**（`scripts/check-55-s13.sh`）；**Escrow.json** 可为精简 ABI，须含 `openDispute` 等与 DApp 调用一致；canonical 全量见 `contracts/abi/Escrow.json`（含 **DisputeOpened** 等事件，供 Indexer/文档单源）。质押/资格页 ABI 常量见 `frontend/lib/stakingAbi.ts`、`registryAbi.ts`。

**对齐检查**：合约接口变更 → 重新编译 → 更新 contracts/abi/*.json → 同步到 frontend/dapp/abis（或引用）→ 后端若用 ABI 同步更新；发版前核对 ABI 与部署合约版本一致。

**有序清单与门禁（运维 / PR）**：[ops/RUNBOOK.md §12.4](../../ops/RUNBOOK.md)（**sync-abi-from-forge** → **`frontend/dapp/abis`** → **check-55-s13** → **Contract ABI Gate**；与上表一致）。
...
**路由域与代码 SSOT（2026-03-24）**：`crates/api/src/routes/mod.rs` 中 `api_router()` 依次 `merge` **17** 个域：`health_meta`（含根路径 `/health`、`/meta`、`/metrics`）、`auth`、`admin`、`me`、`guides`、`orders`、`itineraries`、`discover`、`messages`、`disputes`、`evidence`、`media`、`intents`、`community`、`did_rank`、`governance`、`internal`。与 04 §3.4 路径一致性及 **04/13-1** 前端 IA 机读可跑 `./scripts/run-check-04-routes.sh`（**API 主表** + **`frontend/app`** + **13-1 表 1 ⊆ 04 前端路径表**；**`pre-release-automation`** 亦串联同源四步，见 **[scripts/README](../../scripts/README.md)** §三）；日常 PR 说明见根目录 **[CONTRIBUTING.md](../../CONTRIBUTING.md)**「路由与契约」。`contracts/abi` 与 `frontend/dapp/abis` 关键 ABI 可跑 `./scripts/check-55-s13.sh`（有 Foundry 时另见 `scripts/run-verify-abi-forge.sh`）。

**运维 JSON 快照（evidence · 非 §2.1 表内 HTTP 路径）**：**`scripts/indexer-public-snapshot.sh`**（Windows 委托 **`indexer-public-snapshot.ps1`**）将 **`GET /health`**、**`GET /meta`** 与可选 **`ADMIN_BEARER_TOKEN`** / **`INTERNAL_API_SECRET`** 段（admin/internal 索引器探针，与 **04 §3.4** **`internal`/`admin`** 表一致）合并为**一条** JSON；顶域 **`snapshot_provenance`**（**`script_semver` `1.3.0`** 等）须与 **`GET …/internal/indexer-status`** 等响应 **对读**。**L2 键与段落 SSOT** 见 **[04 §3.4](04-后端与API.md)** 读前摘要「索引器运维 JSON 快照」及 **`internal` API 总述**「运维 JSON 快照」；**流程、脚本矩阵与 CI 锚点** 见 **[110 §3.1.2](110-阶段开发链上索引器与事件同步器.md)**、**[Runbook §2.55](../../ops/RUNBOOK.md)**、**[evidence/README](../../evidence/README.md)**、**`indexer-reconcile-gate`**；**07 §六 6.4** 工程台账。**勿**将合并文件误当作 **§2.1** 表内独立 REST 资源；变更形状时 **04**/**110**/**gate**/**07** 同批。
...
| POST | /api/v1/orders/:id/reviews | 提交评价；**`review.weight_breakdown`**（**`traveltrust_core::ReviewWeight`**；评审者 **`users.created_at`** 账龄） | lib/orders |
...
| **智能合约实现** | **已实现** | `contracts/src/` 已有 `Escrow.sol`、`EscrowFactory.sol`、`Staking.sol`、`Registry.sol`、`FeeRouter.sol`、`RegionVault.sol`、`InvestorDistributionClaim.sol`、`GovernanceTimelock.sol`、`GovernanceTreasury.sol`、`MockERC20.sol` 等。 |
| **ABI 入仓状态** | **已入仓（55-S13 字节对齐子集）** | `contracts/abi/` 与 `frontend/dapp/abis/` 含 **Escrow / EscrowFactory / Staking / Registry / FeeRouter / RegionVault** 等；`forge build` + **`scripts/sync-abi-from-forge`** 维护。 |
