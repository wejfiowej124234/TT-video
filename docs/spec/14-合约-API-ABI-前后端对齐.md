...
- `contracts/src/` 已实现 `Escrow.sol`、`EscrowFactory.sol`、`Staking.sol`、`Registry.sol`、**`FeeRouter.sol`**（经济路由 **Partial**）、**`RegionVault.sol`**（国家桶池 **Partial · MVP**）、**`InvestorDistributionClaim.sol`**（份额分红领取 **Partial · B-087**）、**`GovernanceTimelock.sol`**（**B-089**）、**`TravelTrustGovernor.sol`**、**`GovernanceVotesToken.sol`**、**`GovernanceTreasury.sol`**（治理金库支出 **Partial · B-090**）等。
- `contracts/abi/` 已入仓：`Escrow.json`、`EscrowFactory.json`、**`Staking.json`**、**`Registry.json`**、**`FeeRouter.json`**、**`RegionVault.json`**、**`InvestorDistributionClaim.json`**、**`GovernanceTimelock.json`**、**`GovernanceTreasury.json`**（**Governor/VotesToken** 以 **`forge inspect`** 补 ABI 入仓可与 **14** 对齐；完整清单可由 `forge inspect <Contract> abi` 覆盖更新）。
