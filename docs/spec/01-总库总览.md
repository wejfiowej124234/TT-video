...
**说明**：以下为**目标/规划结构**；当前 workspace 与目录以 [README 实现状态](../../README.md) 与 01 骨架为准（如 chain-client 为可选未入仓、api 子目录随实现逐步补齐）。
...
│
├── frontend/                           # 前端（Next.js + React + TS + wagmi + viem + R3F + drei + Framer Motion），见 05/06/09 §2.6～2.7
│   ├── app/                            # 页面：/pay、/staking、/escrow/:id 等
│   ├── lib/                            # API 客户端、auth
│   ├── dapp/                           # 钱包、EIP-712、tx 状态机、watchContractEvent
│   └── components/                    # UI、3D（R3F/drei）、Framer Motion
│
...
│
└── docs/
    ├── README.md                       # 本目录说明（权威在 spec/，临时文档放本层）
    ├── spec/                           # 权威技术文档（00～27、08 系列等，建议先看 spec/00-文档索引.md）
    │   ├── 00-文档索引.md              # 文档索引（建议先看）
    │   ├── 01-总库总览.md              # 本文（总库总览）
    │   ├── 02～07、08-0～08-5、09～15、17～27、企业级审计报告等 .md
    ├── backend/                        # 后端文档导航与架构目录
    ├── frontend/                       # 前端文档导航与架构目录
    └── dapp/                           # DApp 文档导航与架构目录
```
