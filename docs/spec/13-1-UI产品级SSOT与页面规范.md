...
| **五主路由 ① UI 壳冻结（2026-05-25）** | **[FIVE-MAIN-ROUTES-PHASE1-FREEZE](../../frontend/evidence/GO_local_marketing_front_closure/FIVE-MAIN-ROUTES-PHASE1-FREEZE.md)**（**单一前端版本真源** · **`frontend/` 现码** · **非** `archive/ui-v1`）· **`/`** [`(home)/README`](../../frontend/app/(home)/README.md) · **`/market|did-rank|community` README** · **`/traveltrust`** [`modules/traveltrust-home/README`](../../frontend/modules/traveltrust-home/README.md) · **②③** 另闸 |
| **多重身份 · 旅行收购 PD-009（① · 非第五 `users.role`）** | **[`app/market/acquisition/README`](../../frontend/app/market/acquisition/README.md)** · **[`app/me/identities/README`](../../frontend/app/me/identities/README.md)** · **[acquisition-publish-trust-rules §8.1](artifacts/acquisition-publish-trust-rules.v1.md#81-第一阶段--本地--closed2026-05-27)** · **[04「旅行收购 · ① 实现真源」](04-后端与API.md)** · **烟测** **`bash scripts/dev/smoke-acquisition-pd009-local.sh`**（**非 ②③ GO**） |
...
|------|------|------------------|----------|----------|
| **Experience（情绪层）** | Landing | 品牌叙事 + 规划表单 + **1** 预览卡解锁、引导进入协议 | 不放交易按钮、不堆链上操作 | **`LandingHomeAmbientBackdrop`** + **`LandingHeroForm`** + **`ItineraryResultsSection`（`ITINERARY_CARD_COUNT=1`）** + **`UnlockModal`** + **`landingItinerarySession`**（**`localStorage`** · 跨 tab）+ **`LandingFooter`**；**①** **[FIVE-MAIN-ROUTES](../../frontend/evidence/GO_local_marketing_front_closure/FIVE-MAIN-ROUTES-PHASE1-FREEZE.md)** · **[`(home)/README`](../../frontend/app/(home)/README.md)** · **[LANDING-MARKET-PAGES-CODE-SSOT](../../frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** §2 · **[GO_local_web3_itinerary_l5](../../frontend/evidence/GO_local_web3_itinerary_l5/README.md)** |
| **Network（融资向）** | **TravelTrust 网络（/traveltrust 或 /network）** | 品牌/协议级 **网络叙事** + 合规融资转化入口（见 [85](85-TravelTrust网络落地页-融资级设计与开发规格.md)）；**`/network`** 重定向至 **`/traveltrust`**（与 [04 §3.4](04-后端与API.md) 前端路由表一致） | **不做**单笔订单 Escrow 主流程、**不做**完整 DApp 控制台堆叠、**不做**真实代币认购 UI | **① 冻结（2026-05-25）**：**`modules/traveltrust-home`** + **layout lock** `hero→roles→…`（**无** `#overview` 四卡）；**L1** **`TravelTrustHomeLandingNavSlot`**（portal · z-280）+ 双行 **`TravelTrustLandingChrome`** / **`TravelTrustPulseTicker` inline**（**CSS** 公告跑马灯）；**L5** **`TravelTrustStablecoinGateway`**；**未挂载** legacy 页内树（contract）；**[`app/traveltrust/README`](../../frontend/app/traveltrust/README.md)** · **[FIVE-MAIN-ROUTES](../../frontend/evidence/GO_local_marketing_front_closure/FIVE-MAIN-ROUTES-PHASE1-FREEZE.md)**；**②** **`GET /api/v1/traveltrust/page-brief`** |
...
| **Protocol Console（协议控制台）** | **TT社区（/community）** | **UGC 信息流**：**L1** 动态\|发现\|消息\|好友\|我；**L2** **`CommunitySupportMenu`**（反馈/帮助/规范）。**① API 主路径**（Feed/发帖 multipart/互动/社交/子页）；dev **showcase** 须披露。**`/community/me` redirect** → **`/me/settings/profile`**；子页 **`/community/me/*`**。**`/community/tt`→explore**；**`/community/activity`** 获赞汇总 only。**费路由**见 **`/help`** 等 | **不支付、不撮合** | **暖场 + 静态赛博**（**88**）；**①** **[`app/community/README`](../../frontend/app/community/README.md)** · **[COMMUNITY-PHASE1-FREEZE](../../frontend/evidence/GO_local_marketing_front_closure/COMMUNITY-PHASE1-FREEZE.md)** |
| **Protocol Console（协议控制台）** | **DID 排行榜（/did-rank）** | **竖脊五签**：旅行者 / 向导 / **行程** / 商家 / 旅行收购；**内页翻页**（**30 §1、§4.3**）；**`?board=itinerary|…`** + **`?period=`** / **`?guide_sort=`** / **`?me=`** 深链；**`rank_basis`**（[30](30-DID排行榜-页面规范.md)、[04-附录 did-rank](04-附录-did-rank对接说明.md) §2）；五端点 HTTP + **`prize-pool`**（失败 **不** mock 回退）；档案链 **`/community/user/[id]`** | **不做** Escrow 签名与单笔支付；**不做**社区 Feed | **暖场 + 静态赛博叠层**（**30 §4.1**）；**`framer-motion`** 脊签切换（**30 §6**）；**① 冻结** **[`app/did-rank/README`](../../frontend/app/did-rank/README.md)** · **[DID-RANK-PHASE1-FREEZE](../../frontend/evidence/GO_local_marketing_front_closure/DID-RANK-PHASE1-FREEZE.md)** |
| **Identity · 商家入驻（provider · ①）** | **`/auth/register?role=provider`** → **`/provider/register?step=1..3`** → **`/me/onboarding`** → **`/admin/provider-applications`** → **`/admin/users/[id]`**（审核）→ **`/market/provider`** | 开通 **Shop（API `provider`）** 五步：注册意向 → **KYB + 钱包** → **96-18 准入费** → **运营审核** → **发橱窗 listing** | **不做** 单笔 Escrow 主流程；**不在此路径冒充 ②③ 生产 PSP / staging GO** | **L5 Auth 壳**（**[`PROVIDER-REGISTER-UI-FREEZE`](../../frontend/evidence/GO_local_provider_register_closure/PROVIDER-REGISTER-UI-FREEZE.md)**）；**代码 SSOT** **[`app/provider/README`](../../frontend/app/provider/README.md)**；**烟测** **`bash scripts/dev/smoke-provider-onboarding-local.sh`** |
| **Identity · 旅行收购（PD-009 · ①）** | **`/me/identities`**（收购卡片 **「进入子站」**）→ **`/market/acquisition`** → 绑主钱包 → **`POST …/me/acquisition/publish-bond`**（或信用免押）→ **`POST …/market/acquisition/listings*`** | 开通 **收购发布能力**（**附加能力 · 非第五 `users.role`**）：**`acquisition_publish_gate.rs`**（**非** **`region_steward`** / **96-18 准入费**）→ 目录/草稿/创单（**`order_kind=acquisition_listing`**） | **不做** 商家 KYB 链；**不在此路径冒充 ②③ staging / Production GO** | **Hub UI 冻结** **[`ME-IDENTITIES-UI-FREEZE`](../../frontend/evidence/GO_local_auth_l5/ME-IDENTITIES-UI-FREEZE.md)**；**代码 SSOT** **[`app/market/acquisition/README`](../../frontend/app/market/acquisition/README.md)** · **[`app/me/identities/README`](../../frontend/app/me/identities/README.md)**；**烟测** **`bash scripts/dev/smoke-acquisition-pd009-local.sh`** |
...
| | EscrowDetail（**`/escrow/[id]`** · ① 已收口） | **双壳**（代码 **`EscrowDetail/index.tsx`**）：① **Experience**（`isPreEscrowProtocol && !hasEscrow`）— 创新行程草稿：3 步、发布/选向导、行程 Tab、确认终版；**UI 冻结** [`ESCROW-ORDER-PAGE-PHASE1-CLOSURE`](../../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md)。② **协议 DID**（已上链）— finality、存款/**release**/争议、聊天；**53+30-DID** 霓虹区。**企业级 UI/UX** 见 [53 §4.4](53-阶段开发技术文档.md)。 | Experience：**禁止** 旅行者默认 FeeRouter/聊天/cyan 8 步；协议区以 53+30-DID 为准 | 状态、金额、finality、操作区；草稿见 **80 §0.2.2** |
...
| **个人中心 → TT 社区·我** | `/community/me/posts` 等 | 「我的帖子/收藏」链独立子页；裸 **`/community/me` redirect** 资料编辑（**`me/README`**） |
| **TT 社区「我」**（/community/me） | **社区资料子页** | **裸 `/community/me` → redirect `/me/settings/profile`**；**`/community/me/posts|collects|likes|reports`** 独立 API 页 — [`me/README`](../../frontend/app/community/me/README.md) |
...
| **当前基线** | **71** 个 **`frontend/app/admin/**/page.tsx`**（枚举与验收见 **[`frontend/app/admin/README.md`](../../frontend/app/admin/README.md)** · **70 §3.0.2**）；**① Partial** 含 **`/admin`**（系统概况 **`GET …/metrics/home-overview`**）· **`/admin/permissions`** · **`/admin/onboarding/*`** · **`/admin/steward-applications`** · **`/admin/inbox`** · **`/admin/operator-guide`** · **`/admin/compliance`** · **`/admin/finance-suite`** · **`/admin/trust-growth`** · **`/admin/cross-check`** · **`/admin/drift-summary`** 及表 2-补充各行；L5：**危险写确认** · **41 列表 + 11 详情 SWR** · **M-01～M-03 smoke**（**非 ②③ GO**） |
...
| 页面/路由 | 最小能力 | 门禁 |
|-----------|----------|------|
| `/admin` | Admin 控制台首页（风险与待办聚合） | **Partial**：**`AdminHomeClient`** 壳头 **`Link`** → **`/admin/observability`**（**`admin_observability_title`**，与子页顶栏同口径）；**`app/admin/error.tsx`** 页级错误边界内联快链含 **`/admin/observability`**（与同路径 **hub/子页** 运维矩阵同口径）；**Retry** **`text-meta` **`admin_error_boundary_retry_hint`****+**`form aria-describedby`**+**`type=submit`**** **`reset()`**（**593**；**Home** 仍 **`Link`**）；域分组卡片含 **`/admin/indexer/reconcile-reports`**（对账报告列表，与 **`/admin/indexer`**/**可观测**/**审计** 等并列）；**`admin_home_desc_indexer`**/**`admin_home_desc_reconcile_reports`**（**locales**）互指 **report_type**/**chain_id** 与 **indexer health**/**列表**/**`reconcile/[id]`** 同键；**系统概况** **`AdminHomeSystemOverviewSection`**（**`GET …/metrics/home-overview`** · **`source` postgres/memory 诚实标签** · 聚焦待办 **`frame=compact`** 折叠 · 代码 SSOT **`frontend/app/admin/README.md`**）；仅 Admin |
...
| `/admin/guides` | 向导入驻台账 | **Partial**：只读 **`GET …/admin/guides`** + **`/admin/guides/[id]`**；详情 **`PATCH …/admin/guides/:id`**（**`status`** / **`rejection_*`** · **04** §3.5 · L5 confirm）；**列表筛选** 同 **593** 口径；顶栏 **`Link`** → **`/admin/observability`** |
...
| `/admin/permissions` | **ADM-U02** 权限中心（控制台角色 · 2FA · 路由矩阵） | **Partial（①）**：**`GET …/capabilities`** · **`GET …/rbac/route-matrix`** · **`GET/PATCH …/security/2fa-policy`** · TOTP 面板 · **`AdminPermissionsProductionSafetyPanel`**；角色变更 **`POST …/console-role-change-request`** → **`/admin/approvals`**；直写 **`PUT …/console-role`** 仅本地闸；**SSOT** **[ADM-U02 runbook](../../docs/runbook/ADM-U02-admin-permissions-2fa-approval.md)** · **[`frontend/app/admin/README.md`](../../frontend/app/admin/README.md)** |
| `/admin/onboarding` | **96-18 / 96-09** 入驻运维 hub | **Partial（①）**：链 **`/admin/onboarding/entitlements`**（含 **`[id]`**）· **`webhook-jobs`** · **`payment-events`** · **`compliance-audit`**；API **`GET|POST|PATCH …/admin/onboarding/*`**（**04** §3.5）；**②** 真 Stripe webhook 另闸 |
| `/admin/steward-applications` | 区域主理人申请队列 | **Partial（①）**：**`GET …/steward-applications`**；审核 **`PATCH …/users/:id/steward-application-review`** on **`/admin/users/[id]`** |
| `/admin/inbox` | 统一收件箱（待办深链） | **Partial（①）**：与首页 **`AdminHomeInboxStrip`** 同源计数；**`/admin?focus=inbox`** 聚焦布局 |
| `/admin/operator-guide` | 运营动线指南 | **Partial（①）**：静态 **`operator-guide/page.tsx`** · ① 主路径动线 |
| `/admin/compliance` | DSAR / 合规 hub | **Partial（①）**：链 **`/admin/compliance/requests*`**；**`GET|POST …/compliance/data-requests*`** |
| `/admin/finance-suite` | 财务七件套导航 hub | **Partial（①）**：**`finance-suite/page.tsx`** · ② 页内深度另闸 |
| `/admin/finance-reconciliation` | 财务对账摘要台 | **Partial（①）**：只读对账视图 · 与 **`/admin/finance`** / indexer 互链 |
| `/admin/trust-growth` | **P-OBS1** 信任增长运维 | **Partial（①）**：**`GET|PATCH|POST …/admin/trust-growth/*`** · **`trust-growth/page.tsx`** |
| `/admin/cross-check` | 跨域一致性探针 | **Partial（①）**：**`GET …/admin/cross-check`** |
| `/admin/drift-summary` | 投影/链漂移摘要 | **Partial（①）**：**`GET …/admin/drift-summary`** |
| `/admin/auth-audit-events` | 认证审计事件只读 | **Partial（①）**：**`GET …/admin/auth-audit-events`**（**`admin_audit_http.rs`** · 须 **PG**） |
...
**EscrowDetail 例外（A3）**：**已上链**订单在 **`/escrow/[id]`** 内**协议控制台区**（行程与预算、报价摘要、步骤条、聊天、操作区）以 **[53+30-DID §4](53-阶段开发技术文档.md)** 为准，采用赛博朋克风格（霓虹、光晕）；与表内「禁止霓虹」在该区以 53 为准。**预链上草稿 Experience 壳**（`experienceDraft`）**不适用 A3 霓虹默认** — 见 **[`ESCROW-DRAFT-EXPERIENCE-FREEZE`](../../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-DRAFT-EXPERIENCE-FREEZE.md)**。

**布局约束**：金融区必须使用**统一布局组件**（同一信息密度、同一间距/字体体系）；展示区（Landing / 自由市场 **Experience** 区；**`/discover` 已实现重定向 → `/market`**，**无**独立 Discover 列表 UI）可自由。避免「旅游情绪」混入「签名按钮区」。

**订单与 Escrow 路由（实现真值 · 2026-05-28 · ①）**：**现行主入口** **`/escrow/[id]`**（`app/escrow/[id]/page.tsx` → **`EscrowDetail`**；**04 §3.4** **`Implemented`**）。创新行程解锁后旅行者草稿订单以 **Experience 暖壳** 收口 — **[`ESCROW-ORDER-PAGE-PHASE1-CLOSURE`](../../frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md)** · **[`app/escrow/[id]/README`](../../frontend/app/escrow/[id]/README.md)**。`/orders` 列表与 **`/orders/new`** 等仍作订单索引/创建；**勿** 将下文 **53/N4 历史「单路由 `/orders/[id]`」** 理解为 **`/escrow/[id]` 已废弃**。

**订单与 Escrow 路由（53/N4 · 历史规划 · 对读用）**：原稿设想 **单路由** `/orders/[id]` 内嵌 Escrow 区块；**若与上段冲突，以上段代码为准**。**从 /market 进入订单详情**时：**全局顶栏** **`Header.tsx`** L0（市场 **Dark**；**`/escrow/*`** 等与 **86 §6.0** 对读）；**已上链**主内容区协议控制台 **53+30-DID**，见 [53 附录 D](53-阶段开发技术文档.md)。
