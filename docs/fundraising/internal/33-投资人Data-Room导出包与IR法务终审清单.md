...
| **终稿形态** | **唯一路演 Deck**：`04-PitchDeck-v{release}-*.pptx/.pdf`（**15** 页压缩主 Deck）；**演示视频**：`demo/TravelTrust-Product-Demo-v{release}.mp4`；**机构级 IR PDF 套装**（**编号**习惯 **01–08**，含 `04-IC-Memo` 等；**叙事默认读序**以 [external/00-START-HERE.md](../external/00-START-HERE.md) 与包内 **`DEFAULT READ ORDER`** 为准，**不等同**于按 01→08 文件名顺读）：`01-OnePager` / `02-Executive-Summary` / `03-FAQ` / `05-Litepaper` / `06-Whitepaper` / `07-Protocol-Tokenomics`（CN+EN，除索引）+ **`08-Data-Room-Index-v{release}.pdf`**；**勿**再附带从 storyboard Markdown 另导出的重复 Deck PDF。**Markdown 仅仓库内真源**；外发使用 **`--omit-markdown`**（根目录 **`00-START-HERE.txt`**）。 |
