...
0c. **每日 rhythm** — 1 Batch → Dashboard → Commit → **finalize DoD** · Owner 只看 **`TT_RELEASE_READINESS`** + **`TT_RELEASE_DECISION`**  
0d. **Batch DoD** — Gate + Evidence + Dashboard + Commit + clean tree → 才 PASS；否则 IN_PROGRESS · `finalize-fpc-batch-dod.cjs`  
