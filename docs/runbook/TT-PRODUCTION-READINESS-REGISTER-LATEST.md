...
> 修复路径必须 **Register → Fix → Local → Git SHA → Staging 同 SHA → Evidence → Recalculate**；**禁止**仅本地计入 Coverage PASS。| Final User Journey Acceptance | — | Release / Staging Validation | **不**进 WAIT_WINDOW（防 UX Finding 风暴） |
