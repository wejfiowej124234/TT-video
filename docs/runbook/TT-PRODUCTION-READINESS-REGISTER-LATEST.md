...
| Consistency Step4 · Staging smoke · `PSG-COV-STG-MIG-02` | tip `9528f593` Fly smoke 曾绿后 crash：`20250228000001` checksum modified | Windows worktree `autocrlf` → Docker 上下文 CRLF ≠ DB LF checksum；`.gitattributes eol=lf` + LF worktree 同 tip 重部署 | **OPEN** → 修复中（ΔFix=0 · 非 Web3） |
