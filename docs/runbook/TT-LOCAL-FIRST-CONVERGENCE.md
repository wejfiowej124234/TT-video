...
| 层 | 含义 | 当前锚点（示例） |
|----|------|------------------|
| **本地 HEAD** | 开发 + 配置 SSOT | `6e1fb5ac`（含 build.env governor 修复） |
...
5. **S5 deploy** — Owner 授权 + `TESTNET_FREEZE_OVERRIDE=1`；**仅**正式 Web/API deploy 时带入本地新 commit（如 `6e1fb5ac` build.env）
