...
**动 `frontend/app/provider/register*`、`provider-applications`、商家 market listing 门闸或对应 API 时（建议 · ①）**：**`bash scripts/dev/smoke-provider-onboarding-local.sh`**（须 **`DATABASE_URL`** · **`INTERNAL_API_SECRET`** · **`SEED_TEST_ACCOUNTS=1`**；**SSOT** **[provider/register README](frontend/app/provider/register/README.md)** · **[dev-local-smoke-baseline §8](docs/dev-local-smoke-baseline.md)** · **[TT-9618 §2.1](docs/runbook/TT-9618-onboarding-local-testnet.md)**；**非 ②③ GO**）。

**动 `frontend/app/market/acquisition*`、**`app/me/identities*`**、收购 **`acquisition_publish_gate`** / **`GET /me.trust` 收购字段** 或对应 API 时（建议 · ①）**：**`cd frontend && npx vitest run acquisitionL5 acquisitionL5FullScore meTrust meIdentitiesPage --run`** + **`bash scripts/dev/smoke-acquisition-pd009-local.sh`**（须 **`DATABASE_URL`** · **`SEED_TEST_ACCOUNTS=1`**；**SSOT** **[market/acquisition README](frontend/app/market/acquisition/README.md)** · **[me/identities README](frontend/app/me/identities/README.md)** · **[acquisition-publish-trust-rules §8.1](docs/spec/artifacts/acquisition-publish-trust-rules.v1.md#81-第一阶段--本地--closed2026-05-27)** · **[93 §2.1b](docs/spec/93-全站功能验证矩阵-域别回归清单.md#93-21b-acquisition-pd009)**；**非 ②③ GO**）。

**动 `frontend/app/admin/**`、**`components/admin/**`、Admin RBAC/2FA/系统概况 API 或 **`crates/api/src/routes/admin/**` 时（建议 · ①）**：**`bash scripts/dev/run-admin-l5-green.sh`**；全链 **`bash scripts/dev/verify-admin-audit-closure.sh`**（含 M-03）；RBAC/ADM-U02 另 **`bash scripts/dev/smoke-admin-rbac-matrix-local.sh`** · **`bash scripts/dev/smoke-admin-adm-u02-local.sh`**。**SSOT** **[frontend/app/admin/README.md](frontend/app/admin/README.md)** · **[dev-local-smoke-baseline §11](docs/dev-local-smoke-baseline.md)** · **[70 §3.0.2](docs/spec/70-管理员系统开发文档.md)**；**非 ②③ GO**。

**动治理域 / TTG Cert #1～12 / `docs/spec/governance-token` / HAT-R1 / Four-Ledger / Admin RBAC 对齐证据（建议 · ② · Sepolia 验收轨）**：**`bash scripts/dev/run-ttg-governance-cert-post-change-gate.sh`**（已完成 Cert 机读 smoke 回归 + registry 校验）；**新 Cert** 须同步 **`registry/ttg-governance-cert-gates.v1.yaml`** + **`python scripts/dev/validate-ttg-governance-cert-gates-registry.py`**；**active Cert finalize** 仍单独 **`run-tt-governance-cert-NN-*.sh --finalize`** — **[TTG-CERT-EXECUTION-SESSION-RUNBOOK](docs/runbook/TTG-CERT-EXECUTION-SESSION-RUNBOOK.md)** · **[handbook 05 §3](docs/handbook/engineering/05-本地环境与常用门禁速查.md)** · **[solo-dev-rhythm §6.5 #11](docs/solo-dev-rhythm.md)**；**≠ ① Phase① FULL MASTER** · **≠ ③ GO**。

**动 Web3 旅行首页 `/` 或自由市场主入口 `/market` 数据链时（建议 · ①）**：**`bash scripts/dev/run-web3-itinerary-l5-green.sh`** + 可选 **`bash scripts/dev/smoke-web3-itinerary-full-chain-local.sh`**（**SSOT** **[LANDING-MARKET-PAGES-CODE-SSOT](frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)** · **[dev-local-smoke-baseline §10](docs/dev-local-smoke-baseline.md)** · **[solo-dev-rhythm §6.5](docs/solo-dev-rhythm.md)** · **[AGENTS.md](AGENTS.md)**；**非 ②③ GO**）。

**动 `frontend/components/escrow/EscrowDetail*`、`EscrowDraft*`、`lib/escrowExperienceUi.ts`、`lib/escrowExperienceDevTools.ts` 或创新行程解锁后订单页 UI 时（建议 · ① · 已收口）**：**`bash scripts/dev/run-web3-itinerary-l5-green.sh`**（含 **`escrowDraftExperienceUiFreeze`**）；全链可选 **`bash scripts/dev/smoke-web3-itinerary-full-chain-local.sh`**。**SSOT** **[ESCROW-ORDER-PAGE-PHASE1-CLOSURE](frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-ORDER-PAGE-PHASE1-CLOSURE.md)** · **[ESCROW-DRAFT-EXPERIENCE-FREEZE](frontend/evidence/GO_local_web3_itinerary_l5/ESCROW-DRAFT-EXPERIENCE-FREEZE.md)** · **[app/escrow/[id]/README](frontend/app/escrow/[id]/README.md)**；**非 ②③ GO**。
...
**顶栏五主路由页身（文档-only 对齐 · 不改码时仍须遵守）**：**UI 壳** SSOT **[FIVE-MAIN-ROUTES-PHASE1-FREEZE.md](frontend/evidence/GO_local_marketing_front_closure/FIVE-MAIN-ROUTES-PHASE1-FREEZE.md)**（全仓**仅** `frontend/` 现行树；**禁止** `archive/ui-v1` 或并行文档版 UI）。**`/` + `/market` 四页数据链** SSOT **[LANDING-MARKET-PAGES-CODE-SSOT.md](frontend/evidence/GO_local_web3_pages_closure/LANDING-MARKET-PAGES-CODE-SSOT.md)**（**localStorage** · **300ms debounce** · 收藏 **`localStorage` + F-020 best-effort**（**`marketTravelBookmarksSync`**）→ **②** SLA）。互指 **[88 §一](docs/spec/88-五主路由页身实现快照与UX缺口审计-20260330.md)**、**[AGENTS.md](AGENTS.md)**、**[07 §5.3 2c](docs/spec/07-开发流程与顺序.md)**。
