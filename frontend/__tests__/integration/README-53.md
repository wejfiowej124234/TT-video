...
| 双边确认 | 旅行者/向导各自确认、双勾后步骤进入「确认·待付款」；重复提交幂等 | mock 双边确认 API、幂等键，断言双勾态、禁用态、重复请求不重复生效 | 53-S6、53-S9 |
...
- **53 主路径（Happy Path）**：旅行者创建行程 → 订单进入市场可见 → 向导抢单 → 聊天/订单消息沟通（可选简短）→ 向导确认接单 → 双边确认 → 旅行者付款（deposit）→ 完成 → 评分上传+双方确认 → 释放。
...
| 测试账号与权限 | 旅行者/向导测试账号（13-1 表 2 或 27 一致） | 27-P0 或 13-1、53-S11 验收 |
