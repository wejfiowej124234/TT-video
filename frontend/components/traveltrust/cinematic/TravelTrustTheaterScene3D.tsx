...
function TheaterPulse({ curve, offset }: { curve: THREE.Curve<THREE.Vector3>; offset: number }) {
  ...
  return (
    <mesh ref={ref}>
      ...
      <meshBasicMaterial color={TT_LEGACY_THEATER_3D_L5.node} toneMapped={false} />
    </mesh>
  );
}
...
function TheaterRing({ active }: { active: number }) {
  ...
  return (
    <group ref={group} rotation={[0.55, 0.3, 0]}>
      <Line points={points} color={TT_LEGACY_THEATER_3D_L5.line} transparent opacity={lineOpacity} lineWidth={1} />
      <Line
      ...
      <Line
        ...
        color={TT_LEGACY_THEATER_3D_L5.rim}
        transparent
...
function TheaterSceneContent({ active }: { active: number }) {
  return (
    <>
      ...
      <pointLight position={[2, 2, 4]} intensity={0.55 + active * 0.35} color={TT_LEGACY_THEATER_3D_L5.light} />
...
export function TravelTrustTheaterScene3D() {
  ...
  const wrapOpacity =
    TT_LEGACY_THEATER_3D_L5.wrapperOpacityMin + active * TT_LEGACY_THEATER_3D_L5.wrapperOpacityRange;
  ...
  return (
    <motion.div
      ...
      data-tt-traveltrust-theater-3d-opacity-ramp-l5="1"
      ...
      transition={
        active >= 0.95 && !reduceMotion
          ? {
              opacity: { duration: TT_LEGACY_THEATER_3D_L5.wrapperFadeDuration, ease: "easeOut" },
              scale: {
                duration: TT_LEGACY_THEATER_3D_L5.activeScalePulse.duration,
                repeat: Infinity,
                ease: "easeInOut",
              },
            }
          : { duration: TT_LEGACY_THEATER_3D_L5.wrapperFadeDuration, ease: "easeOut" }
      }
    >
      <Canvas
