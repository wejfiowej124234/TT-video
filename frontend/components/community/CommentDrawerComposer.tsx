...
export interface CommentDrawerComposerProps {
  t: (key: string) => string;
  isLoggedIn: boolean;
  authPending: boolean;
  sending: boolean;
...
export function CommentDrawerComposer({
  t,
  isLoggedIn,
  authPending,
  sending,
...
}: CommentDrawerComposerProps) {
  return (
    <div className={TT_COMMUNITY_DRAWER_L5.postDetailComposerBar}>
      ...
      {replyTarget ? (
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 rounded-[var(--radius-md)] border border-ref-sun/28 bg-ink-800/80 px-3 py-2">
      ...
      {isLoggedIn && !authPending && !sending && !input.trim() && !isShowcasePost ? (
        <ActionGateChecklist
