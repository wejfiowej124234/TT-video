...
export function CommunityMeOrderCancelConfirmDialog({
  open,
  busy,
  t,
  surface = "hub",
  ...
  return createPortal(
    <div
      ...
      data-tt-community-order-cancel-confirm="1"
      onMouseDown={(e) => {
    ...
    >
      ...
      >
        <h2 id={titleId} className="text-body font-semibold text-slate-100">
          {t("community_me_orders_cancel_title")}
        </h2>
        <p id={descId} className="mt-2 text-meta text-slate-300 leading-snug">
          {t("community_me_orders_cancel_confirm")}
        </p>
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <button type="button" className={DANGER_CONFIRM_BTN} onClick={() => void onConfirm()} disabled={busy}>
            {busy ? t("community_me_orders_cancel_menu_pending") : t("community_me_orders_cancel_action")}
          </button>
        </div>
      </div>
    </div>,
