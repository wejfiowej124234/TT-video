...
import type { useCommunityMeDrawerPostDetail } from "@/lib/useCommunityMeDrawerPostDetail";
import type { LocaleTranslateFn } from "@/lib/i18n";

type PostDetailSlice = ReturnType<typeof useCommunityMeDrawerPostDetail>;

/** Hub 帖子抽屉 · PostDetailDrawer portal（与独立页同源 · ①） */
export function CommunityMePostsExperiencePortals({
  t,
  isLoggedIn,
  authPending,
  meUser,
  ...
  isLoggedIn: boolean;
  authPending: boolean;
  meUser: { id: string } | null | undefined;
  postsForVideoNav: readonly CommunityPost[];
  likedIds: Set<string>;
  ...
  onPostVisibilityChange: (postId: string, next: CommunityPostUserVisibility) => void;
      ...
      {detailPost && (
        <PostDetailDrawerPortal
          ...
          onPostVisibilityChange={(next) => {
            onPostVisibilityChange(detailPost.id, next);
          }}
          postVisibilityBusy={visibilityBusyId === detailPost.id}
          {...detailVideoFeedDrawerProps(postsForVideoNav, detailPost, setDetailPost)}
        />
      )}
      {interactionToast ? (
        <div
