...
export function OrbitalArcs({ config }: { config: TravelTrustCinematic3dConfig }) {
  ...
  return (
    <group ref={groupRef}>
      ...
      {pulseCurves[1] ? <TravelPulse curve={pulseCurves[1]} speed={0.07} color="#6ee7b7" /> : null}
    </group>
  );
}
...
export function TravelGlobeNetwork({
  ...
  useFrame((state, delta) => {
    ...
    nodeRefs.current.forEach((mesh, i) => {
      if (!mesh) return;
      const pulse = 1 + Math.sin(t * 2.2 + i * 0.7) * 0.12;
...
export function TravelTrustCinematicScene3DContent({
  ...
  return (
    <>
      ...
      <pointLight position={[4, 3, 6]} intensity={1} color="#23ced9" />
      <pointLight position={[-5, -2, 4]} intensity={0.55} color="#fca47c" />
      <pointLight position={[0, -4, 2]} intensity={0.35} color="#6ee7b7" />
      <Stars
        radius={42}
        depth={28}
        ...
        <ParallaxRig strength={config.parallaxStrength}>
          <Float speed={1.05} rotationIntensity={0.14} floatIntensity={0.22}>
            <group position={[0, -0.12, 0]} rotation={[0.2, 0.42, 0]}>
              {enableGlow ? (
                <>
                  <GlobeBrandAtmosphere radius={config.globeRadius} />
                  <GlobeGlowHalo radius={config.globeRadius} intensity={1.15} />
                </>
              ) : null}
              <TravelGlobeNetwork config={config} />
            </group>
          </Float>
        </ParallaxRig>
      </ScrollSceneRig>
