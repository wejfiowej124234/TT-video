...
/** SSR/客户端同一 UTC 日基准，避免 hydration 因时区/跨日导致日期 input 不一致 */
function defaultLandingDateRangeUtc(): { start: string; end: string } {
  const now = new Date();
  const start = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 7));
  const end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 11));
...
export function useLandingPage(t: (key: string) => string) {
  ...
  const [startDate, setStartDate] = useState(() => defaultLandingDateRangeUtc().start);
  const [endDate, setEndDate] = useState(() => defaultLandingDateRangeUtc().end);
