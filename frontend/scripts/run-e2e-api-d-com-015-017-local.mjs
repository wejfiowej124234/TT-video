/**
 * 本地「直连 API」烟测：**F-015 / F-016 / F-017**（`f015-f016-f017-request.spec.ts`），只起 **traveltrust-api**。
 *
 * 用法：`npm run e2e:api-d-com-015-017-local`（cwd 为 **frontend**）
 */
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-d-com-015-017-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f015-f016-f017-request.spec.ts --project=api-d-com-015-017-chromium";
try {
