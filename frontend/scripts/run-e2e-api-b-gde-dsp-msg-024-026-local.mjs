/**
 * 本地「直连 API」烟测：**F-024 / F-025 / F-026**（`f024-f025-f026-request.spec.ts`），只起 **traveltrust-api**。
 *
 * 用法：`npm run e2e:api-b-gde-dsp-msg-024-026-local`（cwd 为 **frontend**；须 **`DATABASE_URL`** + **`P3_CHAIN_OFF=1`** 以跑 **mock-pay**）
 */
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-b-gde-dsp-msg-024-026-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f024-f025-f026-request.spec.ts --project=api-b-gde-dsp-msg-024-026-chromium";
try {
