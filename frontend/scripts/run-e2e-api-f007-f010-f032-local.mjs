/**
 * 本地「直连 API」烟测：**F-007 / F-010 / F-032**（`f007-f010-f032-request.spec.ts`）。
 *
 * 用法：`npm run e2e:api-f007-f010-f032-local`（cwd **frontend**；须 **`DATABASE_URL`**；**`P3_CHAIN_OFF=1`**；**`TRAVELTRUST_ALLOW_LOCAL_PROFILE_AVATAR=1`** 跑 **F-007** 本机头像路径）。
 */
...
if ((process.env.TRAVELTRUST_ALLOW_LOCAL_PROFILE_AVATAR ?? "").trim() === "") {
  process.env.TRAVELTRUST_ALLOW_LOCAL_PROFILE_AVATAR = "1";
}
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-f007-f010-f032-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f007-f010-f032-request.spec.ts --project=api-f007-f010-f032-chromium";
try {
