/**
 * 本地「直连 API」烟测：**F-021 / F-022 / F-023**（`f021-f022-f023-request.spec.ts`），只起 **traveltrust-api**。
 *
 * 用法：`npm run e2e:api-mkt-guides-021-023-local`（cwd 为 **frontend**；须 **`DATABASE_URL`** 已迁移）
 */
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-mkt-guides-021-023-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f021-f022-f023-request.spec.ts --project=api-mkt-guides-021-023-chromium";
try {
