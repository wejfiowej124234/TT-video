/**
 * 本地「直连 API」烟测：**F-027 / F-028 / F-033**（`f027-f028-f033-request.spec.ts`）。
 *
 * 用法：`npm run e2e:api-f027-f028-f033-local`（cwd **frontend**；须 **`DATABASE_URL`**；**`P3_CHAIN_OFF=1`** 跑 **mock-pay**；**`REQUIRE_IDEMPOTENCY_KEY=1`** 跑 **F-028** 幂等重放）。
 */
...
process.env.REQUIRE_IDEMPOTENCY_KEY = "1";
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-f027-f028-f033-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f027-f028-f033-request.spec.ts --project=api-f027-f028-f033-chromium";
try {
