/**
 * 本地「直连 API」烟测：**F-018 / F-019 / F-020**（`f018-f019-f020-request.spec.ts`），只起 **traveltrust-api**。
 *
 * 用法：`npm run e2e:api-d-com-018-020-local`（cwd 为 **frontend**）
 */
...
if (process.env.SKIP_API_BUILD !== "1") {
  try {
    ...
    console.log("[e2e:api-d-com-018-020-local] cargo build -p traveltrust-api");
    execSync("cargo build -p traveltrust-api", { cwd: repoRoot, stdio: "inherit", env: process.env });
  }
}

const cmd =
  "npx playwright test e2e/f018-f019-f020-request.spec.ts --project=api-d-com-018-020-chromium";
try {
