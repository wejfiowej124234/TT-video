...
/** /itinerary 路由 · 页面级错误边界（含 /itinerary/new）；与 52 行程、订单主路径衔接 */
export default function ItineraryError({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      console.error("Itinerary page error:", error?.message);
    }
  }, [error]);
  ...
  return (
    <main className="mx-auto flex min-h-[50vh] max-w-lg flex-col items-center justify-center px-6 py-12 bg-bg-main" role="alert">
      <div className="rounded-[var(--radius-md)] border border-ink-200 bg-bg-console p-6 shadow-soft">
        <p className="text-meta font-medium text-ink-500 mb-1">{t("itin_title")}</p>
        ...
        <p className="mt-5 text-meta text-ink-600 text-center flex flex-wrap justify-center gap-x-2 gap-y-1">
          <Link href="/itinerary/new" className={`${touchTargetLink44Classes} text-travel-600 hover:underline ${travelFocusRingOffset2Classes}`}>
            {t("itin_title")}
          </Link>
          <span aria-hidden>·</span>
          <Link href="/orders" className={`${touchTargetLink44Classes} text-travel-600 hover:underline ${travelFocusRingOffset2Classes}`}>
            {t("nav_orders")}
          </Link>
          <span aria-hidden>·</span>
