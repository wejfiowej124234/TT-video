...
export default function StewardRegisterPendingPanel({
  successFocusRef,
  t,
}: {
  ...
  return (
    <main
      ...
      aria-label={t("stewardRegister_pendingTitle")}
      {...stewardRegisterL5MainDataAttrs()}
      {...meSettingsExtensionIngressDataAttrs(fromSettings, "data-tt-steward-register-from-settings")}
    >
      <AuthL5PageBackdrop />
      <div className={`${TT_STEWARD_REGISTER_L5.pageColumn} flex flex-1 flex-col items-center justify-center gap-8`}>
        <MeSettingsExtensionIngressBlock
          fromSettings={fromSettings}
          noticeKey="me_settings_steward_register_from_settings_notice"
          t={t}
        />
        <div className={TT_STEWARD_REGISTER_L5.statusCardWrap}>
          <AuthL5Card>
            ...
            <div className={TT_AUTH_REGISTER_FLOW_L5.pendingStatusSection}>
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                <p className="text-h4 font-semibold text-slate-100" role="status" aria-live="polite">
                  {t("stewardRegister_pendingTitle")}
                </p>
                <span className={TT_STEWARD_REGISTER_L5.pendingStatusBadge}>
                  {t("stewardRegister_pendingReviewBadge")}
                </span>
              </div>
              <p className="text-small leading-relaxed text-slate-300/95">{t("stewardRegister_pendingDesc")}</p>
            </div>
            <div className={TT_AUTH_REGISTER_FLOW_L5.pendingHintStack}>
              <p>{t("stewardRegister_pendingOnboardingHint")}</p>
            </div>
            <div className={TT_AUTH_REGISTER_FLOW_L5.pendingActions}>
              <Link
                href={meOnboardingHref("region_steward", { from: "steward_pending" })}
        ...
        <AuthL5CrossNavFooter
          hideFeeRouterLinks
          className={`${TT_STEWARD_REGISTER_L5.footerLinks} ${TT_AUTH_REGISTER_FLOW_L5.pendingCrossNav}`}
        />
      </div>
    </main>
  );
