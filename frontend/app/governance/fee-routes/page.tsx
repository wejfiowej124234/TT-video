...
import { normalizeEvmAddr, rawFeeRouterFromMeta, shortHexAddr } from "@/lib/feeRouterWiring";
...
export default function GovernanceFeeRoutesPage() {
  ...
  /** B-063：分页「加载更多」失败单独条带，避免与首屏 error 混用且须 ApiErrorAlert + 重试/清错 */
  ...
  /** true = pass chain_id from /meta; false = all chains */
  ...
  useEffect(() => {
    fetchJsonWithApiStatusLog<MetaJson>("GovernanceFeeRoutesPage.meta", apiUrl(routes.meta))
      .then(({ res, body }) => {
        if (!res.ok) {
