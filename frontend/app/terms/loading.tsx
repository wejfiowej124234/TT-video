...
/** 与 terms 页 prose 法务文档壳一致 */
export default function TermsLoading() {
  const { t } = useTranslation();
  return (
    <main className="min-h-screen bg-bg-console py-12 px-4" role="status" aria-label={t("terms_title")} aria-busy="true">
      <div className="max-w-2xl mx-auto prose prose-ink" aria-hidden>
        <div className="min-h-[44px] h-11 w-56 bg-ink-200 rounded-[var(--radius-sm)] animate-pulse" />
        <div className="mt-2 h-4 w-full bg-ink-100 rounded-[var(--radius-sm)] animate-pulse" />
        <div className="min-h-[44px] h-11 w-44 bg-ink-200 rounded-[var(--radius-sm)] animate-pulse mt-6" />
        ...
        <div className="mt-8 h-3 w-64 bg-ink-100 rounded-[var(--radius-sm)] animate-pulse" />
