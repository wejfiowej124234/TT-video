    ...
    <main className="mx-auto max-w-6xl p-6 sm:p-8" aria-labelledby={pageTitleId}>
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-body text-ink-600">{t("admin_region_vault_subtitle")}</p>
        </div>
      ...
      {!loading && !error && appliedFilters ? (
        <AdminAppliedFiltersBanner
          ...
          className="mt-6"
        >
          {t("admin_region_vault_applied")}: {JSON.stringify(appliedFilters)}
        </AdminAppliedFiltersBanner>
      ) : null}
        ...
        >
          <table className="min-w-full border-collapse text-left text-small">
            <thead className="bg-ink-50 text-meta font-medium uppercase text-ink-600">
              <tr>
                <th className="px-3 py-2">{t("admin_region_vault_colChain")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colBlockLog")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colTx")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colVault")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colToken")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colTo")}</th>
                <th className="px-3 py-2">{t("admin_region_vault_colAmount")}</th>
              </tr>
            </thead>
      ...
      {!loading && !error && items.length === 0 && summary && (summary.total ?? 0) === 0 && (
        <section
          ...
          aria-label={t("admin_region_vault_events_table_aria")}
        >
          <p className="text-body text-ink-500">{t("admin_region_vault_empty")}</p>
        </section>
      )}
