...
import {
  ...
  ADMIN_CONSOLE_INNER_PANEL_CLASS,
} from "@/lib/adminUi";
...
const WIZARD_STEPS = [1, 2] as const;
...
export function AdminCommunityReportsModerationWizard(props: Props) {
  ...
  const stepLabel = (n: ModerationWizardStep) =>
    n === 1 ? t("admin_reports_wizard_step1") : t("admin_reports_wizard_step2");

  const showPenaltyStep = modStatus === "resolved";

  const goNext = () => {
    if (step === 1) {
      if (!applyStepErrors(validateAdminReportsWizardReviewStep(modExpectedVer, modNotes))) return;
      setFieldErrors({});
      if (showPenaltyStep) setStep(2);
    ...
    >
      ...
      >
        <AdminModalWarmL5Panel className="max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          >
            {WIZARD_STEPS.map((n) => {
              const hidden = n === 2 && !showPenaltyStep;
          ...
          <form className="mt-4 space-y-3 text-small" onSubmit={onSubmit}>
            {step === 1 ? (
              <>
                ...
                {modExpectedVer.trim() ? (
                  <p className={ADMIN_TEXT_META_CLASS} data-tt-admin-reports-wizard-version="1">
                    {t("admin_reports_wizard_version_meta").replace("{{v}}", modExpectedVer.trim())}
                  </p>
                ) : null}
