...
import { resolveOpsListDataSourceStrip } from "@/lib/admin/opsWorkbenchL5";
...
const metaLedgerLinkClass = `${touchTargetLink44Classes} !flex-col !items-stretch !justify-start p-3 text-left text-slate-200 ${ADMIN_HUB_DEPTH_LINK_CARD_CLASS} ${ADMIN_FORM_FIELD_FOCUS_CLASS}`;
...
export function AdminFinanceMetaLedgerSection({
  ...
  const sourceStrip = resolveOpsListDataSourceStrip(meta as Record<string, unknown>);

  return (
    <div className={`${ADMIN_CONSOLE_MUTED_PANEL_CLASS} p-4`}>
      <Link
        href="/admin/indexer/reconcile-reports"
        className={`${touchTargetLink44Classes} !flex !w-full !flex-col !items-stretch !justify-start -mx-1 -mt-1 rounded-[var(--radius-md)] px-1 pt-1 text-left text-slate-200 transition motion-reduce:transition-none hover:text-slate-100 ${ADMIN_FORM_FIELD_FOCUS_CLASS}`}
        aria-labelledby={financeMetaDlHeadingId}
      >
        <h2 id={financeMetaDlHeadingId} className="text-body font-medium text-slate-100">
          {t("admin_finance_meta_title")}
        </h2>
        <dl className="mt-2 grid gap-2 text-small text-slate-300 sm:grid-cols-2">
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_source")}</dt>
            <dd
              className="font-medium text-slate-200"
              data-tt-admin-finance-meta-source={sourceStrip.metaSource ?? undefined}
            >
              {t(sourceStrip.labelKey)}
            </dd>
          </div>
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_generatedAt")}</dt>
            <dd className="font-mono text-slate-200">{generatedAt ?? t("admin_em_dash")}</dd>
          </div>
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_dbOrderCount")}</dt>
            <dd className="font-mono text-slate-200">
              {meta.db_order_count == null ? t("admin_finance_meta_na") : String(meta.db_order_count)}
            </dd>
          </div>
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_dbEscrowCount")}</dt>
            <dd className="font-mono text-slate-200">
          ...
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_projectionReconcileReportCount")}</dt>
            <dd className="font-mono text-slate-200">
          ...
          <div>
            <dt className="text-meta text-slate-400">{t("admin_finance_meta_reconciliationReportsTotalCount")}</dt>
            <dd className="font-mono text-slate-200">
          ...
          <div>
            <dt className="text-meta text-slate-400">
      ...
      <div className={`mt-4 ${ADMIN_INNER_DIVIDER_CLASS} pt-4`}>
        <h3 className="text-small font-semibold text-slate-100">{t("admin_finance_ledger_db_title")}</h3>
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          <Link
            href="/admin/fee-router#admin-fee-router-events"
            className={metaLedgerLinkClass}
          >
            <p className="text-meta font-medium text-slate-400">{t("admin_finance_meta_feeRouterHeading")}</p>
            <dl className="mt-2 text-small text-slate-300">
              <div className="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-2">
                <dt className="text-meta text-slate-400 shrink-0">{t("admin_finance_meta_feeRouterEnvAddress")}</dt>
            ...
            {feeRouterStats ? (
              <dl className="mt-2 space-y-1 text-small text-slate-300">
              ...
              <p className="mt-2 text-small text-slate-400">{t("admin_finance_meta_na")}</p>
            )}
          </Link>
          <Link
            ...
            className={metaLedgerLinkClass}
          >
            <p className="text-meta font-medium text-slate-400">{t("admin_finance_meta_regionVaultHeading")}</p>
            <dl className="mt-2 text-small text-slate-300">
              <div className="flex flex-col gap-0.5 sm:flex-row sm:items-start sm:justify-between sm:gap-2">
                <dt className="text-meta text-slate-400 shrink-0">{t("admin_finance_meta_regionVaultEnvAddress")}</dt>
                <dd className="font-mono text-right break-all sm:text-left">
          ...
          >
            <p className="text-meta font-medium text-slate-400">{t("admin_finance_meta_projectionReconcileHeading")}</p>
            {lastProjReconcile ? (
              <div className="mt-2 space-y-1 text-small text-slate-300">
                {typeof lastProjReconcile.report_type === "string" && lastProjReconcile.report_type.trim() ? (
                  <p className="text-meta font-mono text-slate-300">
                ...
                {lastReportId ? (
                  <p className="pt-1 text-small font-medium text-slate-300">{t("admin_indexer_last_reconcile_open")}</p>
                ) : null}
              </div>
            ) : (
              <p className="mt-2 text-small text-slate-400">{t("admin_finance_meta_na")}</p>
      ...
      <p className="mt-3 text-meta text-slate-400">{t("admin_finance_meta_hint")}</p>
    </div>
  );
}
