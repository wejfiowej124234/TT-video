    ...
    <div className={`mt-5 ${ADMIN_FILTER_CARD_CLASS}`}>
      ...
      >
        ...
        <div className="mt-3 flex flex-wrap items-end gap-4">
          <label className={ADMIN_FILTER_FIELD_LABEL_CLASS} htmlFor={statusFilterId}>
            {t("admin_approvals_statusLabel")}
            <select
              id={statusFilterId}
              name="status"
              className={`mt-1 inline-flex w-full min-h-[44px] min-w-[10rem] items-center justify-start ${ADMIN_FILTER_INPUT_MD_CLASS} px-3 py-2 text-small ${ADMIN_FORM_FIELD_FOCUS_CLASS}`}
              value={draftStatus}
          ...
          <label className={`min-w-[12rem] flex-1 ${ADMIN_FILTER_FIELD_LABEL_CLASS}`}>
