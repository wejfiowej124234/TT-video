              ...
              {sortedItems.map((r, idx) => {
                ...
                return (
                  <tr key={r.id ?? `ts-${idx}`} className={ADMIN_TABLE_ROW_CLASS}>
                    <td className="px-3 py-2 font-mono text-small text-ink-800 max-w-[10rem] truncate" title={r.tenant_key}>
                      {r.tenant_key ?? dash}
                    </td>
                    <td className="px-3 py-2 font-mono text-small text-ink-800">{r.region_code ?? dash}</td>
                    <td className="px-3 py-2 font-mono text-small text-ink-800">{r.scope_class ?? dash}</td>
