...
import { useAdminContentSeoPage } from "./useAdminContentSeoPage";
...
export function AdminContentSeoPageMain() {
  ...
  const {
    ...
    submitReview,
    publish,
  } = useAdminContentSeoPage();
  const [editRows, setEditRows] = useState<Record<string, SeoEditState>>({});

  return (
    <AdminContentPageShell
      titleId={titleId}
      titleKey="admin_content_seo_title"
      subtitleKey="admin_content_seo_subtitle"
      loading={loading}
      error={error}
    >
      <AdminOpsRiskBanner messageKey="admin_ops_risk_banner_catalog_publish" />
      <AdminContentFormCard onSubmit={handleCreate} dataAttr="content-seo-create" className="md:grid-cols-3">
        ...
        <label className="flex flex-col gap-1 md:col-span-2">
          <span className={ADMIN_FILTER_FIELD_LABEL_CLASS}>{t("admin_content_seo_col_title")}</span>
          <input className={ADMIN_FILTER_INPUT_SM_CLASS} value={title} onChange={(e) => setTitle(e.target.value)} />
        </label>
        <label className="flex flex-col gap-1 md:col-span-3">
          <span className={ADMIN_FILTER_FIELD_LABEL_CLASS}>{t("admin_content_seo_col_description")}</span>
          <input
            className={ADMIN_FILTER_INPUT_SM_CLASS}
            value={description}
      ...
      <div data-tt-admin-content-seo-list="1">
        <AdminContentDataTable dataAttr="content-seo-list">
          <AdminContentTableHead>
            <tr>
              ...
              <AdminContentTableTh>{t("admin_content_seo_col_title")}</AdminContentTableTh>
              <AdminContentTableTh>{t("admin_content_seo_col_description")}</AdminContentTableTh>
          ...
          <AdminContentTableBody>
            {items.map((row) => {
              const edit = editRows[row.id] ?? rowEditState(row);
              return (
                <tr key={row.id}>
                  ...
                  <td className={ADMIN_TABLE_TD_CELL_CLASS}>
                    <input
                      className={`w-full max-w-xs ${ADMIN_FILTER_INPUT_SM_CLASS}`}
                      value={edit.title}
                      onChange={(e) =>
                        setEditRows((prev) => ({
                          ...prev,
                          [row.id]: { ...edit, title: e.target.value },
                        }))
                      }
                    />
                  </td>
