...
const RATE_NAV_FOCUS =
  "focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-900 rounded-[var(--radius-sm)]";
...
/** `orderId` 由服务端 `page.tsx` 校验为 UUID 后传入（与 `/escrow/[id]` 一致）。 */
export default function EscrowRatePageClient({ orderId }: { orderId: string }) {
  const id = orderId.trim();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const rateLoginReturnUrl = useMemo(
    () =>
      buildLoginReturnPathWithQuery(pathname, searchParams?.toString() ?? "", `/escrow/${encodeURIComponent(id)}/rate`),
  ...
  const [orderLoadAuthGate, setOrderLoadAuthGate] = useState<null | "login_required" | "forbidden">(null);
    ...
    (refreshOnly = false): Promise<void> => {
      ...
      return getOrder(id)
        .then((raw) => {
          ...
          const orderForMatch =
            rawRec.order != null && typeof rawRec.order === "object" && !Array.isArray(rawRec.order)
              ? rawRec.order
              : raw;
          if (!apiOrderSliceMatchesRoute(orderForMatch, id)) {
            if (refreshOnly) {
              setError(t("orderGet_payloadOrderMismatch"));
            } else {
              setOrder(null);
              setOrderResponseForPrefetch(null);
        ...
        .catch((err) => {
          ...
          const gate: null | "login_required" | "forbidden" =
            msg === "login_required" ? "login_required" : /403|forbidden|权限|暂无权限/i.test(msg) ? "forbidden" : null;
          if (refreshOnly) {
            setError(mapApiReadError(err, t, "escrow_loadFailed"));
          } else {
            setOrder(null);
            setOrderResponseForPrefetch(null);
            setOrderLoadAuthGate(gate);
  ...
  useEffect(() => {
    void loadOrder().catch(() => {
      /* `loadOrder` 内已 `console.error` + setOrderLoadError；此处仅避免未处理的 Promise 拒绝 */
    });
  }, [loadOrder]);
  ...
  const submitUpload = () => {
    ...
    void loadOrder(true)
      ...
      .catch((err) => {
        if (typeof window !== "undefined") {
          console.error("EscrowRatePage submitUpload loadOrder:", err);
        }
      })
      .finally(() => {
        setSubmitting(false);
      ...
      <main className="min-h-screen bg-bg-main text-ink-800" aria-label={t("rate_pageTitle")}>
        ...
        <div className="container py-8 md:py-12 max-w-lg">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {orderLoadAuthGate === "login_required" ? (
              <Link
                href={`/auth/login?returnUrl=${encodeURIComponent(rateLoginReturnUrl)}`}
                className="inline-flex min-h-[44px] items-center justify-center rounded-[var(--radius-sm)] border border-cyan-400/50 bg-cyan-500/20 px-4 py-2 text-small font-medium text-cyan-200 hover:bg-cyan-500/30"
              >
                {t("me_goLogin")}
              </Link>
            ) : null}
            ...
            <Link
              href="/orders"
              className="inline-flex items-center rounded-[var(--radius-sm)] border border-slate-500 px-4 py-2 text-small text-cyan-300 hover:text-cyan-100 hover:bg-ink-700/60"
            >
          ...
          <header className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 justify-end">
              <Link
                ...
                className="text-small font-medium text-cyan-300 hover:text-cyan-100 hover:drop-shadow-scifi-cyan-lg rounded-[var(--radius-sm)] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-900"
              >
                ← {t("escrow_backToOrders")}
              </Link>
              <Link
                ...
                onClick={stashEscrowMainPrefetch}
                className="text-small font-medium text-cyan-300 hover:text-cyan-100 hover:underline rounded-[var(--radius-sm)] focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-ink-900"
              >
                {t("orders_payHub")}
              </Link>
            </div>
          ...
          {order?.rating_deadline &&
            ...
            !Number.isNaN(Date.parse(order.rating_deadline)) && (
              <p className="text-small text-slate-300 mt-1" role="note">
                {t("order_ratingDeadlineHint", {
                  date: new Date(order.rating_deadline).toLocaleString(undefined, {
                    dateStyle: "short",
                    timeStyle: "short",
                  }),
                })}
            ...
            <section className={panelClass} aria-labelledby={rateUploadHeadingId}>
              ...
              >
                ...
                <p id={rateFileHintId} className="text-meta mt-2">
                  {files.length > 0 ? t("rate_filesSelected", { n: files.length }) : ""}
                </p>
              </div>
