...
/** /guide 路由 · 页面级错误边界（含向导申请）；与 staking、07 Phase 4 准入衔接 */
export default function GuideError({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      console.error("Guide page error:", error?.message);
    }
  }, [error]);
  ...
  return (
    <main className="mx-auto flex min-h-[50vh] max-w-lg flex-col items-center justify-center px-6 py-12 bg-bg-main" role="alert">
      <div className="rounded-[var(--radius-md)] border border-ink-200 bg-bg-console p-6 shadow-soft">
        <p className="text-meta font-medium text-ink-500 mb-1">{t("guideRegister_title")}</p>
