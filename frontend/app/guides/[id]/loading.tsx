...
const PANEL = "rounded-[var(--radius-md)] border border-cyan-500/30 bg-slate-900/70 backdrop-blur-md shadow-scifi-panel";
    ...
    <main className="relative min-h-screen" role="status" aria-label={t("guideDetail_title")} aria-busy="true">
      ...
      <div className="relative z-10 min-h-screen px-4 py-8 md:py-12">
        <div className="mx-auto max-w-2xl space-y-6">
          <div className="h-4 w-64 max-w-full bg-slate-600/40 rounded-[var(--radius-sm)] animate-pulse" aria-hidden />
          <section className={`${PANEL} p-6`} aria-hidden>
            <div className="flex flex-col sm:flex-row gap-4 items-start">
              <div className="w-24 h-24 shrink-0 rounded-full bg-slate-700/70 ring-2 ring-cyan-400/30 animate-pulse" />
              <div className="flex-1 space-y-3 min-w-0">
                <div className="min-h-[44px] h-11 w-48 max-w-full bg-slate-600/50 rounded-[var(--radius-sm)] animate-pulse" />
                <div className="h-4 w-56 max-w-full bg-slate-700/45 rounded-[var(--radius-sm)] animate-pulse" />
                <div className="flex flex-wrap gap-2">
                  <div className="min-h-[44px] h-11 w-28 rounded-full bg-success/15 border border-success/30 animate-pulse" />
                  <div className="min-h-[44px] h-11 w-40 rounded-[var(--radius-md)] bg-cyan-500/10 border border-cyan-500/35 animate-pulse" />
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="p-4 grid gap-4 sm:grid-cols-2">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="rounded-[var(--radius-md)] border border-slate-600/50 bg-slate-800/50 p-4 space-y-2">
                  <div className="h-4 w-24 bg-slate-600/45 rounded-[var(--radius-sm)] animate-pulse" />
