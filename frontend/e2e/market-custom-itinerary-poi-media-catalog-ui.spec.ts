/**
 * Custom Itinerary W5 · POI media preview cards (flag=0 · TS images)
 */
...
test.describe("Custom Itinerary W5 POI media (flag=0)", () => {
  test("attraction preview card visible after selecting 故宫", async ({ page }) => {
    ...
    const gugongPill = day1
      .getByRole("group", { name: /Attractions|景区/i })
      .getByRole("button", { name: "故宫" });
    await expect(gugongPill).toBeVisible({ timeout: 30_000 });
    await gugongPill.click();

    const preview = day1.getByRole("button", { name: /故宫|Forbidden/i });
