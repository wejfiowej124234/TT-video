...
import { getDidRankTravelers, getDidRankGuides, getDidRankItineraries } from ".";
...
describe("getDidRankTravelers", () => {
  ...
  it("accepts 200 P2 placeholder (empty travelers + note; did_rank no-db no-chain_off)", async () => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      mockTextResponse(true, {
        status: "ok",
        period: "week",
