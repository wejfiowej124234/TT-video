...
import { getOrderMessages, postOrderMessage } from ".";
...
describe("getOrderMessages", () => {
  ...
  it("rejects HTTP 503 chain_off_unavailable (align routes/messages/mod.rs)", async () => {
    (globalThis.fetch as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      mockTextResponse(false, { error: "chain_off_unavailable", message: "chain_off_unavailable" }, 503)
    );
    await expect(getOrderMessages("00000000-0000-4000-8000-000000000001")).rejects.toThrow(
...
describe("postOrderMessage", () => {
  ...
  it("rejects HTTP 503 chain_off_unavailable (align routes/messages/mod.rs)", async () => {
    ...
    await expect(postOrderMessage("00000000-0000-4000-8000-000000000001", { content: "hi" }, "k")).rejects.toThrow(
      "chain_off_unavailable"
    );
  });
});
