...
describe("ttgPublicUnlockScheduleLocal", () => {
  it("uses Owner geometric unlock inside public 50%", () => {
    ...
    expect(TTG_PUBLIC_UNLOCK_BATCHES[1].amountTtg).toBe(6_250_000_000);
    expect(TTG_PUBLIC_UNLOCK_BATCHES[2].amountTtg).toBe(31_250_000_000);
    expect(TTG_PUBLIC_UNLOCK_BATCHES[3].amountTtg).toBe(312_500_000_000);
    expect(TTG_PUBLIC_UNLOCK_BATCHES[4].amountTtg).toBe(625_000_000_000);
    expect(TTG_PUBLIC_UNLOCK_TOTAL_TTG).toBe(976_250_000_000);
    expect(TTG_PUBLIC_UNLOCK_TOTAL_PCT).toBeCloseTo(0.03905, 5);
  ...
  it("uses Owner batch sale prices and dates", () => {
    ...
    expect(TTG_PUBLIC_UNLOCK_BATCHES.map((b) => b.windowDays)).toEqual([7, 14, 21, 30, 45]);
  ...
  it("never date-drives open — Phase1 cutover pending (W-P0-05)", () => {
    ...
    expect(TTG_PUBLIC_UNLOCK_META).toEqual({
      ...
      saleSchedulePin: "V9_SHORT_WINDOW_FIVE_ROUND",
    });
