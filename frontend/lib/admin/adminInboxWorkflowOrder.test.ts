...
describe("adminInboxWorkflowOrder (①)", () => {
  it("fixes provider → guide → steward → approvals → disputes → reports", () => {
    ...
    expect(adminInboxWorkflowRank("provider")).toBeLessThan(adminInboxWorkflowRank("guide"));
    expect(adminInboxWorkflowRank("guide")).toBeLessThan(adminInboxWorkflowRank("steward"));
