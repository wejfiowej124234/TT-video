...
describe("admin inbox ↔ shell nav alignment (① · U2)", () => {
  ...
  it("AdminShellBar imports queue SSOT for onboarding + community reports", () => {
    const shell = readFileSync(join(fe, "components", "admin", "AdminShellBar.tsx"), "utf8");
    expect(shell).toContain("ADMIN_INBOX_QUEUE_HREFS.provider");
    expect(shell).toContain("ADMIN_INBOX_QUEUE_HREFS.steward");
    expect(shell).toContain("ADMIN_INBOX_QUEUE_HREFS.approvals");
    expect(shell).toContain("ADMIN_INBOX_QUEUE_HREFS.reports");
  });
  ...
  it("config publish + community related links use queue SSOT", () => {
    ...
    expect(readFileSync(join(fe, "components", "admin", "AdminCommunityRelatedLinks.tsx"), "utf8")).toContain(
  ...
  it("home inbox surfaces import ADMIN_INBOX_QUEUE_HREFS for queue deep links", () => {
    for (const rel of [
      ...
      "lib/admin/adminShellSidebarModel.ts",
    ]) {
