...
| ⑨ | 协议强制关闭（三种触发+关闭流程+存量终局+用户资金处理） | §6 | **三种触发**：① 监管勒令（政府/监管机构关停网络服务）② 多签不可恢复（关键多签成员均失联/丧失行为能力，≥12 个月无法恢复）③ 稳定币永久冻结（USDC 等稳定币发行方全球冻结，无法转移）；**关闭流程**： 1️⃣ 立即停止新单（前端下线、API /accept 返回 503）2️⃣ 通知用户 30 天内完成存量订单（邮件/网站公告）3️⃣ 存量订单按默认裁决（证据不足→game退款）或自动清算；**存量终局**：30 天内所有订单状态变更收尾，无卡住订单；**用户资金处理**：Escrow 按约定释放（旅行者取回/向导取回），资金无损失，链上可审计；**证据保全**：关闭前 30 天内生成审计报告与证据 snapshot 入 evidence；**披露**：公开宣布关闭原因与用户资金处理规则，接受查询（已按最佳实践填选） |
...
**O5 环境变量汇总（多环境）**：dev/staging/prod 关键变量与根目录 **`.env.example`** 一致；发版前核对 PORT、DATABASE_URL、CORS_ORIGINS、NEXT_PUBLIC_API_BASE_URL 与 04 §六附续 E2E/53 BB2 一致。根目录与 frontend 各有一份 .env.example；**生产必配**：DATABASE_URL、CORS_ORIGINS（见上表）。**链与 DApp 对齐（Phase 3/4）**：`CHAIN_RPC_URL`、`CHAIN_ID`、`ESCROW_FACTORY_ADDRESS`、`GUIDE_STAKING_ADDRESS`、`STAKING_PROVIDER_ADDRESS`、`REGISTRY_ADDRESS`（后端 ChainConfig / `GET /meta` → `chain.contracts`）；前端 `NEXT_PUBLIC_CHAIN_ID`、`NEXT_PUBLIC_GUIDE_STAKING_ADDRESS`、`NEXT_PUBLIC_STAKING_PROVIDER_ADDRESS`、`NEXT_PUBLIC_REGISTRY_ADDRESS` 须与部署一致。**限流与观测**：`GET /meta` 的 `rate_limits` 与 middleware 同源；运维快照另见 `GET /api/v1/admin/observability/overview`（需 admin 会话）。完整映射见 [08-3 附录 A](../docs/spec/08-3-参数与门禁表.md#附录-a运维与实现映射非-26-key-数值对齐代码与-runbook)。**CI**：`scripts/check-55-s13.sh` 校验 API 路由锚点及 **GuideIdentityStakingPool.json** / **ProviderIdentityStakingPool.json** / **Registry.json** 等在 `contracts/abi` 与 `frontend/dapp/abis` 字节一致（Build workflow 已挂载）。
...
**治理池 DB↔链余额对拍（母表 B-381 · TT-B381）与真实链路验证（母表 B-382 · TT-B382）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_governance_pool_db_vs_chain_balance_drift_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`governance_pool_db_vs_chain_balance_drift_observability`**（锚 **`381-GOVERNANCE-POOL-DB-VS-CHAIN-BALANCE-DRIFT-OBS-V1`**）；**`GET …/admin/observability/overview`** **`overview.governance_pool_db_vs_chain_balance_drift_observability`** 自最新 **`orders_projection_vs_orders`** **`summary`** 回读。**验证**：[`docs/runbook/TT-B382-GOVERNANCE-POOL-DRIFT-REAL-LINK-VERIFY-001.md`](../docs/runbook/TT-B382-GOVERNANCE-POOL-DRIFT-REAL-LINK-VERIFY-001.md)；一键：**`bash scripts/ops/b381-governance-pool-drift-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**FeeRouter `PlatformFeeRouted` 链上日志数 vs DB 投影（母表 B-383 · TT-B383）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_fee_router_platform_fee_routed_log_count_chain_vs_db_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`fee_router_platform_fee_routed_log_count_chain_vs_db_observability`**（锚 **`383-FEE-ROUTER-PLATFORM-FEE-ROUTED-LOG-COUNT-CHAIN-VS-DB-OBS-V1`**）；**`GET …/admin/observability/overview`** **`overview.fee_router_platform_fee_routed_log_count_chain_vs_db_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B383-FEE-ROUTER-PLATFORM-FEE-ROUTED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md`](../docs/runbook/TT-B383-FEE-ROUTER-PLATFORM-FEE-ROUTED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md)；一键：**`bash scripts/ops/b383-fee-router-platform-fee-routed-log-count-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**RegionVault `RegionVaultForwarded` 链上日志数 vs DB 投影（母表 B-384 · TT-B384）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_region_vault_forwarded_log_count_chain_vs_db_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`region_vault_forwarded_log_count_chain_vs_db_observability`**（锚 **`384-REGION-VAULT-FORWARDED-LOG-COUNT-CHAIN-VS-DB-OBS-V1`**）；**`GET …/admin/observability/overview`** **`overview.region_vault_forwarded_log_count_chain_vs_db_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B384-REGION-VAULT-FORWARDED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md`](../docs/runbook/TT-B384-REGION-VAULT-FORWARDED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md)；一键：**`bash scripts/ops/b384-region-vault-forwarded-log-count-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**CountryPoolLedger `CountryLedgerCredited` 链上日志数 vs DB 投影（母表 B-385 · TT-B385）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_p5_country_ledger_credited_log_count_chain_vs_db_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`p5_country_ledger_credited_log_count_chain_vs_db_observability`**（锚 **`385-P5-COUNTRY-LEDGER-CREDITED-LOG-COUNT-CHAIN-VS-DB-OBS-V1`**）；**`GET …/admin/observability/overview`** **`overview.p5_country_ledger_credited_log_count_chain_vs_db_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B385-P5-COUNTRY-LEDGER-CREDITED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md`](../docs/runbook/TT-B385-P5-COUNTRY-LEDGER-CREDITED-LOG-COUNT-CHAIN-VS-DB-VERIFY-001.md)；一键：**`bash scripts/ops/b385-p5-country-ledger-credited-log-count-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**经济投影三腿汇总 bundle（母表 B-386 · TT-B386）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_log_count_chain_vs_db_bundle_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_log_count_chain_vs_db_bundle_observability`**（锚 **`386-REVENUE-PIPELINE-LOG-COUNT-CHAIN-VS-DB-BUNDLE-OBS-V1`**；**汇总** **B-383/B-384/B-385**，**`rollup.marker`** worst-of）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_log_count_chain_vs_db_bundle_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B386-REVENUE-PIPELINE-LOG-COUNT-BUNDLE-CHAIN-VS-DB-VERIFY-001.md`](../docs/runbook/TT-B386-REVENUE-PIPELINE-LOG-COUNT-BUNDLE-CHAIN-VS-DB-VERIFY-001.md)；一键：**`bash scripts/ops/b386-revenue-pipeline-log-count-bundle-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Revenue pipeline bundle 跨 persist `rollup.marker` 对拍（母表 B-387 · TT-B387）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_bundle_cross_snapshot_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_bundle_cross_snapshot_drift_observability`**（锚 **`387-REVENUE-PIPELINE-BUNDLE-CROSS-SNAPSHOT-DRIFT-OBS-V1`**；**隐式** **B-386** **bundle**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_bundle_cross_snapshot_drift_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B387-REVENUE-PIPELINE-BUNDLE-CROSS-SNAPSHOT-DRIFT-VERIFY-001.md`](../docs/runbook/TT-B387-REVENUE-PIPELINE-BUNDLE-CROSS-SNAPSHOT-DRIFT-VERIFY-001.md)；一键（**两步 POST**）：**`bash scripts/ops/b387-revenue-pipeline-bundle-cross-snapshot-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Cross-snapshot 历史趋势 / 连续漂移（母表 B-388 · TT-B388）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_cross_snapshot_history_trend_observability:true`** 时（**须** **B-387**；**隐式** **B-386** **bundle** **+** **B-387** **cross**），**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_cross_snapshot_history_trend_observability`**（锚 **`388-REVENUE-PIPELINE-CROSS-SNAPSHOT-HISTORY-TREND-OBS-V1`**；**`series_newest_first`** **/** **`consecutive_drift_streak`** **/** **`last_flip`** 等）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_cross_snapshot_history_trend_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B388-REVENUE-PIPELINE-CROSS-SNAPSHOT-HISTORY-TREND-OBS-001.md`](../docs/runbook/TT-B388-REVENUE-PIPELINE-CROSS-SNAPSHOT-HISTORY-TREND-OBS-001.md)；一键（**三步 POST**：先 **B-386** **bundle** **`persist`**，再 **B-387** **`cross`**，再 **B-388** **`history`**）：**`bash scripts/ops/b388-revenue-pipeline-cross-snapshot-history-trend-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Revenue pipeline 最近 B-386 bundle 落库新鲜度（母表 B-389 · TT-B389）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_latest_persist_freshness_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_latest_persist_freshness_observability`**（锚 **`389-REVENUE-PIPELINE-LATEST-PERSIST-FRESHNESS-OBS-V1`**；**insert 前** **本链** **最新** **带** **B-386** **bundle** **键** **之** **`reconciliation_reports.created_at`****→****`age_seconds`**；**无** **则** **`observation_note=no_stored_revenue_pipeline_bundle_report`**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_latest_persist_freshness_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B389-REVENUE-PIPELINE-LATEST-PERSIST-FRESHNESS-OBS-001.md`](../docs/runbook/TT-B389-REVENUE-PIPELINE-LATEST-PERSIST-FRESHNESS-OBS-001.md)；一键：**`bash scripts/ops/b389-revenue-pipeline-latest-persist-freshness-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Freshness → drift 关联 suspect（母表 B-390 · TT-B390）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_freshness_drift_suspect_observability:true`** 时（**隐式** **与** **B-389** **同源** **`latest_revenue_pipeline_bundle_report_meta_for_chain`** **一次** **查询**；**须** **同** **请求** **含** **`include_revenue_pipeline_log_count_chain_vs_db_bundle_observability:true`** **以** **填充** **`suspect_due_to_freshness`**），**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_freshness_drift_suspect_observability`**（锚 **`390-REVENUE-PIPELINE-FRESHNESS-DRIFT-SUSPECT-OBS-V1`**；**`freshness_abnormal_reason`** **/** **`suspect_due_to_freshness`**；**`TRAVELTRUST_REVENUE_PIPELINE_FRESHNESS_STALE_SUSPECT_SECS`** **默认** **86400**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_freshness_drift_suspect_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B390-REVENUE-PIPELINE-FRESHNESS-DRIFT-SUSPECT-OBS-001.md`](../docs/runbook/TT-B390-REVENUE-PIPELINE-FRESHNESS-DRIFT-SUSPECT-OBS-001.md)；一键：**`bash scripts/ops/b390-revenue-pipeline-freshness-drift-suspect-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Union max 投影块 vs indexer checkpoint（母表 B-391 · TT-B391）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_union_max_projection_block_vs_indexer_checkpoint_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_union_max_projection_block_vs_indexer_checkpoint_observability`**（锚 **`391-REVENUE-PIPELINE-UNION-MAX-PROJECTION-BLOCK-VS-INDEXER-CHECKPOINT-OBS-V1`**；**`fee_router_routed_events`****/** **`region_vault_forwarded_events`****/** **`p5_country_ledger_lines`** **union** **`max(block_number)`** **vs** **进程** **checkpoint**；**`gap_blocks`**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_union_max_projection_block_vs_indexer_checkpoint_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B391-REVENUE-PIPELINE-UNION-MAX-PROJECTION-BLOCK-VS-INDEXER-CHECKPOINT-OBS-001.md`](../docs/runbook/TT-B391-REVENUE-PIPELINE-UNION-MAX-PROJECTION-BLOCK-VS-INDEXER-CHECKPOINT-OBS-001.md)；一键：**`bash scripts/ops/b391-revenue-pipeline-union-max-projection-block-vs-indexer-checkpoint-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Union max vs `event_log` 尾块覆盖（母表 B-392 · TT-B392）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_union_max_vs_event_log_max_block_tail_coverage_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_union_max_vs_event_log_max_block_tail_coverage_observability`**（锚 **`392-REVENUE-PIPELINE-UNION-MAX-VS-EVENT-LOG-MAX-BLOCK-TAIL-COVERAGE-OBS-V1`**；**三** **投影表** **union max** **vs** **`event_log`** **`MAX(block_number)`**；**`tail_slack_blocks`**；**负** **→** **`marker=drift`**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_union_max_vs_event_log_max_block_tail_coverage_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B392-REVENUE-PIPELINE-UNION-MAX-VS-EVENT-LOG-MAX-BLOCK-TAIL-COVERAGE-OBS-001.md`](../docs/runbook/TT-B392-REVENUE-PIPELINE-UNION-MAX-VS-EVENT-LOG-MAX-BLOCK-TAIL-COVERAGE-OBS-001.md)；一键：**`bash scripts/ops/b392-revenue-pipeline-union-max-vs-event-log-max-block-tail-coverage-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**B-391 + B-392 rollup bundle（母表 B-393 · TT-B393）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_union_max_checkpoint_vs_event_log_tail_bundle_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_union_max_checkpoint_vs_event_log_tail_bundle_observability`**（锚 **`393-REVENUE-PIPELINE-UNION-MAX-CHECKPOINT-VS-EVENT-LOG-TAIL-BUNDLE-OBS-V1`**；**`components`** **内** **嵌** **B-391/B-392** **子** **JSON**；**`rollup.marker`** **worst-of**）；**隐式** **计算** **两** **子** **腿**。**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_union_max_checkpoint_vs_event_log_tail_bundle_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B393-REVENUE-PIPELINE-UNION-MAX-CHECKPOINT-VS-EVENT-LOG-TAIL-BUNDLE-OBS-001.md`](../docs/runbook/TT-B393-REVENUE-PIPELINE-UNION-MAX-CHECKPOINT-VS-EVENT-LOG-TAIL-BUNDLE-OBS-001.md)；一键：**`bash scripts/ops/b393-revenue-pipeline-union-max-checkpoint-vs-event-log-tail-bundle-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Per-leg projection max block spread（母表 B-394 · TT-B394）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_per_leg_projection_max_block_spread_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_per_leg_projection_max_block_spread_observability`**（锚 **`394-REVENUE-PIPELINE-PER-LEG-PROJECTION-MAX-BLOCK-SPREAD-OBS-V1`**；**三腿** **`max(block_number)`** **之** **`spread_blocks`**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_per_leg_projection_max_block_spread_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B394-REVENUE-PIPELINE-PER-LEG-PROJECTION-MAX-BLOCK-SPREAD-OBS-001.md`](../docs/runbook/TT-B394-REVENUE-PIPELINE-PER-LEG-PROJECTION-MAX-BLOCK-SPREAD-OBS-001.md)；一键：**`bash scripts/ops/b394-revenue-pipeline-per-leg-projection-max-block-spread-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread stratified（母表 B-395 · TT-B395）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_stratified_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_stratified_observability`**（锚 **`395-REVENUE-PIPELINE-SPREAD-STRATIFIED-OBS-V1`**；**`spread_anomaly_layer`** **/** **`inter_leg_drift`**；**B-394** **同源** **`spread_blocks`**）；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_stratified_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B395-REVENUE-PIPELINE-SPREAD-STRATIFIED-OBS-001.md`](../docs/runbook/TT-B395-REVENUE-PIPELINE-SPREAD-STRATIFIED-OBS-001.md)；一键：**`bash scripts/ops/b395-revenue-pipeline-spread-stratified-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread vs union–indexer gap（母表 B-396 · TT-B396）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_vs_union_indexer_gap_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_vs_union_indexer_gap_observability`**（锚 **`396-REVENUE-PIPELINE-SPREAD-VS-UNION-INDEXER-GAP-OBS-V1`**；**`dominance_signal`** **/** **`gap_blocks`** **（** **B-391** **同源** **）** **/** **`spread_blocks`** **（** **B-394** **同源** **）** **/** **`inter_leg_drift`** **（** **B-395** **同源** **）** **；** **不** **含** **`spread_anomaly_layer`** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_vs_union_indexer_gap_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B396-REVENUE-PIPELINE-SPREAD-VS-UNION-INDEXER-GAP-OBS-001.md`](../docs/runbook/TT-B396-REVENUE-PIPELINE-SPREAD-VS-UNION-INDEXER-GAP-OBS-001.md)；一键：**`bash scripts/ops/b396-revenue-pipeline-spread-vs-union-indexer-gap-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread vs event_log tail slack（母表 B-397 · TT-B397）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_vs_event_log_tail_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_vs_event_log_tail_observability`**（锚 **`397-REVENUE-PIPELINE-SPREAD-VS-EVENT-LOG-TAIL-OBS-V1`**；**`dominance_signal`** **/** **`tail_slack_blocks`** **（** **B-392** **同源** **`event_log_max−union_max`** **）** **/** **`spread_blocks`** **（** **B-394** **）** **/** **`inter_leg_drift`** **（** **B-395** **）** **；** **不** **含** **`spread_anomaly_layer`** **；** **与** **B-396** **`gap_blocks`** **正交** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_vs_event_log_tail_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B397-REVENUE-PIPELINE-SPREAD-VS-EVENT-LOG-TAIL-OBS-001.md`](../docs/runbook/TT-B397-REVENUE-PIPELINE-SPREAD-VS-EVENT-LOG-TAIL-OBS-001.md)；一键：**`bash scripts/ops/b397-revenue-pipeline-spread-vs-event-log-tail-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread dual-slack triangulation（母表 B-398 · TT-B398）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_dual_slack_triangulation_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_dual_slack_triangulation_observability`**（锚 **`398-REVENUE-PIPELINE-SPREAD-DUAL-SLACK-TRIANGULATION-OBS-V1`**；**`triangulation_signal`** **/** **`gap_blocks`** **（** **B-391** **）** **/** **`tail_slack_blocks`** **（** **B-392** **）** **/** **`spread_blocks`** **（** **B-394** **）** **/** **`inter_leg_drift`** **（** **B-395** **）** **；** **不** **含** **`dominance_signal`** **、** **`spread_anomaly_layer`** **；** **不** **替代** **B-396/B-397** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_dual_slack_triangulation_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B398-REVENUE-PIPELINE-SPREAD-DUAL-SLACK-TRIANGULATION-OBS-001.md`](../docs/runbook/TT-B398-REVENUE-PIPELINE-SPREAD-DUAL-SLACK-TRIANGULATION-OBS-001.md)；一键：**`bash scripts/ops/b398-revenue-pipeline-spread-dual-slack-triangulation-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread dual-axis dominance concordance（母表 B-399 · TT-B399）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_dual_axis_dominance_concordance_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_dual_axis_dominance_concordance_observability`**（锚 **`399-REVENUE-PIPELINE-SPREAD-DUAL-AXIS-DOMINANCE-CONCORDANCE-OBS-V1`**；**`concordance_signal`** **rollup** **B-396** **`dominance_signal`** **与** **B-397** **`dominance_signal`** **；** **不** **含** **`triangulation_signal`** **（** **B-398** **）** **、** **`spread_anomaly_layer`** **（** **B-395** **）** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_dual_axis_dominance_concordance_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B399-REVENUE-PIPELINE-SPREAD-DUAL-AXIS-DOMINANCE-CONCORDANCE-OBS-001.md`](../docs/runbook/TT-B399-REVENUE-PIPELINE-SPREAD-DUAL-AXIS-DOMINANCE-CONCORDANCE-OBS-001.md)；一键：**`bash scripts/ops/b399-revenue-pipeline-spread-dual-axis-dominance-concordance-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Spread triangulation–concordance bundle（母表 B-400 · TT-B400）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_triangulation_concordance_bundle_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_triangulation_concordance_bundle_observability`**（锚 **`400-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-OBS-V1`**；**`components`** **内嵌** **B-398** **与** **B-399** **JSON** **；** **`rollup.marker`** **worst-of** **子** **`marker`** **；** **隐式** **计算** **两** **子** **键** **；** **不** **重算** **B-394～B-397** **腿面** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_triangulation_concordance_bundle_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B400-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-OBS-001.md`](../docs/runbook/TT-B400-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-OBS-001.md)；一键：**`bash scripts/ops/b400-revenue-pipeline-spread-triangulation-concordance-bundle-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**B-400 bundle × B-389 freshness suspect（母表 B-401 · TT-B401）**：**`POST …/internal/indexer-reconcile`** 在 **`persist:true`** 且 **`include_revenue_pipeline_spread_triangulation_concordance_bundle_freshness_suspect_observability:true`** 时，**`200`** 与报告 **`summary`** 含 **`revenue_pipeline_spread_triangulation_concordance_bundle_freshness_suspect_observability`**（锚 **`401-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-FRESHNESS-SUSPECT-OBS-V1`**；**B-389** **persist** **年龄** **×** **B-400** **`marker`** **`suspect`**；**`TRAVELTRUST_REVENUE_PIPELINE_FRESHNESS_STALE_SUSPECT_SECS`** **与** **B-390** **同源**；**隐式** **B-389** **base** **+** **B-400** **bundle** **）**；**`GET …/admin/observability/overview`** **`overview.revenue_pipeline_spread_triangulation_concordance_bundle_freshness_suspect_observability`** 自最新 **`summary`** 回读。**验证**：[`docs/runbook/TT-B401-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-FRESHNESS-SUSPECT-OBS-001.md`](../docs/runbook/TT-B401-REVENUE-PIPELINE-SPREAD-TRIANGULATION-CONCORDANCE-BUNDLE-FRESHNESS-SUSPECT-OBS-001.md)；一键：**`bash scripts/ops/b401-revenue-pipeline-spread-triangulation-concordance-bundle-freshness-suspect-reconcile-admin-overview-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**Revenue suspect 值班聚合（母表 B-426 · TT-B426 · 锚 `B-426-REVENUE-SUSPECT-TRIAGE`）**：**若** **`overview.revenue_pipeline_freshness_drift_suspect_observability`** **或** **`overview.revenue_pipeline_spread_triangulation_concordance_bundle_freshness_suspect_observability`** **出现** **`suspect_due_to_freshness`** **或** **`rollup.marker=suspect`**（**语义** **见** **B-390** **/** **B-401** **）** **→** **先** **核对** **`revenue_pipeline_latest_persist_freshness_observability`**（**B-389**）**与** **Indexer** **checkpoint** **（** **`GET /meta.indexer`** **、** **上表** **indexer** **行** **）** **；** **再** **按** **`revenue_pipeline_spread_dual_slack_triangulation_observability.triangulation_signal`**（**B-398**）**与** **`revenue_pipeline_spread_triangulation_concordance_bundle_observability`**（**B-400**）**components** **定位** **spread** **腿** **。** **一键** **GO** **门禁**（**Runbook** **条文** **+** **overview** **四键** **齐备** **）** **：** **`bash scripts/check-revenue-suspect-runbook-gate.sh`** **（** **`--json`** **`traveltrust.revenue_suspect_runbook_gate.v1`** **）** **；** **正式** **TT** **`TT-B426-GO-OBS-REVENUE-SUSPECT-RUNBOOK-001`** **。**

**订单主路径 E2E 发版语义门禁（母表 B-427 · TT-B427 · 锚 `B-427-ORDER-E2E-01353-GATE`）**：**一键** **`bash scripts/check-order-e2e-01353-gate.sh`** **（** **`--json`** **`traveltrust.order_e2e_01353_gate.v1`** **）** **串联** **`scripts/ops/b409-order-state-primary-acceptance.sh`** **与** **`scripts/ops/b409-order-state-exception-acceptance.sh`** **（** **TT-B409** **主成功链** **+** **`created→cancelled`** **异常链** **；** **叙事** **与** **[`TT-B409`](../docs/runbook/TT-B409-ORDER-STATE-MACHINE-CHAIN-OFF-53-001.md)** **/** **[`TT-B410`](../docs/runbook/TT-B410-USER-FLOW-E2E-ORDER-STATE-UNIFIED-001.md)** **`b410`** **主路径** **`b409`** **一致** **，** **默认** **不** **跑** **Playwright** **）** **；** **正式** **`TT-B427-GO-BIZ-ORDER-E2E-01353-001`** **。** **离线仅条文**：**`B427_SKIP_CARGO=1`** **。**

**GO live 文档互指核对（母表 B-421 · TT-B421 · 锚 `B-421-RUNBOOK-GOLIVE-DOCLINK-GATE`）**：**[`docs/go-live-checklist.md`](../docs/go-live-checklist.md)** ↔ **[`docs/spec/00-文档索引.md`](../docs/spec/00-文档索引.md)** ↔ **[`docs/spec/缺口与待补-官方总表.md`](../docs/spec/缺口与待补-官方总表.md)** **§P0** ↔ **[`15` 附录〇](../docs/spec/15-多维度文档与技术检查报告.md#发版前勾选总表)** ↔ **[`TT-B421`](../docs/runbook/TT-B421-GO-RUNBOOK-GOLIVE-DOCLINK-001.md)** **；** **一键机读** **`bash scripts/check-runbook-golive-doclink-gate.sh`** **`--json`** **`traveltrust.runbook_golive_doclink_gate.v1`** **；** **正式** **`TT-B421-GO-RUNBOOK-GOLIVE-DOCLINK-001`** **。**

**最小 revenue E2E 组合烟测（母表 B-402 · TT-B402）**：**不** **新增** **观测** **键** **；** **单** **请求** **同时** **打开** **B-383**（**`include_fee_router_platform_fee_routed_log_count_chain_vs_db_observability:true`**）与 **B-386**（**`include_revenue_pipeline_log_count_chain_vs_db_bundle_observability:true`**），**`persist:true`**，**校验** **reconcile** **响应** **根级** **`fee_router_platform_fee_routed_log_count_chain_vs_db_observability`** **/** **`revenue_pipeline_log_count_chain_vs_db_bundle_observability`** **锚** **与** **`GET …/admin/observability/overview`** **同** **两** **键** **深相等** **。** **业务** **闭环** **语义** **（** **53** **→** **Escrow** **平台费** **→** **FeeRouter** **`distribute`** **→** **`PlatformFeeRouted`** **→** **`fee_router_routed_events`** **）** **见** **[`docs/runbook/TT-B402-MIN-REVENUE-E2E-DATA-BUSINESS-CLOSE-LOOP-001.md`](../docs/runbook/TT-B402-MIN-REVENUE-E2E-DATA-BUSINESS-CLOSE-LOOP-001.md)** **§1～3** **；** **链上** **尚** **无** **投影** **时** **B-386** **`rollup`** **可能** **drift/unavailable** **（** **属** **预期** **）** **。** **一键**：**`bash scripts/ops/b402-min-revenue-e2e-reconcile-smoke.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**）。

**订单 → 收入观测可查路径（母表 B-412 · TT-B412）**：**不** **新增** **观测** **键** **；** **先** **`GET /api/v1/admin/orders/:id`**（**`B412_ORDER_ID`** **；** **须** **chain_off** **+** **admin**）**锚定** **订单** **与** **可选** **`chain_id`** **，** **再** **复用** **上款** **B-402** **`b402-min-revenue-e2e-reconcile-smoke.sh`** **（** **B-383+B-386** **）** **。** **一键**：**`bash scripts/ops/b412-order-to-revenue-discoverability-smoke.sh`** **（** **须** **`B412_ORDER_ID`** **；** **或** **`B412_SKIP_ORDER=1`** **仅** **跑** **b402** **）** **。** **路径** **/** **断言** **：** **[`docs/runbook/TT-B412-REVENUE-DATA-DISCOVERABILITY-OPS-001.md`](../docs/runbook/TT-B412-REVENUE-DATA-DISCOVERABILITY-OPS-001.md)**。

**Revenue Go-Live 联调收口（母表 B-414 · TT-B414）**：**唯一** **把** **B-402** **+** **B-412** **+** **B-413** **钉成** **一次** **可复跑** **业务** **闭环** **证据** **：** **`bash scripts/ops/b414-revenue-e2e-go-live-closeout.sh`** **（** **订单** **锚** **`B414_ORDER_ID`/`B412_ORDER_ID`** **；** **或** **`B414_SKIP_ORDER=1`** **；** **可选** **`B414_INDEXER_TICK_ROUNDS`** **）** **→** **`persist` `indexer-reconcile`** **根级** **B-383** **/** **B-386** **/** **B-413** **与** **`GET …/admin/observability/overview`** **深相等** **→** **落盘** **`evidence/b414_revenue_e2e_go_live_closeout/run_<UTC>/b414-closeout-record.json`** **。** **Runbook**：[`docs/runbook/TT-B414-REVENUE-E2E-GO-LIVE-CLOSEOUT-001.md`](../docs/runbook/TT-B414-REVENUE-E2E-GO-LIVE-CLOSEOUT-001.md) **；** **与** **[go-live-checklist.md](../docs/go-live-checklist.md)** **§10** **/** **[evidence/README.md](../evidence/README.md#b414-revenue-e2e-go-live-closeout)** **互指** **。**

**订单状态事实 vs `event_log` / `orders_projection`（母表 B-413 · TT-B413）**：**锚** **`413-ORDER-STATE-FACTS-CHAIN-ALIGN-OBS-V1`** **`GET …/admin/observability/overview`** **/** **`POST …/internal/indexer-reconcile`** **根级** **`order_state_transition_facts_chain_align_observability`** **（** **与** **B-415** **聚合** **同源** **底座** **）** **。** **Runbook**：[`docs/runbook/TT-B413-ORDER-STATE-CHAIN-LOG-PROJECTION-ALIGN-001.md`](../docs/runbook/TT-B413-ORDER-STATE-CHAIN-LOG-PROJECTION-ALIGN-001.md) **；** **实现** [`crates/api/src/db/order_state_transition_facts.rs`](../crates/api/src/db/order_state_transition_facts.rs) **。**

**用户订单主路径闸（母表 B-410 · TT-B410）**：**`bash scripts/ops/b410-user-flow-e2e-gate.sh`** **（** **内含** **`b409-order-state-primary-acceptance.sh`** **+** **`b409-order-state-exception-acceptance.sh`** **）** **；** **可选** **`B410_RUN_PLAYWRIGHT=1`** **跑** **Playwright** **。** **Runbook**：[`docs/runbook/TT-B410-USER-FLOW-E2E-ORDER-STATE-UNIFIED-001.md`](../docs/runbook/TT-B410-USER-FLOW-E2E-ORDER-STATE-UNIFIED-001.md) **。**

**FeeRouter 治理事实流只读聚合（母表 B-415 · TT-B415）**：**`POST …/internal/indexer-reconcile`** **与** **`GET …/admin/observability/overview`** **根级** **`fee_router_governance_fact_stream_observability`** **（** **锚** **`415-FEE-ROUTER-GOVERNANCE-FACT-STREAM-OBS-V1`** **）** **；** **聚合** **B-413/B-164/B-381/B-177** **与** **链上** **`owner`/`distributePaused`** **等** **。** **验证**：[`docs/runbook/TT-B415-FEE-ROUTER-GOVERNANCE-FACT-STREAM-001.md`](../docs/runbook/TT-B415-FEE-ROUTER-GOVERNANCE-FACT-STREAM-001.md)。

**治理 execute 后四键并列无漂移（母表 B-430 · TT-B430）**：**单** **`POST …/internal/indexer-reconcile`**（**`persist:true`** **+** **`include_governance_pool_db_vs_chain_balance_drift_observability`** **+** **`include_fee_router_platform_fee_routed_log_count_chain_vs_db_observability`** **+** **`include_region_vault_forwarded_log_count_chain_vs_db_observability`**），**`200`** 根级含 **B-381/B-383/B-384** 与 **B-415**（**无** **B-415** **单独** **`include_*`**）；**随后** **`GET …/admin/observability/overview`**，**四** **键** **reconcile** **根级** **与** **`overview.*`** **须** **JSON** **深相等**（**`GO`** **/** **`SUSPECT`** **见** **脚本** **退出码**）。**一键**：**`bash scripts/ops/b430-gov-post-exec-reconcile-overview-bundle.sh`**（须 **`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`jq`**；可选 **`B430_OUT_DIR`**、**`B430_WRITE_CLOSEOUT_PACK=1`** **（** **默认** **`evidence/b430_gov_post_exec_reconcile_overview/run_<UTC>/`** **）** **、** **`B430_POST_INDEXER_TICK=1`**、**`B430_PROPOSAL_ID`** **（** **`governance_effect_applied`** **）** **、** **`B430_FETCH_PUBLIC_POOL`**）。**Runbook**：[`docs/runbook/TT-B430-GOV-POST-EXEC-RECONCILE-OVERVIEW-BUNDLE-001.md`](../docs/runbook/TT-B430-GOV-POST-EXEC-RECONCILE-OVERVIEW-BUNDLE-001.md) **§3.1** **；** **证据锚** [`evidence/README.md`](../evidence/README.md#b430-gov-post-exec-reconcile-overview) **。** **与** **B-424**：**不** **替代** **meta/overview** **全量** **门禁** **；** **与** **TT-B431**：**链上** **execute** **对拍** **见** **[`TT-B431`](../docs/runbook/TT-B431-GOV-EXECUTE-CHAIN-READ-PAYLOAD-ALIGN-001.md)** **/** **[`evidence/README.md`](../evidence/README.md#b431-gov-execute-chain-read-payload-align)** **，** **本** **卡** **钉** **API** **观测** **面** **无** **分叉** **。**

**FeeRouter 写路径 · 测试网门禁（母表 B-416）**：**目标** **测试网** **须** **先** **部署** **含** **`POST …/internal/indexer-reconcile`** **专用** **HTTP** **超时** **的** **API** **（** **`INTERNAL_INDEXER_RECONCILE_TIMEOUT_SECS`** **；** **上表** **对账** **行** **、** **`.env.example`** **）** **，** **再** **跑** **L0** **/** **封口** **编排** **。** **链上** **写** **前** **先** **`bash scripts/ops/b416-fee-router-write-path-b415-preflight.sh`** **（** **L0** **：** **`reconcile`≙`admin`** **同** **键** **B-415** **）** **；** **L1** **Timelock** **/** **Safe** **/** **`cast`** **受控** **广播** **与** **`b416-print-distribute-cast-template.sh`** **模板** **；** **L2** **最小** **`distribute`** **成功** **链** **复用** **下款** **B-407** **脚本** **。** **真链** **封口** **顺序** **（** **详** **`evidence/b416_fee_router_write_path_testnet/README.md`** **）** **：** **（** **可选** **）** **mint** **`ADMIN_BEARER_TOKEN`** **→** **真实** **L2** **`distribute`** **（** **`B416_RUN_B407=1`** **/** **`B416_DISTRIBUTE_TX_HASH`** **）** **→** **`B416_L1_ATTESTED=1`** **`bash scripts/ops/b416-testnet-closeout-evidence.sh`** **→** **`run_<UTC>/b416-closeout-record.json`** **落盘** **→** **关闸** **重启** **→** **即刻** **B-416** **冻结** **并** **停掉** **全部** **相关** **运维** **（** **仅** **保留** **已** **落盘** **证据** **只读** **与** **审计** **）** **；** **唯一** **沿** **B-417** **/** **TT-B417** **推进** **治理** **执行** **自动化** **（** **L3** **）** **。** **一键** **证据** **包** **（** **L0→L2** **落盘** **`b416-closeout-record.json`** **）** **：** **`bash scripts/ops/b416-testnet-closeout-evidence.sh`** **（** **见** **`evidence/b416_fee_router_write_path_testnet/README.md`** **）** **。** **测试网** **免交互** **`ADMIN_BEARER_TOKEN`** **（** **须** **`TRAVELTRUST_TESTNET_ADMIN_BEARER_MINT=1`** **+** **测试网** **`CHAIN_ID`** **+** **库内** **admin** **）** **：** **`export ADMIN_BEARER_TOKEN="$(bash scripts/ops/b416-mint-testnet-admin-bearer.sh)"`** **（** **`POST …/internal/testnet-mint-admin-bearer`** **）** **；** **封口后** **关闸** **。** **Runbook**：[`docs/runbook/TT-B416-FEE-ROUTER-WRITE-PATH-TESTNET-ADMIN-001.md`](../docs/runbook/TT-B416-FEE-ROUTER-WRITE-PATH-TESTNET-ADMIN-001.md) **§0** **/** **§8** **（** **合并** **交付** **/** **真实** **测试网** **`distribute`** **补跑** **/** **`b416-closeout-record.json`** **最终** **封口** **）** **。** **治理** **L3** **母表** **B-417** **：** [`docs/runbook/TT-B417-GOVERNANCE-EXECUTION-AUTOMATION-L3-001.md`](../docs/runbook/TT-B417-GOVERNANCE-EXECUTION-AUTOMATION-L3-001.md) **（** **登记** **）** **。**

**Governor / Timelock · L3 链上 GO 证据（母表 B-417）**：**落盘** **`evidence/b417_governance_execution_runs/run_<UTC>/`**（**queue** **/** **execute** **侧车** **+** **`b417-governance-execution-report.json`**，**`execution_verdict=GO`**）；**一键**：**`bash scripts/ops/b417-env-gap-check.sh`** **→** **`b417-sepolia-preflight.sh`** **→** **`b417-run-onchain-evidence.sh`** **→** **`bash scripts/ops/b417-evidence-pack-verify.sh`** **`"$B417_RECORD_DIR"`**。**已** **`queue`**（**`state=5`**）**同提案复跑**：**`B417_ALLOW_QUEUED_PREFLIGHT=1`** **（** **预检** **与** **queue** **钩子** **）** **；** **queue** **侧** **可** **复用** **已有** **`b417-chain-step-queue.json`** **或** **设** **`B417_QUEUE_TX_HASH`** **从** **回执** **生成** **侧车** **。** **证据** **README**：[`evidence/b417_governance_execution_runs/README.md`](../evidence/b417_governance_execution_runs/README.md)** **。**

**质押→投票→执行→池/国库 UI 演示闭环（母表 B-428 · TT-B428）**：产品与运维**最短叙事**（手测/录屏证据目录、`GET …/governance/pool` 前后对照、与 **B-417** 执行链互证）见 **[`docs/runbook/TT-B428-GOV-STAKING-TREASURY-UI-CLOSELOOP-001.md`](../docs/runbook/TT-B428-GOV-STAKING-TREASURY-UI-CLOSELOOP-001.md)**、**[`evidence/b428_gov_staking_treasury_ui_closeloop/README.md`](../evidence/b428_gov_staking_treasury_ui_closeloop/README.md)**（**`run_*`/`b428-closeout-record.json`**）。

**治理闭环 UI 防回归壳（母表 B-432 · TT-B432）**：**`bash scripts/run-check-04-routes.sh`** 串联 **`scripts/gates/check-b432-governance-ui-ssot-surface.py`**；前端 **`npm run test:b432`**。与 **04 §零**、**89** 文首工程段互指。

**GO 收口总卡 · release-proof bundle（母表 B-433 · TT-B433）**：**机读** **`evidence/GO_release_proof_staking_gov_bundle/release_proof_manifest.v1.json`** **（** **`bundle_verdict`** **`GO`** **、** **`production_scope`** **、** **`caveats[]`** **为** **说明性** **）** **；** **统一** **对外** **Production** **唯一** **入口** **[`evidence/GO_FINAL_20260416/README.md`](../evidence/GO_FINAL_20260416/README.md)** **/** **[`release_proof.json`](../evidence/GO_FINAL_20260416/release_proof.json)** **/** **[`RELEASE_NOTES_PUBLIC.md`](../evidence/GO_FINAL_20260416/RELEASE_NOTES_PUBLIC.md)** **。** **上线判定一句话** **：** **当且仅当** **B-414** **、** **B-430** **、** **B-431** **在** **目标** **环境** **均为** **GO** **，** **系统** **达到** **Production Go-Live** **（** **工程收益** **+** **治理观测** **+** **链上真实性** **）** **；** **详** **[`TT-B433` §0.5](../docs/runbook/TT-B433-GO-RELEASE-PROOF-STAKING-GOV-BUNDLE-001.md#production-go-live-rule-b433)** **。** **Runbook**：[`docs/runbook/TT-B433-GO-RELEASE-PROOF-STAKING-GOV-BUNDLE-001.md`](../docs/runbook/TT-B433-GO-RELEASE-PROOF-STAKING-GOV-BUNDLE-001.md) **；** **证据** **锚** [`evidence/README.md`](../evidence/README.md#go-master-evidence-entry-production-go-live) **/** [`#b433-go-release-proof-staking-gov-bundle`](../evidence/README.md#b433-go-release-proof-staking-gov-bundle) **。**

**可重复 revenue 索引跑流 L0（母表 B-403 · TT-B403）**：**`bash scripts/ops/b403-revenue-e2e-repeatable-runner.sh`** **（** **默认** **3** **轮** **；** **`B403_ROUNDS=2`** **可调** **）** **—** **每轮** **生成** **`run_id`** **，** **`POST …/internal/indexer-tick`** **后** **复用** **`b402-min-revenue-e2e-reconcile-smoke.sh`** **（** **B-383+B-386** **）** **。** **NDJSON** **留证**：**`evidence/b403_revenue_e2e_runs/b403-run-manifest.jsonl`** **（** **见** **[`docs/runbook/TT-B403-REVENUE-E2E-REPEATABLE-RUNNER-L0-001.md`](../docs/runbook/TT-B403-REVENUE-E2E-REPEATABLE-RUNNER-L0-001.md)** **）** **。** **L1** **只读** **聚合** **（** **母表** **B-404** **）** **：** **`GET /api/v1/internal/revenue-e2e-run-status?run_id=<uuid>`** **—** **manifest** **行** **+** **DB** **快照** **（** **见** **[`docs/runbook/TT-B404-REVENUE-E2E-RUN-STATUS-L1-001.md`](../docs/runbook/TT-B404-REVENUE-E2E-RUN-STATUS-L1-001.md)** **与** **[04 §3.4](../docs/spec/04-后端与API.md)** **）** **，** **与** **L0** **runner** **分轨** **。**

**订单锚 revenue 跑流 L2（母表 B-405 · TT-B405）**：**`bash scripts/ops/b405-revenue-e2e-order-driven-runner.sh`** **（** **默认** **`B405_ROUNDS=2`** **）** **—** **每轮** **`order_id`** **（** **`B405_ORDER_ID`** **或** **`GET …/orders?limit=1`** **）** **、** **`run_id`** **、** **`POST …/internal/indexer-tick`** **、** **`b402-min-revenue-e2e-reconcile-smoke.sh`** **。** **留证**：**`evidence/b405_revenue_e2e_runs/b405-run-manifest.jsonl`** **（** **[`docs/runbook/TT-B405-REAL-ORDER-DRIVEN-REVENUE-E2E-L2-001.md`](../docs/runbook/TT-B405-REAL-ORDER-DRIVEN-REVENUE-E2E-L2-001.md)** **）** **。** **L1** **仍** **用** **同一** **`GET …/revenue-e2e-run-status`** **（** **先** **b403** **manifest** **再** **b405** **manifest** **；** **`manifest_kind=b405_round`** **时** **含** **`orders_row_excerpt`** **）** **。** **须** **`INTERNAL_API_SECRET`** **、** **`ADMIN_BEARER_TOKEN`** **、** **`jq`** **；** **无** **可见** **订单** **时** **设** **`B405_ORDER_ID`** **。**

**自举测试订单 + L2 跑流（母表 B-406 · TT-B406）**：**`bash scripts/ops/b406-revenue-e2e-bootstrap-and-runner.sh`** **—** **`seed-test-accounts`** **（** **可选** **）** **→** **登录** **旅行者** **/** **向导** **→** **`GET …/guides`** **→** **`POST …/orders`** **→** **`/accept`** **→** **`/mock-pay`** **（** **须** **API** **`P3_CHAIN_OFF=1`** **）** **→** **导出** **`B405_ORDER_ID`** **并** **调用** **上款** **`b405-revenue-e2e-order-driven-runner.sh`** **；** **stdout** **提示** **`GET …/revenue-e2e-run-status?run_id=`** **。** **见** **[`docs/runbook/TT-B406-REVENUE-E2E-BOOTSTRAP-ORDER-001.md`](../docs/runbook/TT-B406-REVENUE-E2E-BOOTSTRAP-ORDER-001.md)** **、** **[`evidence/b406_revenue_e2e_runs/README.md`](../evidence/b406_revenue_e2e_runs/README.md)** **。**

**真实链 release/distribute + L2 跑流（母表 B-407 · TT-B407）**：**`bash scripts/ops/b407-revenue-e2e-real-chain-runner.sh`** **—** **不** **使用** **`/mock-pay`** **；** **`POST …/set-escrow-address`** **绑定** **已** **`Funded`** **Escrow** **（** **`B407_ESCROW_ADDRESS`** **）** **后** **调用** **`b407-exec-chain-release-distribute.sh`** **（** **`cast`** **：** **`release()`** **+** **`FeeRouter.distribute`** **）** **→** **导出** **`B405_ORDER_ID`** **并** **调用** **`b405-revenue-e2e-order-driven-runner.sh`** **；** **stdout** **提示** **`GET …/revenue-e2e-run-status?run_id=`** **。** **须** **`B407_RELAYER_PK`** **/** **`B407_OWNER_PK`** **/** **`B407_FEE_ROUTER`** **或** **`FEE_ROUTER_ADDRESS`** **/** **`B407_RPC_URL`** **等** **（** **见** **脚本** **头** **注释** **）** **。** **见** **[`docs/runbook/TT-B407-REAL-CHAIN-REVENUE-E2E-001.md`](../docs/runbook/TT-B407-REAL-CHAIN-REVENUE-E2E-001.md)** **、** **[`evidence/b407_revenue_e2e_runs/README.md`](../evidence/b407_revenue_e2e_runs/README.md)** **。**

**目标环境验收封口（母表 B-408 · TT-B408）**：**`B408_RECORD_DIR=…`** **`bash scripts/ops/b408-revenue-e2e-acceptance-closeout.sh`** **—** **在** **B-407** **跑流** **外** **落盘** **`b407-chain-tx.json`** **/** **`b404-run-status.json`** **/** **`b408-acceptance-record.json`** **（** **tx** **hash** **/** **`run_id`** **/** **B-404** **体** **）** **；** **运维** **回填** **[Runbook §5.1](#51-目标环境-revenue-e2e-可重复验收b-408)** **表** **。** **见** **[`docs/runbook/TT-B408-REVENUE-E2E-ACCEPTANCE-CLOSEOUT-001.md`](../docs/runbook/TT-B408-REVENUE-E2E-ACCEPTANCE-CLOSEOUT-001.md)** **、** **[`evidence/b408_revenue_e2e_acceptance/README.md`](../evidence/b408_revenue_e2e_acceptance/README.md)** **。**
...
**RegionVault claim 广播续跑（B-262 / B-276 / B-277 / B-282 · `scripts/ops/`）**：**B-276** 只做广播前 nonce/RPC 预检与 **`nonce_preflight_report`** 落盘。**B-277**（**`scripts/region-vault-claim-broadcast-signing-order-static.sh`** / **`region_vault_claim_broadcast_signing_order_static.py`** **`emit`/`verify`**）落盘 **`14-REGIONVAULT-CLAIM-BROADCAST-SIGNING-ORDER-STATIC-TABLE-V1`**，枚举 **`(signing_order, batch_plan_id)`** 规范全序及 **`stub_conforms_to_canonical_total_order`**，与 **B-257** 次序规则同源（契约见 **[04 §四](../docs/spec/04-后端与API.md)**「RegionVault claim 广播离线脚本」）。**B-262** **`region_vault_claim_broadcast_execute.py` `execute`** 负责 **`eth_sendRawTransaction`** 与 **`execution_report`**。**B-282**（**`scripts/region-vault-claim-broadcast-pipeline-resume.sh`** / **`region_vault_claim_broadcast_pipeline_resume.py` `resume-execute`** → **`execute --resume-from-execution-report`**）在**不改变 B-262 执行语义**的前提下，按 prior **`execution_report`** 与当前 stub **SHA256**、报告 **anchor/rule_version** 对齐后，仅对已在链上经 **`eth_getTransactionReceipt` 二次确认成功**的步骤跳过重复发送，新报告带 **`resume_from_execution_report_attached`** 与各步 **`idempotent_resume`**。**提醒（预期行为，非缺陷）**：B-282 保证的是**执行幂等**（不重发已确认上链的同一笔 raw tx），**不是**「环境幂等」——**不**据此断言 RPC、内存池、nonce 竞争等运行环境未变；resume / replay 前仍须值班判断环境与 stub 是否仍适用。
...
| [母表 **B-434** · **`TT-B434-FUND-TIMELOCK-TRUTH-ARBITRATION-001`**](../docs/runbook/TT-B434-FUND-TIMELOCK-TRUTH-ARBITRATION-001.md) | **`Deploy.s.sol`** **与** **`DeployGovernanceStack`** **可能** **产出** **两套** **`GovernanceTimelock`** **：** **全栈** **资金** **主线** **开工** **前** **须** **书面** **裁断** **唯一** **真值** **（** **避免** **双套** **假** **闭环** **）** **。 |
| [母表 **B-435** · **`TT-B435-FULLSTACK-FUND-TESTNET-RELEASE-CHAIN-001`**](../docs/runbook/TT-B435-FULLSTACK-FUND-TESTNET-RELEASE-CHAIN-001.md) | **B-434** **裁断** **后** **：** **测试网** **FeeRouter** **/** **池** **/** **meta** **同源** **/** **真** **扣款** **/** **indexer-reconcile** **/** **`runtime-chain-ssot`** **一条** **链** **验收** **。 |
...
**换部署核对**：新地址须同步 **根 `.env` / 前端 `NEXT_PUBLIC_*`**、**`GET /meta` → `chain.contracts`**，并按 **§12.4** 走 **ABI 同步** 与 **55-S13**。**若** **同时** **存在** **「** **`Deploy.s.sol`** **批次** **」** **与** **「** **`DeployGovernanceStack`** **批次** **」** **两套** **Timelock** **可能** **：** **须** **先** **完成** **母表** **B-434** **/** **[`TT-B434`](../docs/runbook/TT-B434-FUND-TIMELOCK-TRUTH-ARBITRATION-001.md)** **裁断** **，** **再** **按** **B-435** **/** **[`TT-B435`](../docs/runbook/TT-B435-FULLSTACK-FUND-TESTNET-RELEASE-CHAIN-001.md)** **执行** **闭环** **。**
...
## 2.7 分轨 B、治理金库与机读纪律：长期盯盘与三项周期演练（签字）

**定位**：[91-协议金库与资金池-技术索引](../docs/spec/91-协议金库与资金池-技术索引.md)（v0.8.x 起）、[84-第一阶段10国Country-Pool发行参数总表](../docs/spec/84-第一阶段10国Country-Pool发行参数总表.md)、[82-治理币-文档总览](../docs/spec/82-治理币-文档总览.md)、[89-治理 UI](../docs/spec/89-治理UI-全球旅游市场治理控制台设计规格.md) 与 [04 §3.4](../docs/spec/04-后端与API.md) 已把**定义与边界**写清。**当前缺口不是「再定义一次」**，而是：**流程能否长期不被绕开**、**提案/运维是否持续按同一口径理解**、**机读与演练是否按期留痕**。本节把**须重复执行的运维动作**固化为周期项，产物与 **§3 / §4** 演练模板兼容；**不**替代 **91/82/84** 正文。

### 2.7.1 四类长期风险（钉住即可）

| # | 盯什么 | 性质 | 说明 |
|---|--------|------|------|
| **1** | **分轨 B「唯一入口」** | 工程纪律 + 机读长期执行 | 入口白名单、**`GET /meta` + evidence + 台账** 三处一致已在 **91** 写成硬约束；**未披露入口视为 UB**。风险在**今后是否有人绕开**（临时多签、临时地址、人工收款），属**流程与习惯**，不是「缺一段 spec」。 |
| **2** | **治理「能花什么资产」** | 提案审查 + 审计口径 | **GovernanceTreasury** 已从「万能钱包」收束为**治理支出金库**；**Escrow 用户资产**禁止治理动用；**84** 已拆手续费路径。剩余风险：**payload 或运维理解偏差**仍可能把「允许治理支出」误解成「任意入库资产都可动」— **每笔 / 定期审查**。 |
| **3** | **显示完整 ≠ 资金全貌** | 产品 + 运营对外口径 | **89 / 04** 已要求余额带 **`track_type` + `source`**。治理页仍是**治理视角**读数，**不是**全协议资产负债表；对外 briefing 须防误读。 |
| **4** | **多轨同 tx** | 已延后的设计债（非当期 bug） | **91** 已明确：当前 **event_log** **单 tx 禁跨轨**；若未来放开须先有组合、冲销、报表与侧带表策略。**本期**优先 **Runbook 演练与签字**，而非再扩文档。 |

### 2.7.2 三项周期动作（最低频率：**每季度至少一次**；**主网/重大升级前须加一轮**）

| 动作 | 核对要点（机械执行） | 证据（建议路径） | 签字 |
|------|----------------------|------------------|------|
| **A · 分轨 B 唯一入口** | ① **91** 白名单/UB 条款 ↔ ② **`GET /meta` → `treasury_track_b_entrances`**（及同批治理 meta 键）↔ ③ 内部金库入口台账 ↔ ④ 最近一次 **evidence/** 或 reconcile 留痕 **四方对读无漂移**；**显式确认**无未登记收款路径、无「临时多签 / 临时地址 / 人工收款」替代白名单 | `evidence/GO_YYYYMMDD/artifacts/track-b-entrance-cadence.md`（或并入该日 **`manifest.json`** 说明字段） | 值班执行 + **批准人**批签 |
| **B · 治理支出前审查** | **凡拟链上 `queue`→`execute` 且动用治理金库**：① 目标合约与 **GovernanceTreasury** 设计边界一致 ② payload **不**触碰 **Escrow 用户资金**语义 ③ 与 **82 / 84** 叙事对读，**书面一句**：「本支出属治理预算腿，非订单用户资产」。**无链上支出的季度**：仍须做一次**桌面复盘**（最近 0 笔亦登记「本季无 execute」）。 | 提案/工单指针 + 审查记录一行；可指向 **§2.56** 表内 **B-100** 证据链 | **批准人**（或治理流程规定的第二人） |
| **C · Pause / 密钥 / `chain_id` 演练** | ① **Pause** 与 **08-3**、本节 **RUNBOOK_PAUSE_ALLOWLIST**、**§1** 场景 ⑦ **对读** ② 运行环境 **`CHAIN_ID` / RPC** 与 **`GET /meta` → `chain` / `runtime-chain-ssot`** **一致** ③ 多签与值班 **可达**（可为桌面推演，不必真链上 Pause） | 与 **§4** 同编号登记，或 `evidence/GO_YYYYMMDD/artifacts/pause-keys-chainid-cadence.md` | **双人**（值班 + 批准人） |

### 2.7.3 与 §3 / §4 的衔接

- 上述 **A/B/C** 可在 **§4 演练历史** 中单列三行（场景名：`cadence-track-b` / `cadence-governance-spend-review` / `cadence-pause-keys-chainid`），或并入既有 **DR-/GO-** 编号；**最低频率**不满足时，**不得**在发版材料中宣称「金库入口/治理支出/暂停链路已运维闭环」。

<a id="check-g-dual-score-gate"></a>

### 2.7.4 Check-G（双评分 · 发版控制绑定）

**目的**：把 **[07 §六 6.3](../docs/spec/07-开发流程与顺序.md#63-全库完成度权重ssot--沟通用)** 篇首 **（1）** **全库综合完成度 %**（**执行维度 · SSOT**）与 **[15 §〇 · 企业级综合维度打分母表](../docs/spec/15-多维度文档与技术检查报告.md#企业级综合维度打分母表)**（**质量维度 · 七维**）**绑成发版闸**，避免「只看 **07** 误判可上线」或「只看 **§〇** 误判系统成熟」。

| 条件 | 须同时满足 | 操作 |
|------|-------------|------|
| **G1 · 执行 %** | 篇首 **（1）** 全库综合 % **≥** 当次发版纪要或 **Go-Live** 批准书记载之**阈值**（未单列时：不低于当次 **Baseline**；由 **批准人** 书面确认） | 与 **[go-live-checklist.md](../docs/go-live-checklist.md)**、**[缺口官方总表 P0](../docs/spec/缺口与待补-官方总表.md)** 并联勾选 |
| **G2 · 质量七维（分阶段阈值）** | 见下表 **阶段 → 每维最低分**；**七维各自**须 ≥ 该阶段阈值。**任维低于阈值**时：须在该次 evidence 中登记 **风险接受（Risk Acceptance）**（理由 + 批准人），**或** 闭环至达标后再发版 | **机读真源**：`dual_score_signoff.v1.json`（模板见 **[example.json](../evidence/GO_YYYYMMDD_template/artifacts/dual_score_signoff.v1.example.json)**；JSON Schema：**[dual_score_signoff.v1.schema.json](../evidence/schemas/dual_score_signoff.v1.schema.json)**）；**一键校验**：`bash scripts/check-dual-score-gate.sh path/to/dual_score_signoff.v1.json`（`--emit-summary` 见脚本 **stderr/stdout**） |

**G2 阶段阈值（每一维独立比较 · 与 §〇 10 分制一致）**：

| 阶段 | `stage` 取值（JSON） | 每维最低分（低于则 **红项**，除非已 **Risk Acceptance**） |
|------|----------------------|------------------------------------------------------------|
| **Test / Internal** | **`test_internal`** | **≥ 7 / 10**（允许探索） |
| **Pre-GO** | **`pre_go`** | **≥ 8 / 10**（允许轻微风险） |
| **GO / External / 企业级稳定态** | **`go_external`** | **≥ 9 / 10**（与 §〇「企业级稳定态」叙事对齐） |

**风险接受（Risk Acceptance）**：当某维 **< 本阶段最低分** 仍须发版时，在 **`dual_score_signoff.v1.json`** 的 **`risk_acceptances[]`** 中为对应 **`dimension_id`（1～7）** 各写一条（**`reason`**、**`approver`** 必填）；**人类可读**摘要可并列写入 **`artifacts/dual-score-signoff.md`**。**未**登记风险接受且存在低于阈值的维 → **机读校验失败**（`check-dual-score-gate.sh` **exit 1**）。

**证据（建议）**：`evidence/GO_YYYYMMDD/artifacts/dual_score_signoff.v1.json`（**SHA256** 可由校验脚本 **stderr** 一行带出，并入 **`manifest.json`** / 工单）；可选并列 **`dual-score-signoff.md`**（非机读唯一真源）。

**签字链落点约定**：**`dual_score_signoff.v1.json`** 须与当次 **`risk_acceptances[]`** 及批准事实可对照，并经由 **`evidence_refs[]`** 可追溯指向会议纪要、**PR**、**TT** 任务卡等（落实「谁 / 何时 / 依据」）。

**meta（与 manifest 绑定 · 可选字段）**：默认在 **`meta`** 中保留 **`manifest_ref`**（如 **`manifest.json#dual_score`**）；**不**默认启用 **`meta.sha256` ↔ `manifest.json` 全文**双哈希互指（易人为失配、操作成本高）。**`meta.sha256`** 仅在高等级对外发布等场景按需选用。

**校验策略**：**`GO_FINAL_*`** 等定型发布包须执行 **`python scripts/dev/validate_evidence_manifest.py validate <GO包根目录> --verify-artifact-files`**（全量核对磁盘 **artifacts**）；日常开发或模板校验可仅 **`validate …`**（不强制 **`--verify-artifact-files`**），以控制成本。

**操作提示**：生成或更新 **`dual_score_signoff.v1.json`** 后，应以 **`python scripts/dev/validate_dual_score_signoff.py <评分件路径> --bundle-root <GO包根目录>`** 与 **`python scripts/dev/validate_evidence_manifest.py validate <GO包根目录>`**（是否 **`--verify-artifact-files`** 见上文「校验策略」）共同校验评分件与 **`manifest.dual_score`** 的路径/哈希一致性。**定型封口**另见下文「封口命令（建议顺序）」。

**环境说明**：部分证据（链上观测、**reconcile** 等）依赖当时 **RPC / 运行时环境**，不保证离线完全复现；审查以生成时记录与 **hash** 为准。

**evidence_refs**：应为可解析引用（仓库内路径、**PR**、或带 **commit** 的永久链接等），避免仅写不可追溯的临时描述。**定型发版**建议在 **`evidence_refs[]`** 中**显式写入** **`commit:`** **+** **40 位小写 hex**（与当次 **`g1_snapshot`** / 发版 **PR** 一致），并可选 **`tag:`**、**`PR:`**，以绑定「本次评分对应的构建 / **release** 状态」— **不**改 schema，仅用既有字符串项。

**封口留痕（非自动化）**：执行「封口命令（建议顺序）」后，将**成功**的终端输出**粘贴**至发版 **PR/工单**，或落入 **`evidence/…/artifacts/check-g-seal.log`**（与 **go-live-checklist §11.18** 一致），供审计核对「曾执行」。

**封口命令（建议顺序）**：
1. **`bash scripts/check-dual-score-gate.sh <评分件路径>`**
2. **`python scripts/dev/validate_dual_score_signoff.py <评分件路径> --bundle-root <GO包根目录>`**
3. **`python scripts/dev/validate_evidence_manifest.py validate <GO包根目录>`** — **`GO_FINAL_*`** **须**带 **`--verify-artifact-files`**；其它 **bundle** 按上文「校验策略」。

**说明**：**G1** 的「% 阈值」仍依赖 **批准人** 对 **07** 与发版纪要的判断 — **脚本不读取链上/仓库外状态**。**Check-G** **不替代** 其它 **CI** 门禁；与 **[15 附录〇](../docs/spec/15-多维度文档与技术检查报告.md#发版前勾选总表)** 发版勾选并列执行。

**演练（建议）**：（1）**审计桌面演练** — 模拟外部审计提问，仅用 **`evidence/GO_*`** bundle（**`manifest.json`**、**`dual_score`**、留痕日志）作答；（2）**发布演练** — 按 **go-live-checklist** + **PR 模板** + 本节**完整跑通**一次 **`GO_FINAL_*`**；（3）**异常演练** — 故意保留错误 **`dual_score`** 或 **`manifest.dual_score`** 哈希，确认机读校验 **失败**（**exit ≠ 0**），再修正后重跑封口。

**线 B（治理执行链）验收清单**：见 [`docs/runbook/TT-LINE-B-GOVERNANCE-EXECUTION-CHECKLIST.md`](../docs/runbook/TT-LINE-B-GOVERNANCE-EXECUTION-CHECKLIST.md)（Governor / Timelock / `GovernanceTreasury` 权限边界；与 **线 A**（FeeRouter + `/meta` + 分轨观测）正交）。

<a id="testnet-acceptance-one-liner-2026-04-17"></a>
...
| 线 B 清单与仍待补证 | [`docs/runbook/TT-LINE-B-GOVERNANCE-EXECUTION-CHECKLIST.md`](../docs/runbook/TT-LINE-B-GOVERNANCE-EXECUTION-CHECKLIST.md)（含 **Treasury.spend**、TT-B435 类全栈证据） |
...
### 5.1 目标环境 Revenue E2E 可重复验收（B-408）

**目的**：在目标环境跑通一次 **[`b407-revenue-e2e-real-chain-runner.sh`](../scripts/ops/b407-revenue-e2e-real-chain-runner.sh)**（真实链 **`Escrow.release` / `FeeRouter.distribute`** + **B-405** / **B-402**），并落盘 **链上 tx hash**、**`b405_run_id`**、**`GET /api/v1/internal/revenue-e2e-run-status`** 成功体，作为运维封口与回归基线（与 [TT-B407](../docs/runbook/TT-B407-REAL-CHAIN-REVENUE-E2E-001.md) 链路径一致）。

**一键封口（落盘）**：设 **`B408_RECORD_DIR`**（建议每批次子目录，避免覆盖）后执行 **`bash scripts/ops/b408-revenue-e2e-acceptance-closeout.sh`** — 须与 **B-407** 相同 env（**`INTERNAL_API_SECRET`**、**`ADMIN_BEARER_TOKEN`**、**`B407_*`** 链变量等）。成功后在 **`$B408_RECORD_DIR`** 生成 **`b407-chain-tx.json`**、**`b404-run-status.json`**、**`b408-acceptance-record.json`** 等。规格见 **[TT-B408](../docs/runbook/TT-B408-REVENUE-E2E-ACCEPTANCE-CLOSEOUT-001.md)**；默认落盘目录说明见 **[`evidence/b408_revenue_e2e_acceptance/README.md`](../evidence/b408_revenue_e2e_acceptance/README.md)**。

| 封口批次 | 日期 (UTC) | 环境 | `b405_order_id` | `release` tx | `distribute` tx | `b405_run_id` | B-404 证据路径 | `b408-acceptance-record.json` |
|----------|------------|------|-----------------|--------------|-----------------|---------------|----------------|-------------------------------|
| B-408-2026-04-15-engineering-attempt（**未闭合**） | 2026-04-15 | 本机 API（`127.0.0.1:8080`）与仓库 `.env` **密钥/链变量未对齐** | — | — | — | — | — | 说明见 [`evidence/b408_revenue_e2e_acceptance/b408_20260415_engineering_attempt/STATUS.md`](../evidence/b408_revenue_e2e_acceptance/b408_20260415_engineering_attempt/STATUS.md) |
| *目标环境正式封口（待填）* | *YYYY-MM-DD* | *staging / prod-like* | *UUID* | *`0x…`* | *`0x…`* | *UUID* | *`…/b404-run-status.json`* | *同批目录 `b408-acceptance-record.json`* |

**说明**：首行为 **2026-04-15** 工程侧试跑留痕（**非**业务正式封口）。**正式封口**须在**目标环境** shell 中 export 与**运行中 API**一致的 **`INTERNAL_API_SECRET`**，并配置链上 **`B407_*`**（含 **Funded Escrow**、**FeeRouter** 同址）后执行 **`b408-revenue-e2e-acceptance-closeout.sh`**，再将**末行**填实或**增行**登记。
...
<a id="line-a-feerouter-pass"></a>

#### 线 A · 验收结论模板（`FeeRouter` + `/meta` + 分轨 A/B · PASS 须全部满足）

以下可直接粘贴为**验收结论**（**线 A** 专项；与 **[91 §七点五](../docs/spec/91-协议金库与资金池-技术索引.md#线-a-验收-pass-判据)** 互指；**不**替代 **线 B**（Governor / Timelock / 金库执行链））。

**PASS（线 A）**：

1. **`chain_id`** 全链路一致（**`GET /meta` → `chain.chain_id`**、**`CHAIN_ID`**、Explorer / 索引器 / 证据包 **`chainId`** 同源对拍，见 **[91 §六.1](../docs/spec/91-协议金库与资金池-技术索引.md#61-跨网络一致性原则multi-chain-consistency)**）。
2. **`/meta` 七键**与部署台账一致（**`GET /api/v1/meta` → `chain.contracts`** 七项协议地址与根 **`.env` / 前端 `NEXT_PUBLIC_*`** / **[14 §1.1](../docs/spec/14-合约-API-ABI-前后端对齐.md)** 同源；见上表「同址三处」与 **[04 §3.4](../docs/spec/04-后端与API.md)** **`chain.contracts`** 规则）。
3. **Track B** 入口机读存在且与披露一致（**`GET /meta` → `governance.treasury_track_b_entrances`** 与 **91 §4.3 / §4.5**、台账 **`TREASURY_ADDRESS` / `TRACK_B_ENTRANCE_*`** 对读）。
4. **FeeRouter** 四向地址链上可读且用途一致（**`countryBucket` / `globalStakers` / `globalReserve` / `globalOps`** — **[91 §4.2](../docs/spec/91-协议金库与资金池-技术索引.md#42-feerouter-四收款地址部署与-meta-红线)**；以链上只读或运维快照为准，**非**仅文档口头一致）。
5. **分轨 A/B** 在 **`event_log`** 与 **API** 中一致表达（**`track_type`** 与 **[04](../docs/spec/04-后端与API.md)** 治理/对账路由；**禁止**裸转账冒充分轨，见 **91 §4.5**）。
6. **projection** 数据未被误用为链上 **SSOT**（**`GET …/governance/fee-pool-aggregates`** 等 **Σ 投影** **不得**顶替 **`GET …/governance/pool`** 根级 **`pool_balance` / `country_pool` / `treasury_pool`** 瞬时链上主读；**[04 §3.4](../docs/spec/04-后端与API.md)** **B110-SSOT-07**）。

**FAIL**：任一条未满足 → **不得**声明 **PASS（线 A）**；须纠偏后重验。
...
**前置**（开闸后仍须全部满足，否则 API **不**写根级 **`treasury_erc20_pool*`** 链上主读）：进程已挂载 **`ChainConfig` + 可用 RPC**；**`TREASURY_ADDRESS`**（与 **`GET /meta` → 链上金库地址配置** 同源，见根目录 **`.env.example`**）；**`GOVERNANCE_TREASURY_SSOT_TOKEN_ADDRESS`**（非空、规范合约地址）。
...
- **仲裁员实质控制防护**（写死）：**仲裁员利益来源披露**（与争议方的利益关系、是否来自同一经济利益群体）；**收入结构透明度**（报酬不与单案结果挂钩）；**仲裁员报酬结构**须为**固定薪酬或与裁决方向/争议数量无关**（写死于本节或治理文档，与 08-4 一致），否则存在激励制造争议风险；**外部/独立仲裁员引入比例**（由运营或治理定稿写死本节）。避免 3 人全部来自向导侧或平台长期合作方等结构性控制。**仲裁员联盟/串谋防护**（写死）：**仲裁员之间是否可能串谋**须有制度缓解；**案件分配随机性证明**（如随机分配、轮询、可验证随机）须写死；**是否公开每个仲裁员历史偏差指数**（匿名化或脱敏）须写死；**是否存在外部审计抽查机制**须写死（由运营/法务定稿）。否则企业级会问「仲裁员是否可能形成联盟」。**仲裁员集体偏差**（写死）：若**80% 仲裁员倾向某类人或某城市**（非单人舞弊、乃群体结构偏差），须有**偏差指数阈值**触发**自动冻结裁决或暂停争议或强制复核**；由风控/运营定稿写死本节，否则集体偏向无制衡。**连续偏向阈值**（写死）：若**连续 N 单同向**（如长期偏向旅行者）须触发**自动审查**；**集体停机机制**（写死）：当集体偏差超过阈值时是否**暂停新争议受理或全平台仲裁暂停**须写死；**外部独立复核机制**（写死）：是否存在**外部审计抽查或独立复核**（非仅内部）须写死；否则仲裁系统可被经济攻击。与 08-3 连续偏向阈值、08-4 企业级审计一致。
...
- **向导质押挤兑（协议信任抛盘）**（写死）：**非币抛盘**— 连续裁决偏向旅行者→向导亏损、质押被扣→向导集体退出并 unstake→订单骤降、质押池缩水、信誉系统崩塌。缓解=**默认裁决平衡**、**连续偏向阈值**触发自动风控（08-3、本节）、**质押覆盖率与 worst-case 模型**、若有**向导 unstake 冷却期**（如 7～14 天）须写死以减缓瞬间挤兑；由风控/产品定稿写死本节或 08-3。**叙事/信任抛盘**（写死）：稳定币冻结、仲裁争议曝光、多签暂停被滥用→用户「卖信任」→用户流失、TVL 下滑。缓解=Pause 滥用缓解（本节）、透明度与冲突矩阵（§6/§11）、OFAC+冻结+Pause 冲突步骤表；由风控/法务定稿。
- **异常争议率与关联账户**：异常争议率（某类服务/地区显著偏离均值）触发告警或**自动冻结**该类别新单；关联账户检测与处置流程由风控定稿写死（防协作套利/沉默攻击）。**系统级拥堵降级策略**（写死）：当**订单暴涨、链拥堵、RPC 不稳定**时，是否自动**限单、提高保证金、延长 SLA** 须写死；由风控/产品定稿写死本节或 08-3，否则规模极限与系统稳定性存在缺口。**争议量 10× 触发阈值（7️⃣）**（写死）：订单 10× 导致争议量暴增（如订单 10,000→争议 800 级）时，**SLA 自动延长/自动暂停新争议/动态提高 arbFee** 的**触发阈值**（如待裁件数 > N 或争议率 > X%）须写死；无触发阈值=规模错觉。见 08-3、W-D3-CAPACITY。**规模失控压力测试（企业级）**（写死）：**10× 争议 alone 不够**；须覆盖 **10× 争议 + 10× Gas + 10× 链拥堵 + 10× 恶意攻击 + 10× 仲裁投诉 + 10× 用户挤兑** 的推演或阈值。须写死或落点：**仲裁处理能力上限**、**仲裁员容量数学模型**（如 max 待裁件数/仲裁员/日）、**自动降级机制**（超载时限单/排队/只读）、**排队公平性算法**（FIFO 或加权）；无=规模幻觉。由风控/运营定稿写死本节或 08-3。**群体攻击（8️⃣）**（写死）：**旅行者集体恶意刷单、向导集体罢工、仲裁员集体拒绝、多签成员串谋**— 是否存在**协议级自动缓冲机制**（如限流、冻结、fallback）还是**仅能人工干预**须写死；由风控/运营定稿。**群体攻击压力测试**（写死）：假设**多组假旅行者+假向导互相制造争议、诱导仲裁拥堵、声誉操控**— 是否可通过系统漏洞套利、通过仲裁拖延冻结资金、通过声誉洗白；须有**群体攻击压力测试或缓解写死**（如关联账户检测、异常争议率冻结、声誉金额加权），否则企业级会问。**经济战**（写死）：**竞争对手大规模制造争议、大规模投诉、大规模链拥堵**— 是否存在**自动降级模式**（如限单、排队、只读）还是**系统崩溃**须写死；由风控/运维定稿写死本节。
...
**公开路径（本层直接放行，不校验上述登录头）**（与代码 `public` 一致，摘要）：**`OPTIONS *`**；**`/health`**、**`/meta`**；**`/auth/*`**；**`GET /api/v1/guides`**、**`GET /api/v1/guides/{id}`**（含 **`…/availability`**；**不含** **`/api/v1/guides/upload-doc`**）；**`GET /api/v1/discover/orders`**；**`GET /api/v1/did-rank/{travelers,guides,itineraries}`**；**`GET /api/v1/community/…`** 只读（**不含**路径含 **`/me/`**、**`/friends/`** 的社区子路径）；**`/api/v1/internal/*`**（**另**由 **`INTERNAL_API_SECRET`** 门禁，见 **04 §7.8**「内网密钥」；**不得**对公网暴露）。
...
2. **项目根** **`bash scripts/sync-abi-from-forge.sh`** 或 **`.\scripts\sync-abi-from-forge.ps1`**：写入 **`contracts/abi/*.json`**（含 **双池**、**RegionVault**、**GovernanceTimelock** / **GovernanceTreasury** / **GovernanceVotesToken**（链上符号 **TTG**）/ **TravelTrustGovernor** 等；及可选 IERC20/MockERC20）；脚本末尾已调用 **`run-verify-abi-forge`**（multiset 与 **contract-abi-gate** 同源）。**治理 Governor/Token JSON** **仅** **须** **留在** **`contracts/abi/`** **（** **55-S13** **不** **复制** **到** **`frontend/dapp/abis`** **）** **。**
3. 按脚本 **Next** 提示将 **`GuideIdentityStakingPool.json`** / **`ProviderIdentityStakingPool.json`** / **`Registry.json` / `FeeRouter.json`** 复制到 **`frontend/dapp/abis/`**（与 **`contracts/abi`** **字节一致**，**55-S13** 硬门禁）；**`Escrow`**：canonical 在 **`contracts/abi/Escrow.json`**；**`frontend/dapp/abis/Escrow.json`** 可为精简 ABI，须与 DApp 调用一致且含 **`openDispute`** 等（见 **14 §1.2**）。
4. 若变更 **Guide/Provider 质押池** / **`Registry`** / **`Escrow`** 对外接口：同步 **`frontend/lib/stakingAbi.ts`**、**`registryAbi.ts`** 等 **`as const`** 镜像（与 JSON 一致）。
...
| 群体攻击（旅行者刷单/向导罢工/仲裁员集体拒绝/多签串谋）自动缓冲 vs 人工 | §11 | 协议级自动缓冲或仅人工干预写死 | 风控/运营 | |
