...
| **`ttg-governance-cert-gates.v1.yaml`** | **②** TTG Cert #1～12 机读注册表（runner / smoke / signoff / `path_triggers`）；维护后 **`python scripts/dev/validate-ttg-governance-cert-gates-registry.py`** · 改动后 **`bash scripts/dev/run-ttg-governance-cert-post-change-gate.sh`** |
