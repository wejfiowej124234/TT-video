//! `POST /auth/forgot-password` extra risk limits: per-IP and global buckets.

use std::env;

use sqlx::postgres::PgPool;

const DEFAULT_PER_IP_MAX_PER_WINDOW: u32 = 20;
const DEFAULT_PER_IP_WINDOW_SECS: u64 = 3600;
const DEFAULT_GLOBAL_MAX_PER_WINDOW: u32 = 2000;
...
pub(crate) async fn try_consume_forgot_per_ip_slot(pool: Option<&PgPool>, client_ip: &str) -> bool {
    let max = read_u32(
...
pub(crate) async fn try_consume_forgot_global_slot(pool: Option<&PgPool>) -> bool {
    let max = read_u32(
