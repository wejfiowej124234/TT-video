    ...
    if public_catalog_only {
        sqlx::query_as::<_, MarketListingRow>(
            r#"SELECT id, variant, owner_user_id, payload, status, data_origin, created_at, updated_at
               ...
               WHERE variant = $1 AND status = 'published' AND data_origin = 'production'
...
pub async fn insert_market_listing(
    ...
    let n = sqlx::query(
        r#"INSERT INTO market_listings (id, variant, owner_user_id, payload, status, data_origin, created_at, updated_at)
