...
pub(super) async fn meta(State(state): State<ApiMetaState>) -> impl IntoResponse {
    ...
    let chain_contracts = state.chain_config.as_ref().map(|c| {
        ...
        json!({
            ...
            "escrow_factory_v2_address": &c.escrow_factory_v2_address,
            "region_steward_stake_pool_address": steward_pool.as_ref(),
            "rule": "仅当 CHAIN_RPC_URL 等已加载 ChainConfig 时有值；guide_staking_address 与 STAKING_ADDRESS/GUIDE_STAKING_ADDRESS 同源；staking_provider_address 与 STAKING_PROVIDER_ADDRESS 同源；governance_token_address 与 GOVERNANCE_VOTES_TOKEN_ADDRESS/GOVERNANCE_TOKEN_ADDRESS 同源；registry_address/escrow_factory_address 与 REGISTRY_ADDRESS/ESCROW_FACTORY_ADDRESS 同源；escrow_factory_v2_address 与 ESCROW_FACTORY_V2_ADDRESS 同源（V2 bilateral · optional until Sepolia broadcast）；region_steward_stake_pool_address 与 REGION_STEWARD_STAKE_POOL_ADDRESS 同源；FEE_ROUTER_ADDRESS 设后 indexer-tick 拉取 PlatformFeeRouted；GOVERNOR_ADDRESS 设后拉取 Governor 事件（B-089）；前端 NEXT_PUBLIC_* 须与部署一致"
        })
    });
